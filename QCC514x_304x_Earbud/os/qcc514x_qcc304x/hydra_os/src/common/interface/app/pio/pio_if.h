 ...
 */
    ...
    ANALOGUE,
    /** Sets the reset Pin (supported on PIO1 only) to reset mode. Not applicable to CSRA68100 chipset */
    RESETB,
    /** The following CHARGER_COMMS functions will only take effect on or
     * after ChargerCommsUART stream creation */
    /** Charger Comms (Single-wire) UART LED pin. */
    CHARGER_COMMS_UART_TX_RX,
    /** Control the comms pull-up enable. */
    CHARGER_COMMS_PULL,
    /** Voltage regulator control. */
    CHARGER_COMMS_VCHG_REG,
    /** Control to isolate the voltage regulator from the Charger Comms (Single-wire) UART */
    CHARGER_COMMS_ISOLATE,
    /** Associates a PIO to something outside of Apps control. */
    OTHER,
    /** Bitserial 2, clock input used in SPI slave and I2C */
    BITSERIAL_2_CLOCK_IN,
    /** Bitserial 2, clock output used in SPI master and I2C */
    BITSERIAL_2_CLOCK_OUT,
    /** Bitserial 2, data input used for SPI master MISO, SPI slave MOSI and I2C */
    BITSERIAL_2_DATA_IN,
    /** Bitserial 2, data output used for SPI master MOSI, SPI slave MISO and I2C */
    BITSERIAL_2_DATA_OUT,
    /** Bitserial 2, chip select input used for SPI slave */
    BITSERIAL_2_SEL_IN,
    /** Bitserial 2, chip select output used for SPI master */
    BITSERIAL_2_SEL_OUT
} pin_function_id;
...
/*! @brief Priority IDs which can be set for a group of pins. This is used in
 *  PioDebounceGroupBankPriority().
 */
typedef enum
{
    /** PIO group debounce handled in non-interrupt context */
    PRIORITY_LOW = 0,
    /** PIO group debounce handled in interrupt context */
    PRIORITY_HIGH
} pio_group_priority;
