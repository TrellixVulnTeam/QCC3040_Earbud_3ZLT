...
bool obexFrameConnectPkt( Obex session, uint8 opcode )
{
    ...
    if( dataLen )
    {
        ...
        /* Add the Target/who header */
         ...
         /*Add app param*/
         if(session->supported_feature.support_features == TRUE && session->supported_feature.Supported_Feature_Param )
         {
            src = StreamRegionSource( session->supported_feature.Supported_Feature_Param, 6 );

            if(!ObexObjAddVariableHdr(session,obex_app_params, 6, src ))
            {
                if (src)
                {
                    SourceEmpty( src );
 ...
 *  len     -   Length of the Connect Packet
 **************************************************************************/
uint8 ObexStoreReturnedSRM(  Obex session, Source source, uint16 *len )
{
   const uint8* pkt = SourceMap(source);
    uint8 SRM_server;
    UNUSED(session);
    ...
    {
        pkt  += (OBEX_PKT_HDR_SIZE + OBEX_PKT_SETPATH_HDR_SIZE);
        *len -=  (OBEX_PKT_HDR_SIZE + OBEX_PKT_SETPATH_HDR_SIZE);
    }


    /* Extract the Connection ID Header from the response  packet */
     SRM_server  = obexGetUint8Header( pkt, *len, OBEX_SRM_HDR );
     return SRM_server;
