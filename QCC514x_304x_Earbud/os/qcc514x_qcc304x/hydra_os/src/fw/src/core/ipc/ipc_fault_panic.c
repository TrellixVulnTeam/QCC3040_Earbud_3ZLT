...
{
    /* It's possible to panic before panic_data has been initialised. */
    if(NULL != panic_data)
    {
...
void panic_interrupt_handler(void)
{
    if(NULL != panic_data)
    {
        panic_from_p0(panic_data->p0_deathbed_confession,
                      panic_data->p0_t,
                      panic_data->p0_diatribe);
    }
    else
    {
        panic_from_p0(PANIC_NO_PANIC_DATA, 0, 0);
    }
}
