...
const goal_entry_t hsgoals[] =
{
    ...
    GOAL_WITH_CONCURRENCY(hs_topology_goal_allow_le_connection, hs_topology_procedure_allow_le_connection,
    ...
    GOAL(hs_topology_goal_disconnect_lru_handset, hs_topology_procedure_disconnect_lru_handset,
         &hs_proc_disconnect_lru_handset_fns, hs_topology_goal_connect_handset), 
};
...
static void headsetTopology_GoalProcComplete(procedure_id proc, procedure_result_t result)
{
    ...
    if (complete_event)
    {
        DEBUG_LOG_VERBOSE("headsetTopology_GoalProcComplete generating event 0x%llx", complete_event);
...
void HeadsetTopology_HandleGoalDecision(Task task, MessageId id, Message message)
{
    ...
    if (td->hstop_state != hstop_state_started)
    {
        DEBUG_LOG_INFO("HeadsetTopology_HandleGoalDecision. Flushed id MESSAGE:headset_topology_goals:0x%x (topology stopping:%d)",
                                id, td->hstop_state);
        return;
    }

    DEBUG_LOG_INFO("HeadsetTopology_HandleGoalDecision id MESSAGE:headset_topology_goals:0x%x", id);
    ...
    switch (id)
    {
        ...
        case HSTOP_GOAL_DISCONNECT_LRU_HANDSET:
            GoalsEngine_ActivateGoal(td->goal_set, hs_topology_goal_disconnect_lru_handset, task, id, message, 0);
            break;
        ...
        default:
            DEBUG_LOG_VERBOSE("HeadsetTopology_HandleGoalDecision, unknown goal decision MESSAGE:headset_topology_goals:0x%x", id);
            break;
    }
