...
uint8 pmalloc_length[PMALLOC_MAX_DEBUG_BLOCKS];
#endif
    ...
    for (i = 0; i < pool_config_len; ++i, ++pool_config_ptr)
    {
        ...
        if (size)
        {
             ...
             if ((void *) new_hwm <= (void *) pmalloc_end)
             {
               ...
               pools_end->size = size;
#ifdef PMALLOC_CUMULATIVE_BLOCKS
               /* At this stage the blocks aren't cumulative - they're
                * converted later in init_pmalloc()
                */
               pools_end->cblocks = blocks;
#else
...
#ifdef PMALLOC_CUMULATIVE_BLOCKS
        /* Convert cblocks to a cumulative count */
        pool->allocated = pool->cblocks;
        pool->cblocks = pmalloc_total_blocks;
        pmalloc_total_blocks += pool->allocated;
#else
...
#ifdef PMALLOC_STATS
        pool->max_ideal_size = 0;
        pool->curr_ideal_size = 0;
#endif
    }
