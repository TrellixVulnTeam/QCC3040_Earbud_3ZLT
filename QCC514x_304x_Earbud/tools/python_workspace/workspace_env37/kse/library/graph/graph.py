...
from kats.core.endpoint_base import ENDPOINT_TYPE_SOURCE, ENDPOINT_TYPE_SINK
...
class Node:
    """
    Representation of a graph terminal

    Args:
        node_type (str): Node type, one of 'stream', 'endpoint', 'operator' or 'koperator'
        node_index (int): Zero based node index
        node_modifier (list/None): Terminal information as ['i', 0] or ['o', 1]
    """
    ...
    def get_index(self):
        """Get zero based terminal index

        Returns:
            int: Terminal node index
        """
        return self._node_index

    def get_modifier(self, index=None):
        """Get terminal modifiers

        Args:
            index (int/None): Modifier to get or None for all

        Returns:
            any/list[any]: Terminal node modifier/s
        """
        if index is None:
...
class Graph:
    ...
    def _check_node(self, node):
        ...
        if out[0].lower().startswith(STREAM):
            ...
            if self.get_stream_num() <= int(out[1]):
                raise ValueError('node %s not found in configuration' % (node))
            return out[0], int(out[1]), []
        if out[0].lower().startswith(ENDPOINT):
            if len(out) != 2:
                raise ValueError('invalid node %s' % (node))
            if self.get_endpoint_num() <= int(out[1]):
                raise ValueError('node %s not found in configuration' % (node))
            return out[0], int(out[1]), []
        if out[0].lower().startswith(OPERATOR):
            if len(out) != 4 or out[2].lower() not in [OPERATOR_INPUT, OPERATOR_OUTPUT]:
                raise ValueError('invalid node %s' % (node))
            if self.get_operator_num() <= int(out[1]):
                raise ValueError('node %s not found in configuration' % (node))
            return out[0], int(out[1]), [out[2], int(out[3])]
        if self._koperator_enable and out[0].lower().startswith(KOPERATOR):
            if len(out) != 4 or out[2].lower() not in [KOPERATOR_INPUT, KOPERATOR_OUTPUT]:
                raise ValueError('invalid node %s' % (node))
            if self.get_koperator_num() <= int(out[1]):
                raise ValueError('node %s not found in configuration' % (node))
            return out[0], int(out[1]), [out[2], int(out[3])]
    ...
    def _validate_from_koperator(self, edge):
        ...
        # NOTE It would be good to check output does not exceed maximum for this koperator
    ...
    def _validate_from_endpoint(self, edge):
        """Connections from an endpoint validation
            ...
            - endpoint source to operator input or endpoint sink
        ...
        if endpoint_type == ENDPOINT_TYPE_SOURCE:
            # from endpoint source can only be connected to operator or sink endpoint
            try:
                _, endpoint_to_type, _, _ = self.get_endpoint(None, edge[GRAPH_TO].get_index())
            except Exception:  # pylint:disable=broad-except
                endpoint_to_type = None
                ...
                if edge[GRAPH_TO].get_modifier(0) != OPERATOR_INPUT:
                    raise ValueError(
                        ...
                        (str(edge[GRAPH_FROM]),
                         endpoint_type,
                         str(edge[GRAPH_TO])))
            elif edge[GRAPH_TO].get_type() == ENDPOINT and endpoint_to_type == ENDPOINT_TYPE_SINK:
                pass
            else:
                raise ValueError(
                    'node %s (%s) connected to %s only can connect operator or endpoint sink' %
                    (str(edge[GRAPH_FROM]),
                     endpoint_type,
                     str(edge[GRAPH_TO])))
    ...
    def _validate_to_koperator(self, edge):
        ...
        # NOTE It would be good to check input does not exceed maximum for this koperator
        # unfortunately that requires connecting to capability_factory.get_class which is
        # kalsim specific
    ...
    def get_node_out_connections(self, node_type, node_index, output_num=None):
        """Get all connections in the graph from this node

        Args:
            node_type (str): Node type, one of 'stream', 'endpoint', 'operator' or 'koperator'
            node_index (int): Zero based node index
            output_num (int): Number of  outputs in node

        Returns:
            list[list[int,int]]: Connections
        """
        res = []
        for entry in self._internal_graph:
            if (entry[GRAPH_FROM].get_type() == node_type and
                    entry[GRAPH_FROM].get_index() == node_index and
                    (output_num is None or (entry[GRAPH_FROM].get_modifier(0) == NODE_OUTPUT and
                                            entry[GRAPH_FROM].get_modifier(1) == output_num))):
                res.append(entry)
        return res

    def get_node_in_connections(self, node_type, node_index, input_num=None):
        """Get all connections in the graph to this node

        Args:
            node_type (str): Node type, one of 'stream', 'endpoint', 'operator' or 'koperator'
            node_index (int): Zero based node index
            input_num (int): Number of inputs in node
        ...
        for entry in self._internal_graph:
            if (entry[GRAPH_TO].get_type() == node_type and
                    entry[GRAPH_TO].get_index() == node_index and
    ...
    def get_stream_node(self, stream_type, index):
        """Get stream node
        ...
        for entry in self._config[STREAM]:
            if not stream_type or entry[STREAM_TYPE] == stream_type:
                if num == index:
                    return Node(STREAM, index)
                num += 1
        raise ValueError('stream_type:%s index:%s not found' % (stream_type, index))

    def get_stream_raw(self, stream_type, index):
        """Get stream configuration raw entry
    ...
    def get_endpoint_node(self, endpoint_type, index):
        """Get endpoint node
        ...
        for entry in self._config[ENDPOINT]:
            if not endpoint_type or entry[ENDPOINT_TYPE] == endpoint_type:
                if num == index:
                    return Node(ENDPOINT, index)
                num += 1
        raise ValueError('endpoint_type:%s index:%s not found' % (endpoint_type, index))

    def get_endpoint_raw(self, endpoint_type, index):
        """Get endpoint configuration raw entry
    ...
    def get_operator_node(self, index):
        """Get operator node
        ...
        """
        num = 0
        for _ in self._config[OPERATOR]:
            if num == index:
                return Node(OPERATOR, index)
            num += 1
        raise ValueError('operator index:%s not found' % (index))

    def get_operator_raw(self, index):
        """Get operator configuration raw entry
    ...
    def get_koperator_node(self, index):
        """Get kats operator node

        Args:
            index (int): Zero-based operator index

        Returns:
            Node: kats operator node
        """
        num = 0
        for _ in self._config[KOPERATOR]:
            if num == index:
                return Node(KOPERATOR, index)
            num += 1
        raise ValueError('operator index:%s not found' % (index))

    def get_koperator_raw(self, index):
        """Get kats operator configuration raw entry

        Args:
            index (int): Zero-based operator index

        Returns:
            dict: kats operator entry
        """
        num = 0
        for entry in self._config[KOPERATOR]:
            if num == index:
