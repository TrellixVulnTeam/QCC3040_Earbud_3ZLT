...
/*! Convert a time from milliseconds to BT slots rounding up */
#define MS_TO_BT_SLOTS(MILLISECONDS) US_TO_BT_SLOTS((MILLISECONDS) * US_PER_MS)

/*! Convert a time from milliseconds to microseconds */
#define MS_TO_US(ms)        ((rtime_t)(ms * US_PER_MS))

/*! Convert a time from seconds to milliseconds */
#define SEC_TO_MS(sec)      ((rtime_t)(sec * MS_PER_SEC))

/*! Convert a time from seconds to microseconds */
#define SEC_TO_US(sec)      ((rtime_t)(sec * US_PER_SEC))
...
/**
 * \brief Convert an absolute time to a milli-second delay.
 * \param time - The time to convert.
 * \return The number of milliseconds from the current time to the specified time.
 * \note If the time is in the past, D_IMMEDIATE will be returned.
 */
Delay RtimeTimeToMsDelay(rtime_t time);
