...
class PoolInfo(Analysis.Analysis):
    ...
    def get_block_from_addr(self, address):
        ...
        for pool in range(self.pool_count):
            ...
            blocks_total = self.debuginfo.get_dm_const(
                self.pooldata[pool]['numBlocksFree'].address,
                0,
            )
    ...
    def get_pool(self, pool):
        ...
        blocks_total = self.debuginfo.get_dm_const(
            self.pooldata[pool]['numBlocksFree'].address, 0
        )
        ...
        # 'get' that.
        pool_var = self.debuginfo.get_var_strict(initial_free_ptr)
        if pool_var.parent is not None and Arch.kal_arch == 4:
            pool_var = self.debuginfo.get_var_strict(str(pool_var.parent.name))

        pool_var.value = self.chipdata.get_data(pool_var.address, pool_var.size)
    ...
    def _lookup_debuginfo(self):
        ...
        self.pool_count = self.debuginfo.get_var_strict(
            'L_aMemoryPoolControl'
        ).array_len
            ...
            for pool in range(self.pool_count):
                ...
                self.p_pool.append(
                    self.debuginfo.get_dm_const(
                        self.pooldata[pool]['pFirstFreeBlock'].address,
                        0
                    )
                )
        else:
    ...
    def _inspect_pool(self, pool):
        ...
        for blockid, blockvar in enumerate(poolvar.members):
            ...
            if self.pmalloc_debug_enabled:
                ...
                if not blockvar.free:
                    file_name_ptr = block_allocation_data['file'].value
        ...
        if Arch.chip_arch == "Hydra":
            ...
            buff_handles_array_len = self.debuginfo.get_var_strict('L_mmu_buffer_handles').array_len
            if Arch.kal_arch == 4:
                try:
                    buff_handles = self.chipdata.get_var_strict(
                        'L_mmu_buffer_handle_list'
                    )
                    buff_handles_array_len = self.debuginfo.get_var_strict('L_mmu_buffer_handle_list').array_len
                except DebugInfoNoVariableError:
                    pass
            for handle in range(buff_handles_array_len):
                if Arch.kal_arch == 4:
