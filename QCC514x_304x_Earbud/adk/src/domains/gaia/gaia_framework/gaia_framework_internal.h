...
typedef enum {
    ...
    APP_GAIA_SERVICE_STOPPED,   		        /*!< A GAIA service has stopped */

    APP_GAIA_MESSAGE_END                        /*! This must be the final message */
} av_headet_gaia_messages;

/*! Data used to inform apps which GAIA transport has stopped */
typedef struct {
    gaia_transport_type transport_type;
} APP_GAIA_SERVICE_STOPPED_T;
 ...
 */
extern void gaiaFrameworkInternal_GaiaDisconnect(GAIA_TRANSPORT *t);
