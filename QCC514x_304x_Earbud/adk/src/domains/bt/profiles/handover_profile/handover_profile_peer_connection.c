...
static const uint16 l2cap_conftab[] =
{
    ...
    L2CAP_AUTOPT_MTU_IN,            895,            /* Maximum inbound MTU - 895 bytes */
    L2CAP_AUTOPT_MTU_OUT,           895,            /* Minimum acceptable outbound MTU - 48 bytes */
...
static void handoverProfile_EnterConnectingLocal(void)
{
    ...
    /* cancel any pending start-up request while establishing connection locally */
    MessageCancelFirst(&ho_inst->task, HANDOVER_PROFILE_INTERNAL_STARTUP_REQ);
}
...
static void handoverProfile_EnterConnectingRemote(void)
{
    ...
    /* Entering to connecting remote possibly happen only in Secondary, but still cancel the any pending start-up request for safer side */
    MessageCancelFirst(&Handover_GetTaskData()->task, HANDOVER_PROFILE_INTERNAL_STARTUP_REQ);
}
...
static void handoverProfile_EnterConnected(void)
{
    ...
    {
        /* Primary will veto handover until secondary firmware version is known */
        ho_inst->secondary_firmware = HANDOVER_PROFILE_SECONDARY_FIRMWARE_UNKNOWN;
    }
    else
    {
        /* Secondary sends primary its firmware versions. Handover will not be
           attempted if the firmware versions do not match. */
        handoverProtocol_SendVersionInd();
...
static void handoverProfile_EnterDisconnected(handover_profile_state_t old_state)
{
    ...
    /* Send connect/disconnect confirmation to requested task if any */
    ...
    if(old_state >= HANDOVER_PROFILE_STATE_CONNECTED)
    {
...
static void handoverProfile_ExitInitialising(void)
{
    ...
    MessageSend(SystemState_GetTransitionTask(), HANDOVER_PROFILE_INIT_CFM, NULL);
}
...
void HandoverProfile_Startup(Task task, const bdaddr *peer_addr)
{
    ...
    /* Currently it's supported only one connetion client, so there shall be only one pending start-up request */
    MessageCancelFirst(&ho_inst->task, HANDOVER_PROFILE_INTERNAL_STARTUP_REQ);
...
static void handoverProfile_handleUnhandledMsg(MessageId id)
{
    ...
    UNUSED(id);
    DEBUG_LOG("handoverProfile_handleUnhandledMsg, state %u, MESSAGE:0x%x",
              HandoverProfile_GetState(ho_inst), id);
    Panic();
}
...
void HandoverProfile_HandleInternalStartupRequest(HANDOVER_PROFILE_INTERNAL_STARTUP_REQ_T *req)
{
    ...
    switch (HandoverProfile_GetState(ho_inst))
    {
        ...
        case HANDOVER_PROFILE_STATE_CONNECTING_REMOTE:
        {
            /* Nothing to do.Just wait for completing connection procedure */
        }
        break;
...
{
    handover_profile_task_data_t *ho_inst = Handover_GetTaskData();
    DEBUG_LOG("HandoverProfile_TerminateSdpPrimitive, state %u", HandoverProfile_GetState(ho_inst));
...
{
    uint16_t status = cfm->status, local_psm = cfm->psm;
    handover_profile_task_data_t *ho_inst = Handover_GetTaskData();

    DEBUG_LOG("HandoverProfile_HandleClL2capRegisterCfm, status %u, psm %u", status, local_psm);
    ...
    if (success == status)
    {
        ...
        sdp_SetHandoverProfilePsm(record, local_psm);

        /* Register service record */
        ConnectionRegisterServiceRecord(&ho_inst->task, 
                                        sdp_GetHandoverProfileServiceRecordSize(), 
                                        record);
        StreamConfigure(VM_STREAM_L2CAP_ADD_LARGE_BUFFER_ON_PSM, ho_inst->local_psm);
    }
    else
    {
...
void HandoverProfile_HandleClSdpRegisterCfm(const CL_SDP_REGISTER_CFM_T *cfm)
{
    ...
    DEBUG_LOG("HandoverProfile_HandleClSdpRegisterCfm, status %d", status);
    PanicFalse(HandoverProfile_GetState(ho_inst) == HANDOVER_PROFILE_STATE_INITIALISING);
...
void HandoverProfile_HandleL2capConnectCfm(const CL_L2CAP_CONNECT_CFM_T *cfm)
{
    ...
    switch (HandoverProfile_GetState(ho_inst))
    {
        ...
        case HANDOVER_PROFILE_STATE_CONNECTING_REMOTE:
        {
            ...
            {
                DEBUG_LOG("HandoverProfile_HandleL2capConnectCfm, connected, conn ID %u, flush remote %u, mtu_remote %u, qos  %u", cfm->connection_id, cfm->flush_timeout_remote, cfm->mtu_remote, cfm->qos_remote.service_type);
                ...
                /* Check for any pending initial messages */
                handoverProtocol_HandleMessage(ho_inst->link_source);
            }
