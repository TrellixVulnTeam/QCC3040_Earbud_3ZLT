#!C:\Qualcomm_Prog\ADK_Toolkit_1.2.10.21_x64\tools\pyenv37\Scripts\python.exe
############################################################################
# CONFIDENTIAL
#
...
# Pythonpath check
# ----------------------------------------------------------------------------
def package_import_is_valid():
    # If pylib is set in PYTHONPATH, check it is local to pydbg
    # If not warn the user.
    # Set the correct pylib path
    my_dir = os.path.abspath(os.path.dirname(__file__))
    pylib_dir = os.path.join(my_dir, "pylib")
    import csr
    csr_path_list = csr.__path__
    if not os.path.exists(pylib_dir):
        if os.path.dirname(csr_path_list[0]).endswith("pylib"):
            # Account for the case when pydbg in an installed package
            return False
    return True


if not package_import_is_valid():
    print("WARNING: The path to csr in PYTHONPATH is not local to pydbg.")
...
def main():
    ...
    if "--version" in sys.argv:
        print(PYDBG_VERSION if PYDBG_VERSION is not None else "<unversioned>")
        sys.exit(0)
