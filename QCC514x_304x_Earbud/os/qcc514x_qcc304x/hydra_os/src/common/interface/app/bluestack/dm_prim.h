...
#define BT_VERSION_MAX              BT_VERSION_5p2
...
typedef struct
{
    dm_prim_t       type;        /* DM_ULP_SET_TRANSMIT_POWER_REPORTING_ENABLE_CFM */
    phandle_t       phandle;     /* destination phandle */
    TP_BD_ADDR_T    tp_addrt;    /* transport Bluetooth device address */
...
typedef struct
{
    ...
    uint32_t        flags;         /* Reserved always set to 0 */
 ...
 *---------------------------------------------------------------------------*/
typedef DM_HCI_STANDARD_COMMAND_CFM_T DM_ULP_EXT_ADV_REGISTER_APP_ADV_SET_CFM_T;
...
typedef struct
{
    ...
    uint16_t        primary_adv_phy;          /* EA Primary and Secondary advertising
                                                 phys in hci.h */
    uint8_t         secondary_adv_max_skip;   /* 0 to 0xFF */
    uint16_t        secondary_adv_phy;        /* EA Primary and Secondary advertising
                                                 phys in hci.h */
 ...
 * PURPOSE
 *
 * Enable/disable advertising for an advertising set. Further info can be found in
 * BLUETOOTH CORE SPECIFICATION Version 5.2: LE Set Extended Advertising Enable Command
 *
 * RETURNS
 ...
 * PURPOSE
 *
 * Enable/disable advertising for 1 to 4 advertising sets. Further info can be found in
 * BLUETOOTH CORE SPECIFICATION Version 5.2: LE Set Extended Advertising Enable Command
 *
...
/* Use the global random address instead of one generated or in field random_addr.
   ...
   supports bluestack running a host based RPA timer for advertising set. Make 
   sure DM_HCI_ULP_SET_ADVERTISING_PARAMETERS_REQ has own address type set to use
   random address otherwise RPA timer maybe disabled. The 
   DM_ULP_EXT_ADV_SET_PARAMS_REQ may also be used to set this on legacy advertising
   set. */
...
typedef struct
{
    ...
    uint8_t         report_periodic; /* Generate periodic advertising reports */
 ...
 * PURPOSE
 *
 * Say for a synced train if advertising reports should be reported.
 *
 * RETURNS
...
typedef struct
{
    ...
    uint8_t             mode;               /* What to do when SyncInfo is received. */
    uint8_t             cte_type;           /* Specify types of Constant Tone Extension. */
...
typedef struct cis_config_test
{
    uint16_t    max_sdu_m_to_s;             /* Maximum SDU Size from master host */
    uint16_t    max_sdu_s_to_m;             /* Maximum SDU Size from slave host */
 ...
 *     then this primitive will be dropped. Typically connection context will
 *     be created using the primitive DM_ISOC_CREATE_BIG_REQ.
 *
