...
class Sym(object):
    ...
    def printfunc(self, name):
        """
        Pretty-print information about a function specified by name.

        This displays the filename and line number containing the
        function definition, low and high addresses in memory (fixed
        if the function is not part of a downloadable capability),
        and information about stack consumption in octets.
        """
        if hasattr(self, "funcs"):
            try:
                ...
                print ("")
                print ("Filename: %s        Line: %d" % (func[0], func[1]))
                print ("Low   PC: 0x%x" % func[2])
                print ("High  PC: 0x%x" % func[3])
                if func[4] == 0:
                    print ("Stack consumption unknown")
                elif func[4] == 1:
                    print ("Maximum stack consumption: %d octets" % func[5])
                elif func[4] == 2:
                    print ("Minimum stack consumption: %d octets" % func[5])
                else:
                    print ("Unknown stack consumption attribute")
                return
            except KeyError:
                print ("Function '%s' not found" % name)
        else:
            raise Exception("No symbol file is loaded, unable to display function info")
