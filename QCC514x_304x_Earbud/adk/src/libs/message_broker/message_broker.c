...
void MessageBroker_RegisterInterestInMsgGroups(Task task, const message_group_t *msg_groups, unsigned num_groups)
{
    ...
    for (msg_group_index = 0; msg_group_index < num_groups; msg_group_index++)
    {
        ...
        for (registration = message_broker_state.registrations;
             ...
             registration++)
        {
            /* Groups can have a range.
               Check both, but use the start range so the registration check
               can have a simpler check */
            if (registration->message_group <= group && group <= registration->last_message_group)
            {
                registration->MessageGroupRegister(task, registration->message_group);
                registered = TRUE;
            }
        }
