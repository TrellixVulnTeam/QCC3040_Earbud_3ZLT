...
bool src_sync_buffer_details(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    {
        /* If a negative buffer size has been set, terminal buffer sizes
         * are disabled.
         */
        term_buffer_size = op_extra_data->buffer_size;
    }
    else
    {
        term_buffer_size =
                op_extra_data->terminal_buffer_sizes.terminal_buffer_sizes
                [(terminal_id & TERMINAL_SINK_MASK) ?
                        SRC_SYNC_INPUT_BUFFER_SIZE_INDEX :
                        SRC_SYNC_OUTPUT_BUFFER_SIZE_INDEX];
    }
    if (term_buffer_size > 0)
    {
        config_buffer_w = term_buffer_size;
    }
    else
    {
        ...
        {
            buffer_kick_mult = - term_buffer_size;
        }
...
void src_sync_refresh_downstream_probe(SRC_SYNC_OP_DATA* op_extra_data)
{
        ...
        (src_grp->common.terminals != NULL))
    {
        ...
        if (single_downstream_hop)
        {
             ...
             */
            unsigned buffer_unit = (kick_period * op_extra_data->source_groups->common.sample_rate) / SECOND;
            ...
            {
                /* Inverse of:
                 * src_sync_usec_to_sec_frac(frac_mult(op_extra_data->cur_params.OFFSET_SS_MAX_LATENCY, (unsigned)stream_if_get_system_kick_period())) << 5;
                 */
                op_extra_data->cur_params.OFFSET_SS_MAX_LATENCY =
                    pl_fractional_divide(
                        src_sync_samples_to_usec(
                            buffer_size - 1,
                            op_extra_data->source_groups->common.inv_sample_rate),
                        kick_period << 5);
...
/*
 * Helper to compare the cbuffer's level with the threshold,
 * with the meaning of positive values as words of data,
 * or the magnitude of negative values as words of space.
 * \param cbuffer Connection buffer to check
 * \param threshold Signed threshold as passed in the
 *                  SET_BACK_KICK_THRESHOLD common operator message
 * \return TRUE if the buffer level is above the threshold
 *         (more data or less space)
 */
static bool src_sync_above_back_kick_threshold(tCbuffer* cbuffer, int threshold)
{
    /* Input level must be above threshold before
     * consuming */
    if (threshold > 0)
    {
        if ((int)cbuffer_calc_amount_data_in_words(cbuffer)
                > threshold)
        {
            return TRUE;
        }
    ...
    {
        /* space < -threshold */
        if ((int)cbuffer_calc_amount_space_in_words(cbuffer)
                + threshold < 0)
        {
            return TRUE;
        }
...
/*
 * Check the preconditions for configured back kick:
 * - exactly one sink group is configured (matching the patch implementation)
 * - the sink group's first channel is connected
 */
static unsigned src_sync_pre_check_back_kick(SRC_SYNC_OP_DATA* op_extra_data, tCbuffer** p_sink0_cbuffer)
{
    unsigned back_kick_mask = 0;
    tCbuffer* sink0_cbuffer;
    int threshold = op_extra_data->back_kick_threshold;
    if (threshold != 0)
    {
        SRC_SYNC_SINK_GROUP* sink_grp = op_extra_data->sink_groups;
        if (sink_grp != NULL)
        {
            SRC_SYNC_SINK_ENTRY* sink_ptr;
            sink_ptr = sink_entries_from_group(sink_grp);
            if (sink_ptr != NULL)
            {
                sink0_cbuffer = sink_ptr->common.buffer;
                if (sink0_cbuffer != NULL)
                {
                    if ((op_extra_data->back_kick_mode !=
                            OPMSG_COMMON_BACK_KICK_MODE_EDGE) ||
                        src_sync_above_back_kick_threshold(
                                sink0_cbuffer, threshold))
                    {
                        back_kick_mask = sink_grp->common.channel_mask;
                        *p_sink0_cbuffer = sink0_cbuffer;
                    }
                }
            }
        }
    }
    return back_kick_mask;
}
...
void src_sync_process_data(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    unsigned back_kick_mask;
    tCbuffer* sink0_cbuffer = NULL;
    back_kick_mask = src_sync_pre_check_back_kick(op_extra_data, &sink0_cbuffer);
    ...
    if (back_kick_mask != 0)
    {
        if (! src_sync_above_back_kick_threshold(
                sink0_cbuffer, op_extra_data->back_kick_threshold))
        {
            touched->sinks |= back_kick_mask;
        }
    }
