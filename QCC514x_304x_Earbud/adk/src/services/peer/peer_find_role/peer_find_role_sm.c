...
static void peer_find_role_enter_discover_connectable(void)
{
    ...
    if (!peerFindRole_EnableAdvertising())
    {
        if (PeerFindRoleGetAdvertBusy())
        {
            MessageSendConditionally(PeerFindRoleGetTask(), 
                                     PEER_FIND_ROLE_INTERNAL_ATTEMPT_ADVERTISING, NULL,
                                     &PeerFindRoleGetAdvertBusy());
        }
        else
        {
            DEBUG_LOG("peer_find_role_enter_discover_connectable Unable to start advertising. Not busy");
            Panic();
        }
    }
}
...
static void peer_find_role_enter_connecting_to_discovered(void)
{
    ...
    MessageSendLater(PeerFindRoleGetTask(),
                     ...
                     NULL,
                     PeerFindRoleConfigConnectingToPeerTimeoutMs());
}
...
static void peer_find_role_exit_advertising_state(void)
{
    ...
    MessageCancelAll(PeerFindRoleGetTask(), PEER_FIND_ROLE_INTERNAL_ATTEMPT_ADVERTISING);
...
static void peer_find_role_exit_connecting_to_discovered(PEER_FIND_ROLE_STATE new_state)
{
    ...
    MessageCancelFirst(PeerFindRoleGetTask(), PEER_FIND_ROLE_INTERNAL_TIMEOUT_PEER_CONNECTION);
...
static void peer_find_role_enter_server_preparing(void)
{
    ...
    peer_find_role_prepare_for_role_selection(PEER_FIND_ROLE_STATE_SERVER);
}
...
{
    DEBUG_LOG("peer_find_role_enter_client_preparing client %d",
                peer_find_role_prepare_client_registered());

    peer_find_role_prepare_for_role_selection(PEER_FIND_ROLE_STATE_CLIENT_DECIDING);
}
...
/*! Internal function for leaving the state #PEER_FIND_ROLE_STATE_CLIENT_DECIDING

    On leaving the state cancel any outstanding timer raised when entering the state
 */
static void peer_find_role_exit_client_deciding(void)
{
    DEBUG_LOG_FN_ENTRY("peer_find_role_exit_client_deciding");
...
void peer_find_role_set_state(PEER_FIND_ROLE_STATE new_state)
{
    ...
    switch (old_state)
    {
        ...
        case PEER_FIND_ROLE_STATE_CLIENT_DECIDING:
            peer_find_role_exit_client_deciding();
            break;
