/*!
    \copyright Copyright (c) 2022 Qualcomm Technologies International, Ltd.
        All Rights Reserved.
        Qualcomm Technologies International, Ltd. Confidential and Proprietary.
    \file chain_input_aac_stereo_mix.c
    \brief The chain_input_aac_stereo_mix chain.

    This file is generated by C:\Qualcomm_Prog\QCC514x_304x_Earbud\adk\tools\packages\chaingen\chaingen_mod\__init__.pyc.
*/

#include <chain_input_aac_stereo_mix.h>
#include <cap_id_prim.h>
#include <opmsg_prim.h>
#include <hydra_macros.h>
#include <../earbud_cap_ids.h>
#include <kymera_chain_roles.h>
static const operator_config_t operators_p0[] =
{
    MAKE_OPERATOR_CONFIG(CAP_ID_RTP_DECODE, OPR_RTP_DECODER),
    MAKE_OPERATOR_CONFIG(EB_CAP_ID_AAC_DECODER, OPR_AAC_DECODER),
    MAKE_OPERATOR_CONFIG(CAP_ID_MIXER, OPR_LEFT_RIGHT_MIXER),
} ;

static const operator_config_t operators_p1[] =
{
    MAKE_OPERATOR_CONFIG(CAP_ID_RTP_DECODE, OPR_RTP_DECODER),
    MAKE_OPERATOR_CONFIG_P1(EB_CAP_ID_AAC_DECODER, OPR_AAC_DECODER),
    MAKE_OPERATOR_CONFIG_P1(CAP_ID_MIXER, OPR_LEFT_RIGHT_MIXER),
} ;

static const operator_endpoint_t inputs[] =
{
    {OPR_RTP_DECODER, EPR_SINK_MEDIA, 0},
} ;

static const operator_endpoint_t outputs[] =
{
    {OPR_LEFT_RIGHT_MIXER, EPR_SOURCE_DECODED_PCM, 0},
} ;

static const operator_connection_t connections[] =
{
    {OPR_RTP_DECODER, 0, OPR_AAC_DECODER, 0, 1},
    {OPR_AAC_DECODER, 0, OPR_LEFT_RIGHT_MIXER, 0, 1},
    {OPR_AAC_DECODER, 1, OPR_LEFT_RIGHT_MIXER, 1, 1},
} ;

const chain_config_t chain_input_aac_stereo_mix_config_p0 = {0, 0, operators_p0, 3, inputs, 1, outputs, 1, connections, 3};

const chain_config_t chain_input_aac_stereo_mix_config_p1 = {0, 0, operators_p1, 3, inputs, 1, outputs, 1, connections, 3};

