...
DESCRIPTION
    Function that provides information if hfp audio is connected or not for a
    given handset.

RETURNS
    TRUE if hfp audio connected, FALSE otherwise.
*/
bool HfpIsAudioConnected(const bdaddr* bd_addr)
{
    ...
    if(link)
        return (link->bitfields.audio_state == hfp_audio_connected);
