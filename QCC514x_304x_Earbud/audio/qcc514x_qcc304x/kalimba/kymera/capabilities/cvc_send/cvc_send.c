...
bool cvc_send_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        *(stats[3])=0;
        ...
        resp = cpsPack2Words(*stats[22] , *stats[23], resp);
        resp = cpsPack2Words(*stats[24] , *stats[25], resp);
        resp = cpsPack2Words(*stats[26] , *stats[27], resp);
        resp = cpsPack2Words(*stats[28] , *stats[29], resp);
        resp = cpsPack2Words(*stats[30] , *stats[31], resp);
