...
bool AvrcpProfile_Veto(avInstanceTaskData *the_inst)
{
    ...
    {
        DEBUG_LOG_INFO("AvrcpProfile_Veto, avrcp.client_responses");
        veto = TRUE;
    }
...
#define STEPS_TO_MILLISECONDS(x) (x*STEP_SIZE)
    ...
    {
        the_inst->connect_avrcp_post_handover = MAX(1, MILLISECONDS_TO_STEPS(MIN(first_due, CAP_MAX_DELAY_MILLISECONDS)));
        DEBUG_LOG_INFO("AvrcpProfile_Veto, AV_INTERNAL_AVRCP_CONNECT_LATER_REQ exists in queue, due in %d millisecs[%d steps]", first_due, the_inst->connect_avrcp_post_handover);
    }
...
void AvrcpProfile_Commit(avInstanceTaskData *the_inst, bool is_primary)
{
    ...
    else
    {
        ...
        {
            delay_in_milliseconds = STEPS_TO_MILLISECONDS(the_inst->connect_avrcp_post_handover);
            DEBUG_LOG("av_Unmarshal, AV_INTERNAL_AVRCP_CONNECT_LATER_REQ message due in %d millisecs", delay_in_milliseconds);
            MessageSendLater(&the_inst->av_task, AV_INTERNAL_AVRCP_CONNECT_LATER_REQ, NULL, delay_in_milliseconds);
            the_inst->connect_avrcp_post_handover=0;
        }
        ...
        if(the_inst->send_avrcp_unrouted_pause_post_handover)
        {
            ...
            DEBUG_LOG("av_Unmarshal, AV_INTERNAL_A2DP_UNROUTED_PAUSE_REQUEST message due in %d millisecs", delay_in_milliseconds);
