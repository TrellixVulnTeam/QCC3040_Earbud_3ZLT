...
LOGGING_PRESERVE_MESSAGE_TYPE(AVRCP_INTERNAL_T)
       ...
       case MESSAGE_STREAM_DISCONNECT:          /* Fall Through */
        ...
        default:
            if(id > AVRCP_MSG_BASE  && id < AVRCP_MSG_BOTTOM)
            {
                ...
                AVRCP_INFO(("Ignoring the Message(MESSAGE:0x%x) in wrong state (%d)\n",
                         id, avrcp->bitfields.state));
            }
            else
            {
                AVRCP_DEBUG((" AVRCP handleUnexpected- State 0x%x MsgId MESSAGE:0x%x\n",
       ...
       case CL_L2CAP_TIMEOUT_IND:
        ...
        default:
            if(id > AVRCP_MSG_BASE  && id < AVRCP_MSG_BOTTOM)
            {
                /* Ignore these messages. 
                   It may happen due to cross over of messages */
                AVRCP_INFO(("Ignoring the Message (MESSAGE:AVRCP_INTERNAL_T:0x%x) \n", id));
            }
            else
            {
                AVRCP_DEBUG(("Unhandled Message = MESSAGE:AVRCP_INTERNAL_T:0x%x\n", id));
