...
FUNCTION
    Ask the DM to update the Auth entries for all devices.

    This can be used to update the resolving list for private addresses.
*/
...
void connectionAddDeviceAtTdlPosition(uint8 pos)
{
    ...
    /* USPSCRUM-2096: If there are LE link keys, then force device privacy mode. */
    if (td->content.data_flags & BLE_ONLY_LINK_KEYS)
    {
        ...
        unpack_td_bdaddr(&tempAddr, td);
        prim->peer_identity_address_type = td->content.addr_type;
        BdaddrConvertVmToBluestack(&prim->peer_identity_address, &tempAddr);

        prim->privacy_mode = (uint8) 0x01; /* device privacy mode */
