...
/*!
  The charger pin (VCHG) or VREG_IN state has changed. 
...
/*!
  Message to indicate the status of the transmission of ChargerComms message
  via ChargerComms Uart.
  The message content is a #MessageChargerCommsUartStatus.

  This message is sent to the task registered with MessageChargerCommsTask().
 */
#define MESSAGE_CHARGERCOMMS_UART_STATUS (SYSTEM_MESSAGE_BASE_ + 84)
...
/*!
  Message type received when charger(VCHG) or VREG_EN pin states have changed.
*/
typedef struct
{
    bool charger_connected; /*!< TRUE if the charger is connected (VCHG is high), FALSE if it is not */
...
/*!
  Message type received when ChargerCommsUart received an ACK/NAK for
  a transmitted message or failed to send the message.
*/
typedef struct
{
    charger_comms_uart_tx_status status; /*!< Status */
    charger_comms_uart_tx_msg_header_t header; /*!< Message header of the message which failed to send or acknowledged by the recipient */
} MessageChargerCommsUartStatus;
...
typedef struct
{
    ...
    uint32              offset; /*!< Offset within sector requested to be
                                     erased. */
    raPartition_handle *handle; /*!< Handle pointer for the partition erased in */
