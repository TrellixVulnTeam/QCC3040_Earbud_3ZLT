 ...
 *   
 *   PS Key ID Name                        | PS Key ID Value | 0..519 space as
 *  used by PsRetrieve
 *   ------------------------------------- | --------------- |
 *  -----------------------------------
...
/**
 *  \brief Remove the given key from the system cache.
 * Only applicable to the PSKEY_READONLY key set, this call removes the specified
 * key from the system cache, freeing up system memory. This also makes it
 *  unreadable
 * until the next power-cycle, so the customer application should take care to not
 * read the key again.
 *         
 *  \param key The persistent store key to drop.
 *  \return           Returns TRUE if the key is successfully removed, FALSE if the key is
 *  not (or doesn't exist).
 *           
 * 
 * \ingroup trapset_core
 */
bool PsDrop(uint16 key);
 ...
 *             
 *  \return             Return False if there are still Appkeys in PsStore.
 *             Return True if the Appkey previously read was the last one.
 *           
 * 
 * \ingroup trapset_core
 */
 ...
 *             
 *  \return             Return False if there are still Audiokeys in PsStore.
 *             Return True if the Audiokey previously read was the last one.
 *           
 * 
 * \ingroup trapset_core
 */
