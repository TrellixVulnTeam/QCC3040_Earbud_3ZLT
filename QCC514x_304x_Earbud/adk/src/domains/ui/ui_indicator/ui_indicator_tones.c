...
#include "ui_indicator_tones_private.h"
...
LOGGING_PRESERVE_MESSAGE_ENUM(ui_internal_tone_messages)
...
static bool uiTones_IsToneMandatory(uint16 tone_index)
{
    ...
    if (!uiTones_IsRepeatingIndication(tone_index))
    {
        ...
        is_mandatory = indicate_on_shutdown || button_press_feedback;
    }
    return is_mandatory;
}

static inline bool uiTones_IsToneQueueable(uint16 tone_index)
{
    return !uiTones_IsRepeatingIndication(tone_index) &&
           the_tones.sys_event_to_tone_data_mappings[tone_index].data.tone.queueable;
}
...
static void uiTones_QueueTone(uint16 tone_index)
{
    ...
    int num_messages_in_queue = MessagesPendingForTask(t, NULL);
    bool tone_is_queueable = uiTones_IsToneQueueable(tone_index);
    bool queue_is_full = num_messages_in_queue >= UI_TONES_MAX_QUEUE_SIZE;
    bool queue_is_empty = num_messages_in_queue == 0;
    bool ready_to_play = queue_is_empty && !ui_IsKymeraResourceLocked();

    if ((tone_is_queueable && !queue_is_full) ||
        (!tone_is_queueable && ready_to_play) ||
        uiTones_IsToneMandatory(tone_index))
    {
...
static void uiTones_HandleShutdownRequest(void)
{
    if (the_tones.tone_playback_enabled)
    {
        int32 time = 0;
        bool tone_is_currently_playing = MessagePendingFirst(&the_tones.task, UI_INTERNAL_TONE_PLAYBACK_COMPLETED, &time);
        if (tone_is_currently_playing)
        {
            /* Await completion of tones that have started playing. */
            the_tones.indicate_when_power_shutdown_prepared = TRUE;
        }
        else
        {
            /* If no tone is playing, then we can shutdown immediately */
...
static void uiTones_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    DEBUG_LOG("uiTones_HandleMessage Id MESSAGE:ui_internal_tone_messages:0x%04x", id);
...
bool UiTones_Init(Task init_task)
{
    ...
    the_tones.tone_playback_enabled = FALSE;
