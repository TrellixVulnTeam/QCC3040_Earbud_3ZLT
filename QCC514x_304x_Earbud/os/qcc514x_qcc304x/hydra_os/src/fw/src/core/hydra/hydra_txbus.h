...
/*
 * From QCC517x and QCC307x there is a hardware timeout mechanism
 * running from the slow clock. This has timeouts defaulting
 * to 2ms. The firmware needs a timeout in excess of that to
 * allow the hardware to recover before the firmware panics.
 * 
 * It is acceptable to use the same timeout on chips that do 
 * not have the hardware timeout.
 */
#define HYDRA_TBRIDGE_TRANSACTION_TIMEOUT_US    10000
 ...
 */
#define HYDRA_TXBUS_RD_TIMEOUT_USECS    HYDRA_TBRIDGE_TRANSACTION_TIMEOUT_US
 ...
 * If we exceed this we will panic.
 */
#define HYDRA_TXBUS_TRANSACTION_SEND_TIMEOUT_USECS  HYDRA_TBRIDGE_TRANSACTION_TIMEOUT_US
