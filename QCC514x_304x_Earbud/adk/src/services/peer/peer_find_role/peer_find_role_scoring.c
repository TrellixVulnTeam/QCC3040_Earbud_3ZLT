...
{
    Pairing_ActivityClientRegister(PeerFindRoleGetTask());
}
...
void peer_find_role_calculate_score(void)
{
    ...
    score += PEER_FIND_ROLE_SCORE(PHY_STATE, (uint16)appPhyStateGetState());
    score += PEER_FIND_ROLE_SCORE(POWERED, Charger_IsConnected());
    score += PEER_FIND_ROLE_SCORE(BATTERY, Soc_GetBatterySoc());
    score += PEER_FIND_ROLE_SCORE(ROLE_CP, peer_find_role_is_central());
    score += PEER_FIND_ROLE_SCORE(USER_PAIRING, scoring->saved_user_pairing);
    score += PEER_FIND_ROLE_SCORE(HANDSET_CONNECTED, appDeviceIsBredrHandsetConnected());
