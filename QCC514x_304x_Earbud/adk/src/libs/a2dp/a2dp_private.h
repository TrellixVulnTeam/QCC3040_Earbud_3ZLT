...
typedef enum
{
    ...
    A2DP_INTERNAL_SIGNALLING_MTU_REQ,
...
typedef struct
{   /* Only be valid during channel establishment */
    uint16      outbound_cid;
    uint16      inbound_cid;
    uint8       inbound_id;
} connection_setup;
...
} signalling_connection_bitfields;

typedef struct
{
    Sink        sink;
    signalling_connection_bitfields bitfields;
    uint8      *received_packet;
} signalling_connection_active;

typedef union
{
    connection_setup setup;                 /* Only be valid during an channel establishment */
    signalling_connection_active active;    /* Only valid once a channel has been established */
...
} signalling_channel_status;

/* The signalling connection information */
typedef struct signalling_channel
{
    signalling_connection connection;
    signalling_channel_status status;
} signalling_channel;
...
typedef struct
{
    unsigned    unused:4;
    unsigned    mtu:12;
} media_connection_active_bitfields;

typedef struct
{
    Sink        sink;
    media_connection_active_bitfields bitfields;
#if 0   /* TODO: */
...
#endif
} media_connection_active;

typedef union
{
    connection_setup setup;         /* Only be valid during an channel establishment */
    media_connection_active active; /* Only valid once a channel has been established */
} media_connection;
...
typedef struct media_channel
{
    ...
    media_channel_status status;
} media_channel;

typedef struct
{
    unsigned                unused:4;
    unsigned                use_large_mtu:1;                        /* Flag to enable use of large MTU for QHS connections */
...
typedef struct
{
    bdaddr          addr;
    bool            use_large_mtu;
} A2DP_INTERNAL_SIGNALLING_MTU_REQ_T;
