...
LOGGING_PRESERVE_MESSAGE_TYPE(HfpMessageId)
...
void HfpInit(Task theAppTask, const hfp_init_params *config, const char* extra_indicators)
{
    ...
    else
    {
        ...
        /* Feature required for v1.7 and later */
        lHfp->hf_supported_features |= HFP_ESCO_S4_SUPPORTED;

        if(config->hf_indicators != hfp_indicator_mask_none)
        {
        ...
        for_all_links(link)
        {
            ...
            /* SWB support enabled by default for each link. */
            link->bitfields.link_supports_swb = TRUE;
