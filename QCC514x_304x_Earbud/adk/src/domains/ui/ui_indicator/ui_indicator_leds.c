...
static void uiLed_PerformLedAction(const ui_led_data_t *config, uint16 led_index)
{
    ...
    case LED_STOP_PATTERN:
        DEBUG_LOG_V_VERBOSE("uiLed_PerformLedAction LED_STOP_PATTERN enum:led_priority_t:%d", config->priority);
        LedManager_StopPattern(config->priority);
        break;
#ifndef FORCE_LED_FLASHES
    case LED_SET_FILTER:
        if (LedManager_GetFilter(config->priority) != config->data.filter)
        {
            DEBUG_LOG_V_VERBOSE("uiLed_PerformLedAction LED_SET_FILTER enum:led_priority_t:%d", config->priority);
    ...
    case LED_CANCEL_FILTER:
        ...
        {
            DEBUG_LOG_V_VERBOSE("uiLed_PerformLedAction LED_CANCEL_FILTER enum:led_priority_t:%d", config->priority);
...
static bool uiLeds_CheckForSupercedingContext(uint16 led_index)
{
    ...
    if (curr_context_index < the_leds.context_mapping_table_size)
    {
        ...
        do
        {
            ...
            DEBUG_LOG_V_VERBOSE("uiLeds_CheckForSupercedingContext curr_context_index=%d, enum:ui_providers_t:%d, context=%02x",
                ...
                curr_context == Ui_GetUiProviderContext(curr_provider))
            {
                ...
                DEBUG_LOG("uiLeds_CheckForSupercedingContext was superceded by enum:ui_providers_t:%d", curr_provider);
                break;
            }
            curr_context_index++;
        }
