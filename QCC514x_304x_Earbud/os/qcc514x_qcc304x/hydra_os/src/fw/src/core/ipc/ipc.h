 ...
 * @param len_bytes Length of message in bytes (if \p msg is NULL, must be
 * 0!). Must be a multiple of 4.
 *
 * \ingroup ipc_send
 */

extern void ipc_send(IPC_SIGNAL_ID msg_id, const void *msg, uint16 len_bytes);
...
/**
 * Helper macro for setting the PIO muxes for the Charger Comms (Single-wire) 
 * UART and auxiliary pins.
 * @param req Pointer to request message indicating which PIOs to set.
 */ 
#define ipc_send_pio_set_func_charger_comms(req) \
    ipc_send_pio(req, IPC_SIGNAL_ID_PIO_SET_FUNC_CHARGER_COMMS)
...
/* The ipc_send_sched function has been removed for IPC_PROTOCOL_ID 2. It
...
/**
 * Pass the given message to the given task, applying filtering as required
 *
 * \note This function logically replaces the BC VM's "vm_send_message_filtered"
 * in the Apps vm_message port.
 *
 * @param msg_type_id Message handler ID.  If this doesn't correspond to a
 * registered task, the message is quietly dropped
 * @param id Message ID
 * @param message Message body
 * @param allow_duplicates Whether to filter the task's queue for duplicates
 */
extern void ipc_send_trap_api_msg_filtered(IPC_MSG_TYPE msg_type_id,
                                           uint16 id,
                                           void *message,
                                           bool allow_duplicates);

/**
 * Pass the given message to the given P1 handler
 *
 * @param handler Handler in P1. This is usually a P1 task (of type Task) and
 * it doesn't need to be registered to receive this message type.
 * @param id Message ID
 * @param message Message body
 * @param message_length The length of @p message in bytes
 * @param allow_duplicates Whether to filter the task's queue for duplicates
 */
extern void ipc_send_trap_api_msg_to_handler(void *handler,
                                             uint16 id,
                                             const void *message,
...
/**
 * Pass the given message to the given sink or source message handler task,
 * applying filtering as required
 *
 * \note This function logically replaces the BC VM's "vm_send_message_filtered"
 * in the Apps vm_message port.
 *
 * @param stream_id Stream whose handler to pass the message to. If there is no
 * handler, the message is quietly dropped
 * @param id Message ID
 * @param message Message body
 * @param allow_duplicates Whether to filter the task's queue for duplicates
 */
extern void ipc_send_trap_api_sink_source_msg_filtered(uint16 stream_id,
                                                       uint16 id,
                                                       void *message,
                                                       bool allow_duplicates);

/**
 * Non-blocking traps should call this before completing
 */
extern void ipc_trap_complete(void);
#endif /* PROCESSOR_P0 */
...
/**
 * @brief Send a message and block waiting for the response.
 *
 * Blocking messages must use this function rather than an ipc_send/ipc_recv pair
 * as future implementations of IPC may be more complicated.
 *
 * @param msg_id ID of message to send.
 * @param msg Message body (note: the ID field is automatically set).
 * @param len_bytes Length of message in bytes (if \p msg is NULL, must be
 * 0!). Must be a multiple of 4.
 * @param rsp_id ID of message to wait until received before returning.
 * @param blocking_msg Pointer to pre-allocated space for the expected message.
 */
extern void ipc_transaction(IPC_SIGNAL_ID msg_id, const void *msg, uint16 len_bytes,
