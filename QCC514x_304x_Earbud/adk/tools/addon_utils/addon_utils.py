...
class XPathCommand(Commander):
    ...
    def do_devkitgroup(self):
        ...
        for item in self._items(".//configuration/", select="devkitGroup"):
            fprint(item.text)
    ...
    def do_pretty(self, fname):
        ...
        encoding = self.xml_pi.get("encoding", None)
        lines = ET.tostring(self.root)
        dom = xml.dom.minidom.parseString(lines.decode(encoding))
        pretty_xml = dom.toprettyxml(indent="    ", encoding=encoding).decode(encoding)
        pretty_xml = "\n".join(l for l in pretty_xml.splitlines() if l.strip())
        if fname == "-":
            fprint(pretty_xml, end="")
        else:
            with open_for_writing(fname) as fob:
