...
#define SAMPLE_RATE_16k                             (16000UL)

#define USB_AUDIO_CHANNELS_MIC                      2
#define USB_AUDIO_CHANNELS_SPEAKER                  2

#ifdef ENABLE_ADAPTIVE_ANC
#define USB_AUDIO_SAMPLE_RATE_AANC_TUNING           SAMPLE_RATE_16k

#define USB_SPKR_SAMPLE_RATE                        USB_AUDIO_SAMPLE_RATE_AANC_TUNING
#define USB_MIC_SAMPLE_RATE                         USB_AUDIO_SAMPLE_RATE_AANC_TUNING

#define USB_STREAMING_NUM_OF_FREQUENCY              (0x01)

#define USB_AUDIO_CHANNELS_MIC_AANC_TUNING          2
#define USB_AUDIO_CHANNELS_SPEAKER_AANC_TUNING      1
#endif
...
#ifdef ENABLE_ADAPTIVE_ANC
#define usbConfigAncTuningEnabled()             (AncStateManager_IsAdaptiveAncTuningModeActive()) /* Configuration for USB Adaptive ANC tuning */
#else
#define usbConfigAncTuningEnabled()             (FALSE)
#endif
...
#ifdef ENABLE_ADAPTIVE_ANC
/** USB Audio Class Descriptor: control interfaces for aanc tuning use case */
static const uint8 usb_interface_descriptor_control_mic_and_speaker_aanc_tuning[] =
{
    /* Class Specific Header */
    0x0A,         /* bLength */
    0x24,         /* bDescriptorType = CS_INTERFACE */
    ...
    0x00,         /* bAssocTerminal = none */
    USB_AUDIO_CHANNELS_MIC_AANC_TUNING,         /* bNrChannels */
    ...
    0x00, 0x00,     /*bmaControls[1] = 0002 (Vol on Right Front)*/
    0x00,           /*iFeature = same as USB product string*/
...
/** AANC Tuning USB streaming interface descriptors for speaker */
static const uint8 usb_interface_descriptor_streaming_speaker_aanc_tuning[] =
{
    /* Class Specific AS interface descriptor */
    0x07,         /* bLength */
    0x24,         /* bDescriptorType = CS_INTERFACE */
    ...
    0x02,         /* bDescriptorSubType = FORMAT_TYPE */
    0x01,         /* bFormatType = FORMAT_TYPE_I */
    USB_AUDIO_CHANNELS_SPEAKER_AANC_TUNING,         /* bNumberOfChannels */
    ...
    USB_SPKR_SAMPLE_RATE & 0xff, /* tSampleFreq = 48000*/
    (USB_SPKR_SAMPLE_RATE >> 8) & 0xff,
    (USB_SPKR_SAMPLE_RATE >> 16) & 0xff,
...
/** AANC Tuning USB streaming interface descriptors for mic */
static const uint8 usb_interface_descriptor_streaming_mic_aanc_tuning[] =
{
    /* Class Specific AS interface descriptor */
    0x07,         /* bLength */
    0x24,         /* bDescriptorType = CS_INTERFACE */
    ...
    0x02,         /* bDescriptorSubType = FORMAT_TYPE */
    0x01,         /* bFormatType = FORMAT_TYPE_I */
    USB_AUDIO_CHANNELS_MIC_AANC_TUNING,         /* bNumberOfChannels */
    0x02,         /* bSubframeSize = 2 bytes */
    0x10,         /* bBitsResolution */
    0x01,         /* bSampleFreqType = 1 discrete sampling freq */
    USB_MIC_SAMPLE_RATE & 0xff, /* tSampleFreq */
    (USB_MIC_SAMPLE_RATE >> 8) & 0xff,
    (USB_MIC_SAMPLE_RATE >> 16) & 0xff,
...
/** Usb Audio configuration for enumerating device for Adaptive ANC tuning case*/
static const usb_device_class_audio_config usb_config_adaptive_anc_specific_descriptors =
{
    {usb_interface_descriptor_control_mic_and_speaker_aanc_tuning,
    sizeof(usb_interface_descriptor_control_mic_and_speaker_aanc_tuning),
    NULL},
    {usb_interface_descriptor_streaming_mic_aanc_tuning,
    sizeof(usb_interface_descriptor_streaming_mic_aanc_tuning),
    usb_epinfo_streaming_mic},
    {usb_interface_descriptor_streaming_speaker_aanc_tuning,
    sizeof(usb_interface_descriptor_streaming_speaker_aanc_tuning),
    usb_epinfo_streaming_speaker}
};
    ...
    switch (id)
    {
        ...
        case MESSAGE_USB_ENUMERATED:
            if(TaskList_Size(Usb_GetClients()))
            {
                ...
                msg->sample_rate = usbConfigAncTuningEnabled() ? SAMPLE_RATE_16k : USB_AUDIO_SAMPLE_RATE;
        ...
        case MESSAGE_USB_DETACHED:
            if(TaskList_Size(Usb_GetClients()))
            {
                TaskList_MessageSendId(Usb_GetClients(), MESSAGE_USB_DETACHED);
            }
            break;

        default:
            DEBUG_LOG("USB_COMMON Message handler, unhandled MESSAGE:0x%x", id);
            break;
    }
}
...
void Usb_TimeCriticalInit(void)
{
    ...
    {
        (void)UsbHubConfigure(NULL);
        if (!UsbHubConfigKey(USB_DEVICE_CFG_PRODUCT_ID, 0x400E))
        {
            DEBUG_LOG_ERROR("UsbHubConfigKey ERROR");
            Panic();
        }

        status = UsbDeviceClassConfigure(USB_DEVICE_CLASS_CONFIG_AUDIO_INTERFACE_DESCRIPTORS, 0, 0, (const uint8 *)&usb_config_adaptive_anc_specific_descriptors);
        DEBUG_LOG("Usb_TimeCriticalInit adaptive anc tuning configuration status is %d",status);
        if (status != usb_device_class_status_success)
        {
            Panic();
        }
    ...
    status = UsbDeviceClassEnumerate(appGetSysTask(), USB_DEVICE_CLASS_TYPE_AUDIO_MICROPHONE | USB_DEVICE_CLASS_TYPE_AUDIO_SPEAKER);
    ...
    (void)StreamConfigure(VM_STREAM_USB_ATTACH_MSG_ENABLED, 1);
}
