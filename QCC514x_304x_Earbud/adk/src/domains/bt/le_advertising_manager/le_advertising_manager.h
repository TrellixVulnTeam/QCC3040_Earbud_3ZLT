...
typedef enum
{
 ...
 le_adv_data_set_extended_handset =  1UL<<3
} le_adv_data_set_t;

#define LE_ADV_MGR_ADVERTISING_SET_LEGACY   (le_adv_data_set_handset_identifiable | le_adv_data_set_handset_unidentifiable | le_adv_data_set_peer)
#define LE_ADV_MGR_ADVERTISING_SET_EXTENDED (le_adv_data_set_extended_handset)
...
/*! \brief Enum to configure the behaviour when LeAdvertisingManager_NotifyDataChange() API is called.*/
typedef enum
{
    le_adv_config_notify_restart_advertising = 0,
    le_adv_config_notify_keep_advertising
}le_adv_config_notify_t;
...
\return Sends LE_ADV_MGR_NOTIFY_DATA_CHANGE_CFM for a succesful call to API for notification.
*/
bool LeAdvertisingManager_NotifyDataChange(Task task, const le_adv_mgr_register_handle handle);

/*! \brief Public API to configure LE advertising behaviour when LeAdvertisingManager_NotifyDataChange() API is called.
\param[in] config Argument of type le_adv_config_notify_t. 
\note For LE advertising to stop and restart when LeAdvertisingManager_NotifyDataChange() API is called, "le_adv_config_notify_restart_advertising" should be passed as the parameter.
      For LE advertising to continue without stopping and restarting when LeAdvertisingManager_NotifyDataChange() API is called, "le_adv_config_notify_keep_advertising" should be passed as the parameter.
      When configured with the parameter "le_adv_config_notify_keep_advertising", LE advertising can continue using old data for a period of time until LE advertising packet is updated with the new data.
\return TRUE for a succesful call to API for notification, FALSE otherwise.
*/
bool LeAdvertisingManager_ConfigureAdvertisingOnNotifyDataChange(le_adv_config_notify_t config);
