...
typedef enum
{
    ...
    VM_SOURCE_USB_TRANSFER_LENGTH = 0x000c,      /*!< Normally RX transfer completes when a short packet is received.
                                                  *   Configuring transfer length also allows the transfer to complete when
                                                  *   the transfer reached that length. The length shall be a multiple
                                                  *   of the endpoint's wMaxPacketSize. */
...
/*!
  @brief Sink filter path values for SinkL2capFilterPackets
*/
typedef enum
{
    SINK_FILTER_PATH_LOCAL = 0, /*!< Filter packets on the local path */
    SINK_FILTER_PATH_RELAY = 1  /*!< Filter packets on the relay path */
} sink_filter_path;
