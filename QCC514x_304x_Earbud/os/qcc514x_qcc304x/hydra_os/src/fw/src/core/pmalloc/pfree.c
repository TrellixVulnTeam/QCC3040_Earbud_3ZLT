...
void pfree(void *ptr)
{
    ...
    /* The supplied pointer may be within the middle of a block, e.g. if a
       ...
       We may also want the offset in the case we're recording lengths.
    */
    n = udiv3216(&rmdr, (uint16)(pool->size),
                 (uint32)((char *) ptr - (char *) (pool == pmalloc_pools
                                           ? pmalloc_blocks
                                           : pool[-1].pool_end))
                );
    ptr = (void *) ((char *) ptr - rmdr);
    ...
    {
        /* Work out which pool we theoretically would have stored it in... */
        /* Go back down the pool list until we get to the right one, and remove it */
        size_t rq_size = PMALLOC_GET_LENGTH(n);

        if (pool == pmalloc_pools)
        {
            pool->curr_ideal_size--;
        }
        else
        {
            /* Shortcut for being on the right pool */
            if (pool[-1].size < rq_size)
            {
                pool->curr_ideal_size--;
            }
            else
            {
                for (pool2 = pool-1; pool2 >= pmalloc_pools; pool2--)
                {
                    if (pool2->size < rq_size)
                    {
                        pool2[1].curr_ideal_size--;
                        break;
                    }
                }
            }
