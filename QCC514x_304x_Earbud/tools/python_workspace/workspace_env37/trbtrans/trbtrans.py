...
from ctypes import c_int, c_uint, c_ubyte, c_void_p, c_char_p, c_char, c_bool, c_uint16, byref, POINTER, Structure, \
    cdll, c_uint8, c_float, c_uint32, c_uint64, c_double
import os
import sys
...
class RemoteSleepState(object):
    """
    States for FPGA deep sleep state machine.
    """
    ...
    STATES_LOOKUP = {
        # Chip is awake
        0 : "REMOTE_SLEEP_STATE_NORMAL",
        # Chip is asleep - has sent a DS_ENTRY message.
        1: "REMOTE_SLEEP_STATE_SLEEP",
        # Chip is asleep. There is a transaction in the dongle's TX queue so, it is sending
        # a DS_WAKE message to wake the chip.
        2: "REMOTE_SLEEP_STATE_SIGNALLING_WAKE",
        # Chip has been sent a DS_WAKE message. Waiting for DS_EXIT message from chip.
        3: "REMOTE_SLEEP_STATE_WAKING"
    }

    def _str_state(self):
        try:
            return RemoteSleepState.STATES_LOOKUP[self.state]
        except KeyError:
            raise ValueError("RemoteSleepState not valid: {0}".format(self.state))
...
class Transaction(Structure):
    ...
    _fields_ = [("timestamp", c_uint),
                ...
                ("payload", ctypes_arr_type(c_ubyte, 9))]
...
class ScarDmaStatistics(Structure):
    ...
    _fields_ = [("channel_stats", ctypes_arr_type(ScarDmaChannelStatistics, 7))]
...
class TrbTimersRaw(Structure):
    """
        Corresponds to the C struct 'trb_timers'.
        """
    _fields_ = [
        ("timestamp_lo", c_uint32),
        ("timestamp_hi", c_uint32),
        ("timestamp_constant", c_uint32),
        ("remote_asleep_timer", c_uint32),
        ("asleep_timer_constant", c_double)
    ]


class TrbDeepSleepRaw(Structure):
    """
    Corresponds to the C struct 'trb_deep_sleep'.
    """
    _fields_ = [
        ("ds_entry_count", c_uint32),
        ("ds_exit_count", c_uint32),
        ("ds_timeout_count", c_uint32),
        ("waking_timeout", c_uint32),
        ("timing_info", TrbTimersRaw),
        ("tbridge_state_machines", c_uint32),
        ("remote_sleep_state", c_int)
    ]
...
class TrbDongle(object):
    def __init__(self, raw_details):
        ...
        self.description = from_cstr(raw_details.description)
...
class ScarDeviceStats(object):
    ...
    def __repr__(self):
        ...
        header = "Channel # | State | Total buffer size | Producer xfer bytes | " \
                 "Consumer xfer bytes | Diff | Description"
...
class TrbTimers(object):
    def __init__(self, raw_timing_info):
        """
        :type raw_timing_info: TrbTimersRaw
        """
        # Retain for later use by trb.calc_deep_sleep_time()
        self.raw_timing_info = raw_timing_info
...
Timestamp_H:           {1:#x}
Full Timestamp:        {2:#x}
Timestamp Constant:    {3}
Remote Asleep Timer:   {4:#x}
Asleep Timer Constant: {5:.03f}
""".format(
            self.timestamp_lo,
            self.timestamp_hi,
            self.timestamp_full,
            self.timestamp_constant,
            self.remote_asleep_timer,
            self.asleep_timer_constant
        )

    @property
    def timestamp_lo(self):
        return self.raw_timing_info.timestamp_lo

    @property
    def timestamp_hi(self):
        return self.raw_timing_info.timestamp_hi

    @property
    def timestamp_full(self):
        return self.timestamp_lo | (self.timestamp_hi << 32)

    @property
    def timestamp_constant(self):
        return self.raw_timing_info.timestamp_constant

    @property
    def remote_asleep_timer(self):
        return self.raw_timing_info.remote_asleep_timer

    @property
    def asleep_timer_constant(self):
        return self.raw_timing_info.asleep_timer_constant


class TrbTimersDiff(object):
    def __init__(self, timestamp_diff_us, time_asleep_us, percent_asleep):
        self.timestamp_diff_us = timestamp_diff_us
        self.time_asleep_us = time_asleep_us
        self.percent_asleep = percent_asleep

    def __repr__(self):
        return """
Timestamp diff:         {0} us ({1:.2f} ms)
Time asleep:            {2} us ({3:.2f} ms)
Percentage time asleep: {4:.1f}
""".format(
            self.timestamp_diff_us,
            self.timestamp_diff_us / 1000.0,
            self.time_asleep_us,
            self.time_asleep_us / 1000.0,
            self.percent_asleep,
        )


class TrbDeepSleep(object):
    def __init__(self, raw_ds_info):
        """
        :type raw_ds_info: TrbDeepSleepRaw
        """
        self.raw_ds_info = raw_ds_info
        self.timing_info = TrbTimers(raw_ds_info.timing_info)
        self.remote_sleep_state = RemoteSleepState(raw_ds_info.remote_sleep_state)
...
Waking timeout:   {3:#x}
{4}
TBRIDGE_STATE_MACHINES: {5:#x}
Remote sleep state:     {6}
""".format(
            self.entry_count,
            self.exit_count,
            self.timeout_count,
            self.waking_timeout,
            self.timing_info,
            self.tbridge_state_machines,
            self.remote_sleep_state
        )

    @property
    def entry_count(self):
        return self.raw_ds_info.ds_entry_count

    @property
    def exit_count(self):
        return self.raw_ds_info.ds_exit_count

    @property
    def timeout_count(self):
        return self.raw_ds_info.ds_timeout_count

    @property
    def waking_timeout(self):
        return self.raw_ds_info.waking_timeout

    @property
    def tbridge_state_machines(self):
        return self.raw_ds_info.tbridge_state_machines
...
class Trb:
    ...
    def _handle_error(self, err):
        ...
        if bool(err):
            ...
            try:
                ex = TRB_ERROR_MAP[err_code](
                    from_cstr(err_str.value),
                    err_action_num
                )
            except IndexError:
...
The system PATH used for the search was:
    ...
    def __init__(self, override_lib_path = None):
        ...
        gen_prototype('trb_get_driver_from_name'  , c_int, [CTypesStrIn, POINTER(c_int)])
        ...
        gen_prototype('trb_decode_transport_string', c_int, [CTypesStrIn, POINTER(c_int), POINTER(c_int)])
        ...
        gen_prototype('trb_start_log'             , c_int, [c_void_p, CTypesStrIn])
        ...
        gen_prototype('trb_configure_io_timeout', c_int, [c_void_p, c_uint, c_float])

        gen_prototype('trb_read_deep_sleep_info', c_int, [c_void_p, POINTER(TrbDeepSleepRaw)])
        gen_prototype(
            'trb_calc_deep_sleep_time',
            c_int,
            [POINTER(TrbTimersRaw), POINTER(TrbTimersRaw), POINTER(c_uint64), POINTER(c_uint32), POINTER(c_double)]
        )

    def driver_from_name(self, driver_name):
        raw_driver = c_int()
        self._handle_error(self._cfuncs['trb_get_driver_from_name'](driver_name.lower(), byref(raw_driver)))
    ...
    def _translate_driver(self, driver):
        ...
        if isinstance(driver, str):
            return self.driver_from_name(driver)
    ...
    def form_transport_string(self, dongle_details):
        ...
        buf_len = 1
        trans_str_buffer = ctypes_arr_type(c_char, buf_len)()
        ...
        return from_cstr(trans_str_buffer.value)
    ...
    def decode_transport_string(self, transport_string):
        ...
        dongle_id = c_int()
        self._handle_error(
            self._cfuncs['trb_decode_transport_string'](
                transport_string,
                byref(driver),
                byref(dongle_id)
            )
    ...
    def get_version(self):
        ...
        """
        return from_cstr(self._cfuncs['trb_get_version']())

    def get_dongle_name(self):
        """
        If a stream is open, returns the descriptive name of the debug dongle.
        """
        return from_cstr(self._cfuncs['trb_get_dongle_name'](self._stream))
    ...
    def get_driver_version(self):
        ...
        """
        return from_cstr(self._cfuncs['trb_get_driver_version'](self._stream))
    ...
    def get_max_transactions(self):
        ...
        Raw I/O is not subject to this limit.
        """
    ...
    def read_raw_transactions(self, num_transactions, timeout_millis):
        ...
        """
        buf = ctypes_arr_type(Transaction, num_transactions)()
        num_actually_got = c_int(0)
        err = self._cfuncs['trb_read_raw_transactions'](
            self._stream,
            buf,
            num_transactions,
            byref(num_actually_got),
            timeout_millis
        )
    ...
    def read(self, subsys, block_id, addr, bytes_per_transaction, num_bytes, action_type_override = None):
        ...
        a[0].data = ctypes_arr_type(c_ubyte, num_bytes)()    # Create a C array of unsigned chars
        self.sequence(a)
    ...
    def write(self, subsys, block, addr, bytes_per_transaction, data_bytes, action_type_override = None):
        ...
        a[0].num_bytes = num_bytes
        a[0].bytes_per_transaction = bytes_per_transaction
        data_carray = ctypes_arr_type(c_ubyte, num_bytes)()
    ...
    def configure_io_timeout(self, base_timeout_millis = 0, scale_factor_millis = 0.0):
        """
        Configure I/O timeouts. For a particular piece of I/O, the full timeout is
        calculated as
        base_timeout_millis + scale_factor_millis * num_transactions
        By default, the library sets base_timeout_millis and scale_factor_millis such that timeouts
        are conservative, to avoid false positive timeouts.

        NOTE: this function should not be needed in normal operation. Its primary purposes is to
        aid in diagnosis of I/O timeouts. Normal operation should generally use the defaults.
        In particular, excessive timeout values effectively mean that I/O can block for a very long
        time when there is no possibility of a valid response.

        :param base_timeout_millis the base timeout; see above.
        Pass zero to use the library default.
        :param scale_factor_millis the per-transaction scale factor; see above.
        Pass zero to use the library default.
        """
        self._handle_error(
            self._cfuncs['trb_configure_io_timeout'](self._stream, base_timeout_millis, scale_factor_millis)
        )

    def read_deep_sleep_info(self):
        """
        Read deep-sleep-related information from the dongle FPGA.
        """
        raw_ds_info = TrbDeepSleepRaw()
        self._handle_error(
            self._cfuncs['trb_read_deep_sleep_info'](self._stream, byref(raw_ds_info))
        )
        return TrbDeepSleep(raw_ds_info)

    def remote_sleep_state(self):
        """
        Convenience shortcut for read_deep_sleep_info().remote_sleep_state.
        """
        return self.read_deep_sleep_info().remote_sleep_state

    def calc_deep_sleep_time(self, timers1, timers2):
        """
        Given two TrbTimers instances read via two calls to read_deep_sleep_info() (TrbDeepSleep.timing_info),
        calculate:-
        - the microsecond difference in the free-running timestamp timer,
        - the microsecond difference in the asleep timer,
        - the percentage time the chip was in deep sleep.
        :type timers1: TrbTimers | TrbDeepSleep
        :type timers2: TrbTimers | TrbDeepSleep
        """
        if isinstance(timers1, TrbDeepSleep):
            timers1 = timers1.timing_info
        if isinstance(timers2, TrbDeepSleep):
            timers2 = timers2.timing_info

        timestamp_diff_us = c_uint64()
        time_asleep_us = c_uint32()
        percent_asleep = c_double()
        self._handle_error(
            self._cfuncs['trb_calc_deep_sleep_time'](
                byref(timers1.raw_timing_info),
                byref(timers2.raw_timing_info),
                byref(timestamp_diff_us),
                byref(time_asleep_us),
                byref(percent_asleep)
            )
        )
        return TrbTimersDiff(timestamp_diff_us.value, time_asleep_us.value, percent_asleep.value)
