...
VARIABLE_DEPENDENCIES = {
    'strict': ('L_faultinfo', '$_fault_data', '$_fault_data1')
}
...
class Fault(Analysis.Analysis):
    ...
    occurred has it's diatribe and id stored in fault_data structures.
    ...
    def run_all(self):
        ...
        Displays the contents of the faults from faultinfo array and fault_data
        structure.
        """
        self.formatter.section_start('Fault Info')
        self.analyse_faultinfo()
        self.analyse_fault_data_struct()
    ...
    def analyse_fault_data_struct(self):
        """Retrieve and display last fault recorded from preserve block."""
        if self.chipdata.processor == 0:
            fault = self.chipdata.get_var_strict('$_fault_data')
            last_fault_id = fault['last_id'].value
            last_fault_diatribe = fault['last_arg'].value
        else:
            fault = self.chipdata.get_var_strict('$_fault_data1')
            last_fault_id = fault['last_id'].value
            last_fault_diatribe = fault['last_arg'].value
        ...
        if last_fault_id != 0 and (last_fault_id & 0xFF0000 == 0):
            ...
            try:
                ...
                if 'Invalid enum value' in str(exception):
                    self.formatter.output(
                        'Invalid fault_id on fault_data structure -'
