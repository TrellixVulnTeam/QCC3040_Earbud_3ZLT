...
Source StreamAclMarshalSource(const tp_bdaddr * tpaddr)
{
    ...
    if (tpaddr != NULL)
    {
        ...
        id = ipc_recv_prim.ret;
    }

    return SOURCE_FROM_ID(id);
}
...
Sink StreamAclMarshalSink(const tp_bdaddr * tpaddr)
{
    ...
    return SINK_FROM_ID(id);
}

Sink StreamAclEstablishSink(const tp_bdaddr * tpaddr)
{
    IPC_STREAM_ACL_ESTABLISH_SINK ipc_send_prim;
    IPC_SINK_RSP ipc_recv_prim;
    TP_BD_ADDR_T tpaddr_fw;
    uint16 id = 0;
    ...
    if (tpaddr != NULL)
    {
        ...
        ipc_transaction(IPC_SIGNAL_ID_STREAM_ACL_ESTABLISH_SINK, &ipc_send_prim, sizeof(ipc_send_prim),
                        IPC_SIGNAL_ID_STREAM_ACL_ESTABLISH_SINK_RSP, &ipc_recv_prim);
