...
/*! \brief Facilitate transition of the Audio subsystem to low power mode.
           NOTE: Will only work correctly if at least one operator has been configured
           to run in low power mode (e.g. through OperatorFrameworkTriggerNotificationStart).
    \param chain The handle of the corresponding chain.
    \param operators_to_exclude List of the operators in the chain to exclude from preservation.
           Those would be (for example) any delegated operators or operators used to send notifications
           to the application while in low power mode.
           Can be NULL.
*/
void ChainSleep(kymera_chain_handle_t chain, const operator_list_t *operators_to_exclude);

/*! \brief Facilitate transition of the Audio subsystem out of low power mode.
           Used to reverse the effects of ChainSleep API.
    \param chain The handle of the corresponding chain.
    \param operators_to_exclude List of the operators in the chain to exclude from preservation release.
           Should be the same list of operators used in ChainSleep.
           Can be NULL.
*/
void ChainWake(kymera_chain_handle_t chain, const operator_list_t *operators_to_exclude);

/*! \brief Searches chain config for downloadable capabiltiies, and loads the bundles of any found.
    \param chain_config The chain config to search.
*/
void ChainLoadDownloadableCapsFromChainConfig(const chain_config_t * chain_config);

/*! \brief Searches the chain config for downloadable capabilities, and unloads the bundles of any found.
 *         Reverses the effects of the ChainLoadDownloadableCapsFromChainConfig API.
 *  \param chain_config The chain config to search.
*/
void ChainUnloadDownloadableCapsFromChainConfig(const chain_config_t * chain_config);

/*! \brief Checks the availability of a capability ID inside a chain.
 *  \param chain The handle of the corresponding chain.
 *  \param cap_id capability ID to check in the chain.
 *  \return True: Capability ID found in the chain.
*/
bool ChainCheckCapabilityId(kymera_chain_handle_t chain, capability_id_t cap_id);
