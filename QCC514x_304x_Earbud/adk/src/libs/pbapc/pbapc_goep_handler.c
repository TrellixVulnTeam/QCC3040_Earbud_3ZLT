                        ...
                        uint16       *channel )
{
	...
	uint16 psm[2];
	psm[0] = channel[0]; /*local_psm if L2cap otherwise RFcomm channel*/
    bool L2CAP_conn = 0;
    ObexConnParams params;
    if (state->L2CAP_conn)
    {
        psm[1] = channel[1];
        L2CAP_conn = state->L2CAP_conn;
    }

    params.connTaskData.handler = pbapcGoepHandler;
    params.sizeConnTask = sizeof(pbapcState);
    params.target = pbapTarget;
    params.sizeTarget = sizeof(pbapTarget);
    params.auth = FALSE;
    params.suportedFeatures.support_features = FALSE;

    if(state->support_features == TRUE)
    {
       params.suportedFeatures.support_features = TRUE;
       params.suportedFeatures.Supported_Feature_Param = (uint8*)PanicUnlessMalloc(sizeof(uint8) * HEADER_LEN);
       /*Frame 4byte Supported Features*/
       pbapcFrame4ByteAppParams( params.suportedFeatures.Supported_Feature_Param, pbapc_app_Supported_Features, FEATURE_ALL );
    }
    ...
    state = (pbapcState*)ObexConnectRequest(addr, &psm[0],  (const ObexConnParams *)&params, state->L2CAP_conn);
