...
try:
    ...
    from vm_build.base_builder import BaseBuilder
    from vm_build.builder.nvscmd import NvsCmd
    from vm_build.builder.pydbg_flash_image import Pyflash
    from vm_build.builder.device_checker import check_device_with_timeout
...
def main(cmd_line_args):
    ...
    if single_image.projects_exist():
        ...
        # Flash apps so that SqifAudioMaskROM and SqifBTMaskROM in file-system
        # are available to flash BT and Audio
        if apps_image:
            print("Program the apps SQIF with image: {}".format(apps_image))
            nvscmd.burn(apps_image, "apps")

        if audio_image or btss_image :
            print("Device reset without Apps")
            pyflash.device_reset_no_apps()

        if not check_device_with_timeout(args.device_uri, timeout=5, devkit_path=args.devkit_root) :
            return False
