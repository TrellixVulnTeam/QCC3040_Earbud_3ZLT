...
bool AudioOutputDisconnect(void)
{
    ...
    forEachOutput(audio_out)
    {
        ...
        if (audio_sink != (Sink)NULL)
        {
            ...
            audioOutputCheckUseCase(FALSE, config->mapping[audio_out].hardware_type);
        }
    }
...
bool AudioOutputConfigRequiresI2s(void)
{
    ...
    forEachOutput(audio_out)
    {
        if (config->mapping[audio_out].hardware_type == audio_output_type_i2s)
        {
...
audio_output_hardware_type_t AudioOutputGetHardwareType(audio_output_t audio_out)
{
    ...
    {
        return audio_output_type_none;
    }

    return config->mapping[audio_out].hardware_type;
}
...
bool AudioOutputIsOutputMapped(audio_output_t audio_out)
{
    ...
    if(config->mapping[audio_out].hardware_type != audio_output_type_none)
    {
        return TRUE;
    }
...
static void audioOutputSetupI2sIfEnabled(const audio_output_params_t* params)
{
    ...
    if (i2sOutputEnabled(0))
    {
        ...
        if (!state.i2s_devices_active)
        {
            ...
            AudioI2SDeviceInitialise(params->sample_rate);
...
static void enableMclkIfRequired(bool enable)
{
         ...
         instance++)
    {
        ...
        forEachOutput(audio_out)
        {
            if (config->mapping[audio_out].hardware_instance == instance &&
                config->mapping[audio_out].hardware_type == audio_output_type_i2s)
            {
                Sink audio_sink = audioOutputGetSinkIfMapped(audio_out);
...
static bool connect(audio_output_params_t* params)
{
    ...
    forEachOutput(audio_out)
    {
        ...
        audioOutputCheckUseCase(TRUE, config->mapping[audio_out].hardware_type);
                          ...
                          const audio_output_params_t *params)
{
    ...
    if(config->output_resolution_mode == audio_output_24_bit)
    {
...
static audio_instance getAudioHardwareInstance(audio_output_t audio_out)
{
    switch (config->mapping[audio_out].hardware_instance)
    {
...
static audio_channel getAudioHardwareChannel(audio_output_t audio_out)
{
    ...
    switch (config->mapping[audio_out].hardware_type)
    {
        case audio_output_type_dac:
        {
            switch (config->mapping[audio_out].channel)
            {
