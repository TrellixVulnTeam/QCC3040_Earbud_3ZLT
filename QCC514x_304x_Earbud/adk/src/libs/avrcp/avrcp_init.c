...
LOGGING_PRESERVE_MESSAGE_TYPE(AvrcpMessageId)
                        ...
                        avrcpState   state)
{
    ...
    avrcp->bitfields.connection_incoming = 0;
}
...
******************************************************************************/
void avrcpAddTaskToList(AVRCP *avrcp, const bdaddr *bd_addr, const bool incoming)
{
    avrcpList *newList;
    ...
    /* Store if this is an incoming or outgoing connection */
    newList->avrcp->bitfields.connection_incoming = incoming?1:0;
...
AVRCP *avrcpFindTask(const bdaddr *bd_addr,
                     const bool search_connection_dir,
                     const bool incoming)
{
    ...
    while (list != NULL)
    {
        ...
        /* Check whether the connetion exists for this particular device address. */      
        if (BdaddrIsSame(bd_addr, &avrcp->bd_addr))
        {
            if (search_connection_dir)
            {
                if (avrcp->bitfields.connection_incoming == (incoming?1:0))
                {
                     return avrcp;
                }
            }
                        ...
                        const bool incoming)
{
    ...
    avrcpAddTaskToList(avrcp, bd_addr, incoming);
...
{
    *avrcp = avrcpFindTask(bd_addr, FALSE, FALSE);
