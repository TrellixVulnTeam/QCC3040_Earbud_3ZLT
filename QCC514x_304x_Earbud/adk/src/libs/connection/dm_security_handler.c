        ...
        )
{
    ...
    if (ind->flags & DM_SM_FLAGS_SECURITY_MANAGER)
    {
         ...
         */
        message->authentication_requirements = ind->authentication_requirements;
    }
    else
    {
        ...
        )
{
    ...
    if(infoState->version != bluetooth_unknown)
    {
        if(!smState->setSecurityModeLock)
        {
             ...
             * stop SDP ping.
             */
            prim->config = 0; /*Keep the already set config bits */
...
DESCRIPTION
    Handle the Bluestack response to a request for the encryption of the 
    SIRK
*/
void connectionHandleSirkOperationCfm(const DM_SM_SIRK_OPERATION_CFM_T * cfm)
{
    Task theAppTask = (Task) cfm->context;
    
    if(theAppTask)
    {
        MAKE_CL_MESSAGE(CL_SM_SIRK_OPERATION_CFM);
        ...
        {
            memmove(&message->sirk_key, cfm->sirk_key, CL_SM_SIRK_KEY_LEN * sizeof(uint8));
        }
        else
        {
            /* Set Zero to sirk_key in case of failure */
            memset(&message->sirk_key, 0, CL_SM_SIRK_KEY_LEN * sizeof(uint8));
        }

        MessageSend(theAppTask, CL_SM_SIRK_OPERATION_CFM, message);
    }
#ifdef CONNECTION_DEBUG_LIB
    else
    {
        CL_DEBUG(("Failed to get task context from DM_SM_SIRK_OPERATION_CFM message. \n"));
    }
#endif
}
