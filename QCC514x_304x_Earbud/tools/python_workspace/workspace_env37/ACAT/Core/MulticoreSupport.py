...
from ACAT.Core.exceptions import (
    AnalysisError, AmbiguousSymbolError,
    InvalidDmConstAddressError, InvalidDmConstLengthError,
    InvalidDmAddressError, InvalidDmLengthError, DebugInfoNoVariableError
)
...
class Functions(object):
    ...
    def get(self, identifier, elf_id=None, datalen=None):
        ...
        if (isinstance(identifier, numbers.Integral) and
                Arch.get_pm_region(identifier, False) in ("PMROM", "PMRAM", "DM_AS_PM") and
                datalen is None):
        ...
        # Also look to see whether it's a constant name.
        try:
            const = self.debuginfo.get_constant_strict(identifier, elf_id)
            hit_flag = True
        ...
        except AmbiguousSymbolError as amb_const:
            const = None
            logger.warning(
                "Multiple constants found for {}. Use `search('*{}*')` to see "
                "the results clearer.\n".format(
                    str(identifier),
                    str(identifier)
                )
            )
            apology += str(amb_const) + "\n"

        # We got at least one hit
        if hit_flag:
            if cu.all_none((pm_addr, reg, var, const)):
                ...
                return None
            elif pm_addr is not None and cu.all_none((reg, var, const)):
                return pm_addr
            elif reg is not None and cu.all_none((var, pm_addr, const)):
                return reg
            elif var is not None and cu.all_none((reg, pm_addr, const)):
                return var
            elif const is not None and cu.all_none((pm_addr, reg, var)):
                return const
            else:
    ...
    def get_var(self, identifier, elf_id=None, datalen=None):
        ...
        return ct.ChipdataVariable(ret_var, self.chipdata)
...
class Processor(Functions):
    ...
    def __init__(self, chipdata, debuginfo,
        ...
        self.debuginfo = debuginfo
        self.debuginfo.set_table(MappedTable(chipdata, self.debuginfo))
    ...
    def run_all(self):
        """Execute all the analyses available to this processor."""
    ...
    def is_booted(self):
        ...
        The booted state is decided by the STACK_POINTER value.
