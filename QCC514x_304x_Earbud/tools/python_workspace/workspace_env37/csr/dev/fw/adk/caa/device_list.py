...
class Device(FirmwareComponent, UserDict):
    ...
    def _decode_key_and_value(self, key, value):
        ...
        def _decode_ui_user_config_gesture(value):
            user_table = []

            try:
                size_user_table = value[0].value * 4

                for i in range(1,(size_user_table+1)):
                    if ((i-1)%4 == 0):
                        user_table.append(self.env.cast(value[i:], 'ui_user_gesture_table_content_t'))

                empty_list = []
                length_empty_list = value[1+size_user_table].value
                for i in range(size_user_table+1+1,size_user_table+1+1+length_empty_list):
                    empty_list.append(self.env.cast(value[i], 'ui_user_config_gesture_id_t'))

                if (size_user_table and length_empty_list):
                    return "user={} empty={}".format(user_table, empty_list)
                elif (size_user_table):
                    return user_table
                else:
                    return empty_list
        ...
        key_map = {
            ...
            "ui_user_gesture" : _decode_ui_user_config_gesture,
...
class DeviceList(FirmwareComponent):
    ...
    @property
    def sinks(self):
        ''' Returns a filtered list of sink devices '''
        sink_filter = lambda device: (device.type.value == self.env.econst.DEVICE_TYPE_SINK)
        return list(filter(sink_filter, self.devices))
