...
/*! Brief sanity check that a passed connection is within range */
#define connection_valid(_conn) (&connections[0] <= _conn && _conn < &connections[CON_MANAGER_MAX_CONNECTIONS])

/******************************************************************************/
void conManagerResolveTpaddr(const tp_bdaddr * tpaddr, tp_bdaddr * resolved_tpaddr)
{
...
/******************************************************************************/
cm_connection_t *ConManagerFindNextActiveLink(cm_connection_t *conn, cm_transport_t transport_mask)
{
    ...
    while (++conn < &connections[CON_MANAGER_MAX_CONNECTIONS])
    {
        cm_transport_t connection_transport = TransportToCmTransport(conn ->tpaddr.transport);

        if (   connection_transport & transport_mask
            && conn->bitfields.state != ACL_DISCONNECTED
            && conn->bitfields.state != ACL_DISCONNECTED_LINK_LOSS)
        {
            DEBUG_LOG("ConManagerFindNextActiveLink. Found 0x%x. State: %x Lock:%x Users:%x Trans:0x%x",
                    conn->tpaddr.taddr.addr.lap, 
                    conn->bitfields.state, conn->lock,
                    conn->bitfields.users,
                    conn->tpaddr.transport);
            return conn;
        }
    }

    DEBUG_LOG("ConManagerFindNextActiveLink. None found for transport mask 0x%x", transport_mask);
    return NULL;
}
...
/******************************************************************************/
bool conManagerGetLpState(cm_connection_t *connection, lpPerConnectionState* lp_state)
{
