 ...
 * \ingroup trap_api_impl
 */
extern void trap_api_send_sink_source_message_filtered(uint16 stream_id,
                                                       uint16 id,
                                                       void *message,
                                                       bool allow_duplicates);
...
/**
 * Pass the given pio message to the given firmware message channel handler task,
 * applying filtering as required
 *
 * \note This function is essentially equivalent to "vm_send_message_filtered"
 * in vm_message.c in the BC VM
 *
 * @param group PIO group to identify task registered for that group
 * @param id Message int
 * @param message Message body
 * @param allow_duplicates Whether to filter the task's queue for duplicates
 *
 * \ingroup trap_api_impl
 */
extern void trap_api_send_pio_message_filtered(uint16 group,
                                    uint16 id,
                                    void *message,
                                    bool allow_duplicates);
