...
enum proximity_messages
{
    ...
    PROXIMITY_MESSAGE_END
};
ASSERT_MESSAGE_GROUP_NOT_OVERFLOWED(PROXIMITY, PROXIMITY_MESSAGE_END)
...
/*! \brief Turn on/off Proximity Sensor.
    \param task The task that turns Sensor on/off
    \param enable TRUE if turning on Sensor
                  FALSE if turning off Sensor      
*/
#if defined(INCLUDE_PROXIMITY)
extern void appProximityEnableSensor(Task task, bool enable);
#else
#define appProximityEnableSensor(_task, _enable) (UNUSED(_task), UNUSED(_enable))
#endif
