...
/**
 *  \brief Allocates sz bytes and returns a pointer to the memory if successful. If the
 *  memory allocation fails, the application is panicked.
 * The use sizeof() to allocate a structure is recommended and for allocating an
 *  array multiply sizeof() by the length of the array.
 *         
 * 
 * \ingroup trapset_core
 */
