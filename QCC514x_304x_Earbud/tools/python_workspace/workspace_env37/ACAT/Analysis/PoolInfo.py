...
class PoolInfo(Analysis.Analysis):
    ...
    def __init__(self, **kwarg):
        ...
        self.p_pool_ends = []
        self.core = 0
        self.dmprofiling_enabled = False
    ...
    def get_dmprofiling_pool_blocks(self, task_list, tf_list, f_list):
        """Construct a list of pool owner sizes. Each size is the sum
           of all pools with a given owner id (for every 'task_list' entry).

        Args:
           task_list: an ordered list of unique owner ids

        It outputs the list of pool owner sizes, the size of all pools
        with an owner not in 'task_list', and a list of descriptions of
        pool blocks owned by 'task_list' entries.
        """
        sz_list = [0] * len(task_list)
        blk_list = [[] for _ in range(len(task_list))]
        ...
        for pool in range(self.pool_count):
            (sizes, block_list, szunk) = self._find_poolblock_owner(pool, task_list, tf_list, f_list)

            for tskid in range(len(task_list)):
                sz_list[tskid] = sz_list[tskid] + sizes[tskid]
    ...
    def get_block_from_addr(self, address):
        ...
        for pool in range(self.pool_count):
            ...
            block_size_words = self.pooldata[pool]['blockSizeWords'].value + self.block_overhead_words
    ...
    def get_pool(self, pool):
        ...
        block_size = self.pooldata[pool]['blockSizeWords'].value + self.block_overhead_words

        # get total number of blocks
        blocks_total = self._get_blockcount(pool)

        try:
            poolLimits = self.chipdata.get_var_strict("L_memPoolLimits")
            ...
            fake_pool_var = ct.Variable(
                name="Dummy",
                address=poolLimits.address + index * Arch.addr_per_word,
                size=Arch.addr_per_word,
                value=copy.deepcopy(poolLimits[index].value),
                var_type=None,
                debuginfo=None,
                members=[],
                parent=None
            ...
            pool_var = self.debuginfo.get_var_strict(poolname)
            if pool_var.parent is not None and Arch.kal_arch == 4:
                pool_var = self.debuginfo.get_var_strict(str(pool_var.parent.name))

            pool_var.value = self.chipdata.get_data(pool_var.address, pool_var.size)[0]
            # Because we don't want to modify the real pool variable use deepcopy
            # for the values
            fake_pool_var = ct.Variable(
                name=pool_var.name,
    ...
    def ret_get_watermarks(self):
        ...
        for pool in range(self.pool_count):
            ...
            blk_size = self.pooldata[pool]['blockSizeWords'].value

            # get total number of blocks
            blks_total = self._get_blockcount(pool)

            blks_free = self.pooldata[pool]['numBlocksFree'].value
            blks_min = self.pooldata[pool]['minBlocksFree'].value
    ...
    def clear_watermarks(self):
        ...
        for pool in range(self.pool_count):
            blocks_free = self.pooldata[pool]['numBlocksFree'].value
            # Wash down the watermark (min available =  current available)
            self.chipdata.set_data(
                self.pooldata[pool]['minBlocksFree'].address,
                [blocks_free]
            )
    ...
    def display_configuration(self):
        ...
        for pool in range(self.pool_count):
            ...
            # get total number of blocks
            blocks_total = self._get_blockcount(pool)
            block_size = self.pooldata[pool]['blockSizeWords'].value
    ...
    def _lookup_debuginfo(self):
        ...
        # Check for PMALLOC_DEBUG If tPlMemoryBlockHeader has a member called
        #  "line" then PMALLOC_DEBUG is enabled
        try:
            _ = self.chipdata.cast(0, "tPlMemoryBlockHeader")['line'].value
            self.pmalloc_debug_enabled = True
        except AttributeError:
            self.pmalloc_debug_enabled = False

        # Check for INSTALL_DM_MEMORY_PROFILING If DMPROFILING_OWNER exists
        # then INSTALL_DM_MEMORY_PROFILING is enabled
        try:
        ...
        self.pool_count = len(self.pooldata)
        ...
        try:
            poolLimits = self.chipdata.get_var_strict('L_memPoolLimits')
            length = poolLimits.size // Arch.addr_per_word

            # Go through all the pools
            for pool in range(length):
                if (pool % (length // 2)) != (length // 2 - 1):
                    self.p_pool.append(poolLimits[pool].value)
                else:
                    self.p_pool_ends.append(poolLimits[pool].value)
    ...
    def _get_pool_id(self, value):
        """Retrieves id from pool block value.
           This fw-set value is (pool-id<<8)|(task-id)
           and top 16 bits are used only if a pointer
        Args:
            value (int): Pool block value.

        Return:
            int: Pool Identification number.
        """
        if self.dmprofiling_enabled:
            pool = (value >> 8) & 0xFF
            pool = pool | (value & 0xFFFF0000)
    ...
    def _get_owner_id(value):
        """Retrieves owner from pool block value.
           This fw-set value is (pool-id<<8)|(task-id)
           and top 16 bits are used only if a pointer
        Args:
            value (int): Pool block value.
        ...
        Return:
            int: Owner identification number.
        """
        owner = value & 0xFF
        return owner

    def _get_poolsz(self, pool):
        sz = 4 * self.pooldata[pool]['blockSizeWords'].value
        return sz
    ...
    def _inspect_pool(self, pool):
        ...
        block_size = self.pooldata[pool]['blockSizeWords'].value
        blocks_free = self.pooldata[pool]['numBlocksFree'].value
        blocks_min = self.pooldata[pool]['minBlocksFree'].value
        free_ptr = self.pooldata[pool]['pFirstFreeBlock'].value
        ...
        for blockid, blockvar in enumerate(poolvar.members):
            ...
            # word.
            if self._get_pool_id(blockvar.value[0]) == pool and \
                ...
                if self.pmalloc_debug_enabled:
                  ...
                  (self._get_pool_id(blockvar.value[0]) == 0 and
                   blockid == last_free_block)):
            ...
            if self.pmalloc_debug_enabled:
                ...
                if not blockvar.free:
                    file_name_ptr = \
                        block_allocation_data['file'].value
                    line = block_allocation_data['line'].value
                    if line == 0:
        ...
        search_range = list(range(Arch.dRegions['DM1RAM'][0],
                                  Arch.dRegions['DM1RAM'][1],
                                  Arch.addr_per_word))
    ...
    def _is_transform_dm_memory(self, blockvar, length, pool, tf_list):
        ...
        # pointers are memory pointers, e.g. +4 octets off.
        found_trf = False
        name = "POOL #%d" % pool
        for list in [1,2,3]:
            pos = 0
            while True:
                try:
                    idx = tf_list[list+13].index(blockvar.address+4, pos)
                    # Amend to poolinfo format
                    tf_list[list][idx] = blockvar.address
                    tf_list[list+7][idx] += length
    ...
    def _is_file_dm_memory(self, blockvar, length, pool, f_list):
        """Check if the address is in the file list.
        
        Args:
            blockvar: Pool block.
            length: Size of heap block.
            pool: Which pool (0..5)
            f_list: A list of file related memory blocks.
        ...
        # Note that 'address' is a node pointer, the file list
        # pointers are memory pointers, e.g. +4 octets off.
        found_file = False
        name = "POOL #%d" % pool
        for list in [1,2,3]:
            ...
            while True:
                try:
                    idx = f_list[list+13].index(blockvar.address+4, pos)
                    # Amend to poolinfo format
                    f_list[list][idx] = blockvar.address
                    f_list[list+7][idx] += length
    ...
    def _find_poolblock_owner(self, pool, task_list, tf_list, f_list):
        """Examines a memory pool.
        ...
        """
        sz_list = [0] * len(task_list)
        block_list = [[] for _ in range(len(task_list))]
        ...
        for blockid, blockvar in enumerate(poolvar.members):
            ...
            if self._get_pool_id(blockvar.value[0]) == pool and \
                ...
                length = self._get_poolsz(pool)
                               
                if not self._is_transform_dm_memory(blockvar, length, pool, tf_list) and \
                   not self._is_file_dm_memory(blockvar, length, pool, f_list):
                    sz_unknown = sz_unknown + length
                    for tskid in range(len(task_list)):
                        if (task_list[tskid] & 0xFF) == self._get_owner_id(blockvar.value[0]):
                            sz_unknown = sz_unknown - length
                            sz_list[tskid] = sz_list[tskid] + length
                            name = "POOL #%d" % pool
                            block = [blockvar.address, length, name]
        ...
        return sz_list, block_list, sz_unknown
    ...
    def _is_core_offloaded(self):
        ...
        if self.pooldata[0]['pFirstFreeBlock'].value == 0 and \
                self.pooldata[0]['numBlocksFree'].value != 0:
            return True
