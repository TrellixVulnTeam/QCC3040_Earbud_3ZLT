...
/*!  Hfp Supported Features Flag Defines
    ...
    These map to the bitfield of the AT+BRSF command.
*/
...
/*!
    @brief Setting this flag indicates that this device supports Enhanced Voice
    Recognition Activation (EVRA).
*/
#define HFP_EVRA_SUPPORTED          (1<<10)
/*!
    @brief Setting this flag indicates that this device supports Enhanced Voice
    Recognition Activation (EVRA) text updates.
*/
#define HFP_EVRA_TEXT_SUPPORTED     (1<<11)
...
#define HFP_SUPPORTED_FEATURES (HFP_NREC_FUNCTION | HFP_THREE_WAY_CALLING | \
                                ...
                                HFP_EVRA_SUPPORTED | HFP_EVRA_TEXT_SUPPORTED)
...
typedef enum
{
    ...
    /*! As defined in the handsfree profile specification.*/
    hfp_handsfree_profile       = 1<<1,
    FORCE_ENUM_TO_MIN_16BIT(hfp)
} hfp_profile;

/*!
    @brief The options for HfpConnectRequestEx()
*/
typedef enum
{
    /*! Connect with swb  */
    ...
    /*! Connect with no swb  */
    hfp_connect_ex_no_swb,

    FORCE_ENUM_TO_MIN_16BIT(hfp_connect_ex)
} hfp_connect_ex;

/** Bitmasks for all HSP/HFP **/
#define hfp_headset_all           (hfp_headset_profile)
#define hfp_handsfree_all         (hfp_handsfree_profile)
...
/*!
    @brief Voice recognition actions that can be used in HfpVoiceRecognitionEnableRequestEx.
*/
typedef enum
{
    /*! Disable Voice Recognition.  Can also be used by HF to terminate a current
        VR session or reject a VR session request from the AG. */    
    hfp_evra_action_disable = 0x00,    
    /*! Enable Voice Recognition. */
    hfp_evra_action_enable  = 0x01,
    /*! HF is ready for an Enhanced Voice Recognition (EVRA) session.
        The HF may send this value during an ongoing VR session to terminate audio output
        from the AG (if there is any) and prepare the AG for new audio input.     */
    hfp_evra_action_ready   = 0x02    
} hfp_evra_action;

/*!
     @brief Current EVRA VR status as reported by the AG.
*/
typedef enum
{
    /*! EVRA VR Disabled */    
    hfp_evra_status_disabled       = 0x00,    
    /*! EVRA VR Enabled. */
    hfp_evra_status_enabled        = 0x01
} hfp_evra_status;

/*!
    @brief Bitmask of current EVRA VR state as reported by the AG.    
*/
typedef enum
{
    /*! AG No VR session. */
    hfp_evra_ag_none        = 0x00, 
    /*! AG VR ready to accept audio input. */    
    hfp_evra_ag_ready       = 0x01,    
    /*! AG is sending audio to the HF. */
    hfp_evra_ag_sending     = 0x02,
    /*! AG VR processing the audio input. */
    hfp_evra_ag_processing  = 0x04
} hfp_evra_ag_state;


/*!
     @brief Type of text being sent in a EVRA status update as reported by the AG.
*/
typedef enum
{
    /*! Text recognised by the AG from the audio input provided by the HF */    
    hfp_evra_audio_text_recognised  = 0x00,    
    /*! Text of the audio output from the AG */
    hfp_evra_audio_text_output      = 0x01,       
    /*! Text of the audio output from the AG that contains a question */
    hfp_evra_audio_text_question    = 0x02,
    /*! Text of the audio output from the AG that contains an error description */
    hfp_evra_audio_text_error       = 0x03
} hfp_evra_audio_text;


/*!
     @brief The operation to be carried out on the text being sent in a EVRA status 
            update as reported by the AG.
*/
typedef enum
{
    /*! Text operation ID reserved */    
    hfp_evra_text_reserved  = 0x00,    
    /*! Indicates that a new text started. Shall be used when the text ID has changed */
    hfp_evra_text_new       = 0x01,
    /*! Replace any existing text with the same textID */
    hfp_evra_text_replace   = 0x02,
    /*! Attach new text to existing text and keep the same textID */
    hfp_evra_text_attach    = 0x03
} hfp_evra_text_operation;
...
/* 0x5A30 */    HFP_HF_INDICATORS_REPORT_IND,
/* 0x5A31 */    HFP_HF_INDICATORS_IND,

/* Enhanced Voice Recognition */
/* 0x5A32 */    HFP_VOICE_RECOGNITION_EVRA_IND,

/* Library message limit */
/* 0x5A33 */    HFP_MESSAGE_TOP
}HfpMessageId ;
...
/*!
    @brief This is an unsolicited message sent in response to a enhanced voice
    recognition indication received from the AG.
*/
typedef struct
{
    /*! The priority of the link. */
    hfp_link_priority priority;
    /*! The current status of the AG's voice recognition function. */
    hfp_evra_status    enable;
    /*! Bitmask that reflects the current state of the voice recognition engine
        on the AG. */
    hfp_evra_ag_state    state;
    /*! Unique ID of the current text as a hexadecimal string */
    uint16 textId;
    /*! Text type identifier */
    hfp_evra_audio_text    textType;
    /*! Text operation identifier */
    hfp_evra_text_operation    textOperation;
    /*! The size of the VR text (in bytes).*/
    uint16 size_vr_text;
    /*! Pointer to the VR Text. The client should not attempt to free this 
    ...
    uint8  vr_text[1];
} HFP_VOICE_RECOGNITION_EVRA_IND_T;
...
/*!
    @brief Control the voice recognition function on the AG, extended for EVRA feature

    @param priority The priority of the AG to perform this command on.
    
    @param Enable, disable or ready.  See hfp_evra_action

    Only hfp_primary_link and hfp_secondary_link are valid settings for the
    priority parameter.
*/
void HfpVoiceRecognitionEnableRequestEx(hfp_link_priority priority, hfp_evra_action action);
...
/*!
    @brief Provides information if hfp audio is connected or not for a
           given handset link.

    @param bd_addr address of the link to get the hfp audio connect state
           of this link.

    @returns TRUE if hfp audio connected, FALSE otherwise.
*/
bool HfpIsAudioConnected(const bdaddr* bd_addr);
...
/*!
    ...
    @param options Additional options to enable or disable super wide
           band speech.
...
/*!
    @brief Update the bitmask of super wide band support.

    This will be done when SLC is being created with the handset
    during remote initiated connection.

    @param addr Handset address for which swb support status needs to be updated.
           swp_support_status  TRUE If swb is supported for this handset or FALSE
           if swb is not supported for this handset.

*/
void HfpUpdateHandsetSwbSupportStatus(const bdaddr *addr, bool swp_support_status);
