...
bool AudioSources_IsAudioRouted(audio_source_t source)
{
    ...
    if ((interface != NULL) && (interface->IsAudioRouted))
    {
        is_available = interface->IsAudioRouted(source);
        DEBUG_LOG("AudioSources_IsAudioRouted source enum:audio_source_t:%d, available=%d", source, is_available);
    }
    return is_available;
}
...
source_status_t AudioSources_SetState(audio_source_t source, source_state_t state)
{
    ...
    DEBUG_LOG_FN_ENTRY("AudioSources_SetState enum:audio_source_t:%d, enum:source_state_t:%d", source, state);

    if ((interface != NULL) && (interface->SetState))
    {
        status = interface->SetState(source, state);
    }
    return status;
}
