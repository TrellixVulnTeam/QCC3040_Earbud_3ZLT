...
static void keySync_AddDeviceAttributes(bdaddr* bd_addr)
{
    ...
    BtDevice_AddSupportedProfiles((const bdaddr *)bd_addr, DEVICE_PROFILE_AVRCP|DEVICE_PROFILE_A2DP|DEVICE_PROFILE_HFP);
...
bool KeySync_IsDeviceInSync(device_t device)
{
    ...
    else if(type == DEVICE_TYPE_HANDSET)
    {
        ...
        if(Device_GetPropertyU16(device, device_property_flags, &flags))
        {
            bool in_sync = (flags & DEVICE_FLAGS_HANDSET_ADDRESS_FORWARD_REQD) != DEVICE_FLAGS_HANDSET_ADDRESS_FORWARD_REQD;
