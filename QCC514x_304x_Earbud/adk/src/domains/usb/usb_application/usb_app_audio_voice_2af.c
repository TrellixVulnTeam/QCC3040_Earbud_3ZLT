...
static const usb_audio_config_params_t usb_voice_config =
{
    .rev                     = USB_AUDIO_CLASS_REV_1,
    .volume_config.min_db    = -45,
    .volume_config.max_db    = 0,
...
static void usbAppAudioVoice2Af_ConfigDevice(usb_device_index_t dev_index)
{
    usb_result_t result;
...
static void usbAppAudioVoice2Af_Create(usb_device_index_t dev_index)
{
    usb_result_t result;

    DEBUG_LOG_INFO("usbAppAudioVoice2Af: Create");
    ...
    result = UsbDevice_RegisterConfig(dev_index,
                                      usbAppAudioVoice2Af_ConfigDevice);
    assert(result == USB_RESULT_OK);
...
static void usbAppAudioVoice2Af_Attach(usb_device_index_t dev_index)
{
    usb_result_t result;
...
static void usbAppAudioVoice2Af_Detach(usb_device_index_t dev_index)
{
    usb_result_t result;
...
static void usbAppAudioVoice2Af_Close(usb_device_index_t dev_index)
{
    UNUSED(dev_index);

    DEBUG_LOG_INFO("usbAppAudioVoice2Af: Close");

    /* nothing to clear */
}

const usb_app_interface_t usb_app_audio_voice_2af =
{
    .Create = usbAppAudioVoice2Af_Create,
    .Attach = usbAppAudioVoice2Af_Attach,
    .Detach = usbAppAudioVoice2Af_Detach,
    .Destroy = usbAppAudioVoice2Af_Close
};
