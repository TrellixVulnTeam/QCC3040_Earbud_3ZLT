...
LOGGING_PRESERVE_MESSAGE_TYPE(leakthrough_msg_t)
...
ASSERT_MESSAGE_GROUP_NOT_OVERFLOWED(LEAKTHROUGH, LEAKTHROUGH_MESSAGE_END)
...
static void aecLeakthrough_msg_handler(Task task, MessageId id, Message msg)
{
    ...
    switch(id)
    {
        ...
        default:
            DEBUG_LOG("aecLeakthrough_msg_handler: Event not handled. MESSAGE:0x%x", id);
            break;
    }
}

/*! \brief Notify Leakthrough state update to registered clients. */
static void aecLeakthrough_MsgRegisteredClientsOnStateUpdate(bool enable)
{
    leakthroughTaskData *theLeakThrough = AecLeakthrough_GetTaskData();
    ...
    if(theLeakThrough->client_tasks)
    {
        if(enable)
        {
            MESSAGE_MAKE(ind, LEAKTHROUGH_UPDATE_STATE_IND_T);
            ind->state = ENABLED;
            ind->mode = AecLeakthrough_GetMode();
            TaskList_MessageSend(theLeakThrough->client_tasks, LEAKTHROUGH_UPDATE_STATE_IND, ind);
        }
        else
        {
            MESSAGE_MAKE(ind, LEAKTHROUGH_UPDATE_STATE_IND_T);
...
/*! \brief Notify Leakthrough mode update to registered clients. */
static void aecLeakthrough_MsgRegisteredClientsOnModeUpdate(void)
{
    leakthroughTaskData *theLeakThrough = AecLeakthrough_GetTaskData();
     /* Check if any of the clients registered */
    if(theLeakThrough->client_tasks)
    {
        MESSAGE_MAKE(ind, LEAKTHROUGH_UPDATE_MODE_IND_T);
        ind->state = AecLeakthrough_IsLeakthroughEnabled();
        ind->mode = AecLeakthrough_GetMode();
        TaskList_MessageSend(theLeakThrough->client_tasks, LEAKTHROUGH_UPDATE_MODE_IND, ind);
    }
}
...
{
    leakthroughTaskData *theLeakThrough = AecLeakthrough_GetTaskData();
    DEBUG_LOG("AecLeakthrough_IsLeakthroughEnabled: %d", theLeakThrough->leakthrough_status);
...
void AecLeakthrough_Enable(void)
{
    ...
    if(!AecLeakthrough_IsLeakthroughEnabled())
    {
        ...
        theLeakThrough->leakthrough_status = ENABLED;
        Kymera_EnableLeakthrough();
        /*! Notify Leakthrough state update on enable and disable event to the registered clients */
        aecLeakthrough_MsgRegisteredClientsOnStateUpdate(ENABLED);
    }
    else
    {
...
void AecLeakthrough_Disable(void)
{
    ...
    if(AecLeakthrough_IsLeakthroughEnabled())
    {
        ...
        /*! Update the leak-through status */
        theLeakThrough->leakthrough_status = DISABLED;
        /*! Notify Leakthrough state update on enable and disable event to the registered clients */
        aecLeakthrough_MsgRegisteredClientsOnStateUpdate(DISABLED);
    }
    else
    {
...
void AecLeakthrough_SetMode(leakthrough_mode_t mode)
{
    ...
    if(AecLeakthrough_IsLeakthroughEnabled() && (mode < (MAX_NUM_LEAKTHROUGH_MODE)))
    {
        ...
        Kymera_LeakthroughUpdateMode(mode);

        /* Notify Leakthrough mode update to registered clients */
        aecLeakthrough_MsgRegisteredClientsOnModeUpdate();
    }
    else
    {
