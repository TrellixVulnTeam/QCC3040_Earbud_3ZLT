                                ...
                                const PbapcPullPhonebookParams *params)
{
    ...
    /*  valid requests are those with supported repository and pbap_current */
    if((repository & reps) || (repository == pbap_current))
    {
        /* make sure we don't request repositories that are not
        supported */
        repository &= reps;
