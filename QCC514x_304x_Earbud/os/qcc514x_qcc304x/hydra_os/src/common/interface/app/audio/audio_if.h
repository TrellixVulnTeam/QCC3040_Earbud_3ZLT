...
/*!
  @brief audio_anc_config_status used in \c audio_anc_response.
*/
typedef enum {
    AUDIO_ANC_STATUS_OK   = 0x00, /*!< Command sequence processed successfully  */
    AUDIO_ANC_INVALID_PAYLOAD     = 0x02, /*!<The payload wasn't recognised as a sequence of ANC commands or couldn't be processed at this time. */
    AUDIO_ANC_COMMAND_FAILED      = 0x05 /*!< One of the commands in the sequence failed. */
} audio_anc_config_status;

/*
 * ANC Configure status returned in \c AudioAncConfig().
 */
typedef struct audio_anc_response
{
    audio_anc_config_status status;    /*!< Error code. */
    uint16 count; /*!< Number of ANC parameters configured. */
    uint16 failed_key;   /*!< Key ID of the first failed ANC command in the sequence. */
}audio_anc_response;
