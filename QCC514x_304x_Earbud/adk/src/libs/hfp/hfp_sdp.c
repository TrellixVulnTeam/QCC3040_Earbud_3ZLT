    ...
    if(service->sdp_record_handle == 0 && !hfpGetVisibleServiceFromProfile(service->bitfields.profile))
    {
        if(!theHfp->busy_channel)
        {
            ...
            else if (supportedProfileIsHfp(service->bitfields.profile))
            {
                ...
                /* If features and RFC channel match use a constant record */
                if(features == HFP_DEFAULT_FEATURES)
                {
                ...
                if(!service_record)
                    service_record = hfpSdpRecordCreate(theHfp->busy_channel, HFP_DEFAULT_VERSION, features);
            }
            else
            {
...
void hfpShowServiceRecords(void)
{
    ...
    service = hfpGetServiceFromProfile(hfp_handsfree_profile);
    if (service)
    {
        hfpRegisterServiceRecord(service);
    }
