...
#include "nfc/nfc/nfc_faultids.h"
    ...
    /**
     * CPU1 has tried to access at the bottom 512b of BSS
     */
    FAULT_CPU1_RESTRICTED_ACCESS = 0x17,

    /**
     * UART rx has overflowed. This used to be a terminal panic.
     */
    FAULT_STREAMS_UART_BUFFER_OVERFLOW = 0x18,

    /**
     * An IPC message was received for a task that doesn't exist.
     * It's possible the message is a response for a task that was recently
     * deleted.
     * The diatribe is the client ID that the message was supposed to be
     * delivered to.
     */
    FAULT_IPC_MESSAGE_FOR_UNKNOWN_TASK = 0x19,

    /**
     * The flash header code has read an invalid (all 0xff) nonce.
     * Probably due to reading an unformatted upgrade bank.
     */
    FAULT_FLASH_HEADER_READ_INVALID_NONCE = 0x1a,
