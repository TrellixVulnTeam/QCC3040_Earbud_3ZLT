...
static void appAncTuningHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        case USB_DEVICE_CLASS_MSG_SPEAKER_SAMPLE_RATE_IND:
            DEBUG_LOG_VERBOSE("USB_DEVICE_CLASS_MSG_SPEAKER_SAMPLE_RATE_IND");
            break;

        case USB_DEVICE_CLASS_MSG_MIC_SAMPLE_RATE_IND:
            DEBUG_LOG_VERBOSE("USB_DEVICE_CLASS_MSG_MIC_SAMPLE_RATE_IND");
            break;

        case USB_DEVICE_CLASS_MSG_AUDIO_LEVELS_IND:
            DEBUG_LOG_VERBOSE("USB_DEVICE_CLASS_MSG_AUDIO_LEVELS_IND");
            break;

        default:
            DEBUG_LOG_VERBOSE("appAncTuningHandleMessage, id MESSAGE:0x%x", id);
            break;
    }
}
...
void appAncTuningEnable(bool enable)
{
    ...
    if (enable)
    {
        DEBUG_LOG_VERBOSE("appAncTuningEnable, enabling");
        if (AncStateManager_IsEnabled())
        {
            DEBUG_LOG_VERBOSE("appAncTuningEnable, disabling ANC");
    ...
    {
        DEBUG_LOG_VERBOSE("appAncTuningEnable, disabling");
