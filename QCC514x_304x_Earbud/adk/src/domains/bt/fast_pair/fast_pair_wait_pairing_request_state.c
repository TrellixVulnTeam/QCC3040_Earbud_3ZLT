...
static bool fastPair_SendKbPResponse(fast_pair_state_event_crypto_encrypt_args_t* args)
{
    ...
    {
        uint8 encr_additional_data_pname[ADDITIONAL_DATA_WITH_PNAME_ENCRYPTED_PKT_LEN];
        uint8 encr_additional_data_pname_len=0;
        ...
        {
            if(TRUE == fastPair_GetEncryptedAdditionalDataHavingPName(encr_additional_data_pname,&encr_additional_data_pname_len))
            {
                fastPair_SendFPAdditionalDataNotification(encr_additional_data_pname,encr_additional_data_pname_len);
            }
        }
