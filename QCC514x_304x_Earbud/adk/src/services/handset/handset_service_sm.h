...
typedef struct
{
    ...
    /*! Flag that this state machine could be for an unpaired device.
        This can a problem if the device is using resolvable random
        addresses (RRA), as the resolving list in the controller 
        cannot be updated if advertising or scanning. */
    ...
    /*! Flag indicates that prior to the disconnect request, a connection was not
        completely established with the handset, i.e. profiles were not connected. */
    bool connection_was_not_complete_at_disconnect_request;

} handset_service_state_machine_t;

#define FOR_EACH_HANDSET_SM(_sm) for(handset_service_state_machine_t *_sm = handset_service.state_machine;\
                                     _sm < &handset_service.state_machine[HANDSET_SERVICE_MAX_SM];\
                                     _sm++)
...
/*! \brief Check if any LE ACL is connected

    This function will check if a LE ACL is currently connected to the 
    handset represented by the given sm.

    \note This function does not check for any BREDR connections.
    \note An ACL will be counted if it is disconnecting.

    \param sm Handset state machine to check.

    \return TRUE if sm is valid and LE is connected; FALSE otherwise.
*/
...
/*! \brief Find out if any handsets could pair

    This function checks if there are any active handset state machines
    for a device that might pair and require an update to the resolving 
    list in the controller.

    \return TRUE if any handset might pair
*/
bool HandsetServiceSm_CouldDevicesPair(void);

/*! \brief Get the cached device from a state machine entry

    \param sm The state machine instance.

    \param device The device_t handle if cached handset_device handle is
