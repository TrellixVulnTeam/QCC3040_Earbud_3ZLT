...

{
    ...
    "layout": [
        ...
        ("apps_p1",         {"capacity": 272 * 4 * 1024,  "authenticate": False}),
        ...
        # The partition can hold: (current model size + headroom) + rafs FAT table = (65Kb + 4Kb) + 4Kb = 73Kb
        # => 76Kb (rounded to the nearest block) = 19 blocks
        ("ra_partition",    {"capacity":  20 * 4 * 1024,  "id": "VMdl"}),
        ("ro_cfg_fs",       {"capacity":  64 * 4 * 1024,  "authenticate": False}),
        ("ro_fs",           {"capacity": 340 * 4 * 1024,  "authenticate": False})
    ]
}
