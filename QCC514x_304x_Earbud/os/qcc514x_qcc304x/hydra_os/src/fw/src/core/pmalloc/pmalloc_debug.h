...
/**
  * Increase max. debug depending on pmalloc available on chip.
  * Current thinking is to leave the 'big' blocks untracked to  
  * save RAM as these should be easier to guess who has them. 
  * 
  * Note:The redistribution of unused RAM can alter this between builds.
  */

#define PMALLOC_MAX_DEBUG_BLOCKS (460)   /* Generic value for P1 */
...
#define PMALLOC_SET_LENGTH_ATOMIC(n,s) \
    do                              \
    {                               \
        BLOCK_INTERRUPTS();         \
        PMALLOC_SET_LENGTH(n,s);    \
        UNBLOCK_INTERRUPTS();       \
    } while(0)
...
#define PMALLOC_SET_LENGTH_ATOMIC(n,s) PMALLOC_SET_LENGTH(n,s)
#endif /* __KALIMBA__ */
