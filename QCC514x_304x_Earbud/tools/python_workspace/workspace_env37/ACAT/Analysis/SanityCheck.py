...
VARIABLE_DEPENDENCIES = {
    ...
    'one_strict': (('$_panic_data', '$_panic_data1'),)
}
ENUM_DEPENDENCIES = {
...
class SanityCheck(Analysis.Analysis):
    ...
    def get_panic_time(self):
        ...
        # has these bits set.
        time_arr = panic_var['last_time'].value
        if Arch.kal_arch == 4:
            # 32 bit
            panic_time = time_arr
        else:
    ...
    def get_panic_data(self):
        """Returns a variable containing the panic data.

        This is the current format of the panic_info stuct containing
        panid_data. It's obviously subject to change in future.

        $_panic_data
            $_panic_data.last_id
            $_panic_data.last_arg
            $_panic_data.last_time (2 words)
        """
        if self.chipdata.processor == 0:
            return self.chipdata.get_var_strict('$_panic_data')

        return self.chipdata.get_var_strict('$_panic_data1')
    ...
    def analyse_firmware_id(self, id_mismatch_allowed=None, verbose=True):
        ...
        Args:
            ...
            verbose (bool): Prints the info message at the end of the
                call.
        """
    ...
    def analyse_panic_state(self):
        ...
        """
        panic_data = self.get_panic_data()
        ...
        #    faults.
        panic_id = panic_data['last_id'].value
        panic_diatribe = panic_data['last_arg'].value

        self.formatter.section_start('Panic status')
        if self.chip_has_panicked() and preserve_panic_valid:
            # Struct is initialized to 0, so unless a fault has occurred 
            # the fault_id will most probably be invalid
            try:
        ...
        fault_analysis.analyse_fault_data_struct()
