...
/*! \brief resolve address if possible
    \param      tpaddr           Address to resolve
    \param[out] resolved_tpaddr  Pointer to resolved address 
    Resolves the supplied address. If the address is not BLE and Random
    or cannot be resolved, then the supplied address is copied to the
    resolved_tpaddr */
...
/*! \brief Get the connection low power state.
    ...
    If either parameter is NULL, the function will return FALSE
...
/*! \brief Set the connection low power state.
    ...
    If connection is NULL no setting will be changed
...
/*! \brief Find the next active connection

    Passed the last connection returned, attempts to find another active connection
    after the one passed.

    \param[in] conn The connection to start at.
    \param transport_mask Set to cm_transport_all to check all connections, or 
           cm_transport_bredr/ble to only look for connections on specific transport

    \return The next connection that is not in a disconnected state or NULL if
            not found.
 */
