                                ...
                                PBAPC_INT_GET_VCARD_T *msg )
{
    ...
    if (state->L2CAP_conn == TRUE)
        ObexSetSrm(state->handle, TRUE);
    ...
    if(state->L2CAP_conn == TRUE)
    {
        ...
        if( ok )
        {
            ...
            src = StreamRegionSource( obj, 0x01 );
            ok &= (src &&
              ObexObjAddOneByteHdr(state->handle, obex_SRM_hdr, 0x01));/*Send SRM enabe request to the server*/
                                    ...
                                    PBAPC_INT_GET_VCARD_LIST_T *msg )
{
    ...
    if (state->L2CAP_conn == TRUE)
    {
        ...
        if( ok )
        {
           ...
           src = StreamRegionSource( obj, 0x01 );
           ok &= (src &&
              ObexObjAddOneByteHdr(state->handle, obex_SRM_hdr, 0x01));
...
void pbapcIntHandler( Task task, MessageId id, Message message)
{
    ...
    switch ( id )
    {
        ...
        case PBAPC_INT_GET_CONTINUE:
            ...
            break;
        default:
            PBAPC_DEBUG(("Unexpected id - MESSAGE:pbapc_internal_message_t:%x\n", id));
            break;
    }
}
