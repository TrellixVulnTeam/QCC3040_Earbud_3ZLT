...
typedef enum
{
    ...
    IPC_SIGNAL_ID_VM_READ_SECURITY =                                         76,
    IPC_SIGNAL_ID_VM_READ_SECURITY_RSP =                                     77,
    IPC_SIGNAL_ID_VM_ENABLE_SECURITY =                                       78,
    IPC_SIGNAL_ID_VM_ENABLE_SECURITY_RSP =                                   79,
    IPC_SIGNAL_ID_PIO_SET_FUNC_CHARGER_COMMS =                               80,
#include "ipc/ipc_bitserial_signal.h"
#include "ipc/gen/ipc_trap_api_signals.h"
} IPC_SIGNAL_ID;


/**
 * A type for storing a unique identifier for a specific IPC client.
 * E.g. A value identifying a FreeRTOS task.
 */
typedef uint8 IPC_CLIENT_ID;
#define IPC_CLIENT_ID_NULL ((IPC_CLIENT_ID)0)

/**
 * A type for storing the priority of the IPC message.
 * This needs to be large enough to store all FreeRTOS priorities defined by
 * configMAX_PRIORITIES.
 * tskIDLE_PRIORITY is always the lowest priority and defined as 0.
 * Higher numbers imply higher priority.
 */
typedef uint8 IPC_CLIENT_PRIORITY;
...
typedef struct
{
    /** The signal ID, determines the type of message this is. */
    IPC_SIGNAL_ID id;
#ifdef IPC_ADD_TIMESTAMPS
    /** An optional microsecond timestamp useful for profiling. */
    uint32 timestamp_us;
#endif
#if IPC_PROTOCOL_ID == 2
    /** The length of the entire message including the header in bytes. */
    uint16 length_bytes;
    /** A value indicating which Task has sent this message, for P1 to P0
        messages, or which Task a response is for, for P0 to P1 messages.
        For P0 messages that aren't responses this value is unused. */
    IPC_CLIENT_ID client;
    /** The priority of this message, a higher value implies a higher priority.
        Only used for P1 to P0 messages. */
    IPC_CLIENT_PRIORITY priority;
#endif
} IPC_HEADER;
...
typedef enum
{
    IPC_PIO_MUX_CURATOR     = SYSTEM_BUS_CURATOR,
    IPC_PIO_MUX_HOST        = SYSTEM_BUS_HOST_SYS,
    IPC_PIO_MUX_BT          = SYSTEM_BUS_BT_SYS,
    IPC_PIO_MUX_AUDIO       = SYSTEM_BUS_AUDIO_SYS,
    IPC_PIO_MUX_APPS        = SYSTEM_BUS_APPS_SYS,
    IPC_PIO_MUX_LED_CTRL    = SYSTEM_BUS_NUM_SUBSYSTEMS
} IPC_PIO_MUX_ID;
...
/**
 * DIR FROM_P1 Request P0 to read security bits
 */
typedef struct
{
    IPC_HEADER header; /**< id = \c IPC_SIGNAL_ID_VM_READ_SECURITY */
    uint16 read;
} IPC_VM_READ_SECURITY;

/**
 * DIR FROM_P1 Request P0 to enable security bits
 */
typedef struct
{
    IPC_HEADER header; /**< id = \c IPC_SIGNAL_ID_VM_ENABLE_SECURITY */
    uint16 enable;
} IPC_VM_ENABLE_SECURITY;

#include "ipc/ipc_bitserial_prim.h"
