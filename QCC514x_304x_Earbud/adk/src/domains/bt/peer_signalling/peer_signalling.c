...
ASSERT_MESSAGE_GROUP_NOT_OVERFLOWED(PEER_SIG, PEER_SIG_MESSAGE_END)

#endif

/* Make the type used for message IDs available in debug tools */
LOGGING_PRESERVE_MESSAGE_ENUM(peer_signalling_messages)
LOGGING_PRESERVE_MESSAGE_TYPE(PEER_SIG_INTERNAL_MSG)
ASSERT_INTERNAL_MESSAGES_NOT_OVERFLOWED(PEER_SIG_INTERNAL_MESSAGE_END)
...
static void appPeerSigEnterConnected(void)
{
    ...
    AancQuietMode_SetWallClock(sink);
...
static void appPeerSigExitConnected(void)
{
    ...
    AancQuietMode_SetWallClock((Sink)0);
...
static void appPeerSigEnterDisconnected(void)
{
    ...
    /* Discard any data in the source stream */
   if (!StreamConnectDispose(peer_sig->link_source))
   {
        DEBUG_LOG("appPeerSigEnterDisconnected StreamConnectDispose failed, force source empty");
        SourceEmpty(peer_sig->link_source);   
   }
   peer_sig->link_source = NULL;
   peer_sig->link_sink = NULL;
...
static void appPeerSigExitInitialising(void)
{
    ...
    MessageSend(SystemState_GetTransitionTask(), PEER_SIG_INIT_CFM, NULL);
}
...
{
    UNUSED(id);
    DEBUG_LOG("appPeerSigError, state %u, MESSAGE:0x%x", appPeerSigGetState(), id);
    Panic();
}
...
static void appPeerSigHandleInternalStartupRequest(PEER_SIG_INTERNAL_STARTUP_REQ_T *req)
{
    ...
    DEBUG_LOG("appPeerSigHandleInternalStartupRequest, state %u", appPeerSigGetState());
        ...
        case PEER_SIG_STATE_CONNECTING_SDP_SEARCH:
            {
                ...
                /* There is no L2CAP connection so go directly to disconnected state */
...
void appPeerSigTerminateSdpPrimitive(void)
{
    peerSigTaskData* peer_sig = PeerSigGetTaskData();
    DEBUG_LOG("appPeerSigTerminateSdpPrimitive, state %u", appPeerSigGetState());
...
static void appPeerSigHandleCmL2caRegisterCfm(uint8 status, uint16 psm)
{	
    DEBUG_LOG("appPeerSigHandleClL2capRegisterCfm, status %u, psm %u", status, psm);
    ...
    if (success == status)
    {
        ...
        appSdpSetPeerSigPsm(record, psm);
...
static void appPeerSigHandleClSdpRegisterCfm(uint8 status)
{	
    DEBUG_LOG("appPeerSigHandleClSdpRegisterCfm, status %d", status);
    PanicFalse(appPeerSigGetState() == PEER_SIG_STATE_INITIALISING);

    if (status == sds_status_success)
    {
...
static void appPeerSigHandleClSdpServiceSearchAttributeCfm(const CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM_T *cfm)
{
    ...
    switch (appPeerSigGetState())
    {
        ...
        case PEER_SIG_STATE_DISCONNECTED:
        {
            DEBUG_LOG("appPeerSigHandleClSdpServiceSearchAttributeCfm, state machine is disconnected state to client calling disconnect request");
        }
        break;
    ...
    switch (appPeerSigGetState())
    {
        ...
        case PEER_SIG_STATE_CONNECTING_REMOTE:
        {
            ...
            if (l2cap_connect_success == cfm->status)
            {
                ...
                PanicFalse(SourceConfigure(peer_sig->link_source, VM_SOURCE_MESSAGES, VM_MESSAGES_SOME));
...
static void appPeerSigHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        {
            CL_L2CAP_REGISTER_CFM_T * cfm = (CL_L2CAP_REGISTER_CFM_T *)message;
            appPeerSigHandleCmL2caRegisterCfm(cfm->status, cfm->psm);
            break;
        }

        case CL_SDP_REGISTER_CFM:
            appPeerSigHandleClSdpRegisterCfm(((const CL_SDP_REGISTER_CFM_T *)message)->status);
            break;
        ...
        default:
            DEBUG_LOG("appPeerSigHandleMessage. Unhandled message MESSAGE:PEER_SIG_INTERNAL_MSG:0x%04x", id);
            break;
    }
}
...
bool appPeerSigCheckForPendingMarshalledMsg(void)
{
    ...
    for (channel = 0; channel < PEER_SIG_MSG_CHANNEL_MAX; channel++)
    {
        ...
        {
            msg_pending = TRUE;
                DEBUG_LOG("appPeerSigCheckForPendingMarshalledMsg message is pending");
            break;
        }
    }
...
/*!
    \brief Get the number of messages pending to transfer over PeerSig.

    \return Number of pending messages
*/
uint8 appPeerSigGetPendingMarshalledMsgNum(void)
{
    peerSigMsgChannel channel;
    uint8 msg_pending_num = 0;

    DEBUG_LOG("appPeerSigGetPendingMarshalledMsgNum");

    /* Sum up the pending messages across all channel tasks*/
    for (channel = 0; channel < PEER_SIG_MSG_CHANNEL_MAX; channel++)
    {
        marshal_msg_channel_data_t *mmcd = appPeerSigGetChannelData(channel);
        if (mmcd->client_task)
        {
            msg_pending_num += MessagesPendingForTask(&mmcd->channel_task, NULL);
        }
    }
    DEBUG_LOG("appPeerSigGetPendingMarshalledMsgNum, total pending_messages:%d", msg_pending_num);
    return msg_pending_num;
}

/*!
    \brief Calculate PeerSig tranmission delay based on peer link policy state

    \return time delay in micro seconds
*/
uint32 appPeerGetPeerRelayDelayBasedOnSystemContext(void)
{
    ...
    {
        if (Av_IsA2dpSinkStreaming() || HfpProfile_IsScoActive())
        {
