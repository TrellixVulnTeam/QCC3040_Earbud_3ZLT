...
typedef enum
{
    ...
    obex_SRM_hdr    = 0x97,     /**< The Single Response Mode header */
    obex_SRMP_hdr   = 0x98      /**< The Single Response Mode Param header */
} ObexSeqHeaderId;
...
/*!
    ...
    @param Channel RFCOMM Server channel or L2CAP PSM to use for the connection.
...
uint32 ObexObjExtractByteHdrValue( ObexByteHeaderId hdrId,
                                   ...
                                   Source src );
/*!
    @brief Extract 1 byte header value

    @param hdrId Header Id to be extracted.
    @param srcLen Length of the source
    @param src The Source buffer

    Extract 1 byte header from the source.
*/
