/*****************************************************************************
            ...
            (c) Qualcomm Technologies International, Ltd. 2021
...
/*******************************************************************************
  ...
  DESCRIPTION
    AANC operator configuration messages

 VALUES
    SET_AANC_STATIC_GAIN    - Sets the static gain for AANC
    SET_AANC_PLANT_COEFFS   - Sets the plant model coefficients for AANC
    SET_AANC_CONTROL_COEFFS - Sets the control model coefficients for AANC

*******************************************************************************/
typedef enum
{
    OPMSG_AANC_ID_SET_AANC_STATIC_GAIN = 0x0001,
    OPMSG_AANC_ID_SET_AANC_PLANT_COEFFS = 0x0002,
    OPMSG_AANC_ID_SET_AANC_CONTROL_COEFFS = 0x0003
} OPMSG_AANC_ID;
/*******************************************************************************
...
/*******************************************************************************
 ...
 VALUES
    SET_OUTPUT_BLOCK_SIZE         - AEC REFERENCE inherits block size from
                                    connected operator, if connected operator
                                    doesn't have block size it will work in
                                    non-block based mode in which the latency
                                    between REF and mic output isn't tightly
                                    controlled. This message will set the block
                                    size directly and if set have precedence to
                                    inherited block size.
...
typedef enum
{
    OPMSG_AEC_REFERENCE_ID_SET_OUTPUT_BLOCK_SIZE = 0x0006,
...
/*******************************************************************************
  ...
  NAME
    OPMSG_APTX_ADAPTIVE_DEC_ID

  DESCRIPTION
    Aptx Adaptive Decoder configuration messages

 VALUES
    SET_CHANNEL_EXTRACT_MODE - This message is used for setting the mode that
                               can be used to extract a single channel or
                               downmixed channel out of the decoder
    SET_INTERNAL_ADJUST      - Internal adjust of the latency of the decoder

*******************************************************************************/
typedef enum
{
    OPMSG_APTX_ADAPTIVE_DEC_ID_SET_CHANNEL_EXTRACT_MODE = 0x0001,
    OPMSG_APTX_ADAPTIVE_DEC_ID_SET_INTERNAL_ADJUST = 0x0002
} OPMSG_APTX_ADAPTIVE_DEC_ID;
/*******************************************************************************
...
/*******************************************************************************
  ...
  NAME
    OPMSG_APTX_CLASSIC_DEC_ID

  DESCRIPTION
    Aptx Classic Decoder configuration messages

 VALUES
    SET_SILENCE_MODE - Used to set the decoder forced silence mode

*******************************************************************************/
typedef enum
{
    OPMSG_APTX_CLASSIC_DEC_ID_SET_SILENCE_MODE = 0x0001
} OPMSG_APTX_CLASSIC_DEC_ID;
/*******************************************************************************
 ...
 VALUES
    ...
    OPMSG_COMMON_SET_RATE_ADJUST_PASSTHROUGH_MODE
                   - Message to set/clear the passthrough mode of a standalone
                     ...
                     used by the framework only, and shall not be used directly
                     by user.
    OPMSG_COMMON_SET_TTP_STATE
                   - Set the new time-to-play state for the operator.
    OPMSG_COMMON_ADJUST_TTP_TIMESTAMP
                   - Adjust the time to play timestamp value calculation using
                     this signed value.
    OPMSG_COMMON_SET_TTP_SPADJ
                   - Set the sample rate adjustment used for time to play
                     calculation.
    OPMSG_COMMON_REINIT_ALGORITHM
                   - Reinitialize all algorithm specific data in an operator to
                     their initial values
    OPMSG_COMMON_SET_BACK_KICK_THRESHOLD
                   - Set the input buffer level at which a capability kicks
                     backwards. The message consists of 2 words: signed
                     threshold value, and sink terminal bit mask.
    OPMSG_COMMON_RESYNC_TTP
                   - Resync the time-to-play state for the operator
...
typedef enum
{
    ...
    OPMSG_COMMON_SET_TTP_STATE = 0x201A,
    OPMSG_COMMON_ADJUST_TTP_TIMESTAMP = 0x201B,
    OPMSG_COMMON_SET_TTP_SPADJ = 0x201C,
    OPMSG_COMMON_REINIT_ALGORITHM = 0x201D,
    OPMSG_COMMON_SET_BACK_KICK_THRESHOLD = 0x2020,
    OPMSG_COMMON_RESYNC_TTP = 0x2022
} OPMSG_COMMON_ID;
/*******************************************************************************
 ...
 VALUES
    OPMSG_OP_TERMINAL_SHIFT
                   - Key for passing cbops parameter shift.
...
typedef enum
{
    OPMSG_OP_TERMINAL_SHIFT = 0x0001,
...
/*******************************************************************************
  ...
  DESCRIPTION
    GVA configuration messages

 VALUES
    GET_HOTWORD_LIBRARY_VERSION - Get's the GVA's underlying hotword library's
                                  version number

*******************************************************************************/
typedef enum
{
    OPMSG_GVA_ID_GET_HOTWORD_LIBRARY_VERSION = 0x000A
} OPMSG_GVA_ID;
/*******************************************************************************
...
/*******************************************************************************
 ...
 VALUES
    ...
    SET_LATENCY_CHANGE_NOTIFICATION - Enables latency change nofitication for
                                      aptX adaptive
    SET_TTP_NOTIFICATION            - Enables time to play notifictaions.
...
typedef enum
{
    ...
    OPMSG_RTP_DECODE_ID_SET_TTP_NOTIFICATION = 0x000A
} OPMSG_RTP_DECODE_ID;
/*******************************************************************************
 ...
 VALUES
    VA_TRIGGER                  - This message is sent 1) by a VAD operator to
                                  its client when the VAD operator detects audio
                                  activity, 2) by a VTD operator to its client
                                  when the VTD operator detects a trigger
                                  phrase, 3) by a VCI operator to its client
                                  when the VCI operator detects a voice command.
    VA_NEGATIVE_TRIGGER         - This message is sent 1) by a VTD operator to
                                  its client when the VTD operator detects NO
                                  trigger phrase, 2) by a VCI operator to its
                                  client when the VCI operator detects NO voice
                                  command.
    LATENCY_CHANGED             - This  message is sent by the RTP DECODE
                                  operator whenever there is an latency change
                                  due to an aptX adaptive source change.
    TTP_RTP_SEQ                 - This  message is sent by the RTP DECODE
                                  operator to notify the time to play values and
                                  the RTP sequence number.
    AANC_TRIGGER                - This message is sent by the AANC capability to
                                  provide an event based indication in
                                  simulation.
    AANC_EVENT_TRIGGER          - This message is sent by the AANC capability to
                                  provide event based logging to apps
    AANC_EVENT_NEGATIVE_TRIGGER - This message is sent by the AANC capability to
                                  provide event based logging to apps
    SOURCE_SYNC_STALL_CHANGED   - This message is sent by the Source Sync
                                  capability for sink stall state changes
                                  between stalled and other states
    EFT_EVENT_TRIGGER           - This message is sent by the EFT capability to
                                  indicate earbud fit quality to apps
    FADEOUT_DONE                - For more information see section 4.2 in
                                  BlueCore DSPManager Specification document
...
typedef enum
{
    ...
    OPMSG_REPLY_ID_TTP_RTP_SEQ = 0x0006,
    OPMSG_REPLY_ID_AANC_TRIGGER = 0x0007,
    OPMSG_REPLY_ID_AANC_EVENT_TRIGGER = 0x0008,
    OPMSG_REPLY_ID_AANC_EVENT_NEGATIVE_TRIGGER = 0x0009,
    OPMSG_REPLY_ID_SOURCE_SYNC_STALL_CHANGED = 0x000a,
    OPMSG_REPLY_ID_EFT_EVENT_TRIGGER = 0x000b,
...
/*******************************************************************************
 ...
 VALUES
    ...
    SET_BLOCK_SIZE     - Select block size to use with SPC#1 -> aptx passthrough
...
typedef enum
{
    ...
    OPMSG_SPC_ID_SET_BLOCK_SIZE = 0x0006
} OPMSG_SPC_ID;
/*******************************************************************************
 ...
 VALUES
    PASSTHROUGH        - Copy all data from the input 0 to the output 0.
    CONSUMER           - Discard all data from the input.
    PASSTHROUGH1       - Copy all data from the input 1 to the output 1.
    TAGSYNC_0          - Copy all data from the SPC#1 input 0 to the SPC#1
                         output 0 (with tag synchronisation).
    TAGSYNC_1          - Copy all data from the SPC#1 input 1 to the SPC#1
                         output 1 (with tag synchronisation).
    TAGSYNC_DUAL       - Copy all data from the input SPC#1 0&1 to the output
                         SPC#1 0&1 (with tag synchronisation).
    SPC2_DATASYNC_0    - Copy all data from the SPC#2 input 0 to the SPC#2
                         output 0 (with data synchronisation).
    SPC2_DATASYNC_1    - Copy all data from the SPC#2 input 1 to the SPC#2
                         output 1 (with data synchronisation).
    SPC2_DATASYNC_DUAL - Copy all data from the SPC#2 input 0&1 to the SPC#2
                         output 0&1 (with data synchronisation).
...
typedef enum
{
    ...
    OPMSG_SPC_MODE_PASSTHROUGH1 = 0x2,
    OPMSG_SPC_MODE_TAGSYNC_0 = 0x3,
    OPMSG_SPC_MODE_TAGSYNC_1 = 0x4,
    OPMSG_SPC_MODE_TAGSYNC_DUAL = 0x5,
    OPMSG_SPC_MODE_SPC2_DATASYNC_0 = 0x6,
    OPMSG_SPC_MODE_SPC2_DATASYNC_1 = 0x7,
    OPMSG_SPC_MODE_SPC2_DATASYNC_DUAL = 0x8
} OPMSG_SPC_MODE;
/*******************************************************************************
...
/*******************************************************************************
 ...
 VALUES
    ...
    SET_STALL_NOTIFICATION_ENABLE - Enable the SOURCE_SYNC_STALL_CHANGED
                                    unsolicited operator message
    SET_TRACE_ENABLE              -
...
typedef enum
{
    ...
    OPMSG_SRC_SYNC_ID_SET_STALL_NOTIFICATION_ENABLE = 0x0005,
...
typedef enum
{
    OPMSG_SWBS_DEC_ID_FRAME_COUNTS = 0x0001,
    OPMSG_SWBS_DEC_ID_FORCE_PLC_OFF = 0x0002,
    OPMSG_SWBS_DEC_ID_CODEC_MODE = 0x0003,
    OPMSG_SWBS_DEC_ID_PROC_TIME_MEASURE = 0x000C,
    OPMSG_SWBS_DEC_ID_SET_FROM_AIR_INFO = 0x00FE
} OPMSG_SWBS_DEC_ID;
/*******************************************************************************
...
typedef enum
{
    OPMSG_SWBS_ENC_ID_CODEC_MODE = 0x0001,
    OPMSG_SWBS_ENC_ID_SET_TO_AIR_INFO = 0x00FD,
    OPMSG_SWBS_ENC_ID_FORWARD_ALL_KICKS = 0x00FF
} OPMSG_SWBS_ENC_ID;
/*******************************************************************************
...
/*******************************************************************************
  ...
  DESCRIPTION
    TEST_VA configuration messages.

 VALUES
    SET_QID - Set the queue id.
    FWD_ME  - Forward to the queue.

*******************************************************************************/
typedef enum
{
    OPMSG_TEST_VA_ID_SET_QID = 0x0F01,
    OPMSG_TEST_VA_ID_FWD_ME = 0x0F02
} OPMSG_TEST_VA_ID;
/*******************************************************************************
...
/*******************************************************************************
 ...
 VALUES
    ...
    SET_FRAME_SIZE - Sets the frame size for the VAD

*******************************************************************************/
typedef enum
{
    OPMSG_VAD_ID_MODE_CHANGE = 0x0001,
    OPMSG_VAD_ID_SET_FRAME_SIZE = 0x0002
} OPMSG_VAD_ID;
/*******************************************************************************
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Common_Back_Kick_Mode

  DESCRIPTION

 VALUES
    EDGE  - Back kick once when the buffer level falls below the threshold
    LEVEL - Back kick whenever the buffer level is below the threshold

*******************************************************************************/
typedef enum
{
    OPMSG_COMMON_BACK_KICK_MODE_EDGE = 0x0000,
    OPMSG_COMMON_BACK_KICK_MODE_LEVEL = 0x0001
} OPMSG_COMMON_BACK_KICK_MODE;
/*******************************************************************************
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Source_Sync_Sink_Stall_State

  DESCRIPTION
    Enumerates a stalled or non-stalled state

 VALUES
    NOT_STALLED - Any sink state other than stalled
    STALLED     - Sink state stalled

*******************************************************************************/
typedef enum
{
    OPMSG_SOURCE_SYNC_SINK_STALL_STATE_NOT_STALLED = 0,
    OPMSG_SOURCE_SYNC_SINK_STALL_STATE_STALLED = 1
} OPMSG_SOURCE_SYNC_SINK_STALL_STATE;
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Common_Msg_Resync_Ttp

  DESCRIPTION
    Operator common message for OPMSG_COMMON_RESYNC_TTP
...
/* The following macros take OPMSG_COMMON_MSG_RESYNC_TTP *opmsg_common_msg_resync_ttp_ptr */
#define OPMSG_COMMON_MSG_RESYNC_TTP_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_COMMON_MSG_RESYNC_TTP_MESSAGE_ID_GET(opmsg_common_msg_resync_ttp_ptr) ((OPMSG_COMMON_ID)(opmsg_common_msg_resync_ttp_ptr)->_data[0])
#define OPMSG_COMMON_MSG_RESYNC_TTP_MESSAGE_ID_SET(opmsg_common_msg_resync_ttp_ptr, message_id) ((opmsg_common_msg_resync_ttp_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_COMMON_MSG_RESYNC_TTP_WORD_SIZE (1)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_COMMON_MSG_RESYNC_TTP_CREATE(message_id) \
    (uint16)(message_id)
#define OPMSG_COMMON_MSG_RESYNC_TTP_PACK(opmsg_common_msg_resync_ttp_ptr, message_id) \
    do { \
        (opmsg_common_msg_resync_ttp_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
    } while (0)

#define OPMSG_COMMON_MSG_RESYNC_TTP_MARSHALL(addr, opmsg_common_msg_resync_ttp_ptr) memcpy((void *)(addr), (void *)(opmsg_common_msg_resync_ttp_ptr), 1)
#define OPMSG_COMMON_MSG_RESYNC_TTP_UNMARSHALL(addr, opmsg_common_msg_resync_ttp_ptr) memcpy((void *)(opmsg_common_msg_resync_ttp_ptr), (void *)(addr), 1)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Source_Sync_Set_Stall_Notification_Enable

  DESCRIPTION
    Enable or disable notifications about changes of sink state between
    stalled and other states.

  MEMBERS
    message_id - message id
    enable     - Enable the stall changed notification
...
/* The following macros take OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE *opmsg_source_sync_set_stall_notification_enable_ptr */
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_MESSAGE_ID_GET(opmsg_source_sync_set_stall_notification_enable_ptr) ((OPMSG_SRC_SYNC_ID)(opmsg_source_sync_set_stall_notification_enable_ptr)->_data[0])
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_MESSAGE_ID_SET(opmsg_source_sync_set_stall_notification_enable_ptr, message_id) ((opmsg_source_sync_set_stall_notification_enable_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_ENABLE_WORD_OFFSET (1)
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_ENABLE_GET(opmsg_source_sync_set_stall_notification_enable_ptr) ((BOOL)(((opmsg_source_sync_set_stall_notification_enable_ptr)->_data[1] & 0x0)))
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_ENABLE_SET(opmsg_source_sync_set_stall_notification_enable_ptr, enable) ((opmsg_source_sync_set_stall_notification_enable_ptr)->_data[1] =  \
                                                                                                                                     (uint16)(((opmsg_source_sync_set_stall_notification_enable_ptr)->_data[1] & ~0x0) | (((enable)) & 0x0)))
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_WORD_SIZE (1)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_CREATE(message_id, enable) \
    (uint16)(message_id), \
    (uint16)(((enable)) & 0x0)
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_PACK(opmsg_source_sync_set_stall_notification_enable_ptr, message_id, enable) \
    do { \
        (opmsg_source_sync_set_stall_notification_enable_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_source_sync_set_stall_notification_enable_ptr)->_data[1] = (uint16)((uint16)(((enable)) & 0x0)); \
    } while (0)

#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_MARSHALL(addr, opmsg_source_sync_set_stall_notification_enable_ptr) memcpy((void *)(addr), (void *)(opmsg_source_sync_set_stall_notification_enable_ptr), 1)
#define OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE_UNMARSHALL(addr, opmsg_source_sync_set_stall_notification_enable_ptr) memcpy((void *)(opmsg_source_sync_set_stall_notification_enable_ptr), (void *)(addr), 1)
...
/*******************************************************************************
  ...
  MEMBERS
    ...
    is_two_mic      - number of ANC mics per instance is two mic (TRUE) or one
                      (MIC)
    is_parallel_anc - Two ANC instances are used for parallel ANC (TRUE) or only
                      one ANC instance is used
...
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_IS_PARALLEL_ANC_WORD_OFFSET (3)
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_IS_PARALLEL_ANC_GET(opmsg_anc_tuning_frontend_config_ptr) ((opmsg_anc_tuning_frontend_config_ptr)->_data[3])
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_IS_PARALLEL_ANC_SET(opmsg_anc_tuning_frontend_config_ptr, is_parallel_anc) ((opmsg_anc_tuning_frontend_config_ptr)->_data[3] = (uint16)(is_parallel_anc))
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_WORD_SIZE (4)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_CREATE(message_id, is_stereo, is_two_mic, is_parallel_anc) \
    (uint16)(message_id), \
    (uint16)(is_stereo), \
    (uint16)(is_two_mic), \
    (uint16)(is_parallel_anc)
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_PACK(opmsg_anc_tuning_frontend_config_ptr, message_id, is_stereo, is_two_mic, is_parallel_anc) \
    do { \
        ...
        (opmsg_anc_tuning_frontend_config_ptr)->_data[3] = (uint16)((uint16)(is_parallel_anc)); \
    } while (0)

#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_MARSHALL(addr, opmsg_anc_tuning_frontend_config_ptr) memcpy((void *)(addr), (void *)(opmsg_anc_tuning_frontend_config_ptr), 4)
#define OPMSG_ANC_TUNING_FRONTEND_CONFIG_UNMARSHALL(addr, opmsg_anc_tuning_frontend_config_ptr) memcpy((void *)(opmsg_anc_tuning_frontend_config_ptr), (void *)(addr), 4)


/*******************************************************************************

  NAME
    OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION

  DESCRIPTION
    Get's the GVA's underlying hotword library's version number
...
/* The following macros take OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION *opmsg_gva_get_hotword_library_version_ptr */
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_MESSAGE_ID_GET(opmsg_gva_get_hotword_library_version_ptr) ((OPMSG_GVA_ID)(opmsg_gva_get_hotword_library_version_ptr)->_data[0])
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_MESSAGE_ID_SET(opmsg_gva_get_hotword_library_version_ptr, message_id) ((opmsg_gva_get_hotword_library_version_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_WORD_SIZE (1)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_CREATE(message_id) \
    (uint16)(message_id)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_PACK(opmsg_gva_get_hotword_library_version_ptr, message_id) \
    do { \
        (opmsg_gva_get_hotword_library_version_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
    } while (0)

#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_MARSHALL(addr, opmsg_gva_get_hotword_library_version_ptr) memcpy((void *)(addr), (void *)(opmsg_gva_get_hotword_library_version_ptr), 1)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_UNMARSHALL(addr, opmsg_gva_get_hotword_library_version_ptr) memcpy((void *)(opmsg_gva_get_hotword_library_version_ptr), (void *)(addr), 1)


/*******************************************************************************

  NAME
    OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP

  DESCRIPTION
    Operator response for the op message GET_HOTWORD_LIBRARY_VERSION

  MEMBERS
    message_id  - message id
    version_msw - Library version msw
    version_lsw - Library version lsw
...
/* The following macros take OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP *opmsg_gva_get_hotword_library_version_resp_ptr */
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_MESSAGE_ID_GET(opmsg_gva_get_hotword_library_version_resp_ptr) ((OPMSG_COMMON_ID)(opmsg_gva_get_hotword_library_version_resp_ptr)->_data[0])
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_MESSAGE_ID_SET(opmsg_gva_get_hotword_library_version_resp_ptr, message_id) ((opmsg_gva_get_hotword_library_version_resp_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_VERSION_MSW_WORD_OFFSET (1)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_VERSION_MSW_GET(opmsg_gva_get_hotword_library_version_resp_ptr) ((opmsg_gva_get_hotword_library_version_resp_ptr)->_data[1])
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_VERSION_MSW_SET(opmsg_gva_get_hotword_library_version_resp_ptr, version_msw) ((opmsg_gva_get_hotword_library_version_resp_ptr)->_data[1] = (uint16)(version_msw))
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_VERSION_LSW_WORD_OFFSET (2)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_VERSION_LSW_GET(opmsg_gva_get_hotword_library_version_resp_ptr) ((opmsg_gva_get_hotword_library_version_resp_ptr)->_data[2])
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_VERSION_LSW_SET(opmsg_gva_get_hotword_library_version_resp_ptr, version_lsw) ((opmsg_gva_get_hotword_library_version_resp_ptr)->_data[2] = (uint16)(version_lsw))
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_WORD_SIZE (3)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_CREATE(message_id, version_msw, version_lsw) \
    (uint16)(message_id), \
    (uint16)(version_msw), \
    (uint16)(version_lsw)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_PACK(opmsg_gva_get_hotword_library_version_resp_ptr, message_id, version_msw, version_lsw) \
    do { \
        (opmsg_gva_get_hotword_library_version_resp_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_gva_get_hotword_library_version_resp_ptr)->_data[1] = (uint16)((uint16)(version_msw)); \
        (opmsg_gva_get_hotword_library_version_resp_ptr)->_data[2] = (uint16)((uint16)(version_lsw)); \
    } while (0)

#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_MARSHALL(addr, opmsg_gva_get_hotword_library_version_resp_ptr) memcpy((void *)(addr), (void *)(opmsg_gva_get_hotword_library_version_resp_ptr), 3)
#define OPMSG_GVA_GET_HOTWORD_LIBRARY_VERSION_RESP_UNMARSHALL(addr, opmsg_gva_get_hotword_library_version_resp_ptr) memcpy((void *)(opmsg_gva_get_hotword_library_version_resp_ptr), (void *)(addr), 3)
...
/*******************************************************************************
  ...
  NAME
    OPMSG_SET_AANC_CONTROL_MODEL

  DESCRIPTION
    AANC set control model coefficients message

  MEMBERS
    message_id                  - Message ID
    quantity_denominator_coeffs - Number of denominator coefficients
    quantity_numerator_coeffs   - Number of numerator coefficients
    denominator_coeffs          - Control denominator (MSB, LSB) coefficients
                                  a1-aN
    numerator_coeffs            - Control numerator (MSB, LSB) coefficients
                                  b0-bN
...
/* The following macros take OPMSG_SET_AANC_CONTROL_MODEL *opmsg_set_aanc_control_model_ptr */
#define OPMSG_SET_AANC_CONTROL_MODEL_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SET_AANC_CONTROL_MODEL_MESSAGE_ID_GET(opmsg_set_aanc_control_model_ptr) ((OPMSG_AANC_ID)(opmsg_set_aanc_control_model_ptr)->_data[0])
#define OPMSG_SET_AANC_CONTROL_MODEL_MESSAGE_ID_SET(opmsg_set_aanc_control_model_ptr, message_id) ((opmsg_set_aanc_control_model_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SET_AANC_CONTROL_MODEL_QUANTITY_DENOMINATOR_COEFFS_WORD_OFFSET (1)
#define OPMSG_SET_AANC_CONTROL_MODEL_QUANTITY_DENOMINATOR_COEFFS_GET(opmsg_set_aanc_control_model_ptr) ((opmsg_set_aanc_control_model_ptr)->_data[1])
#define OPMSG_SET_AANC_CONTROL_MODEL_QUANTITY_DENOMINATOR_COEFFS_SET(opmsg_set_aanc_control_model_ptr, quantity_denominator_coeffs) ((opmsg_set_aanc_control_model_ptr)->_data[1] = (uint16)(quantity_denominator_coeffs))
#define OPMSG_SET_AANC_CONTROL_MODEL_QUANTITY_NUMERATOR_COEFFS_WORD_OFFSET (2)
#define OPMSG_SET_AANC_CONTROL_MODEL_QUANTITY_NUMERATOR_COEFFS_GET(opmsg_set_aanc_control_model_ptr) ((opmsg_set_aanc_control_model_ptr)->_data[2])
#define OPMSG_SET_AANC_CONTROL_MODEL_QUANTITY_NUMERATOR_COEFFS_SET(opmsg_set_aanc_control_model_ptr, quantity_numerator_coeffs) ((opmsg_set_aanc_control_model_ptr)->_data[2] = (uint16)(quantity_numerator_coeffs))
#define OPMSG_SET_AANC_CONTROL_MODEL_DENOMINATOR_COEFFS_WORD_OFFSET (3)
#define OPMSG_SET_AANC_CONTROL_MODEL_DENOMINATOR_COEFFS_GET(opmsg_set_aanc_control_model_ptr) ((opmsg_set_aanc_control_model_ptr)->_data[3])
#define OPMSG_SET_AANC_CONTROL_MODEL_DENOMINATOR_COEFFS_SET(opmsg_set_aanc_control_model_ptr, denominator_coeffs) ((opmsg_set_aanc_control_model_ptr)->_data[3] = (uint16)(denominator_coeffs))
#define OPMSG_SET_AANC_CONTROL_MODEL_NUMERATOR_COEFFS_WORD_OFFSET (4)
#define OPMSG_SET_AANC_CONTROL_MODEL_NUMERATOR_COEFFS_GET(opmsg_set_aanc_control_model_ptr) ((opmsg_set_aanc_control_model_ptr)->_data[4])
#define OPMSG_SET_AANC_CONTROL_MODEL_NUMERATOR_COEFFS_SET(opmsg_set_aanc_control_model_ptr, numerator_coeffs) ((opmsg_set_aanc_control_model_ptr)->_data[4] = (uint16)(numerator_coeffs))
#define OPMSG_SET_AANC_CONTROL_MODEL_WORD_SIZE (5)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SET_AANC_CONTROL_MODEL_CREATE(message_id, quantity_denominator_coeffs, quantity_numerator_coeffs, denominator_coeffs, numerator_coeffs) \
    (uint16)(message_id), \
    (uint16)(quantity_denominator_coeffs), \
    (uint16)(quantity_numerator_coeffs), \
    (uint16)(denominator_coeffs), \
    (uint16)(numerator_coeffs)
#define OPMSG_SET_AANC_CONTROL_MODEL_PACK(opmsg_set_aanc_control_model_ptr, message_id, quantity_denominator_coeffs, quantity_numerator_coeffs, denominator_coeffs, numerator_coeffs) \
    do { \
        (opmsg_set_aanc_control_model_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_set_aanc_control_model_ptr)->_data[1] = (uint16)((uint16)(quantity_denominator_coeffs)); \
        (opmsg_set_aanc_control_model_ptr)->_data[2] = (uint16)((uint16)(quantity_numerator_coeffs)); \
        (opmsg_set_aanc_control_model_ptr)->_data[3] = (uint16)((uint16)(denominator_coeffs)); \
        (opmsg_set_aanc_control_model_ptr)->_data[4] = (uint16)((uint16)(numerator_coeffs)); \
    } while (0)

#define OPMSG_SET_AANC_CONTROL_MODEL_MARSHALL(addr, opmsg_set_aanc_control_model_ptr) memcpy((void *)(addr), (void *)(opmsg_set_aanc_control_model_ptr), 5)
#define OPMSG_SET_AANC_CONTROL_MODEL_UNMARSHALL(addr, opmsg_set_aanc_control_model_ptr) memcpy((void *)(opmsg_set_aanc_control_model_ptr), (void *)(addr), 5)


/*******************************************************************************

  NAME
    OPMSG_SET_AANC_PLANT_MODEL

  DESCRIPTION
    AANC set plant model coefficients message

  MEMBERS
    message_id                  - Message ID
    quantity_denominator_coeffs - Number of denominator coefficients
    quantity_numerator_coeffs   - Number of numerator coefficients
    denominator_coeffs          - Plant denominator (MSB, LSB) coefficients
                                  a1-aN
    numerator_coeffs            - Plant numerator (MSB, LSB) coefficients b0-bN
...
/* The following macros take OPMSG_SET_AANC_PLANT_MODEL *opmsg_set_aanc_plant_model_ptr */
#define OPMSG_SET_AANC_PLANT_MODEL_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SET_AANC_PLANT_MODEL_MESSAGE_ID_GET(opmsg_set_aanc_plant_model_ptr) ((OPMSG_AANC_ID)(opmsg_set_aanc_plant_model_ptr)->_data[0])
#define OPMSG_SET_AANC_PLANT_MODEL_MESSAGE_ID_SET(opmsg_set_aanc_plant_model_ptr, message_id) ((opmsg_set_aanc_plant_model_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SET_AANC_PLANT_MODEL_QUANTITY_DENOMINATOR_COEFFS_WORD_OFFSET (1)
#define OPMSG_SET_AANC_PLANT_MODEL_QUANTITY_DENOMINATOR_COEFFS_GET(opmsg_set_aanc_plant_model_ptr) ((opmsg_set_aanc_plant_model_ptr)->_data[1])
#define OPMSG_SET_AANC_PLANT_MODEL_QUANTITY_DENOMINATOR_COEFFS_SET(opmsg_set_aanc_plant_model_ptr, quantity_denominator_coeffs) ((opmsg_set_aanc_plant_model_ptr)->_data[1] = (uint16)(quantity_denominator_coeffs))
#define OPMSG_SET_AANC_PLANT_MODEL_QUANTITY_NUMERATOR_COEFFS_WORD_OFFSET (2)
#define OPMSG_SET_AANC_PLANT_MODEL_QUANTITY_NUMERATOR_COEFFS_GET(opmsg_set_aanc_plant_model_ptr) ((opmsg_set_aanc_plant_model_ptr)->_data[2])
#define OPMSG_SET_AANC_PLANT_MODEL_QUANTITY_NUMERATOR_COEFFS_SET(opmsg_set_aanc_plant_model_ptr, quantity_numerator_coeffs) ((opmsg_set_aanc_plant_model_ptr)->_data[2] = (uint16)(quantity_numerator_coeffs))
#define OPMSG_SET_AANC_PLANT_MODEL_DENOMINATOR_COEFFS_WORD_OFFSET (3)
#define OPMSG_SET_AANC_PLANT_MODEL_DENOMINATOR_COEFFS_GET(opmsg_set_aanc_plant_model_ptr) ((opmsg_set_aanc_plant_model_ptr)->_data[3])
#define OPMSG_SET_AANC_PLANT_MODEL_DENOMINATOR_COEFFS_SET(opmsg_set_aanc_plant_model_ptr, denominator_coeffs) ((opmsg_set_aanc_plant_model_ptr)->_data[3] = (uint16)(denominator_coeffs))
#define OPMSG_SET_AANC_PLANT_MODEL_NUMERATOR_COEFFS_WORD_OFFSET (4)
#define OPMSG_SET_AANC_PLANT_MODEL_NUMERATOR_COEFFS_GET(opmsg_set_aanc_plant_model_ptr) ((opmsg_set_aanc_plant_model_ptr)->_data[4])
#define OPMSG_SET_AANC_PLANT_MODEL_NUMERATOR_COEFFS_SET(opmsg_set_aanc_plant_model_ptr, numerator_coeffs) ((opmsg_set_aanc_plant_model_ptr)->_data[4] = (uint16)(numerator_coeffs))
#define OPMSG_SET_AANC_PLANT_MODEL_WORD_SIZE (5)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SET_AANC_PLANT_MODEL_CREATE(message_id, quantity_denominator_coeffs, quantity_numerator_coeffs, denominator_coeffs, numerator_coeffs) \
    (uint16)(message_id), \
    (uint16)(quantity_denominator_coeffs), \
    (uint16)(quantity_numerator_coeffs), \
    (uint16)(denominator_coeffs), \
    (uint16)(numerator_coeffs)
#define OPMSG_SET_AANC_PLANT_MODEL_PACK(opmsg_set_aanc_plant_model_ptr, message_id, quantity_denominator_coeffs, quantity_numerator_coeffs, denominator_coeffs, numerator_coeffs) \
    do { \
        (opmsg_set_aanc_plant_model_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_set_aanc_plant_model_ptr)->_data[1] = (uint16)((uint16)(quantity_denominator_coeffs)); \
        (opmsg_set_aanc_plant_model_ptr)->_data[2] = (uint16)((uint16)(quantity_numerator_coeffs)); \
        (opmsg_set_aanc_plant_model_ptr)->_data[3] = (uint16)((uint16)(denominator_coeffs)); \
        (opmsg_set_aanc_plant_model_ptr)->_data[4] = (uint16)((uint16)(numerator_coeffs)); \
    } while (0)

#define OPMSG_SET_AANC_PLANT_MODEL_MARSHALL(addr, opmsg_set_aanc_plant_model_ptr) memcpy((void *)(addr), (void *)(opmsg_set_aanc_plant_model_ptr), 5)
#define OPMSG_SET_AANC_PLANT_MODEL_UNMARSHALL(addr, opmsg_set_aanc_plant_model_ptr) memcpy((void *)(opmsg_set_aanc_plant_model_ptr), (void *)(addr), 5)
...
/*******************************************************************************
  ...
  DESCRIPTION
    AANC set static gain message

  MEMBERS
    message_id            - Message ID
    ff_coarse_static_gain - ANC feedforward coarse static gain value
    ff_fine_static_gain   - ANC feedforward fine static gain value
    fb_coarse_static_gain - ANC feedback coarse static gain value
    fb_fine_static_gain   - ANC feedback fine static gain value
    ec_coarse_static_gain - ANC EC coarse static gain value
    ec_fine_static_gain   - ANC EC fine static gain value
...
/* The following macros take OPMSG_SET_AANC_STATIC_GAIN *opmsg_set_aanc_static_gain_ptr */
#define OPMSG_SET_AANC_STATIC_GAIN_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SET_AANC_STATIC_GAIN_MESSAGE_ID_GET(opmsg_set_aanc_static_gain_ptr) ((OPMSG_AANC_ID)(opmsg_set_aanc_static_gain_ptr)->_data[0])
#define OPMSG_SET_AANC_STATIC_GAIN_MESSAGE_ID_SET(opmsg_set_aanc_static_gain_ptr, message_id) ((opmsg_set_aanc_static_gain_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SET_AANC_STATIC_GAIN_FF_COARSE_STATIC_GAIN_WORD_OFFSET (1)
#define OPMSG_SET_AANC_STATIC_GAIN_FF_COARSE_STATIC_GAIN_GET(opmsg_set_aanc_static_gain_ptr) ((opmsg_set_aanc_static_gain_ptr)->_data[1])
#define OPMSG_SET_AANC_STATIC_GAIN_FF_COARSE_STATIC_GAIN_SET(opmsg_set_aanc_static_gain_ptr, ff_coarse_static_gain) ((opmsg_set_aanc_static_gain_ptr)->_data[1] = (uint16)(ff_coarse_static_gain))
#define OPMSG_SET_AANC_STATIC_GAIN_FF_FINE_STATIC_GAIN_WORD_OFFSET (2)
#define OPMSG_SET_AANC_STATIC_GAIN_FF_FINE_STATIC_GAIN_GET(opmsg_set_aanc_static_gain_ptr) ((opmsg_set_aanc_static_gain_ptr)->_data[2])
#define OPMSG_SET_AANC_STATIC_GAIN_FF_FINE_STATIC_GAIN_SET(opmsg_set_aanc_static_gain_ptr, ff_fine_static_gain) ((opmsg_set_aanc_static_gain_ptr)->_data[2] = (uint16)(ff_fine_static_gain))
#define OPMSG_SET_AANC_STATIC_GAIN_FB_COARSE_STATIC_GAIN_WORD_OFFSET (3)
#define OPMSG_SET_AANC_STATIC_GAIN_FB_COARSE_STATIC_GAIN_GET(opmsg_set_aanc_static_gain_ptr) ((opmsg_set_aanc_static_gain_ptr)->_data[3])
#define OPMSG_SET_AANC_STATIC_GAIN_FB_COARSE_STATIC_GAIN_SET(opmsg_set_aanc_static_gain_ptr, fb_coarse_static_gain) ((opmsg_set_aanc_static_gain_ptr)->_data[3] = (uint16)(fb_coarse_static_gain))
#define OPMSG_SET_AANC_STATIC_GAIN_FB_FINE_STATIC_GAIN_WORD_OFFSET (4)
#define OPMSG_SET_AANC_STATIC_GAIN_FB_FINE_STATIC_GAIN_GET(opmsg_set_aanc_static_gain_ptr) ((opmsg_set_aanc_static_gain_ptr)->_data[4])
#define OPMSG_SET_AANC_STATIC_GAIN_FB_FINE_STATIC_GAIN_SET(opmsg_set_aanc_static_gain_ptr, fb_fine_static_gain) ((opmsg_set_aanc_static_gain_ptr)->_data[4] = (uint16)(fb_fine_static_gain))
#define OPMSG_SET_AANC_STATIC_GAIN_EC_COARSE_STATIC_GAIN_WORD_OFFSET (5)
#define OPMSG_SET_AANC_STATIC_GAIN_EC_COARSE_STATIC_GAIN_GET(opmsg_set_aanc_static_gain_ptr) ((opmsg_set_aanc_static_gain_ptr)->_data[5])
#define OPMSG_SET_AANC_STATIC_GAIN_EC_COARSE_STATIC_GAIN_SET(opmsg_set_aanc_static_gain_ptr, ec_coarse_static_gain) ((opmsg_set_aanc_static_gain_ptr)->_data[5] = (uint16)(ec_coarse_static_gain))
#define OPMSG_SET_AANC_STATIC_GAIN_EC_FINE_STATIC_GAIN_WORD_OFFSET (6)
#define OPMSG_SET_AANC_STATIC_GAIN_EC_FINE_STATIC_GAIN_GET(opmsg_set_aanc_static_gain_ptr) ((opmsg_set_aanc_static_gain_ptr)->_data[6])
#define OPMSG_SET_AANC_STATIC_GAIN_EC_FINE_STATIC_GAIN_SET(opmsg_set_aanc_static_gain_ptr, ec_fine_static_gain) ((opmsg_set_aanc_static_gain_ptr)->_data[6] = (uint16)(ec_fine_static_gain))
#define OPMSG_SET_AANC_STATIC_GAIN_WORD_SIZE (7)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SET_AANC_STATIC_GAIN_CREATE(message_id, ff_coarse_static_gain, ff_fine_static_gain, fb_coarse_static_gain, fb_fine_static_gain, ec_coarse_static_gain, ec_fine_static_gain) \
    (uint16)(message_id), \
    (uint16)(ff_coarse_static_gain), \
    (uint16)(ff_fine_static_gain), \
    (uint16)(fb_coarse_static_gain), \
    (uint16)(fb_fine_static_gain), \
    (uint16)(ec_coarse_static_gain), \
    (uint16)(ec_fine_static_gain)
#define OPMSG_SET_AANC_STATIC_GAIN_PACK(opmsg_set_aanc_static_gain_ptr, message_id, ff_coarse_static_gain, ff_fine_static_gain, fb_coarse_static_gain, fb_fine_static_gain, ec_coarse_static_gain, ec_fine_static_gain) \
    do { \
        (opmsg_set_aanc_static_gain_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_set_aanc_static_gain_ptr)->_data[1] = (uint16)((uint16)(ff_coarse_static_gain)); \
        (opmsg_set_aanc_static_gain_ptr)->_data[2] = (uint16)((uint16)(ff_fine_static_gain)); \
        (opmsg_set_aanc_static_gain_ptr)->_data[3] = (uint16)((uint16)(fb_coarse_static_gain)); \
        (opmsg_set_aanc_static_gain_ptr)->_data[4] = (uint16)((uint16)(fb_fine_static_gain)); \
        (opmsg_set_aanc_static_gain_ptr)->_data[5] = (uint16)((uint16)(ec_coarse_static_gain)); \
        (opmsg_set_aanc_static_gain_ptr)->_data[6] = (uint16)((uint16)(ec_fine_static_gain)); \
    } while (0)

#define OPMSG_SET_AANC_STATIC_GAIN_MARSHALL(addr, opmsg_set_aanc_static_gain_ptr) memcpy((void *)(addr), (void *)(opmsg_set_aanc_static_gain_ptr), 7)
#define OPMSG_SET_AANC_STATIC_GAIN_UNMARSHALL(addr, opmsg_set_aanc_static_gain_ptr) memcpy((void *)(opmsg_set_aanc_static_gain_ptr), (void *)(addr), 7)


/*******************************************************************************

  NAME
    OPMSG_VAD_SET_FRAME_SIZE

  DESCRIPTION
    Sets the frame size for the VAD

  MEMBERS
    message_id - message id
    frame_size - Value for frame size
...
/* The following macros take OPMSG_VAD_SET_FRAME_SIZE *opmsg_vad_set_frame_size_ptr */
#define OPMSG_VAD_SET_FRAME_SIZE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_VAD_SET_FRAME_SIZE_MESSAGE_ID_GET(opmsg_vad_set_frame_size_ptr) ((OPMSG_VAD_ID)(opmsg_vad_set_frame_size_ptr)->_data[0])
#define OPMSG_VAD_SET_FRAME_SIZE_MESSAGE_ID_SET(opmsg_vad_set_frame_size_ptr, message_id) ((opmsg_vad_set_frame_size_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_VAD_SET_FRAME_SIZE_FRAME_SIZE_WORD_OFFSET (1)
#define OPMSG_VAD_SET_FRAME_SIZE_FRAME_SIZE_GET(opmsg_vad_set_frame_size_ptr) ((opmsg_vad_set_frame_size_ptr)->_data[1])
#define OPMSG_VAD_SET_FRAME_SIZE_FRAME_SIZE_SET(opmsg_vad_set_frame_size_ptr, frame_size) ((opmsg_vad_set_frame_size_ptr)->_data[1] = (uint16)(frame_size))
#define OPMSG_VAD_SET_FRAME_SIZE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_VAD_SET_FRAME_SIZE_CREATE(message_id, frame_size) \
    (uint16)(message_id), \
    (uint16)(frame_size)
#define OPMSG_VAD_SET_FRAME_SIZE_PACK(opmsg_vad_set_frame_size_ptr, message_id, frame_size) \
    do { \
        (opmsg_vad_set_frame_size_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_vad_set_frame_size_ptr)->_data[1] = (uint16)((uint16)(frame_size)); \
    } while (0)

#define OPMSG_VAD_SET_FRAME_SIZE_MARSHALL(addr, opmsg_vad_set_frame_size_ptr) memcpy((void *)(addr), (void *)(opmsg_vad_set_frame_size_ptr), 2)
#define OPMSG_VAD_SET_FRAME_SIZE_UNMARSHALL(addr, opmsg_vad_set_frame_size_ptr) memcpy((void *)(opmsg_vad_set_frame_size_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Aec_Set_Output_Block_Size

  DESCRIPTION
    AEC operator message for SET_OUTPUT_BLOCK_SIZE.

  MEMBERS
    message_id - message id
    block_size - Block size in samples. Any non zero value will enable output
                 block size, 0 will use inherited block size if available.
...
/* The following macros take OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE *opmsg_aec_set_output_block_size_ptr */
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_MESSAGE_ID_GET(opmsg_aec_set_output_block_size_ptr) ((SET_OUTPUT_BLOCK_SIZE)(opmsg_aec_set_output_block_size_ptr)->_data[0])
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_MESSAGE_ID_SET(opmsg_aec_set_output_block_size_ptr, message_id) ((opmsg_aec_set_output_block_size_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_BLOCK_SIZE_WORD_OFFSET (1)
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_BLOCK_SIZE_GET(opmsg_aec_set_output_block_size_ptr) ((opmsg_aec_set_output_block_size_ptr)->_data[1])
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_BLOCK_SIZE_SET(opmsg_aec_set_output_block_size_ptr, block_size) ((opmsg_aec_set_output_block_size_ptr)->_data[1] = (uint16)(block_size))
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_CREATE(message_id, block_size) \
    (uint16)(message_id), \
    (uint16)(block_size)
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_PACK(opmsg_aec_set_output_block_size_ptr, message_id, block_size) \
    do { \
        (opmsg_aec_set_output_block_size_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_aec_set_output_block_size_ptr)->_data[1] = (uint16)((uint16)(block_size)); \
    } while (0)

#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_MARSHALL(addr, opmsg_aec_set_output_block_size_ptr) memcpy((void *)(addr), (void *)(opmsg_aec_set_output_block_size_ptr), 2)
#define OPMSG_AEC_SET_OUTPUT_BLOCK_SIZE_UNMARSHALL(addr, opmsg_aec_set_output_block_size_ptr) memcpy((void *)(opmsg_aec_set_output_block_size_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Aptx_Adaptive_Dec_Set_Channel_Extract_Mode

  DESCRIPTION
    APTX ADAPTIVE Decoder Operator message for extracting a single output
    channel.

  MEMBERS
    message_id           - message id
    channel_extract_mode - channel extract mode
...
/* The following macros take OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE *opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr */
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_MESSAGE_ID_GET(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr) ((OPMSG_APTX_ADAPTIVE_DEC_ID)(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr)->_data[0])
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_MESSAGE_ID_SET(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr, message_id) ((opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_CHANNEL_EXTRACT_MODE_WORD_OFFSET (1)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_CHANNEL_EXTRACT_MODE_GET(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr) ((opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr)->_data[1])
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_CHANNEL_EXTRACT_MODE_SET(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr, channel_extract_mode) ((opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr)->_data[1] = (uint16)(channel_extract_mode))
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_CREATE(message_id, channel_extract_mode) \
    (uint16)(message_id), \
    (uint16)(channel_extract_mode)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_PACK(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr, message_id, channel_extract_mode) \
    do { \
        (opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr)->_data[1] = (uint16)((uint16)(channel_extract_mode)); \
    } while (0)

#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_MARSHALL(addr, opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr) memcpy((void *)(addr), (void *)(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr), 2)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_CHANNEL_EXTRACT_MODE_UNMARSHALL(addr, opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr) memcpy((void *)(opmsg_aptx_adaptive_dec_set_channel_extract_mode_ptr), (void *)(addr), 2)


/*******************************************************************************

  NAME
    Opmsg_Aptx_Adaptive_Dec_Set_Internal_Adjust

  DESCRIPTION
    APTX ADAPTIVE Decoder Operator message for setting internal delay

  MEMBERS
    message_id      - message id
    internal_delay  - How much to add to the internal delay
    delay_mode      - mode if the delay
    connection_mode - mode if the delay
...
/* The following macros take OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST *opmsg_aptx_adaptive_dec_set_internal_adjust_ptr */
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_MESSAGE_ID_GET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr) ((OPMSG_APTX_ADAPTIVE_DEC_ID)(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[0])
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_MESSAGE_ID_SET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr, message_id) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_INTERNAL_DELAY_WORD_OFFSET (1)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_INTERNAL_DELAY_GET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[1])
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_INTERNAL_DELAY_SET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr, internal_delay) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[1] = (uint16)(internal_delay))
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_DELAY_MODE_WORD_OFFSET (2)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_DELAY_MODE_GET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[2])
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_DELAY_MODE_SET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr, delay_mode) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[2] = (uint16)(delay_mode))
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_CONNECTION_MODE_WORD_OFFSET (3)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_CONNECTION_MODE_GET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[3])
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_CONNECTION_MODE_SET(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr, connection_mode) ((opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[3] = (uint16)(connection_mode))
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_WORD_SIZE (4)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_CREATE(message_id, internal_delay, delay_mode, connection_mode) \
    (uint16)(message_id), \
    (uint16)(internal_delay), \
    (uint16)(delay_mode), \
    (uint16)(connection_mode)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_PACK(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr, message_id, internal_delay, delay_mode, connection_mode) \
    do { \
        (opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[1] = (uint16)((uint16)(internal_delay)); \
        (opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[2] = (uint16)((uint16)(delay_mode)); \
        (opmsg_aptx_adaptive_dec_set_internal_adjust_ptr)->_data[3] = (uint16)((uint16)(connection_mode)); \
    } while (0)

#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_MARSHALL(addr, opmsg_aptx_adaptive_dec_set_internal_adjust_ptr) memcpy((void *)(addr), (void *)(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr), 4)
#define OPMSG_APTX_ADAPTIVE_DEC_SET_INTERNAL_ADJUST_UNMARSHALL(addr, opmsg_aptx_adaptive_dec_set_internal_adjust_ptr) memcpy((void *)(opmsg_aptx_adaptive_dec_set_internal_adjust_ptr), (void *)(addr), 4)
...
/*******************************************************************************
  ...
  MEMBERS
    ...
    compatibility   - The compatibility field specifies encoder version to be
                      created based on source application or Test AG request.
...
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_PACKET_SIZE_WORD_OFFSET (2)
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_PACKET_SIZE_GET(opmsg_aptx_adaptive_enc_set_encoding_params_ptr) ((opmsg_aptx_adaptive_enc_set_encoding_params_ptr)->_data[2])
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_PACKET_SIZE_SET(opmsg_aptx_adaptive_enc_set_encoding_params_ptr, packet_size) ((opmsg_aptx_adaptive_enc_set_encoding_params_ptr)->_data[2] = (uint16)(packet_size))
...
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_COMPATIBILITY_WORD_OFFSET (6)
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_COMPATIBILITY_GET(opmsg_aptx_adaptive_enc_set_encoding_params_ptr) ((opmsg_aptx_adaptive_enc_set_encoding_params_ptr)->_data[6])
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_COMPATIBILITY_SET(opmsg_aptx_adaptive_enc_set_encoding_params_ptr, compatibility) ((opmsg_aptx_adaptive_enc_set_encoding_params_ptr)->_data[6] = (uint16)(compatibility))
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_WORD_SIZE (7)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_CREATE(message_id, primary_profile, packet_size, channels, bit_rate, sample_rate, compatibility) \
    (uint16)(message_id), \
...
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_PACK(opmsg_aptx_adaptive_enc_set_encoding_params_ptr, message_id, primary_profile, packet_size, channels, bit_rate, sample_rate, compatibility) \
    do { \
        ...
        (opmsg_aptx_adaptive_enc_set_encoding_params_ptr)->_data[2] = (uint16)((uint16)(packet_size)); \
        ...
        (opmsg_aptx_adaptive_enc_set_encoding_params_ptr)->_data[6] = (uint16)((uint16)(compatibility)); \
    } while (0)

#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_MARSHALL(addr, opmsg_aptx_adaptive_enc_set_encoding_params_ptr) memcpy((void *)(addr), (void *)(opmsg_aptx_adaptive_enc_set_encoding_params_ptr), 7)
#define OPMSG_APTX_ADAPTIVE_ENC_SET_ENCODING_PARAMS_UNMARSHALL(addr, opmsg_aptx_adaptive_enc_set_encoding_params_ptr) memcpy((void *)(opmsg_aptx_adaptive_enc_set_encoding_params_ptr), (void *)(addr), 7)


/*******************************************************************************

  NAME
    Opmsg_Aptx_CLassic_Dec_Set_Channel_Silence_Mode

  DESCRIPTION
    APTX Classic Decoder Operator message for enabling forced silence.

  MEMBERS
    message_id   - message id
    silence_mode - channel silence mode. 0 - dont force silence. 1 - force
                   silence
...
/* The following macros take OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE *opmsg_aptx_classic_dec_set_channel_silence_mode_ptr */
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_MESSAGE_ID_GET(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr) ((OPMSG_APTX_CLASSIC_DEC_ID)(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr)->_data[0])
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_MESSAGE_ID_SET(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr, message_id) ((opmsg_aptx_classic_dec_set_channel_silence_mode_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_SILENCE_MODE_WORD_OFFSET (1)
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_SILENCE_MODE_GET(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr) ((opmsg_aptx_classic_dec_set_channel_silence_mode_ptr)->_data[1])
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_SILENCE_MODE_SET(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr, silence_mode) ((opmsg_aptx_classic_dec_set_channel_silence_mode_ptr)->_data[1] = (uint16)(silence_mode))
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_CREATE(message_id, silence_mode) \
    (uint16)(message_id), \
    (uint16)(silence_mode)
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_PACK(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr, message_id, silence_mode) \
    do { \
        (opmsg_aptx_classic_dec_set_channel_silence_mode_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_aptx_classic_dec_set_channel_silence_mode_ptr)->_data[1] = (uint16)((uint16)(silence_mode)); \
    } while (0)

#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_MARSHALL(addr, opmsg_aptx_classic_dec_set_channel_silence_mode_ptr) memcpy((void *)(addr), (void *)(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr), 2)
#define OPMSG_APTX_CLASSIC_DEC_SET_CHANNEL_SILENCE_MODE_UNMARSHALL(addr, opmsg_aptx_classic_dec_set_channel_silence_mode_ptr) memcpy((void *)(opmsg_aptx_classic_dec_set_channel_silence_mode_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Common_Msg_Adjust_TTP_Timestamp

  DESCRIPTION
    Operator common message for adjusting the TTP timestamp

  MEMBERS
    message_id - message id
    ttp_adj_ms - Most-significant 16 bits of the signed ttp adjustment value.
    ttp_adj_ls - Least-significant 16 bits of the signed ttp adjustment value.
...
/* The following macros take OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP *opmsg_common_msg_adjust_ttp_timestamp_ptr */
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_MESSAGE_ID_GET(opmsg_common_msg_adjust_ttp_timestamp_ptr) ((OPMSG_COMMON_ID)(opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[0])
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_MESSAGE_ID_SET(opmsg_common_msg_adjust_ttp_timestamp_ptr, message_id) ((opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_TTP_ADJ_MS_WORD_OFFSET (1)
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_TTP_ADJ_MS_GET(opmsg_common_msg_adjust_ttp_timestamp_ptr) ((opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[1])
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_TTP_ADJ_MS_SET(opmsg_common_msg_adjust_ttp_timestamp_ptr, ttp_adj_ms) ((opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[1] = (uint16)(ttp_adj_ms))
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_TTP_ADJ_LS_WORD_OFFSET (2)
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_TTP_ADJ_LS_GET(opmsg_common_msg_adjust_ttp_timestamp_ptr) ((opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[2])
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_TTP_ADJ_LS_SET(opmsg_common_msg_adjust_ttp_timestamp_ptr, ttp_adj_ls) ((opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[2] = (uint16)(ttp_adj_ls))
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_WORD_SIZE (3)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_CREATE(message_id, ttp_adj_ms, ttp_adj_ls) \
    (uint16)(message_id), \
    (uint16)(ttp_adj_ms), \
    (uint16)(ttp_adj_ls)
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_PACK(opmsg_common_msg_adjust_ttp_timestamp_ptr, message_id, ttp_adj_ms, ttp_adj_ls) \
    do { \
        (opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[1] = (uint16)((uint16)(ttp_adj_ms)); \
        (opmsg_common_msg_adjust_ttp_timestamp_ptr)->_data[2] = (uint16)((uint16)(ttp_adj_ls)); \
    } while (0)

#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_MARSHALL(addr, opmsg_common_msg_adjust_ttp_timestamp_ptr) memcpy((void *)(addr), (void *)(opmsg_common_msg_adjust_ttp_timestamp_ptr), 3)
#define OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP_UNMARSHALL(addr, opmsg_common_msg_adjust_ttp_timestamp_ptr) memcpy((void *)(opmsg_common_msg_adjust_ttp_timestamp_ptr), (void *)(addr), 3)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Common_Msg_Set_Back_Kick_Threshold

  DESCRIPTION
    Operator message format for OPMSG_COMMON_SET_BACK_KICK_THRESHOLD

  MEMBERS
    message_id -
    threshold  - A positive value specifies amount of data in words. The
                 magnitude of a negative value specifies amount of space in
                 words.
    mode       - Backward kicking rule to apply
    sinks      - Sink terminal bit mask. Bits 0..15 correspond to terminals
                 0..15. Implementation of this field is capability defined.
...
/* The following macros take OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD *opmsg_common_msg_set_back_kick_threshold_ptr */
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MESSAGE_ID_GET(opmsg_common_msg_set_back_kick_threshold_ptr) ((OPMSG_COMMON_ID)(opmsg_common_msg_set_back_kick_threshold_ptr)->_data[0])
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MESSAGE_ID_SET(opmsg_common_msg_set_back_kick_threshold_ptr, message_id) ((opmsg_common_msg_set_back_kick_threshold_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_THRESHOLD_WORD_OFFSET (1)
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_THRESHOLD_GET(opmsg_common_msg_set_back_kick_threshold_ptr) ((opmsg_common_msg_set_back_kick_threshold_ptr)->_data[1])
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_THRESHOLD_SET(opmsg_common_msg_set_back_kick_threshold_ptr, threshold) ((opmsg_common_msg_set_back_kick_threshold_ptr)->_data[1] = (uint16)(threshold))
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MODE_WORD_OFFSET (2)
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MODE_GET(opmsg_common_msg_set_back_kick_threshold_ptr) ((OPMSG_COMMON_BACK_KICK_MODE)(opmsg_common_msg_set_back_kick_threshold_ptr)->_data[2])
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MODE_SET(opmsg_common_msg_set_back_kick_threshold_ptr, mode) ((opmsg_common_msg_set_back_kick_threshold_ptr)->_data[2] = (uint16)(mode))
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_SINKS_WORD_OFFSET (3)
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_SINKS_GET(opmsg_common_msg_set_back_kick_threshold_ptr) ((opmsg_common_msg_set_back_kick_threshold_ptr)->_data[3])
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_SINKS_SET(opmsg_common_msg_set_back_kick_threshold_ptr, sinks) ((opmsg_common_msg_set_back_kick_threshold_ptr)->_data[3] = (uint16)(sinks))
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_WORD_SIZE (4)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_CREATE(message_id, threshold, mode, sinks) \
    (uint16)(message_id), \
    (uint16)(threshold), \
    (uint16)(mode), \
    (uint16)(sinks)
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_PACK(opmsg_common_msg_set_back_kick_threshold_ptr, message_id, threshold, mode, sinks) \
    do { \
        (opmsg_common_msg_set_back_kick_threshold_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_common_msg_set_back_kick_threshold_ptr)->_data[1] = (uint16)((uint16)(threshold)); \
        (opmsg_common_msg_set_back_kick_threshold_ptr)->_data[2] = (uint16)((uint16)(mode)); \
        (opmsg_common_msg_set_back_kick_threshold_ptr)->_data[3] = (uint16)((uint16)(sinks)); \
    } while (0)

#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_MARSHALL(addr, opmsg_common_msg_set_back_kick_threshold_ptr) memcpy((void *)(addr), (void *)(opmsg_common_msg_set_back_kick_threshold_ptr), 4)
#define OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD_UNMARSHALL(addr, opmsg_common_msg_set_back_kick_threshold_ptr) memcpy((void *)(opmsg_common_msg_set_back_kick_threshold_ptr), (void *)(addr), 4)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Common_Msg_Set_TTP_Spadj

  DESCRIPTION
    Operator common message to set TTP sample period adjustment.

  MEMBERS
    message_id - message id
    sp_adj_ms  - Most-significant 16 bits of sample period adjustment.
    sp_adj_ls  - Least-significant 16 bits of sample period adjustment.
...
/* The following macros take OPMSG_COMMON_MSG_SET_TTP_SPADJ *opmsg_common_msg_set_ttp_spadj_ptr */
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_MESSAGE_ID_GET(opmsg_common_msg_set_ttp_spadj_ptr) ((OPMSG_COMMON_ID)(opmsg_common_msg_set_ttp_spadj_ptr)->_data[0])
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_MESSAGE_ID_SET(opmsg_common_msg_set_ttp_spadj_ptr, message_id) ((opmsg_common_msg_set_ttp_spadj_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_SP_ADJ_MS_WORD_OFFSET (1)
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_SP_ADJ_MS_GET(opmsg_common_msg_set_ttp_spadj_ptr) ((opmsg_common_msg_set_ttp_spadj_ptr)->_data[1])
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_SP_ADJ_MS_SET(opmsg_common_msg_set_ttp_spadj_ptr, sp_adj_ms) ((opmsg_common_msg_set_ttp_spadj_ptr)->_data[1] = (uint16)(sp_adj_ms))
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_SP_ADJ_LS_WORD_OFFSET (2)
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_SP_ADJ_LS_GET(opmsg_common_msg_set_ttp_spadj_ptr) ((opmsg_common_msg_set_ttp_spadj_ptr)->_data[2])
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_SP_ADJ_LS_SET(opmsg_common_msg_set_ttp_spadj_ptr, sp_adj_ls) ((opmsg_common_msg_set_ttp_spadj_ptr)->_data[2] = (uint16)(sp_adj_ls))
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_WORD_SIZE (3)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_CREATE(message_id, sp_adj_ms, sp_adj_ls) \
    (uint16)(message_id), \
    (uint16)(sp_adj_ms), \
    (uint16)(sp_adj_ls)
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_PACK(opmsg_common_msg_set_ttp_spadj_ptr, message_id, sp_adj_ms, sp_adj_ls) \
    do { \
        (opmsg_common_msg_set_ttp_spadj_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_common_msg_set_ttp_spadj_ptr)->_data[1] = (uint16)((uint16)(sp_adj_ms)); \
        (opmsg_common_msg_set_ttp_spadj_ptr)->_data[2] = (uint16)((uint16)(sp_adj_ls)); \
    } while (0)

#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_MARSHALL(addr, opmsg_common_msg_set_ttp_spadj_ptr) memcpy((void *)(addr), (void *)(opmsg_common_msg_set_ttp_spadj_ptr), 3)
#define OPMSG_COMMON_MSG_SET_TTP_SPADJ_UNMARSHALL(addr, opmsg_common_msg_set_ttp_spadj_ptr) memcpy((void *)(opmsg_common_msg_set_ttp_spadj_ptr), (void *)(addr), 3)


/*******************************************************************************

  NAME
    Opmsg_Common_Msg_Set_TTP_State

  DESCRIPTION
    Operator common message for SET_TTP_STATE.

  MEMBERS
    message_id       - message id
    mode             - Mode of TTP operation
    ttp_timestamp_ms - Most-significant 16 bits of TTP timestamp.
    ttp_timestamp_ls - Least-significant 16 bits of TTP timestamp.
    sp_adj_ms        - Most-significant 16 bits of sample period adjustment.
    sp_adj_ls        - Least-significant 16 bits of sample period adjustment.
    latency_ms       - Most-significant 16 bits of target latency, in
                       microseconds.
    latency_ls       - Least-significant 16 bits of target latency, in
                       microseconds.
...
/* The following macros take OPMSG_COMMON_MSG_SET_TTP_STATE *opmsg_common_msg_set_ttp_state_ptr */
#define OPMSG_COMMON_MSG_SET_TTP_STATE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_MESSAGE_ID_GET(opmsg_common_msg_set_ttp_state_ptr) ((OPMSG_COMMON_ID)(opmsg_common_msg_set_ttp_state_ptr)->_data[0])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_MESSAGE_ID_SET(opmsg_common_msg_set_ttp_state_ptr, message_id) ((opmsg_common_msg_set_ttp_state_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_MODE_WORD_OFFSET (1)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_MODE_GET(opmsg_common_msg_set_ttp_state_ptr) ((OPMSG_TTP_MODE)(opmsg_common_msg_set_ttp_state_ptr)->_data[1])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_MODE_SET(opmsg_common_msg_set_ttp_state_ptr, mode) ((opmsg_common_msg_set_ttp_state_ptr)->_data[1] = (uint16)(mode))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_TTP_TIMESTAMP_MS_WORD_OFFSET (2)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_TTP_TIMESTAMP_MS_GET(opmsg_common_msg_set_ttp_state_ptr) ((opmsg_common_msg_set_ttp_state_ptr)->_data[2])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_TTP_TIMESTAMP_MS_SET(opmsg_common_msg_set_ttp_state_ptr, ttp_timestamp_ms) ((opmsg_common_msg_set_ttp_state_ptr)->_data[2] = (uint16)(ttp_timestamp_ms))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_TTP_TIMESTAMP_LS_WORD_OFFSET (3)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_TTP_TIMESTAMP_LS_GET(opmsg_common_msg_set_ttp_state_ptr) ((opmsg_common_msg_set_ttp_state_ptr)->_data[3])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_TTP_TIMESTAMP_LS_SET(opmsg_common_msg_set_ttp_state_ptr, ttp_timestamp_ls) ((opmsg_common_msg_set_ttp_state_ptr)->_data[3] = (uint16)(ttp_timestamp_ls))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_SP_ADJ_MS_WORD_OFFSET (4)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_SP_ADJ_MS_GET(opmsg_common_msg_set_ttp_state_ptr) ((opmsg_common_msg_set_ttp_state_ptr)->_data[4])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_SP_ADJ_MS_SET(opmsg_common_msg_set_ttp_state_ptr, sp_adj_ms) ((opmsg_common_msg_set_ttp_state_ptr)->_data[4] = (uint16)(sp_adj_ms))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_SP_ADJ_LS_WORD_OFFSET (5)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_SP_ADJ_LS_GET(opmsg_common_msg_set_ttp_state_ptr) ((opmsg_common_msg_set_ttp_state_ptr)->_data[5])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_SP_ADJ_LS_SET(opmsg_common_msg_set_ttp_state_ptr, sp_adj_ls) ((opmsg_common_msg_set_ttp_state_ptr)->_data[5] = (uint16)(sp_adj_ls))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_LATENCY_MS_WORD_OFFSET (6)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_LATENCY_MS_GET(opmsg_common_msg_set_ttp_state_ptr) ((opmsg_common_msg_set_ttp_state_ptr)->_data[6])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_LATENCY_MS_SET(opmsg_common_msg_set_ttp_state_ptr, latency_ms) ((opmsg_common_msg_set_ttp_state_ptr)->_data[6] = (uint16)(latency_ms))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_LATENCY_LS_WORD_OFFSET (7)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_LATENCY_LS_GET(opmsg_common_msg_set_ttp_state_ptr) ((opmsg_common_msg_set_ttp_state_ptr)->_data[7])
#define OPMSG_COMMON_MSG_SET_TTP_STATE_LATENCY_LS_SET(opmsg_common_msg_set_ttp_state_ptr, latency_ls) ((opmsg_common_msg_set_ttp_state_ptr)->_data[7] = (uint16)(latency_ls))
#define OPMSG_COMMON_MSG_SET_TTP_STATE_WORD_SIZE (8)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_COMMON_MSG_SET_TTP_STATE_CREATE(message_id, mode, ttp_timestamp_ms, ttp_timestamp_ls, sp_adj_ms, sp_adj_ls, latency_ms, latency_ls) \
    (uint16)(message_id), \
    (uint16)(mode), \
    (uint16)(ttp_timestamp_ms), \
    (uint16)(ttp_timestamp_ls), \
    (uint16)(sp_adj_ms), \
    (uint16)(sp_adj_ls), \
    (uint16)(latency_ms), \
    (uint16)(latency_ls)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_PACK(opmsg_common_msg_set_ttp_state_ptr, message_id, mode, ttp_timestamp_ms, ttp_timestamp_ls, sp_adj_ms, sp_adj_ls, latency_ms, latency_ls) \
    do { \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[1] = (uint16)((uint16)(mode)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[2] = (uint16)((uint16)(ttp_timestamp_ms)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[3] = (uint16)((uint16)(ttp_timestamp_ls)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[4] = (uint16)((uint16)(sp_adj_ms)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[5] = (uint16)((uint16)(sp_adj_ls)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[6] = (uint16)((uint16)(latency_ms)); \
        (opmsg_common_msg_set_ttp_state_ptr)->_data[7] = (uint16)((uint16)(latency_ls)); \
    } while (0)

#define OPMSG_COMMON_MSG_SET_TTP_STATE_MARSHALL(addr, opmsg_common_msg_set_ttp_state_ptr) memcpy((void *)(addr), (void *)(opmsg_common_msg_set_ttp_state_ptr), 8)
#define OPMSG_COMMON_MSG_SET_TTP_STATE_UNMARSHALL(addr, opmsg_common_msg_set_ttp_state_ptr) memcpy((void *)(opmsg_common_msg_set_ttp_state_ptr), (void *)(addr), 8)
...
/*******************************************************************************
  ...
  MEMBERS
    ...
    transform_mask_15 - Word 15 of array of 16 words times 16 bits forming a 256
                        bit mask. See description of transform_mask_0.
    timed_data_ep_id  - Source endpoint ID to capture written data
...
#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_TIMED_DATA_EP_ID_WORD_OFFSET (22)
#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_TIMED_DATA_EP_ID_GET(opmsg_ksp_set_stream_timing_trace_ptr) ((opmsg_ksp_set_stream_timing_trace_ptr)->_data[22])
#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_TIMED_DATA_EP_ID_SET(opmsg_ksp_set_stream_timing_trace_ptr, timed_data_ep_id) ((opmsg_ksp_set_stream_timing_trace_ptr)->_data[22] = (uint16)(timed_data_ep_id))
#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_WORD_SIZE (23)
/*lint -e(773) allow unparenthesized*/
...
#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_PACK(opmsg_ksp_set_stream_timing_trace_ptr, message_id, stream_id, nr_samples, setup_buffer_size, event_buffer_size, enable_mask, transform_mask_0, transform_mask_1, transform_mask_2, transform_mask_3, transform_mask_4, transform_mask_5, transform_mask_6, transform_mask_7, transform_mask_8, transform_mask_9, transform_mask_10, transform_mask_11, transform_mask_12, transform_mask_13, transform_mask_14, transform_mask_15, timed_data_ep_id) \
    do { \
        ...
        (opmsg_ksp_set_stream_timing_trace_ptr)->_data[22] = (uint16)((uint16)(timed_data_ep_id)); \
    } while (0)

#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_MARSHALL(addr, opmsg_ksp_set_stream_timing_trace_ptr) memcpy((void *)(addr), (void *)(opmsg_ksp_set_stream_timing_trace_ptr), 23)
#define OPMSG_KSP_SET_STREAM_TIMING_TRACE_UNMARSHALL(addr, opmsg_ksp_set_stream_timing_trace_ptr) memcpy((void *)(opmsg_ksp_set_stream_timing_trace_ptr), (void *)(addr), 23)
...
/*******************************************************************************
  ...
  DESCRIPTION
    RTP DECODE's unsolicited operator message to notify the current ttp
    values.

  MEMBERS
    seq_num              - Sequence number of the last processed RTP packet.
    sample_count         - Number of samples in the RTP packet.
    total_num_packets_ms - Most-significant 16 bits of RTP packets processed.
    total_num_packets_ls - Least-significant 16 bits of RTP packets processed.
    ttp_timestamp_ms     - Most-significant 16 bits of TTP timestamp. .
    ttp_timestamp_ls     - Least-significant 16 bits of TTP timestamp. .
    latency_ms           - Most-significant 16 bits of target latency, in
                           microseconds.
    latency_ls           - Least-significant 16 bits of target latency, in
                           microseconds.
    sp_adj_ms            - Most-significant 16 bits of sample period adjustment.
    sp_adj_ls            - Least-significant 16 bits of sample period
                           adjustment.
    ttp_resync_count     - Number of times ttp resync'ed since opertaor started.
...
/* The following macros take OPMSG_REPLY_TTP_SEQ *opmsg_reply_ttp_seq_ptr */
#define OPMSG_REPLY_TTP_SEQ_SEQ_NUM_WORD_OFFSET (0)
#define OPMSG_REPLY_TTP_SEQ_SEQ_NUM_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[0])
#define OPMSG_REPLY_TTP_SEQ_SEQ_NUM_SET(opmsg_reply_ttp_seq_ptr, seq_num) ((opmsg_reply_ttp_seq_ptr)->_data[0] = (uint16)(seq_num))
#define OPMSG_REPLY_TTP_SEQ_SAMPLE_COUNT_WORD_OFFSET (1)
#define OPMSG_REPLY_TTP_SEQ_SAMPLE_COUNT_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[1])
#define OPMSG_REPLY_TTP_SEQ_SAMPLE_COUNT_SET(opmsg_reply_ttp_seq_ptr, sample_count) ((opmsg_reply_ttp_seq_ptr)->_data[1] = (uint16)(sample_count))
#define OPMSG_REPLY_TTP_SEQ_TOTAL_NUM_PACKETS_MS_WORD_OFFSET (2)
#define OPMSG_REPLY_TTP_SEQ_TOTAL_NUM_PACKETS_MS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[2])
#define OPMSG_REPLY_TTP_SEQ_TOTAL_NUM_PACKETS_MS_SET(opmsg_reply_ttp_seq_ptr, total_num_packets_ms) ((opmsg_reply_ttp_seq_ptr)->_data[2] = (uint16)(total_num_packets_ms))
#define OPMSG_REPLY_TTP_SEQ_TOTAL_NUM_PACKETS_LS_WORD_OFFSET (3)
#define OPMSG_REPLY_TTP_SEQ_TOTAL_NUM_PACKETS_LS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[3])
#define OPMSG_REPLY_TTP_SEQ_TOTAL_NUM_PACKETS_LS_SET(opmsg_reply_ttp_seq_ptr, total_num_packets_ls) ((opmsg_reply_ttp_seq_ptr)->_data[3] = (uint16)(total_num_packets_ls))
#define OPMSG_REPLY_TTP_SEQ_TTP_TIMESTAMP_MS_WORD_OFFSET (4)
#define OPMSG_REPLY_TTP_SEQ_TTP_TIMESTAMP_MS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[4])
#define OPMSG_REPLY_TTP_SEQ_TTP_TIMESTAMP_MS_SET(opmsg_reply_ttp_seq_ptr, ttp_timestamp_ms) ((opmsg_reply_ttp_seq_ptr)->_data[4] = (uint16)(ttp_timestamp_ms))
#define OPMSG_REPLY_TTP_SEQ_TTP_TIMESTAMP_LS_WORD_OFFSET (5)
#define OPMSG_REPLY_TTP_SEQ_TTP_TIMESTAMP_LS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[5])
#define OPMSG_REPLY_TTP_SEQ_TTP_TIMESTAMP_LS_SET(opmsg_reply_ttp_seq_ptr, ttp_timestamp_ls) ((opmsg_reply_ttp_seq_ptr)->_data[5] = (uint16)(ttp_timestamp_ls))
#define OPMSG_REPLY_TTP_SEQ_LATENCY_MS_WORD_OFFSET (6)
#define OPMSG_REPLY_TTP_SEQ_LATENCY_MS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[6])
#define OPMSG_REPLY_TTP_SEQ_LATENCY_MS_SET(opmsg_reply_ttp_seq_ptr, latency_ms) ((opmsg_reply_ttp_seq_ptr)->_data[6] = (uint16)(latency_ms))
#define OPMSG_REPLY_TTP_SEQ_LATENCY_LS_WORD_OFFSET (7)
#define OPMSG_REPLY_TTP_SEQ_LATENCY_LS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[7])
#define OPMSG_REPLY_TTP_SEQ_LATENCY_LS_SET(opmsg_reply_ttp_seq_ptr, latency_ls) ((opmsg_reply_ttp_seq_ptr)->_data[7] = (uint16)(latency_ls))
#define OPMSG_REPLY_TTP_SEQ_SP_ADJ_MS_WORD_OFFSET (8)
#define OPMSG_REPLY_TTP_SEQ_SP_ADJ_MS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[8])
#define OPMSG_REPLY_TTP_SEQ_SP_ADJ_MS_SET(opmsg_reply_ttp_seq_ptr, sp_adj_ms) ((opmsg_reply_ttp_seq_ptr)->_data[8] = (uint16)(sp_adj_ms))
#define OPMSG_REPLY_TTP_SEQ_SP_ADJ_LS_WORD_OFFSET (9)
#define OPMSG_REPLY_TTP_SEQ_SP_ADJ_LS_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[9])
#define OPMSG_REPLY_TTP_SEQ_SP_ADJ_LS_SET(opmsg_reply_ttp_seq_ptr, sp_adj_ls) ((opmsg_reply_ttp_seq_ptr)->_data[9] = (uint16)(sp_adj_ls))
#define OPMSG_REPLY_TTP_SEQ_TTP_RESYNC_COUNT_WORD_OFFSET (10)
#define OPMSG_REPLY_TTP_SEQ_TTP_RESYNC_COUNT_GET(opmsg_reply_ttp_seq_ptr) ((opmsg_reply_ttp_seq_ptr)->_data[10])
#define OPMSG_REPLY_TTP_SEQ_TTP_RESYNC_COUNT_SET(opmsg_reply_ttp_seq_ptr, ttp_resync_count) ((opmsg_reply_ttp_seq_ptr)->_data[10] = (uint16)(ttp_resync_count))
#define OPMSG_REPLY_TTP_SEQ_WORD_SIZE (11)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_REPLY_TTP_SEQ_CREATE(seq_num, sample_count, total_num_packets_ms, total_num_packets_ls, ttp_timestamp_ms, ttp_timestamp_ls, latency_ms, latency_ls, sp_adj_ms, sp_adj_ls, ttp_resync_count) \
    (uint16)(seq_num), \
    (uint16)(sample_count), \
    (uint16)(total_num_packets_ms), \
    (uint16)(total_num_packets_ls), \
    (uint16)(ttp_timestamp_ms), \
    (uint16)(ttp_timestamp_ls), \
    (uint16)(latency_ms), \
    (uint16)(latency_ls), \
    (uint16)(sp_adj_ms), \
    (uint16)(sp_adj_ls), \
    (uint16)(ttp_resync_count)
#define OPMSG_REPLY_TTP_SEQ_PACK(opmsg_reply_ttp_seq_ptr, seq_num, sample_count, total_num_packets_ms, total_num_packets_ls, ttp_timestamp_ms, ttp_timestamp_ls, latency_ms, latency_ls, sp_adj_ms, sp_adj_ls, ttp_resync_count) \
    do { \
        (opmsg_reply_ttp_seq_ptr)->_data[0] = (uint16)((uint16)(seq_num)); \
        (opmsg_reply_ttp_seq_ptr)->_data[1] = (uint16)((uint16)(sample_count)); \
        (opmsg_reply_ttp_seq_ptr)->_data[2] = (uint16)((uint16)(total_num_packets_ms)); \
        (opmsg_reply_ttp_seq_ptr)->_data[3] = (uint16)((uint16)(total_num_packets_ls)); \
        (opmsg_reply_ttp_seq_ptr)->_data[4] = (uint16)((uint16)(ttp_timestamp_ms)); \
        (opmsg_reply_ttp_seq_ptr)->_data[5] = (uint16)((uint16)(ttp_timestamp_ls)); \
        (opmsg_reply_ttp_seq_ptr)->_data[6] = (uint16)((uint16)(latency_ms)); \
        (opmsg_reply_ttp_seq_ptr)->_data[7] = (uint16)((uint16)(latency_ls)); \
        (opmsg_reply_ttp_seq_ptr)->_data[8] = (uint16)((uint16)(sp_adj_ms)); \
        (opmsg_reply_ttp_seq_ptr)->_data[9] = (uint16)((uint16)(sp_adj_ls)); \
        (opmsg_reply_ttp_seq_ptr)->_data[10] = (uint16)((uint16)(ttp_resync_count)); \
    } while (0)

#define OPMSG_REPLY_TTP_SEQ_MARSHALL(addr, opmsg_reply_ttp_seq_ptr) memcpy((void *)(addr), (void *)(opmsg_reply_ttp_seq_ptr), 11)
#define OPMSG_REPLY_TTP_SEQ_UNMARSHALL(addr, opmsg_reply_ttp_seq_ptr) memcpy((void *)(opmsg_reply_ttp_seq_ptr), (void *)(addr), 11)
...
/*******************************************************************************
  ...
  MEMBERS
    ...
    enable_latency_notify - enable or disable latency change notification for
                            ...
                            notification. 1 = Enable Latency change notification.
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Rtp_Set_TTP_Notification

  DESCRIPTION
    RTP DECODE operator message for SET_TTP_NOTIFICATION.

  MEMBERS
    message_id     - message id
    interval_count - Number of RTP packets processed to repeat the notifictaion.
                     0               = Disable TTP notification. any other value
                     = Repeatedly notify on processing this number of RTP
                     packets.
...
/* The following macros take OPMSG_RTP_SET_TTP_NOTIFICATION *opmsg_rtp_set_ttp_notification_ptr */
#define OPMSG_RTP_SET_TTP_NOTIFICATION_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_RTP_SET_TTP_NOTIFICATION_MESSAGE_ID_GET(opmsg_rtp_set_ttp_notification_ptr) ((OPMSG_RTP_DECODE_ID)(opmsg_rtp_set_ttp_notification_ptr)->_data[0])
#define OPMSG_RTP_SET_TTP_NOTIFICATION_MESSAGE_ID_SET(opmsg_rtp_set_ttp_notification_ptr, message_id) ((opmsg_rtp_set_ttp_notification_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_RTP_SET_TTP_NOTIFICATION_INTERVAL_COUNT_WORD_OFFSET (1)
#define OPMSG_RTP_SET_TTP_NOTIFICATION_INTERVAL_COUNT_GET(opmsg_rtp_set_ttp_notification_ptr) ((opmsg_rtp_set_ttp_notification_ptr)->_data[1])
#define OPMSG_RTP_SET_TTP_NOTIFICATION_INTERVAL_COUNT_SET(opmsg_rtp_set_ttp_notification_ptr, interval_count) ((opmsg_rtp_set_ttp_notification_ptr)->_data[1] = (uint16)(interval_count))
#define OPMSG_RTP_SET_TTP_NOTIFICATION_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_RTP_SET_TTP_NOTIFICATION_CREATE(message_id, interval_count) \
    (uint16)(message_id), \
    (uint16)(interval_count)
#define OPMSG_RTP_SET_TTP_NOTIFICATION_PACK(opmsg_rtp_set_ttp_notification_ptr, message_id, interval_count) \
    do { \
        (opmsg_rtp_set_ttp_notification_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_rtp_set_ttp_notification_ptr)->_data[1] = (uint16)((uint16)(interval_count)); \
    } while (0)

#define OPMSG_RTP_SET_TTP_NOTIFICATION_MARSHALL(addr, opmsg_rtp_set_ttp_notification_ptr) memcpy((void *)(addr), (void *)(opmsg_rtp_set_ttp_notification_ptr), 2)
#define OPMSG_RTP_SET_TTP_NOTIFICATION_UNMARSHALL(addr, opmsg_rtp_set_ttp_notification_ptr) memcpy((void *)(opmsg_rtp_set_ttp_notification_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  MEMBERS
    message_id - message id
    block_size - Select block size to use with SPC#1->Aptx passthrough
...
/* The following macros take OPMSG_SPC_SET_BLOCK_SIZE *opmsg_spc_set_block_size_ptr */
#define OPMSG_SPC_SET_BLOCK_SIZE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SPC_SET_BLOCK_SIZE_MESSAGE_ID_GET(opmsg_spc_set_block_size_ptr) ((OPMSG_SPC_ID)(opmsg_spc_set_block_size_ptr)->_data[0])
#define OPMSG_SPC_SET_BLOCK_SIZE_MESSAGE_ID_SET(opmsg_spc_set_block_size_ptr, message_id) ((opmsg_spc_set_block_size_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SPC_SET_BLOCK_SIZE_BLOCK_SIZE_WORD_OFFSET (1)
#define OPMSG_SPC_SET_BLOCK_SIZE_BLOCK_SIZE_GET(opmsg_spc_set_block_size_ptr) ((opmsg_spc_set_block_size_ptr)->_data[1])
#define OPMSG_SPC_SET_BLOCK_SIZE_BLOCK_SIZE_SET(opmsg_spc_set_block_size_ptr, block_size) ((opmsg_spc_set_block_size_ptr)->_data[1] = (uint16)(block_size))
#define OPMSG_SPC_SET_BLOCK_SIZE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SPC_SET_BLOCK_SIZE_CREATE(message_id, block_size) \
    (uint16)(message_id), \
    (uint16)(block_size)
#define OPMSG_SPC_SET_BLOCK_SIZE_PACK(opmsg_spc_set_block_size_ptr, message_id, block_size) \
    do { \
        (opmsg_spc_set_block_size_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_spc_set_block_size_ptr)->_data[1] = (uint16)((uint16)(block_size)); \
    } while (0)

#define OPMSG_SPC_SET_BLOCK_SIZE_MARSHALL(addr, opmsg_spc_set_block_size_ptr) memcpy((void *)(addr), (void *)(opmsg_spc_set_block_size_ptr), 2)
#define OPMSG_SPC_SET_BLOCK_SIZE_UNMARSHALL(addr, opmsg_spc_set_block_size_ptr) memcpy((void *)(opmsg_spc_set_block_size_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Source_Sync_Stall_Changed

  DESCRIPTION
    Source Sync unsolicited message for monitoring stall state.

  MEMBERS
    sink_group  - Number of the sink group reporting the change
    stall_state - Simplified sink state
    timestamp   - System time of the change
...
/* The following macros take OPMSG_SOURCE_SYNC_STALL_CHANGED *opmsg_source_sync_stall_changed_ptr */
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_SINK_GROUP_WORD_OFFSET (0)
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_SINK_GROUP_GET(opmsg_source_sync_stall_changed_ptr) ((opmsg_source_sync_stall_changed_ptr)->_data[0])
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_SINK_GROUP_SET(opmsg_source_sync_stall_changed_ptr, sink_group) ((opmsg_source_sync_stall_changed_ptr)->_data[0] = (uint16)(sink_group))
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_STALL_STATE_WORD_OFFSET (1)
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_STALL_STATE_GET(opmsg_source_sync_stall_changed_ptr) ((OPMSG_SOURCE_SYNC_SINK_STALL_STATE)(opmsg_source_sync_stall_changed_ptr)->_data[1])
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_STALL_STATE_SET(opmsg_source_sync_stall_changed_ptr, stall_state) ((opmsg_source_sync_stall_changed_ptr)->_data[1] = (uint16)(stall_state))
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_TIMESTAMP_WORD_OFFSET (2)
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_TIMESTAMP_GET(opmsg_source_sync_stall_changed_ptr)  \
    (((uint32)((opmsg_source_sync_stall_changed_ptr)->_data[2]) | \
      ((uint32)((opmsg_source_sync_stall_changed_ptr)->_data[3]) << 16)))
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_TIMESTAMP_SET(opmsg_source_sync_stall_changed_ptr, timestamp) do { \
        (opmsg_source_sync_stall_changed_ptr)->_data[2] = (uint16)((timestamp) & 0xffff); \
        (opmsg_source_sync_stall_changed_ptr)->_data[3] = (uint16)((timestamp) >> 16); } while (0)
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_WORD_SIZE (4)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_CREATE(sink_group, stall_state, timestamp) \
    (uint16)(sink_group), \
    (uint16)(stall_state), \
    (uint16)((timestamp) & 0xffff), \
    (uint16)((timestamp) >> 16)
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_PACK(opmsg_source_sync_stall_changed_ptr, sink_group, stall_state, timestamp) \
    do { \
        (opmsg_source_sync_stall_changed_ptr)->_data[0] = (uint16)((uint16)(sink_group)); \
        (opmsg_source_sync_stall_changed_ptr)->_data[1] = (uint16)((uint16)(stall_state)); \
        (opmsg_source_sync_stall_changed_ptr)->_data[2] = (uint16)((uint16)((timestamp) & 0xffff)); \
        (opmsg_source_sync_stall_changed_ptr)->_data[3] = (uint16)(((timestamp) >> 16)); \
    } while (0)

#define OPMSG_SOURCE_SYNC_STALL_CHANGED_MARSHALL(addr, opmsg_source_sync_stall_changed_ptr) memcpy((void *)(addr), (void *)(opmsg_source_sync_stall_changed_ptr), 4)
#define OPMSG_SOURCE_SYNC_STALL_CHANGED_UNMARSHALL(addr, opmsg_source_sync_stall_changed_ptr) memcpy((void *)(opmsg_source_sync_stall_changed_ptr), (void *)(addr), 4)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Swbs_Dec_Codec_Mode

  DESCRIPTION
    SWBS_DEC operator message for CODEC_MODE.

  MEMBERS
    message_id - message id
    codec_mode - set SWBS codec mode
...
/* The following macros take OPMSG_SWBS_DEC_CODEC_MODE *opmsg_swbs_dec_codec_mode_ptr */
#define OPMSG_SWBS_DEC_CODEC_MODE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SWBS_DEC_CODEC_MODE_MESSAGE_ID_GET(opmsg_swbs_dec_codec_mode_ptr) ((OPMSG_SWBS_DEC_ID)(opmsg_swbs_dec_codec_mode_ptr)->_data[0])
#define OPMSG_SWBS_DEC_CODEC_MODE_MESSAGE_ID_SET(opmsg_swbs_dec_codec_mode_ptr, message_id) ((opmsg_swbs_dec_codec_mode_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SWBS_DEC_CODEC_MODE_CODEC_MODE_WORD_OFFSET (1)
#define OPMSG_SWBS_DEC_CODEC_MODE_CODEC_MODE_GET(opmsg_swbs_dec_codec_mode_ptr) ((opmsg_swbs_dec_codec_mode_ptr)->_data[1])
#define OPMSG_SWBS_DEC_CODEC_MODE_CODEC_MODE_SET(opmsg_swbs_dec_codec_mode_ptr, codec_mode) ((opmsg_swbs_dec_codec_mode_ptr)->_data[1] = (uint16)(codec_mode))
#define OPMSG_SWBS_DEC_CODEC_MODE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SWBS_DEC_CODEC_MODE_CREATE(message_id, codec_mode) \
    (uint16)(message_id), \
    (uint16)(codec_mode)
#define OPMSG_SWBS_DEC_CODEC_MODE_PACK(opmsg_swbs_dec_codec_mode_ptr, message_id, codec_mode) \
    do { \
        (opmsg_swbs_dec_codec_mode_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_swbs_dec_codec_mode_ptr)->_data[1] = (uint16)((uint16)(codec_mode)); \
    } while (0)

#define OPMSG_SWBS_DEC_CODEC_MODE_MARSHALL(addr, opmsg_swbs_dec_codec_mode_ptr) memcpy((void *)(addr), (void *)(opmsg_swbs_dec_codec_mode_ptr), 2)
#define OPMSG_SWBS_DEC_CODEC_MODE_UNMARSHALL(addr, opmsg_swbs_dec_codec_mode_ptr) memcpy((void *)(opmsg_swbs_dec_codec_mode_ptr), (void *)(addr), 2)


/*******************************************************************************

  NAME
    Opmsg_Swbs_Dec_Force_Plc_Off

  DESCRIPTION
    SWBS_DEC operator message for FORCE_PLC_OFF.
...
/* The following macros take OPMSG_SWBS_DEC_FORCE_PLC_OFF *opmsg_swbs_dec_force_plc_off_ptr */
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_MESSAGE_ID_GET(opmsg_swbs_dec_force_plc_off_ptr) ((opmsg_swbs_dec_force_plc_off_ptr)->_data[0])
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_MESSAGE_ID_SET(opmsg_swbs_dec_force_plc_off_ptr, message_id) ((opmsg_swbs_dec_force_plc_off_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_FORCE_PLC_OFF_WORD_OFFSET (1)
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_FORCE_PLC_OFF_GET(opmsg_swbs_dec_force_plc_off_ptr) ((opmsg_swbs_dec_force_plc_off_ptr)->_data[1])
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_FORCE_PLC_OFF_SET(opmsg_swbs_dec_force_plc_off_ptr, force_plc_off) ((opmsg_swbs_dec_force_plc_off_ptr)->_data[1] = (uint16)(force_plc_off))
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_CREATE(message_id, force_plc_off) \
    (uint16)(message_id), \
    (uint16)(force_plc_off)
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_PACK(opmsg_swbs_dec_force_plc_off_ptr, message_id, force_plc_off) \
    do { \
        (opmsg_swbs_dec_force_plc_off_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_swbs_dec_force_plc_off_ptr)->_data[1] = (uint16)((uint16)(force_plc_off)); \
    } while (0)

#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_MARSHALL(addr, opmsg_swbs_dec_force_plc_off_ptr) memcpy((void *)(addr), (void *)(opmsg_swbs_dec_force_plc_off_ptr), 2)
#define OPMSG_SWBS_DEC_FORCE_PLC_OFF_UNMARSHALL(addr, opmsg_swbs_dec_force_plc_off_ptr) memcpy((void *)(opmsg_swbs_dec_force_plc_off_ptr), (void *)(addr), 2)


/*******************************************************************************

  NAME
    Opmsg_Swbs_Enc_Codec_Mode

  DESCRIPTION
    SWBS_ENC operator message for CODEC_MODE.
...
/* The following macros take OPMSG_SWBS_ENC_CODEC_MODE *opmsg_swbs_enc_codec_mode_ptr */
#define OPMSG_SWBS_ENC_CODEC_MODE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_SWBS_ENC_CODEC_MODE_MESSAGE_ID_GET(opmsg_swbs_enc_codec_mode_ptr) ((OPMSG_SWBS_ENC_ID)(opmsg_swbs_enc_codec_mode_ptr)->_data[0])
#define OPMSG_SWBS_ENC_CODEC_MODE_MESSAGE_ID_SET(opmsg_swbs_enc_codec_mode_ptr, message_id) ((opmsg_swbs_enc_codec_mode_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_SWBS_ENC_CODEC_MODE_CODEC_MODE_WORD_OFFSET (1)
#define OPMSG_SWBS_ENC_CODEC_MODE_CODEC_MODE_GET(opmsg_swbs_enc_codec_mode_ptr) ((opmsg_swbs_enc_codec_mode_ptr)->_data[1])
#define OPMSG_SWBS_ENC_CODEC_MODE_CODEC_MODE_SET(opmsg_swbs_enc_codec_mode_ptr, codec_mode) ((opmsg_swbs_enc_codec_mode_ptr)->_data[1] = (uint16)(codec_mode))
#define OPMSG_SWBS_ENC_CODEC_MODE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_SWBS_ENC_CODEC_MODE_CREATE(message_id, codec_mode) \
    (uint16)(message_id), \
    (uint16)(codec_mode)
#define OPMSG_SWBS_ENC_CODEC_MODE_PACK(opmsg_swbs_enc_codec_mode_ptr, message_id, codec_mode) \
    do { \
        (opmsg_swbs_enc_codec_mode_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_swbs_enc_codec_mode_ptr)->_data[1] = (uint16)((uint16)(codec_mode)); \
    } while (0)

#define OPMSG_SWBS_ENC_CODEC_MODE_MARSHALL(addr, opmsg_swbs_enc_codec_mode_ptr) memcpy((void *)(addr), (void *)(opmsg_swbs_enc_codec_mode_ptr), 2)
#define OPMSG_SWBS_ENC_CODEC_MODE_UNMARSHALL(addr, opmsg_swbs_enc_codec_mode_ptr) memcpy((void *)(opmsg_swbs_enc_codec_mode_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Test_VA_FWD_ME

  DESCRIPTION
    TEST_VA operator message for FWD_ME.

  MEMBERS
    message_id - message id
    sender     - the (faked) sender of this message
    number     - A magic number to identify the message instance
...
/* The following macros take OPMSG_TEST_VA_FWD_ME *opmsg_test_va_fwd_me_ptr */
#define OPMSG_TEST_VA_FWD_ME_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_TEST_VA_FWD_ME_MESSAGE_ID_GET(opmsg_test_va_fwd_me_ptr) ((OPMSG_TEST_VA_ID)(opmsg_test_va_fwd_me_ptr)->_data[0])
#define OPMSG_TEST_VA_FWD_ME_MESSAGE_ID_SET(opmsg_test_va_fwd_me_ptr, message_id) ((opmsg_test_va_fwd_me_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_TEST_VA_FWD_ME_SENDER_WORD_OFFSET (1)
#define OPMSG_TEST_VA_FWD_ME_SENDER_GET(opmsg_test_va_fwd_me_ptr) ((opmsg_test_va_fwd_me_ptr)->_data[1])
#define OPMSG_TEST_VA_FWD_ME_SENDER_SET(opmsg_test_va_fwd_me_ptr, sender) ((opmsg_test_va_fwd_me_ptr)->_data[1] = (uint16)(sender))
#define OPMSG_TEST_VA_FWD_ME_NUMBER_WORD_OFFSET (2)
#define OPMSG_TEST_VA_FWD_ME_NUMBER_GET(opmsg_test_va_fwd_me_ptr) ((opmsg_test_va_fwd_me_ptr)->_data[2])
#define OPMSG_TEST_VA_FWD_ME_NUMBER_SET(opmsg_test_va_fwd_me_ptr, number) ((opmsg_test_va_fwd_me_ptr)->_data[2] = (uint16)(number))
#define OPMSG_TEST_VA_FWD_ME_WORD_SIZE (3)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_TEST_VA_FWD_ME_CREATE(message_id, sender, number) \
    (uint16)(message_id), \
    (uint16)(sender), \
    (uint16)(number)
#define OPMSG_TEST_VA_FWD_ME_PACK(opmsg_test_va_fwd_me_ptr, message_id, sender, number) \
    do { \
        (opmsg_test_va_fwd_me_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_test_va_fwd_me_ptr)->_data[1] = (uint16)((uint16)(sender)); \
        (opmsg_test_va_fwd_me_ptr)->_data[2] = (uint16)((uint16)(number)); \
    } while (0)

#define OPMSG_TEST_VA_FWD_ME_MARSHALL(addr, opmsg_test_va_fwd_me_ptr) memcpy((void *)(addr), (void *)(opmsg_test_va_fwd_me_ptr), 3)
#define OPMSG_TEST_VA_FWD_ME_UNMARSHALL(addr, opmsg_test_va_fwd_me_ptr) memcpy((void *)(opmsg_test_va_fwd_me_ptr), (void *)(addr), 3)


/*******************************************************************************

  NAME
    Opmsg_Test_VA_Set_QID

  DESCRIPTION
    TEST_VA operator message for SET_QID.
...
/* The following macros take OPMSG_TEST_VA_SET_QID *opmsg_test_va_set_qid_ptr */
#define OPMSG_TEST_VA_SET_QID_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_TEST_VA_SET_QID_MESSAGE_ID_GET(opmsg_test_va_set_qid_ptr) ((OPMSG_TEST_VA_ID)(opmsg_test_va_set_qid_ptr)->_data[0])
#define OPMSG_TEST_VA_SET_QID_MESSAGE_ID_SET(opmsg_test_va_set_qid_ptr, message_id) ((opmsg_test_va_set_qid_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_TEST_VA_SET_QID_QID_WORD_OFFSET (1)
#define OPMSG_TEST_VA_SET_QID_QID_GET(opmsg_test_va_set_qid_ptr) ((opmsg_test_va_set_qid_ptr)->_data[1])
#define OPMSG_TEST_VA_SET_QID_QID_SET(opmsg_test_va_set_qid_ptr, qid) ((opmsg_test_va_set_qid_ptr)->_data[1] = (uint16)(qid))
#define OPMSG_TEST_VA_SET_QID_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_TEST_VA_SET_QID_CREATE(message_id, qid) \
    (uint16)(message_id), \
    (uint16)(qid)
#define OPMSG_TEST_VA_SET_QID_PACK(opmsg_test_va_set_qid_ptr, message_id, qid) \
    do { \
        (opmsg_test_va_set_qid_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_test_va_set_qid_ptr)->_data[1] = (uint16)((uint16)(qid)); \
    } while (0)

#define OPMSG_TEST_VA_SET_QID_MARSHALL(addr, opmsg_test_va_set_qid_ptr) memcpy((void *)(addr), (void *)(opmsg_test_va_set_qid_ptr), 2)
#define OPMSG_TEST_VA_SET_QID_UNMARSHALL(addr, opmsg_test_va_set_qid_ptr) memcpy((void *)(opmsg_test_va_set_qid_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  DESCRIPTION
    Unsolicited message indicating AANC event
...
/* The following macros take OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER *opmsg_unsolicited_aanc_event_trigger_ptr */
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_ID_WORD_OFFSET (0)
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_ID_GET(opmsg_unsolicited_aanc_event_trigger_ptr) ((opmsg_unsolicited_aanc_event_trigger_ptr)->_data[0])
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_ID_SET(opmsg_unsolicited_aanc_event_trigger_ptr, id) ((opmsg_unsolicited_aanc_event_trigger_ptr)->_data[0] = (uint16)(id))
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_PAYLOAD_WORD_OFFSET (1)
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_PAYLOAD_GET(opmsg_unsolicited_aanc_event_trigger_ptr) ((opmsg_unsolicited_aanc_event_trigger_ptr)->_data[1])
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_PAYLOAD_SET(opmsg_unsolicited_aanc_event_trigger_ptr, payload) ((opmsg_unsolicited_aanc_event_trigger_ptr)->_data[1] = (uint16)(payload))
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_CREATE(id, payload) \
    (uint16)(id), \
    (uint16)(payload)
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_PACK(opmsg_unsolicited_aanc_event_trigger_ptr, id, payload) \
    do { \
        (opmsg_unsolicited_aanc_event_trigger_ptr)->_data[0] = (uint16)((uint16)(id)); \
        (opmsg_unsolicited_aanc_event_trigger_ptr)->_data[1] = (uint16)((uint16)(payload)); \
    } while (0)

#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_MARSHALL(addr, opmsg_unsolicited_aanc_event_trigger_ptr) memcpy((void *)(addr), (void *)(opmsg_unsolicited_aanc_event_trigger_ptr), 2)
#define OPMSG_UNSOLICITED_AANC_EVENT_TRIGGER_UNMARSHALL(addr, opmsg_unsolicited_aanc_event_trigger_ptr) memcpy((void *)(opmsg_unsolicited_aanc_event_trigger_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  DESCRIPTION
    Unsolicited message to communicate AANC information

  MEMBERS
    flags          - AANC capability flags
    ff_coarse_gain - AANC feedforward coarse gain
    ff_fine_gain   - AANC feedforward fine gain
    fb_coarse_gain - AANC feedback coarse gain
    fb_fine_gain   - AANC feedback fine gain
    ec_coarse_gain - AANC EC coarse gain
    ec_fine_gain   - AANC EC fine gain
    anc_instance   - ANC HW instance
...
/* The following macros take OPMSG_UNSOLICITED_AANC_INFO *opmsg_unsolicited_aanc_info_ptr */
#define OPMSG_UNSOLICITED_AANC_INFO_FLAGS_WORD_OFFSET (0)
#define OPMSG_UNSOLICITED_AANC_INFO_FLAGS_GET(opmsg_unsolicited_aanc_info_ptr)  \
    (((uint32)((opmsg_unsolicited_aanc_info_ptr)->_data[0]) | \
      ((uint32)((opmsg_unsolicited_aanc_info_ptr)->_data[1]) << 16)))
#define OPMSG_UNSOLICITED_AANC_INFO_FLAGS_SET(opmsg_unsolicited_aanc_info_ptr, flags) do { \
        (opmsg_unsolicited_aanc_info_ptr)->_data[0] = (uint16)((flags) & 0xffff); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[1] = (uint16)((flags) >> 16); } while (0)
#define OPMSG_UNSOLICITED_AANC_INFO_FF_COARSE_GAIN_WORD_OFFSET (2)
#define OPMSG_UNSOLICITED_AANC_INFO_FF_COARSE_GAIN_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[2])
#define OPMSG_UNSOLICITED_AANC_INFO_FF_COARSE_GAIN_SET(opmsg_unsolicited_aanc_info_ptr, ff_coarse_gain) ((opmsg_unsolicited_aanc_info_ptr)->_data[2] = (uint16)(ff_coarse_gain))
#define OPMSG_UNSOLICITED_AANC_INFO_FF_FINE_GAIN_WORD_OFFSET (3)
#define OPMSG_UNSOLICITED_AANC_INFO_FF_FINE_GAIN_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[3])
#define OPMSG_UNSOLICITED_AANC_INFO_FF_FINE_GAIN_SET(opmsg_unsolicited_aanc_info_ptr, ff_fine_gain) ((opmsg_unsolicited_aanc_info_ptr)->_data[3] = (uint16)(ff_fine_gain))
#define OPMSG_UNSOLICITED_AANC_INFO_FB_COARSE_GAIN_WORD_OFFSET (4)
#define OPMSG_UNSOLICITED_AANC_INFO_FB_COARSE_GAIN_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[4])
#define OPMSG_UNSOLICITED_AANC_INFO_FB_COARSE_GAIN_SET(opmsg_unsolicited_aanc_info_ptr, fb_coarse_gain) ((opmsg_unsolicited_aanc_info_ptr)->_data[4] = (uint16)(fb_coarse_gain))
#define OPMSG_UNSOLICITED_AANC_INFO_FB_FINE_GAIN_WORD_OFFSET (5)
#define OPMSG_UNSOLICITED_AANC_INFO_FB_FINE_GAIN_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[5])
#define OPMSG_UNSOLICITED_AANC_INFO_FB_FINE_GAIN_SET(opmsg_unsolicited_aanc_info_ptr, fb_fine_gain) ((opmsg_unsolicited_aanc_info_ptr)->_data[5] = (uint16)(fb_fine_gain))
#define OPMSG_UNSOLICITED_AANC_INFO_EC_COARSE_GAIN_WORD_OFFSET (6)
#define OPMSG_UNSOLICITED_AANC_INFO_EC_COARSE_GAIN_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[6])
#define OPMSG_UNSOLICITED_AANC_INFO_EC_COARSE_GAIN_SET(opmsg_unsolicited_aanc_info_ptr, ec_coarse_gain) ((opmsg_unsolicited_aanc_info_ptr)->_data[6] = (uint16)(ec_coarse_gain))
#define OPMSG_UNSOLICITED_AANC_INFO_EC_FINE_GAIN_WORD_OFFSET (7)
#define OPMSG_UNSOLICITED_AANC_INFO_EC_FINE_GAIN_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[7])
#define OPMSG_UNSOLICITED_AANC_INFO_EC_FINE_GAIN_SET(opmsg_unsolicited_aanc_info_ptr, ec_fine_gain) ((opmsg_unsolicited_aanc_info_ptr)->_data[7] = (uint16)(ec_fine_gain))
#define OPMSG_UNSOLICITED_AANC_INFO_ANC_INSTANCE_WORD_OFFSET (8)
#define OPMSG_UNSOLICITED_AANC_INFO_ANC_INSTANCE_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[8])
#define OPMSG_UNSOLICITED_AANC_INFO_ANC_INSTANCE_SET(opmsg_unsolicited_aanc_info_ptr, anc_instance) ((opmsg_unsolicited_aanc_info_ptr)->_data[8] = (uint16)(anc_instance))
#define OPMSG_UNSOLICITED_AANC_INFO_FILTER_CONFIG_WORD_OFFSET (9)
#define OPMSG_UNSOLICITED_AANC_INFO_FILTER_CONFIG_GET(opmsg_unsolicited_aanc_info_ptr) ((opmsg_unsolicited_aanc_info_ptr)->_data[9])
#define OPMSG_UNSOLICITED_AANC_INFO_FILTER_CONFIG_SET(opmsg_unsolicited_aanc_info_ptr, filter_config) ((opmsg_unsolicited_aanc_info_ptr)->_data[9] = (uint16)(filter_config))
#define OPMSG_UNSOLICITED_AANC_INFO_WORD_SIZE (10)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_UNSOLICITED_AANC_INFO_CREATE(flags, ff_coarse_gain, ff_fine_gain, fb_coarse_gain, fb_fine_gain, ec_coarse_gain, ec_fine_gain, anc_instance, filter_config) \
    (uint16)((flags) & 0xffff), \
    (uint16)((flags) >> 16), \
    (uint16)(ff_coarse_gain), \
    (uint16)(ff_fine_gain), \
    (uint16)(fb_coarse_gain), \
    (uint16)(fb_fine_gain), \
    (uint16)(ec_coarse_gain), \
    (uint16)(ec_fine_gain), \
    (uint16)(anc_instance), \
    (uint16)(filter_config)
#define OPMSG_UNSOLICITED_AANC_INFO_PACK(opmsg_unsolicited_aanc_info_ptr, flags, ff_coarse_gain, ff_fine_gain, fb_coarse_gain, fb_fine_gain, ec_coarse_gain, ec_fine_gain, anc_instance, filter_config) \
    do { \
        (opmsg_unsolicited_aanc_info_ptr)->_data[0] = (uint16)((uint16)((flags) & 0xffff)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[1] = (uint16)(((flags) >> 16)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[2] = (uint16)((uint16)(ff_coarse_gain)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[3] = (uint16)((uint16)(ff_fine_gain)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[4] = (uint16)((uint16)(fb_coarse_gain)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[5] = (uint16)((uint16)(fb_fine_gain)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[6] = (uint16)((uint16)(ec_coarse_gain)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[7] = (uint16)((uint16)(ec_fine_gain)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[8] = (uint16)((uint16)(anc_instance)); \
        (opmsg_unsolicited_aanc_info_ptr)->_data[9] = (uint16)((uint16)(filter_config)); \
    } while (0)

#define OPMSG_UNSOLICITED_AANC_INFO_MARSHALL(addr, opmsg_unsolicited_aanc_info_ptr) memcpy((void *)(addr), (void *)(opmsg_unsolicited_aanc_info_ptr), 10)
#define OPMSG_UNSOLICITED_AANC_INFO_UNMARSHALL(addr, opmsg_unsolicited_aanc_info_ptr) memcpy((void *)(opmsg_unsolicited_aanc_info_ptr), (void *)(addr), 10)
...
/*******************************************************************************
  ...
  DESCRIPTION
    Unsolicited message indicating EFT event
...
/* The following macros take OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER *opmsg_unsolicited_eft_event_trigger_ptr */
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_ID_WORD_OFFSET (0)
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_ID_GET(opmsg_unsolicited_eft_event_trigger_ptr) ((opmsg_unsolicited_eft_event_trigger_ptr)->_data[0])
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_ID_SET(opmsg_unsolicited_eft_event_trigger_ptr, id) ((opmsg_unsolicited_eft_event_trigger_ptr)->_data[0] = (uint16)(id))
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_PAYLOAD_WORD_OFFSET (1)
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_PAYLOAD_GET(opmsg_unsolicited_eft_event_trigger_ptr) ((opmsg_unsolicited_eft_event_trigger_ptr)->_data[1])
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_PAYLOAD_SET(opmsg_unsolicited_eft_event_trigger_ptr, payload) ((opmsg_unsolicited_eft_event_trigger_ptr)->_data[1] = (uint16)(payload))
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_CREATE(id, payload) \
    (uint16)(id), \
    (uint16)(payload)
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_PACK(opmsg_unsolicited_eft_event_trigger_ptr, id, payload) \
    do { \
        (opmsg_unsolicited_eft_event_trigger_ptr)->_data[0] = (uint16)((uint16)(id)); \
        (opmsg_unsolicited_eft_event_trigger_ptr)->_data[1] = (uint16)((uint16)(payload)); \
    } while (0)

#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_MARSHALL(addr, opmsg_unsolicited_eft_event_trigger_ptr) memcpy((void *)(addr), (void *)(opmsg_unsolicited_eft_event_trigger_ptr), 2)
#define OPMSG_UNSOLICITED_EFT_EVENT_TRIGGER_UNMARSHALL(addr, opmsg_unsolicited_eft_event_trigger_ptr) memcpy((void *)(opmsg_unsolicited_eft_event_trigger_ptr), (void *)(addr), 2)
