...
void sppServiceHandler(Task task, MessageId id, Message message)
{
    ...
    if (sppsClientTask)
    {
        ...
        default:
            ...
            SPP_DEBUG(("sppServiceHandler - unexpected msg type MESSAGE:0x%x\n", id));
            break;
        }
    }
    else
...
void SppStartServiceWithChannel(Task theAppTask, uint8 requested_rfcomm)
{
...
{
    SppStartServiceWithChannel(theAppTask, SPP_DEFAULT_CHANNEL);
}
