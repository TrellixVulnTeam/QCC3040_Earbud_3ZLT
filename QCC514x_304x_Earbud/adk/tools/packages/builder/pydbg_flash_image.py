...
class Pyflash(object):
    ...
    def device_reset_no_apps(self):
        print("Reset the chip without starting Apps")
        cucmd = self.device.chip.curator_subsystem.core.fw.cucmd 
        toolcmdid, payload = 48, [2]
        cucmd._write_params([len(payload)+1, toolcmdid] + payload)
        cucmd._send_cmd(cucmd._cmd_code.TUNNEL_TOOLCMD_REQ, timeout=None, wait_for_completion=False)
