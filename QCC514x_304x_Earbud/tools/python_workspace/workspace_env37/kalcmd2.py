...
class kalcmd2_version(object):
    '''
        ...
        to compare to the version returned by Kalsim. Additionally
        gives a revision number that indicates changes to the kalcmd2
        Python interface that do not affect the network interface.
    '''
    MAJOR = 2
    MINOR = 8
    REVISION = 2
...
class kalmsg_connection(object):
    '''
    A kalmsg_connection represents a connection to Kalsim. This class
    handles sending and receiving messages. It should be subclassed to handle
    connection to kalcmd2 in client or server mode.
    '''
    ...
    def send(self, msg):
        '''
        Send a single framed message to Kalsim. msg must be a bytearray or a sequence or collection that returns integers in the range [0,255].
        '''
        if (len(msg) // 256) > 255:
...
class kalmsg_server_connection(kalmsg_connection):
    '''
    A kalmsg_connection that handles connection to Kalsim with this script
    acting as the server.
    '''

    def __init__(self, accepted_socket):
        '''
        Construct a new kalmsg_connection communicating over the given accepted
...
class kalmsg_client_connection(kalmsg_connection):
    ...
    def __init__(self, host="localhost", port=7522):
        '''
        Construct a new kalmsg_connection by connecting to Kalsim at the given
        host and port.
        '''
...
class kalmem_spaces(object):
    '''
    Possible memory spaces that Kalcmd2 can use when accessing Kalsim memory.
...
class kal_widths(object):
    '''
    Possible data widths to use when accessing Kalsim memory.
    '''
    EIGHT_BIT = 0
    SIXTEEN_BIT = 1
    TWENTYFOUR_BIT = 2
...
class kal_regs(object):
    '''
    Kalimba registers accessible via Kalcmd2.
    '''
    REG_B0 = 0
    REG_B1 = 1
    REG_B4 = 2
...
class kalcmd_bluecore_msg_header(kalcmd_msg_header):
    '''
    Specific implementation for BlueCore messaging. At the moment it represents
...
class kalcmd(object):
    ...
    For queries it will return the data along with the return_code.
    '''

    def __init__(self, connection):
        '''
        Create an instance of Kalcmd using a kalmsg_connection instance.
        '''
        self.first_run = True
    ...
    def send_message(self, platform_header, data):
        '''
        Send a message to Kalsim. Only returns the response. Accepts both
    ...
    def get_version(self):
        '''
        Returns the major and minor Kalcmd2 version numbers of the currently
        connected Kalsim.
        '''
    ...
    def stream_create(self):
        ...
        if recv_buffer is None:
            return {'return_code': kalcmd_response.KALCMD_ERROR}
        elif recv_buffer[0] == kalcmd_response.KALCMD_OK:
    ...
    def stream_query_property(self, stream_id, key):
        '''
        Query a stream property when provided with a key. (Key is a string).
        '''
        ...
        for i in range(0, len(key)):
            try:
                send_buf[3 + i] = ord(key[i])   # key is str
            except TypeError:
                send_buf[3 + i] = key[i]        # key is bytes
        self.con.send(send_buf)
    ...
    def stream_change_property(self, stream_id, key, value):
        ...
        for i in range(0, len(key)):
            try:
                send_buf[4 + i] = ord(key[i])   # key is str
            except TypeError:
                send_buf[4 + i] = key[i]        # key is bytes

        for i in range(0, len(value)):
            try:
                send_buf[4 + len(key) + i] = ord(value[i])
            except TypeError:
                send_buf[4 + len(key) + i] = value[i]
    ...
    def stream_extract(self, stream_id, num_of_samples, sign_extend):
        ...
        if recv_buffer is None:
            ...
            if stream_format == kal_widths.EIGHT_BIT:
                ...
                # nonsense returned
                print("unknown format {0}".format(stream_format))
                return {'return_code': 1}
    ...
    # The maximum words for a block memory read or write that will fit in one kalcmd2 message
    # is 16382 (messages are limited to 2^16-1 bytes, and there are 6 bytes of overhead).
    # If an attempt is made to read or write more words than that, break it up.
    _MAX_WORDS_PER_BLOCK_MEM_OP = 16382
    ...
    def block_mem_write(self, mem_space, address, words):
        ...
        buf_size = (self._MAX_WORDS_PER_BLOCK_MEM_OP if num_words >= self._MAX_WORDS_PER_BLOCK_MEM_OP else num_words) * 4 + 6
        send_buf = bytearray([0] * buf_size)
        send_buf[0] = kalcmd_cmds.BLOCK_MEM_WRITE_CMD
        send_buf[1] = mem_space
        
        while words_written < num_words:
            num_words_this_message = num_words - words_written
            if num_words_this_message > self._MAX_WORDS_PER_BLOCK_MEM_OP:
                num_words_this_message = self._MAX_WORDS_PER_BLOCK_MEM_OP
                
            this_buf_size = num_words_this_message * 4 + 6
            if this_buf_size != buf_size:   # Happens on final loop of a multi-part write only.
            ...
            for i in range(num_words_this_message):
                word = words[i+words_written]
            ...
            words_written += num_words_this_message
            if words_written < num_words:
                if self._kal_arch == None:
                    res = self.get_kal_arch()
                    if res['return_code'] != kalcmd_response.KALCMD_OK:
                        return res
                if self._kal_arch == 4:
                    address += num_words_this_message * 4
                else:
                    address += num_words_this_message
    ...
    def block_mem_read(self, mem_space, address, num_words):
        ...
        words = None
        num_words_read = 0
        while num_words_read < num_words:
            num_words_this_message = num_words - num_words_read
            if num_words_this_message > self._MAX_WORDS_PER_BLOCK_MEM_OP:
                num_words_this_message = self._MAX_WORDS_PER_BLOCK_MEM_OP
            ...
            send_buf[4] = (address >> 16) & 0xFF
            send_buf[5] = (address >> 24) & 0xFF
            send_buf[6] = num_words_this_message & 0xFF
            send_buf[7] = (num_words_this_message >> 8) & 0xFF
            self.con.send(send_buf)
        
            recv_buffer = self.__receive_cmd_response(kalcmd_cmds.BLOCK_MEM_READ_CMD)
            if recv_buffer is None:
                return {'return_code': kalcmd_response.KALCMD_ERROR}
            if recv_buffer[0] != kalcmd_response.KALCMD_OK:
                return {'return_code': recv_buffer[0]}

            num_samples = int((len(recv_buffer) - 1) // 4)
            new_words = [0] * num_samples
            ...
            num_words_read += num_samples
            
            if num_words_read < num_words:
                if self._kal_arch == None:
                    res = self.get_kal_arch()
                    if res['return_code'] != kalcmd_response.KALCMD_OK:
                        return res
                if self._kal_arch == 4:
                    address += num_samples * 4
                else:
                    address += num_samples
                    
        return {'return_code': kalcmd_response.KALCMD_OK, 'words': words}
    ...
    def print_message(self, message):
        ...
        for c in message:
            try:
                send_buf[i] = ord(c)    # message is a str
            except TypeError:
                send_buf[i] = c         # message is a bytearray or bytes
            i = i + 1
        self.con.send(send_buf)
    ...
    def stream_flush(self, stream_id):
        '''
            Flush the stream with the given stream_id. Accepts
