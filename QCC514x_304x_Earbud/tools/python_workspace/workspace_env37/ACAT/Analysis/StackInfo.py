...
INT_FRAME_DESCRIPTORS_HYDRA_EXT = [
    'FP',
    'r0',
    'r1',
    'r2',
...
class StackInfo(Analysis.Analysis):
    ...
    def __init__(self, **kwarg):
        ...
        if Arch.chip_arch == "Hydra":
            ...
            self.int_frame_descriptors_ext = INT_FRAME_DESCRIPTORS_HYDRA_EXT
        else:
    ...
    def _is_interrupt_frame(self, frame_name, frame_length):
        ...
        return (
            ...
            len(self.int_frame_descriptors) <= frame_length  # no need to check for int_frame_descriptors_ext
        )
        ...
        for index, value in enumerate(stack_frame):
            ...
            if self.is_saved_rlink(value):
                ...
                if self._is_interrupt_frame(frame_name, len_stack_frame):
                    ...
                    if len_stack_frame == len(self.int_frame_descriptors):
                    ...
                    else:  # this is the newer implementation so use it default. It is a guess anyways
                        if self.int_frame_descriptors_ext[-(index + 1)] == "rLink":
                            # the next frame name will be ..
                            next_frame_name = label_name
                            guessed_return_address = address
    ...
    def _frame_to_string(self, frame, frame_name, frame_address):
        ...
        for index, address in enumerate(stack_frame):
            ...
            if self._is_interrupt_frame(frame_name, frame_length):
                ...
                # register.
                if frame_length == len(self.int_frame_descriptors):
                    len_int_frame = len(self.int_frame_descriptors)
                    frame_descriptors = self.int_frame_descriptors
                else:  # same reason
                    len_int_frame = len(self.int_frame_descriptors_ext)
                    frame_descriptors = self.int_frame_descriptors_ext

                int_frame_index = len_int_frame - 1 - index
                if int_frame_index >= 0:
                    register_name = frame_descriptors[int_frame_index]
                else:
