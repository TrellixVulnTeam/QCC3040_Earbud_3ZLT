...
MAX_SAMPLE_WRITE_SIZE = 10000


@wrapt.decorator
def read_data(wrapped, instance, args, kwargs):  # @DontTrace
    """log exception decorator, logs an exception if any exception is raised in the decorated
    function
...
class AudioRaw(AudioBase):
    ...
    def _sample_width_to_struct_fmt(self, sample_width, num=1):
        ...
        mod += '%s' % (num)
        if sample_width == 8:
            mod += 'B'  # NOTE this is unsigned data, others signed, to match wav file format
    ...
    def write(self):
        ...
        with open(self._filename, 'wb') as handler:
            ...
            for ind in range(0, len(self._audio[0]), MAX_SAMPLE_WRITE_SIZE):
                num = MAX_SAMPLE_WRITE_SIZE if (ind + MAX_SAMPLE_WRITE_SIZE) <= len(
                    self._audio[0]) else len(self._audio[0]) - ind
                tmp = [self._audio[chan][ind:ind + num] for chan in range(len(self._audio))]
                multi_data = [int(val) for pair in zip(*tmp) for val in pair]
                fmt = self._sample_width_to_struct_fmt(
                    self._param.sample_width, len(self._audio) * num)
