...
static Task sdp_search_cancel_task = NULL;

/****************************************************************************
NAME
    resetAppTaskMatchingSDPSearchRequests

DESCRIPTION
    Reset AppTask of matched pending SDP search messages which signifies to ignore handling such requests when delivered

RETURNS
    TRUE, if message is matched. FALSE otherwise.
*/
static bool resetAppTaskMatchingSDPSearchRequests(Task task, MessageId id, Message msg)
{
    bool matched = FALSE;
    UNUSED(task);
    ...
    switch (id)
    {
        ...
        {
            CL_INTERNAL_SDP_SERVICE_SEARCH_REQ_T *service_search_req = (CL_INTERNAL_SDP_SERVICE_SEARCH_REQ_T*) msg;
            if (service_search_req->theAppTask == sdp_search_cancel_task)
            {
                service_search_req->theAppTask = NULL;
                matched = TRUE;
            }
        }
        break;
        ...
        {
            CL_INTERNAL_SDP_ATTRIBUTE_SEARCH_REQ_T *attr_search_req = (CL_INTERNAL_SDP_ATTRIBUTE_SEARCH_REQ_T*) msg;
            if (attr_search_req->theAppTask == sdp_search_cancel_task)
            {
                attr_search_req->theAppTask = NULL;
                matched = TRUE;
            }
        }
        break;
        ...
        {
            CL_INTERNAL_SDP_SERVICE_SEARCH_ATTRIBUTE_REQ_T *service_search_attr_req = (CL_INTERNAL_SDP_SERVICE_SEARCH_ATTRIBUTE_REQ_T*) msg;
            if (service_search_attr_req->theAppTask == sdp_search_cancel_task)
            {
                service_search_attr_req->theAppTask = NULL;
                matched = TRUE;
            }
        }
        break;
        default:
        {
            /* Not interested in other CL internal messages */
        }
        break;
    }
    return matched;
...
{
    if (req->theAppTask == NULL)
    {
        /* Looks like client task has cancelled this request, ignore handling */
        return;
    }
...
void connectionHandleSdpTerminatePrimitiveRequest(const connectionSdpState *state, const CL_INTERNAL_SDP_TERMINATE_PRIMITIVE_REQ_T *req)
{
    ...
    /* Reset the application task in pending SDP search messages which are matching with the requesting AppTask.
     * As a result, these SDP search requests get ignored upon delivery based on validation of AppTask.
     * Note: It's important to look for pending messages although requesting appTask is same as active SDP search task.
     *          Because there could be messages pending for same appTask due to multiple search requests.
	*/
    sdp_search_cancel_task = req->theAppTask;
    /* Just making sure to seek through all vm queued messages. There could be multiple search requests received from same client task */
    MessagePendingMatch(connectionGetCmTask(), FALSE, resetAppTaskMatchingSDPSearchRequests);
