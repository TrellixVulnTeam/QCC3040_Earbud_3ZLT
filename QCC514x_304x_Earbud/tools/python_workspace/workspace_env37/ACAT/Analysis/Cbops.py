...
class Cbops(Analysis.Analysis):
    ...
    def analyse_cbops_graph(
        ...
        num_io = cbops_graph['num_io'].value
    ...
    def _get_cbops_graph_buffers(self, cbops_graph, show_graph_buffers=False):
        ...
        buffers_p = cbops_graph['buffers'].address
        ...
        for buf_no in range(num_io):
            ...
            if cbops_buffer['buffer'].value != 0:
                ...
                if show_graph_buffers:
                    ...
                    buf_type = self._get_cbops_buffer_type(
                        cbops_buffer['type'].value
                    )
                    self.formatter.output("type: {}".format(buf_type))
                    self.formatter.output(
                        "buffer: 0x{0:x}".
                        format(cbops_buffer['buffer'].value)
                    )
                    self.formatter.output(
                        "base: 0x{0:x}".
                        format(cbops_buffer['base'].value)
                    )
                    self.formatter.output(
                        "size: 0x{0:x}".
                        format(cbops_buffer['size'].value)
                    )
                    self.formatter.output(
                        "rw_ptr: 0x{0:x}".
                        format(cbops_buffer['rw_ptr'].value)
                    )
                    self.formatter.output(
                        "transfer_ptr: 0x{0:x}".
                        format(cbops_buffer['transfer_ptr'].value)
                    )
                    self.formatter.output(
                        "transfer_amount: 0x{0:x}".format(
                            cbops_buffer['transfer_amount'].value
                        )
                    )
                    self.formatter.section_end()
    ...
    def _analyse_cbops_graph_operators(self, cbops_graph):
        ...
        cbops_head = cbops_graph['first']
        tot_operators = 0

        try:
            parsed_linked_list = ch.parse_linked_list(
    ...
    def _analyse_override_operator(self, cbops_graph):
        ...
        override_data = cbops_graph['override_data'].value
        override_funcs = cbops_graph['override_funcs']
    ...
    def _analyse_cbops_operator(self, cbops_op):
        ...
        cbops_op_parameter_area_start = cbops_op['parameter_area_start'].address

        # now read the header parameters
        cbops_param_hdr = self.chipdata.cast(
            cbops_op_parameter_area_start, "cbops_param_hdr"
        )
        nr_inputs = cbops_param_hdr['nr_inputs'].value
        nr_outputs = cbops_param_hdr['nr_outputs'].value
        index_table_addr = cbops_param_hdr['index_table'].address
        ...
        prev_operator_addr = cbops_op['prev_operator_addr'].value
        next_operator_addr = cbops_op['next_operator_addr'].value
