...
{
    /* Whether the bundle should be protected from unloading */
...
void customOperatorProtectBundle(FILE_INDEX bundle_file_index)
{
    ...
    for(bundle_node = &head_of_bundle_list; *bundle_node != NULL; bundle_node = &(*bundle_node)->next)
    {
        if((*bundle_node)->file_index == bundle_file_index)
        {
            (*bundle_node)->protected = TRUE;
        }
    }
}
...
void customOperatorUnloadProtectedBundleFromCapId(capability_id_t cap_id)
{
    ...
    for(bundle_node = &head_of_bundle_list; *bundle_node != NULL; bundle_node = &(*bundle_node)->next)
    {
        if ((*bundle_node)->file_index == file_index && (*bundle_node)->protected)
        {
            (*bundle_node)->protected = FALSE;
            unloadBundleIfNoUsers(*bundle_node);
            break;
        }
    }
...
uint32 customOperatorBundleSize(capability_id_t cap_id)
{
    ...
    dkcs_info_t info;
    if (dkcsReadInfo(cap_id, file_index, &info))
    {
        result = info.kdc_pm_length;
    }
