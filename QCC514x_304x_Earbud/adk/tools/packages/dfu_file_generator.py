...
# In the crescendo.upd file, name of the rw config fs file
RWCONFIGFS = "user_ps_filesystem.xuv"
...
class dfu_file_generator():
    ...
    def initiate(self):
        ...
        self.rw_config_fs = self.create_rw_config_fs()
    ...
    # Apps read-write config filesystem
    def create_rw_config_fs(self):
        ret = False
        for file_type in range(len(flash0["layout"])):
            if flash0["layout"][file_type][0] == "rw_config":
                try:
                    ret = self.offset_xuv(os.path.join(self.dfu_dir, "dfu_rw_config.xuv"), RWCONFIGFS)
                except KeyError:
                    print("Partial update: no %s\n" % flash0["layout"][file_type][0])
                    sys.stdout.flush()
                    return True
