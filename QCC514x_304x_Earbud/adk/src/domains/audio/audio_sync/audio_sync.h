...
/*! \brief Simplified audio states used for synchronisation.
    ...
    AUDIO_SYNC_STATE_READY:
        a2dp media channel is in the streaming state

    AUDIO_SYNC_STATE_ACTIVE:
        a2dp media channel is in the streaming state and audio is routed
*/
typedef enum
{
...
typedef enum
{
    ...
    AUDIO_SYNC_PREPARE_IND,
    AUDIO_SYNC_PREPARE_RES,
...
/*! \brief Request to prepare for a change to "Active" state.

    Sent by the audio source to request that the audio sink does what is
    needed to prepare for the AUDIO_SYNC_STATE_ACTIVE state. 

    The audio sink must send a AUDIO_SYNC_PREPARE_RES in response, even if
    it cannot reach the requested state.
*/
typedef struct
{
    ...
    uint8 seid; /*!< Currently active SEID */
    uint8 sync_id; /*!< The sequence ID for this sync indication. */
} AUDIO_SYNC_PREPARE_IND_T;

/*! \brief Response to a AUDIO_SYNC_PREPARE_IND */
typedef struct
{
    uint8 sync_id; /*!< The sequence ID for this sync indication. */
    audio_sync_reason_t  reason; /*!< Indicates whether the response is from the sink or due to timeout */
} AUDIO_SYNC_PREPARE_RES_T;
...
/*! \brief Indicate that an audio sink must become active and expect a reply when complete.
    ...
    The indication is sent as a AUDIO_SYNC_PREPARE_IND to an audio sink instance.

    The audio sink instance represented by #sync_inst must do what is necessary
    to prepare for active state. This should include any connection establishment
    or time consuming processes necessary for the sink to become active

    When the prepared the audio sink must reply with an AUDIO_SYNC_PREPARE_RES to 
    #reply_task with reason audio_sync_success

    This function will also send a response to #reply_task to cover the
    following cases:

    1) If #sync_inst is NULL (e.g. no audio_sync_t instance has been registered
         with the caller). A reponse message will be sent 'now' with reason
         audio_sync_not_required
    2) If the audio sink does not send the response in time. A response
         message will be sent with a time delay of #AudioSync_MaxPrepareSyncDelayMs()
         with reason audio_sync_timeout

    It is assumed that the caller will wait for the response message
    before continuing its normal operation.

    The caller will always wait for a response from the sink unless #sync_inst is NULL
    before indicating that audio has been connected. This will delay creation of the 
    audio chain until the #sync_inst is ready to become active.
    ...
    \param sync_id The sequence id of this specific connect indication message.
*/
void AudioSync_PrepareIndication(struct audio_sync_t *sync_inst, Task reply_task, audio_source_t source_id, uint8 seid, uint8 sync_id);
...
/*! \brief Maximum time to wait for audio sync connection or active transactions
    to complete after which initiating task will continue.
*/
#define AudioSync_MaxSyncDelayMs() 500

/*! \brief Maximum time to wait for audio sync prepare transactions to complete
    after which initiating task will continue.
*/
#define AudioSync_MaxPrepareSyncDelayMs() 1000
