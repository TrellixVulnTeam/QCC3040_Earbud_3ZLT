...
static void earbud_HandleUnexpectedMessage(MessageId id)
{
    ...
    DEBUG_LOG_VERBOSE("earbud_HandleUnexpectedMessage, MESSAGE:0x%x, sm = %d, av = %d", id, SmGetTaskData()->state, AvGetTaskData()->bitfields.state);
#elif defined(INCLUDE_AV)
    DEBUG_LOG_VERBOSE("earbud_HandleUnexpectedMessage, MESSAGE:0x%x, sm = %d, av = %d", id,  SmGetTaskData()->state, AvGetTaskData()->bitfields.state);
#elif defined(INCLUDE_HFP)
    DEBUG_LOG_VERBOSE("earbud_HandleUnexpectedMessage, MESSAGE:0x%x, sm = %d", id,  SmGetTaskData()->sm.state);
#else
    UNUSED(id);
#endif
}
...
{
    DEBUG_LOG_VERBOSE("earbud_HandleUnexpectedSysMessage, id = MESSAGE:0x%x", id);
}
