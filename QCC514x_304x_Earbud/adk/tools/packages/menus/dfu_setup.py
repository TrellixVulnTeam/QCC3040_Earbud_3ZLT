...
from menus import gen_rsa_pss_constants
...
def process_project(project, filespec, nowstring):
    """
    ...
    """
    test_string = "rsa_pss_constants" + os.sep + os.path.basename(filespec)
    for config in project.configurations.values():
        if config.name == "makefile_project":
            for list_file in project.files:
                if list_file.endswith(test_string):
                    return rename_and_replace_file(list_file, filespec, nowstring)
...
def main(args):
    ...
    replacepath = os.path.dirname(parsed_args.workspace)
    replace_rsa_pss_constants_c_file = os.path.join(replacepath, "rsa_pss_constants.c")
    os.chmod(replace_rsa_pss_constants_c_file, stat.S_IWRITE)
    shutil.copyfile(rsa_pss_constants_c_file, replace_rsa_pss_constants_c_file)
    print("'%s' copied" % replace_rsa_pss_constants_c_file)
    print("\nPlease rebuild library and application")
