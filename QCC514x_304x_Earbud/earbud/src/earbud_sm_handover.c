...
const marshal_type_info_t earbud_sm_marshal_types[] = {
    MARSHAL_TYPE_INFO(earbud_sm_timers, MARSHAL_TYPE_CATEGORY_GENERIC),
};

/*! Application handover interface types configuration. */ 
const marshal_type_list_t earbud_sm_marshal_types_list = {earbud_sm_marshal_types, ARRAY_DIM(earbud_sm_marshal_types)};
                              ...
                              void **marshal_obj)
{
    ...
    switch (type)
    {
        case MARSHAL_TYPE(earbud_sm_timers):
        {
            ...
            if (MessagePendingFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_BROADCAST, &first_due))
            {
                SmGetTaskData()->timers.out_of_ear_broadcast_mute = appSm_HandoverTimerAbsoluteTime(first_due);
                SmGetTaskData()->timers.out_of_ear_broadcast_mute_valid = 1;
                DEBUG_LOG_VERBOSE("appSm_HandoverMarshal SM_INTERNAL_TIMEOUT_OUT_OF_EAR_BROADCAST due ms %d", first_due);
                status = TRUE;
            }
            ...
            if (MessagePendingFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_IN_EAR_BROADCAST_START, &first_due))
            {
                SmGetTaskData()->timers.in_ear_broadcast_resume = appSm_HandoverTimerAbsoluteTime(first_due);
                SmGetTaskData()->timers.in_ear_broadcast_resume_valid = 1;
                DEBUG_LOG_VERBOSE("appSm_HandoverMarshal SM_INTERNAL_TIMEOUT_IN_EAR_BROADCAST_START due %d", first_due);
                status = TRUE;
            }
...
static void appSm_HandoverCommit(bool is_primary)
{
    ...
    */
    if (!is_primary)
    {
        ...
        MessageCancelFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_BROADCAST);
        ...
        MessageCancelFirst(SmGetTask(), SM_INTERNAL_TIMEOUT_IN_EAR_BROADCAST_START);
    }
    else
    {
        ...
        if (SmGetTaskData()->timers.out_of_ear_broadcast_mute_valid)
        {
            uint32 offset = appSm_HandoverTimerOffset(SmGetTaskData()->timers.out_of_ear_broadcast_mute);
            DEBUG_LOG_VERBOSE("appSm_HandoverCommit SM_INTERNAL_TIMEOUT_OUT_OF_EAR_BROADCAST %d", offset);
            MessageSendLater(SmGetTask(), SM_INTERNAL_TIMEOUT_OUT_OF_EAR_BROADCAST, NULL, offset);
        }
        ...
        if (SmGetTaskData()->timers.in_ear_broadcast_resume_valid)
        {
            uint32 offset = appSm_HandoverTimerOffset(SmGetTaskData()->timers.in_ear_broadcast_resume);
            DEBUG_LOG_VERBOSE("appSm_HandoverCommit SM_INTERNAL_TIMEOUT_IN_EAR_BROADCAST_START %d", offset);
            MessageSendLater(SmGetTask(), SM_INTERNAL_TIMEOUT_IN_EAR_BROADCAST_START, NULL, offset);
        }
    }

    /* Reset earbud_sm timers after handover commit */
    appSm_HandoverTimersReset();
}
