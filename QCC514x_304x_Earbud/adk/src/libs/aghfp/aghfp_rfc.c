...
void aghfpHandleRfcommClientConnectCfm(AGHFP *aghfp, CL_RFCOMM_CLIENT_CONNECT_CFM_T* cfm)
{
    ...
    /* Handle if a locally initiated SLC connection
       else ignore
    */
    else if (aghfpIsSlcLocallyInitiated(aghfp))
    {
        aghfpHandleRfcommConnectCfm(aghfp, cfm->status);
    }
    else
    {
        PRINT(("aghfpHandleRfcommClientConnectCfm ignoring CL_RFCOMM_CLIENT_CONNECT_CFM\n"));
    }
}
...
{
    /* Ignore failure-CFM from a remotely-initiated SLC,
       if a locally-initiated SLC is already in progress
    */
    if (!aghfpIsSlcLocallyInitiated(aghfp))
    {
        aghfpHandleRfcommConnectCfm(aghfp, cfm->status);
    }
    else
    {
        PRINT(("aghfpHandleRfcommServerConnectCfm ignoring SERVER_CONNECT_CFM. Status: %d\n", cfm->status));
    }
}
...
void aghfpHandleRfcommConnectInd(AGHFP *aghfp, const CL_RFCOMM_CONNECT_IND_T *ind)
{
    ...
    /* Set as a remotely initiated SLC connection */
    aghfpSlcLocallyInitiated(aghfp, FALSE);
