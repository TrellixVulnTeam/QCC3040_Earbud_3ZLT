...
try:
    from vm_build.elf2xuv import elf2xuv
    from vm_build.workspace_parse.workspace import Workspace
    
except ImportError:
    try:
        from elf2xuv.elf2xuv import elf2xuv
        
    except ImportError:
        from elf2xuv import elf2xuv
...
class PrepareSingleImage(object):
    ...
    def _determine_flash_config(self):
        ...
        if flash_config_file:
            workspace_directory = os.path.dirname(self._workspace_file)

            # The flash configuration file has been found
            if not os.path.isfile(flash_config_file):
                flash_config_file = os.path.abspath(os.path.join(workspace_directory, flash_config_file))
            ...
            if os.path.isfile(flash_config_file):
                try:
                    ...
                    self.flash_config['file_paths'] = {
                        'flash_config_file': flash_config_file.replace('\\', '/'),
                        'workspace_file': self._workspace_file.replace('\\', '/')
                    }
    ...
    def _do_elf2xuv(self, source, destination=None):
        ...
        """
        elf2xuv(0, source, xuv_filename=destination)
    ...
    @staticmethod
    def _relocate_image(input_file, output_file):
        """
    ...
    def _process_apps1_project(self, proj_parser, output, outpath, config):
        ...
        if self._build_output_folder is None:
        ...
        map = {}
        for i in range(len(self.flash_config["layout"])):
            map[self.flash_config["layout"][i][0]] = i
        if self.flash_config["layout"][map["apps_p1"]][1].get("src_file_signed"):
            # Signed .xuv file already exists, just copy it into place
            xuv_filename = output + ".xuv"
            source = os.path.abspath(os.path.join(build_output_folder, xuv_filename))
            self._do_copy(source, destination)
        else:
            # Convert .elf file into .xuv
    ...
    def determine_ro_fs_size(self, output_filename):
        """
        ...
        found = False
        for project in ws_projects.values():
            if "ro_fs" in project.filename:
                print("Processing project ro_fs")
                self.calculate_ro_fs_size(project.filename, output_filename)
                found = True
                break
        else:
            print("Processing failed project ro_fs not found")
        return found

    def calculate_ro_fs_size(self, project, output_filename):
        """
        calculate ro_fs edkcs, prompts, wuw and if write to json file
        """
        import maker.parse_proj_file as pproj
        proj_parser = pproj.Project(project, self._devkit_root, self._workspace_file)
        print("Determining size of ro_fs project='{}' -- output_filename='{}'".format(project, output_filename))

        edkcs_size = 0
        prompts_size = 0
        wuw_size = 0
        for srcfile in proj_parser.get_source_files():
            if srcfile.endswith(".edkcs"):
                edkcs_size += os.path.getsize(srcfile)
            elif srcfile.endswith(".sbc"):
                prompts_size += os.path.getsize(srcfile)
            elif 'WUW_bin' in srcfile:
                wuw_size += os.path.getsize(srcfile)

        table_data = {
            'edkcs': edkcs_size,
            'prompts_size': prompts_size,
            'wuw_size': wuw_size
        }

        # Serializing json
        json_object = json.dumps(table_data, sort_keys=True, indent=4)
        ...
        try:
            ...
            print("Unable to create %s: %s" % (output_filename, err))
        ...
        for config in config_list:
            ...
            if project_type is not None:
                if project_type not in self._projects.keys():
                    ...
                    elif project_type in ("firmware_config", "device_config", "customer_ro", "user_ps", "customer_va"):
    ...
    def create_flash_image(self, devkit, output_path, encrypted = False,
        ...
        flash_usage_datafile = None, ro_fs_usage_datafile= None):
        """
        ...
        xuv_filename = None
        if "customer_va" in self._projects:
            for ptn_index, ptn in enumerate(flash0["layout"]):
                # Cannot use map to find partition with a duplicated name
                if "ra_partition" in ptn and 'id' in ptn[1]:
                    xuv_filename = self._projects["customer_va"]
                    flash0["layout"][ptn_index] = self._add_layout("va_fs",
                        xuv_filename, flash0["layout"][ptn_index], encrypted)
                    break
        ...
        if flash_usage_datafile:
            print('Creating Flash usage data file [' + flash_usage_datafile + "]")
            PrepareSingleImage.save_flash_usage_table_to_file(builder, flash_usage_datafile)
        ...
        if ro_fs_usage_datafile:
            if self.determine_ro_fs_size(ro_fs_usage_datafile):
                print("ro_fs usage calculated\n")
    ...
    def print_flash_usage_table(builder):
        ...
        column_width = 15
        column_width_name = max(column_width, max(len(section["name"]) for section in builder.image_header.sections))

        header_row = "{:^{nw}s} |" + "{:^{w}s} |" * 4
        unit_blocks = "Blocks"
        unit_bytes = "Bytes"
        table_header = [
            header_row.format("Name", "Capacity", "Used", "Unused", "Image size", nw=column_width_name, w=column_width),
            header_row.format("", unit_blocks, unit_blocks, unit_blocks, unit_bytes, nw=column_width_name, w=column_width)
        ]
        ...
        for section in builder.image_header.sections:
            ...
            table_data.append(data_row.format(
                section["name"],
                "{:2d}".format(capacity_blocks),
                "{:2d}".format(used_blocks),
                "{:2d}".format(unused_blocks),
                "{:9d}".format(section["size"]),
                nw=column_width_name,
                w=column_width
            ))
        ...
        table_data.append(data_row.format(
            "Totals:",
            "{:2d}".format(totals['capacity']),
            "{:2d}".format(totals['used']),
            "{:2d}".format(totals['unused']),
            "{:9d}".format(totals['size']),
            nw=column_width_name,
            w=column_width
        ))
    ...
    @staticmethod
    def save_flash_usage_table_to_file(builder, output_filename):
        """
        ...
        table_data = {
            "file_paths": builder.config['file_paths'],
            "flash_device": {
        ...
        for section in builder.image_header.sections:
            ...
            table_data['layout'] += [
                {
                    'name': name,
                    'capacity': section.get("capacity", 0) // block_size,
                    'used': section["size"] // block_size + (section["size"] % block_size > 0),
                    'unused': section.get("unused", 0) // block_size,
                    'size': section["size"]
                }
            ]
