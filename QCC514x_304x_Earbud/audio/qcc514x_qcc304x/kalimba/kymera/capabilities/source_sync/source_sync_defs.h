...
#define SRC_SYNC_VERSION_MINOR 5

/** Downloadable build link compatibility, the operator data struct
 * is renamed from the ROM name when it grows.
 */
#define SRC_SYNC_OP_DATA SRC_SYNC_OP_DATA_V3_5
...
/** This describes the order of terminal buffer size fields */
enum {
    SRC_SYNC_OUTPUT_BUFFER_SIZE_INDEX = 0,
    SRC_SYNC_INPUT_BUFFER_SIZE_INDEX = 1,
    SRC_SYNC_TERMINAL_BUFFER_SIZE_COUNT = 2
};
...
/**
 * This describes the input and output buffer size fields
 * as an array for efficient access
 */
typedef struct src_sync_in_out_buffer_sizes
{
    uint16 terminal_buffer_sizes[SRC_SYNC_TERMINAL_BUFFER_SIZE_COUNT];

} SRC_SYNC_IN_OUT_BUFFER_SIZES;

/* capability-specific extra operator data */
typedef struct src_sync_exop_v3_5
{
    ...
    /**
     * Back kick mode set by SET_BACK_KICK_THRESHOLD message
     */
    OPMSG_COMMON_BACK_KICK_MODE back_kick_mode;
    ...
    /** Back kick threshold value from SET_BACK_KICK_THRESHOLD message */
    int16                   back_kick_threshold;
    ...
    /* Terminal buffer sizes, only distinguishing inputs and outputs */
    SRC_SYNC_IN_OUT_BUFFER_SIZES terminal_buffer_sizes;

#ifdef CAPABILITY_DOWNLOAD_BUILD
    /* Context for measuring downstream data, where possible including
     * amount in sink MMU buffer above silence insertion threshold */
    STREAM_SHORT_DOWNSTREAM_PROBE_RESULT downstream_probe;
#endif /* CAPABILITY_DOWNLOAD_BUILD */

    /** Keep pointer for calling capability API functions which need it */
    OPERATOR_DATA* op_data;

} SRC_SYNC_OP_DATA_V3_5;
...
#else
extern unsigned src_sync_sample_rate_to_inv_sample_rate(unsigned sample_rate);
#endif
