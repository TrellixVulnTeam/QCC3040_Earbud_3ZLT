...
#include "volume_control/vol_ctrl_gen_c.h"
#else
...
/* Capability Version */
#define VOL_CTRL_CAP_VERSION_MINOR            1
...
/** Bit mask with N LS bits 1, others 0 */
#define VOL_CTRL_LSBITS_MASK(N) (~(~0u<<(N)))

/** Bit mask for valid input terminals */
#define VOL_CTRL_INPUT_TERM_MASK VOL_CTRL_LSBITS_MASK(2*VOL_CTRL_CONSTANT_NUM_CHANNELS)

/** Bit mask for main input terminals: 8 least significant even bits */
#define VOL_CTRL_MAIN_INPUT_TERM_MASK (0x55555555u & VOL_CTRL_INPUT_TERM_MASK)

/** Bit mask for aux input terminals: 8 least significant odd bits */
#define VOL_CTRL_AUX_INPUT_TERM_MASK (0xaaaaaaaau & VOL_CTRL_INPUT_TERM_MASK)

/** Bit mask for main output terminals: 8 least significant bits */
#define VOL_CTRL_MAIN_OUTPUT_TERM_MASK VOL_CTRL_LSBITS_MASK(VOL_CTRL_CONSTANT_NUM_CHANNELS)
...
#endif
   /** Configured buffer size for main in/out */
   unsigned main_buffer_size_cfg;
   /** Configured aux input buffer size */
   unsigned aux_buffer_size_cfg;
 ...
 */
/* Assembly Functions */
extern void vol_ctrl_update_aux_state(VOL_CTRL_DATA_T *op_extra_data,unsigned num_auxs,vol_time_constants_t *lpvcs);
extern void vol_ctrl_compute_time_constants(unsigned sample_rate,unsigned vol_tc,vol_time_constants_t *lpvcs);
extern void vol_ctrl_update_channel(VOL_CTRL_DATA_T *op_extra_data, vol_ctrl_channel_t *chan_ptr,vol_ctrl_gains_t *volptr, vol_time_constants_t *lpvcs);
...
extern bool vol_ctlr_opmsg_set_buffer_size(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data);
extern bool vol_ctlr_opmsg_set_terminal_buffer_size(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data);
#ifdef VOLUME_CONTROL_AUX_TTP_SUPPORT
