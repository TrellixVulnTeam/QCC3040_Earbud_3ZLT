...
void stateProxy_GetInitialBatteryState(void)
{
    ...
    proxy->local_state->battery = BatteryRegion_GetState();
    proxy->local_state->battery_voltage = appBatteryGetVoltageAverage();
}

/*! \brief Handle local events for battery state change. */
static void stateProxy_HandleBatteryLevelUpdateStateImpl(const MESSAGE_BATTERY_REGION_UPDATE_STATE_T* state,
                                                    state_proxy_source source)
{
    ...
    {
        stateProxy_MarshalToConnectedPeer(MARSHAL_TYPE(MESSAGE_BATTERY_REGION_UPDATE_STATE_T), state, sizeof(*state));
    }
}
...
static void stateProxy_HandleBatteryLevelUpdateVoltageImpl(const MESSAGE_BATTERY_LEVEL_UPDATE_VOLTAGE_T* voltage,
                                                           state_proxy_source source)
{
    DEBUG_LOG("stateProxy_HandleBatteryLevelUpdateVoltage enum:state_proxy_source:%d, %d mV", source, voltage->voltage_mv);
...
void stateProxy_HandleBatteryLevelUpdateState(const MESSAGE_BATTERY_REGION_UPDATE_STATE_T* state)
{
...
void stateProxy_HandleRemoteBatteryLevelState(const MESSAGE_BATTERY_REGION_UPDATE_STATE_T* state)
{
...
void stateProxy_RegisterBatteryClient(void)
{
    ...
    (void)BatteryRegion_Register(stateProxy_GetTask());
