...
/* Macro to select the conftab to use, either the default or for Q2Q mode.
   remote_device should be passed in */
#define SELECT_CONFTAB(remDevice) (*((getAptxAdQ2qMode(remDevice) && remDevice->bitfields.use_large_mtu)?&a2dp_q2q_conftab:&a2dp_conftab))
...
static const uint16 a2dp_q2q_conftab[] =
{
    L2CAP_AUTOPT_SEPARATOR,                             /* START */
    L2CAP_AUTOPT_MTU_IN,            0x03F7,             /* Maximum inbound MTU for Q2Q - 1015 bytes */
    L2CAP_AUTOPT_MTU_OUT,           0x0030,             /* Minimum acceptable outbound MTU - 48 bytes */
    L2CAP_AUTOPT_FLUSH_IN,          0x0000, 0x0000,     /* Min acceptable remote flush timeout - zero*/
                                    0xFFFF, 0xFFFF,     /* Max acceptable remote flush timeout - infinite*/
    L2CAP_AUTOPT_FLUSH_OUT,         0xFFFF, 0xFFFF,     /* Min local flush timeout - infinite */
...
{
    DEBUG_LOG_FN_ENTRY("startLinklossTimer %p",(void*)device);
...
{
    DEBUG_LOG_FN_ENTRY("stopLinklossTimer %p",(void*)device);
...
static remote_device* findDeviceFromSink (Sink sink)
{
    ...
    if (sink != (Sink)NULL)
    {
        ...
        if ( SinkIsValid(sink) && SinkGetBdAddr(sink, &tpaddr) )
        {
            for (i=0; i<A2DP_MAX_REMOTE_DEVICES; i++)
            {
                ...
                {
                    DEBUG_LOG("findDeviceFromSink [%04X %02X %06lX] ", tpaddr.taddr.addr.nap, tpaddr.taddr.addr.uap, tpaddr.taddr.addr.lap);
                    DEBUG_LOG("findDeviceFromSink sink=%p id=%u ", (void*)sink, i);
    ...
    DEBUG_LOG_INFO("Sink=%p id=UNKNOWN",(void*)sink);
    return NULL;
}
...
static remote_device* findDeviceFromBdaddr (const bdaddr *addr)
{
    ...
    if ( !BdaddrIsZero( addr ) )
    {
        for (i=0; i<A2DP_MAX_REMOTE_DEVICES; i++)
        {
            ...
            {
                DEBUG_LOG("findDeviceFromBdaddr [%04X %02X %06lX] id=%u", addr->nap, addr->uap, addr->lap, i);
                return &a2dp->remote_conn[i];
            }
        }
    }

    DEBUG_LOG_INFO("findDeviceFromBdaddr [%04X %02X %06lX] id=UNKNOWN", addr->nap, addr->uap, addr->lap);
    return NULL;
}
    ...
    if ( !BdaddrIsZero( addr ) )
    {
        for (i=0; i<A2DP_MAX_REMOTE_DEVICES; i++)
        {
            ...
            {   /* Ok to add a new device - init the data structure */
                ...
                DEBUG_LOG("a2dpAddDevice device_id=%u [device=%p]", i, (void*)device);
                return device;
            }
        }
    }

    DEBUG_LOG_WARN("a2dpAddDevice failed");
    return NULL;
}
...
{
    DEBUG_LOG_FN_ENTRY("removeDevice[%p]",(void*)device);
    if (device != NULL)
    {
...
{
    DEBUG_LOG_FN_ENTRY("initiateSignalling device=%p connection_id=%u identifier=%u", (void*)device, connection_id, identifier);
    ...
    if (device != NULL)
    {
        ...
        {   /* Ok to add a new device - init the data structure */
            ...
            DEBUG_LOG("initiateSignalling signalling=%p", (void*)signalling);
...
{
    DEBUG_LOG_FN_ENTRY("completeSignalling signalling=%p sink=%p mtu=%u", (void*)signalling, (void*)sink, mtu);
...
{
    DEBUG_LOG_FN_ENTRY("requestRemoteA2dpVersion from [%p]", (void*)device);
...
void a2dpHandleSdpServiceSearchAttributeCfm (const CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM_T *cfm)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleSdpServiceSearchAttributeCfm");
    ...
    {
        DEBUG_LOG("a2dpHandleSdpServiceSearchAttributeCfm device=%p connection_state=%u",(void*)device,device->signal_conn.status.connection_state);
        ...
        DEBUG_LOG("a2dpHandleSdpServiceSearchAttributeCfm cfm->status=%X cfm->error_code=%X",(uint16)cfm->status,cfm->error_code);
        ...
        {   /* Response obtained, examine it */
            ...
            {
                DEBUG_LOG("a2dpHandleSdpServiceSearchAttributeCfm A2DP Version=0x%X",version);
                ...
                if (device->signal_conn.status.connection_state == avdtp_connection_paging)
                {
                    ...
                    {
                        a2dpL2capConnectRequest(CONNECT_DELAY_IMMEDIATE, device, (CONFTAB_LEN(SELECT_CONFTAB(device))), (uint16 *)SELECT_CONFTAB(device));
                    }
                }
            }
...
static media_channel* initiateMedia (remote_device *device, uint16 connection_id, uint8 identifier)
{
    ...
    DEBUG_LOG_FN_ENTRY("initiateMedia device=%p connection_id=%u identifier=%u", (void*)device, connection_id, identifier);
    ...
    if (device != NULL)
    {
        for (i=0; i<A2DP_MAX_MEDIA_CHANNELS; i++)
        {
            ...
            {   /* Ok to add a new device - init the data structure */
                ...
                DEBUG_LOG("initiateMedia media_id=%u [media=%p]", i, (void*)media);
...
{
    DEBUG_LOG_FN_ENTRY("completeMedia media=%p cid=%u sink=%p mtu=%u", (void*)media, connection_id, (void*)sink, mtu);
...
{
    DEBUG_LOG_FN_ENTRY("removeMedia[%p]", (void*)media);
    if (media != NULL)
    {
        memset( media, 0, sizeof(media_channel) );  /* Assumes enumerated type defaults are zero in value */
        return TRUE;
...
{
    DEBUG_LOG_FN_ENTRY("a2dpL2capConnectRequest delay=%ld",delay);
    ...
    if (delay==CONNECT_DELAY_RANDOM)
    {
        ...
        DEBUG_LOG_INFO("a2dpL2capConnectRequest random delay=%lu", delay);
    }
...
void a2dpHandleSignallingConnectReq(const A2DP_INTERNAL_SIGNALLING_CONNECT_REQ_T *req)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleL2capSignallingConnectReq");
    ...
    {   /* We are connecting/have connected to this device */
        ...
        DEBUG_LOG_INFO("a2dpHandleL2capSignallingConnectReq signalling enum:avdtp_connection_state:%d",signalling->status.connection_state);
...
void a2dpMediaConnectReq(remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpMediaConnectReq signalling enum:avdtp_connection_state:%d", signalling->status.connection_state);
    switch (signalling->status.connection_state)
    {
    case avdtp_connection_idle:
    case avdtp_connection_paging:
...
void a2dpHandleSignallingConnectRes(const A2DP_INTERNAL_SIGNALLING_CONNECT_RES_T *res)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleSignallingConnectRes device=%p", (void*)res->device);
    
    if (signalling != NULL)
    {
        DEBUG_LOG_INFO("a2dpHandleSignallingConnectRes signalling enum:avdtp_connection_state:%d",signalling->status.connection_state);
        ...
        case avdtp_connection_disconnect_pending:
            ...
            DEBUG_LOG_INFO("a2dpHandleSignallingConnectRes UNEXPECTED A2DP LIBRARY STATE");
            break;
...
void a2dpHandleSignallingDisconnectReq(const A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ_T *req)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleL2capSignallingDisconnectReq device=%p", (void*)req->device);
    
    if ( device )
    {
        signalling_channel *signalling = &device->signal_conn;
        
        DEBUG_LOG_INFO("a2dpHandleL2capSignallingDisconnectReq signalling enum:avdtp_connection_state:%d enum:avdtp_stream_state:%d",
                       signalling->status.connection_state, signalling->status.stream_state);
        ...
        case avdtp_connection_crossover:
            ...
            A2DP_FATAL_IN_DEBUG("a2dpHandleL2capSignallingDisconnectReq invalid state");
            break;
        case avdtp_connection_connected:
...
/*****************************************************************************/
void a2dpHandleSignallingUseLargeMTU(const A2DP_INTERNAL_SIGNALLING_MTU_REQ_T *req)
{
    remote_device *device = findDeviceFromBdaddr(&req->addr);

    DEBUG_LOG_FN_ENTRY("a2dpHandleSignallingUseLargeMTU device=%p, enable=%d", (void*)device, req->use_large_mtu);

    if (device != NULL)
    {
        device->bitfields.use_large_mtu = req->use_large_mtu;
    }
}
...
void a2dpMediaDisconnectReq(Sink sink)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpMediaDisconnectReq device=%p sink=%p", (void*)device, (void*)sink);
    ...
    if ( device )
    {
        ...
        {   /* Request to disconnect a media channel */
            DEBUG_LOG_INFO("a2dpMediaDisconnectReq media enum:avdtp_connection_state:%d",media->status.conn_info.connection_state);
            ...
            case avdtp_connection_crossover:
                /* Should not be able to get here - we wont have a sink until a connection has been established */
                DEBUG_LOG_INFO("a2dpMediaDisconnectReq UNEXPECTED A2DP LIBRARY STATE");
                break;
            case avdtp_connection_connected:
                media->status.conn_info.connection_state = avdtp_connection_disconnecting;
...
{
    DEBUG_LOG_FN_ENTRY("a2dpHandleL2capConnectInd");
    ...
    if(ind->psm == AVDTP_PSM)
    {
        ...
        {   /* We are connecting/have connected to this device */
            ...
            DEBUG_LOG_INFO("a2dpHandleL2capConnectInd signalling enum:avdtp_connection_state:%d, use_large_mtu=%u", signalling->status.connection_state, device->bitfields.use_large_mtu);
            ...
            case avdtp_connection_idle:
                ...
                ConnectionL2capConnectResponse(&a2dp->task, TRUE, AVDTP_PSM, ind->connection_id, ind->identifier, CONFTAB_LEN(SELECT_CONFTAB(device)), SELECT_CONFTAB(device));
                break;
            ...
            case avdtp_connection_crossover:
                ...
                ConnectionL2capConnectResponse(&a2dp->task, FALSE, ind->psm, ind->connection_id, ind->identifier, CONFTAB_LEN(SELECT_CONFTAB(device)), SELECT_CONFTAB(device));
                break;
            ...
            case avdtp_connection_connected:
                ...
                if ( (signalling->status.stream_state==avdtp_stream_remote_opening) && (initiateMedia(device, ind->connection_id, ind->identifier)!=NULL))
                {
                    ...
                    ConnectionL2capConnectResponse(&a2dp->task, TRUE, ind->psm, ind->connection_id, ind->identifier, CONFTAB_LEN(SELECT_CONFTAB(device)), SELECT_CONFTAB(device));
                }
                else
...
void a2dpHandleL2capConnectCfm(const CL_L2CAP_CONNECT_CFM_T *cfm)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleL2capConnectCfm for device=%p cid=%u enum:l2cap_connect_status:%d", (void*)device, cfm->connection_id, cfm->status);
    ...
    if (device != NULL)
    {
        ...
        DEBUG_LOG_INFO("a2dpHandleL2capConnectCfm signalling enum:avdtp_connection_state%d", signalling->status.connection_state);
        ...
        case avdtp_connection_idle:
            ...
            {
                /* Should not be able to get here */ 
                DEBUG_LOG_INFO("a2dpHandleL2capConnectCfm UNEXPECTED A2DP LIBRARY STATE");
                if (cfm->sink)
        ...
        case avdtp_connection_crossover:
            ...
            {   /* Successful connection of outbound request */
                ...
                {   /* Non-zero cid and id indicate that a response is outstanding, reject the inbound connection */
                    ConnectionL2capConnectResponse(&a2dp->task, FALSE, AVDTP_PSM, signalling->connection.setup.inbound_cid, signalling->connection.setup.inbound_id, CONFTAB_LEN(SELECT_CONFTAB(device)), SELECT_CONFTAB(device));
                }
            }
                ...
                {   /* A singular connection setup being managed */
                    ...
                    {   /* Trying to re-establish a signalling channel after a link loss */
                        ...
                        {   /* Make another connection attempt */
                            ...
                            a2dpL2capConnectRequest(CONNECT_DELAY_RANDOM, device, (CONFTAB_LEN(SELECT_CONFTAB(device))), (uint16*)SELECT_CONFTAB(device));
                        }
                        else
        ...
        case avdtp_connection_disconnecting:        /* Could get here if an L2CAP renegoitation occurred just before disconnect req */
            ...
            {
                DEBUG_LOG_INFO("a2dpHandleL2capConnectCfm media enum:avdtp_connection_state:%d", media->status.conn_info.connection_state);
                ...
                case avdtp_connection_crossover:        /* A media channel can never be in this state */
                    /* Should not be able to get here */
                    DEBUG_LOG_INFO("a2dpHandleL2capConnectCfm UNEXPECTED A2DP MEDIA STATE");
                    if (cfm->sink)
                    {   /* Closing the L2CAP channel when idle is likely to be the best course of action */
                        ConnectionL2capDisconnectRequest(&a2dp->task, cfm->sink);
                    }
            ...
            {   /* Received a confirmation message with an unrecognised connection id - should be due to a signalling crossover condition */
                DEBUG_LOG_INFO("a2dpHandleL2capConnectCfm unrecognised cid 0x%X",cfm->connection_id);
                if (cfm->sink)
...
void a2dpHandleL2capDisconnect(Sink sink, l2cap_disconnect_status status)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleL2capDisconnect device=%p sink=%p enum:l2cap_disconnect_status:%d", (void*)device, (void*)sink, status);
    ...
    {   /* Device is recognised by A2DP lib */
        ...
        {   /* A signalling channel disconnected */
            DEBUG_LOG_INFO("a2dpHandleL2capDisconnect signalling enum:avdtp_connection_state:%d",signalling->status.connection_state);
            ...
            case avdtp_connection_crossover:
                /* Should not be able to get here - we wont have a sink until a connection has been established */
                DEBUG_LOG_INFO("a2dpHandleL2capDisconnect UNEXPECTED A2DP SIGNALLING STATE");
            case avdtp_connection_connected:
            case avdtp_connection_disconnect_pending:
            case avdtp_connection_disconnecting:
        ...
        {   /* A media channel disconnected */
            DEBUG_LOG_INFO("a2dpHandleL2capDisconnect media enum:avdtp_connection_state:%d", media->status.conn_info.connection_state);
            ...
            case avdtp_connection_crossover:
                /* Should not be able to get here - we wont have a sink until a connection has been established */
                DEBUG_LOG_INFO("a2dpHandleL2capDisconnect UNEXPECTED A2DP MEDIA STATE");
                break;
            case avdtp_connection_connected:
            case avdtp_connection_disconnect_pending:
            case avdtp_connection_disconnecting:
                ...
                if ( !isMediaConnected( device ) )
                {
                    ...
                    {   /* There is no signalling channel disconnect pending */
                        ...
                        {   /* Signalling channel linkloss notification received before media channel(s) */
                            ...
                            if (signalling->status.connection_state == avdtp_connection_idle)
                            {   /* Remote device has not started reconnection, so initiate signalling channel connection now */
                                initiateSignalling( device, AVDTP_OUTGOING_CONNECTION_ID, 0 );
                                a2dpL2capConnectRequest(CONNECT_DELAY_RANDOM, device, (CONFTAB_LEN(SELECT_CONFTAB(device))), (uint16*)SELECT_CONFTAB(device));
                            }
                        }
