...
class KaCore:
    ...
    # Set to true the first time _add_script_dir_to_path is called. Used to avoid extending the path multiple times in _add_script_dir_to_path.
    _added_script_dir_to_path = False
    
    @staticmethod
    def _add_script_dir_to_path():
        """
        Add the absolute path of this script to the system path. Does nothing if this has been called already.
        """
        if not KaCore._added_script_dir_to_path:
            KaCore._added_script_dir_to_path = True

            mydir = os.path.abspath(os.path.dirname(__file__))
            path_element_separator = ":" if (sys.platform.startswith('linux') or sys.platform.startswith('cygwin')) else ";"
        
            cur_path = os.environ.get("PATH", "")
            if len(cur_path) > 0:
                os.environ["PATH"] = mydir + path_element_separator + cur_path
            else:
                os.environ["PATH"] = mydir
                
            # And thanks to changes in DLL loading in Python 3.8, also add it to the DLL search path if the capability to do that exists.
    ...
    def _load_kalaccess(self):
        ...
        if sys.platform.startswith('linux'):
            ...
            try:
                ...
                message += "\n    ".join(os.environ.get('PATH', '').split(":" if sys.platform.startswith('cygwin') else ";"))
                message += "\n\nInner Python exception : %r" % ex
                raise OSError(message)
        else:
