...
/**
 * The HYDRA_LOG_FIRM_BASE_ADDR value should match the start address of the
 * debug_strings section in the link map.
 */
#define HYDRA_LOG_FIRM_BASE_ADDR 0x13500000
#define /*lint --e{428} */ HYDRA_LOG_STRING_SUB_BASE(str) (&(str)[-HYDRA_LOG_FIRM_BASE_ADDR])
#define /*lint --e{416} */ HYDRA_LOG_STRING_ADD_BASE(str) (&(str)[HYDRA_LOG_FIRM_BASE_ADDR])
...
#define /*lint --e{774}  --e{607} --e{960} --e{685} --e{568} --e{42} --e{40} */ HYDRA_LOG0(lvl, fmt) \
    ...
    {                                                   \
        ...
        {                                               \
            if ((lvl) == 0 ||                           \
                HYDRA_LOG_LEVEL_CURRENT_SYM >= (lvl))   \
            {                                           \
                HYDRA_LOG_STRING(log_fmt, fmt);         \
                hydra_log_firm_0(                       \
                    HYDRA_LOG_STRING_SUB_BASE(log_fmt)  \
                );                                      \
            }                                           \
        }                                               \
    }                                                   \
...
/**
 * As \c hydra_log_firm_variadic() but taking the dereferenced variadic pointer
 * obtained from va_start for the variable length arguments. This supports
 * implementing a logging function that takes variadic arguments and calls
 * into the hydra_log.
 */
extern void hydra_log_firm_va_arg(const char *event_key,
                                    size_t n_args,
                                    va_list va_argp);

/**
 * Initialise the firmware log. This must be called once at early start-up
 * before making any use of the loging macros.
 */
extern void hydra_log_init(void);
