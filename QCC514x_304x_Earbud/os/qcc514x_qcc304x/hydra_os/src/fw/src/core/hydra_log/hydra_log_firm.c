...
#ifdef FIRMWARE_LOG_LINE_COUNT
/**
 * Incrementing line count that is combined with the event_key to help detect
 * discontinuities in the log. The python log decoder detects the presence of
 * this variable to know how to decode the event key.
 */
static uint8 debugBufferLineCount;
#endif /* FIRMWARE_LOG_LINE_COUNT */
...
)
{
    ...
    /* P1 may still be using full value pointer. Only add the base if required. */
    if (event_key < (const char *)HYDRA_LOG_FIRM_BASE_ADDR)
    {
        event_key = HYDRA_LOG_STRING_ADD_BASE(event_key);
    }

    va_start(va_argp, n_args);
    hydra_log_firm_va_arg(event_key, n_args, va_argp);
    va_end(va_argp);
}

void hydra_log_firm_va_arg(const char *event_key, size_t n_args, va_list va_argp)
{
    size_t pos;
    hydra_log_elem log_event_key = (hydra_log_elem)event_key;

    block_interrupts();

#ifdef FIRMWARE_LOG_LINE_COUNT
    /* A counter is written into in the upper byte of the event key to help
     * detect discontinuities in the log. The python log decoder supports
     * extracting the event key and counter from this when it detects the
     * presence of the 'debugBufferLineCount' variable in the firmware.
     */
    ++debugBufferLineCount;
    log_event_key = (log_event_key & 0xFFFFFF) | (debugBufferLineCount << 24);
#endif /* FIRMWARE_LOG_LINE_COUNT */
    ...
    if (hydra_log_trb_cfg.enable)
    {
        ...
        do
        {
            ...
            while (arg_i < 2)
            {
                ...
                if (n_args_trb)
                {
                    /*lint -e{737} Loss of sign in promotion */
...
void hydra_log_init(void)
{
#if defined(INSTALL_HYDRA_LOG_PER_MODULE_LEVELS)
    size_t i;
    hydra_log_level *l = (hydra_log_level *)&hydra_log.levels;
    for (i = 0; i < sizeof(hydra_log.levels)/sizeof(hydra_log_level); i++)
    {
        l[i] = HYDRA_LOG_INITIAL_LEVEL;
    }
#else /* !defined(INSTALL_HYDRA_LOG_PER_MODULE_LEVELS) */
    /* Init global level */
    hydra_log.level = HYDRA_LOG_INITIAL_LEVEL;
