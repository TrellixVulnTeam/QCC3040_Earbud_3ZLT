...
/**
 *  \brief Generate and output the master clock (MCLK) independently of the audio
 *  subsystem.
 * The clock will be output onto MCLK PIO of the device. Refer to the chip
 * documentation for which PIO is used for this purpose and the range of
 *  frequencies
 * that are supported for MCLK output.
 * If the audio subsystem is configured to use the internal MCLK then the chosen
 * frequency from that interface and this one MUST be the same.
 *         
 *  \param frequency Generate the MCLK at the specified frequency (Hz). A value of 0 will disable
 *  the clock output.
 *  \return TRUE if configuration succeeded, FALSE otherwise. MCLK cannot be reconfigured
 *  while in use.
 * 
 * \ingroup trapset_audio_mclk
 */
bool GenerateMasterClockOutput(uint32 frequency);
