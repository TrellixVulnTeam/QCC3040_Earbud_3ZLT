...
uint16 ObexObjNew( Obex session )
{
    ...
    return obexNewPacket(session,OBEX_EMPTY_BODY_SIZE,OBEX_GET);
}
...
/***********************************************************************
 * NAME
 *  ObexObjAddOneByteHdr
 *
 * DESCRIPTION
 *  Add a Unsigned 1 byte header to the object.
 *
 * PARAMETERS
 *  Refer obex.h
 *************************************************************************/
bool ObexObjAddOneByteHdr( Obex session, ObexByteHeaderId hdrId, uint8 hdrVal)
{
    if( !session ) return FALSE;
    return obexAddUint8Header( session, hdrId, hdrVal );
}
...
uint8 ObexObjExtractOneByteHdrValue( ObexByteHeaderId hdrId,
                                   uint16  srcLen,
                                   Source  src )
{
    const uint8* pkt = SourceMap( src );

    if(pkt == NULL ) return OBEX_INVALID_UINT8;
    return obexGetUint8Header( pkt, srcLen, hdrId );
}
/***********************************************************************
 * NAME
 *  ObexObjMapHdrValue
 ...
 *
 *DESCRIPTION
 * Set SRM to be sent to PSE
 *
 *PARAMETERS
 *  Refer obex.h
 * ***********************************************************************/
Obex ObexSetSrm(Obex session, bool srm_enable_req)
{
    if (!session ) return OBEX_INVALID_LEN;
    session->srm_status = obex_srm_disable; /*SRM disabled until PSE response with SRM on*/
    session->SRM = srm_enable_req; /*send this to PSE  */
    return session;
}
