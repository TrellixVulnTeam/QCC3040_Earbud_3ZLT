...
class CDefinition(Indented):
    ...
    def __init__(self, type, name, array=False, typedef=False, eol=",", doc="", outfile=None):
        self.type = type
        self.name = name
        self.array = array
        ...
        self.list = []
        self.docs = []
        super(CDefinition, self).__init__(outfile)
    ...
    def __exit__(self, type, value, traceback):
        ...
        for i in range(items):
            ...
            print(self.indent() + self.list.pop(0) + self.eol, file=self.outfile)
...
class Enumeration(CDefinition):
    ...
    def __init__(self, name, doc="", eol=",", outfile=None):
        CDefinition.__init__(self, 'enum', name, doc=doc, eol=eol, outfile=outfile)

class EnumerationTypedef(CDefinition):
    ''' Generate a c enumeration '''
    def __init__(self, name, doc="", outfile=None):
        CDefinition.__init__(self, 'enum', name, typedef=True, doc=doc, outfile=outfile)

class Array(CDefinition):
    ''' Generate a c array '''
    def __init__(self, array_type, name, doc="", eol=",", outfile=None):
        CDefinition.__init__(self, array_type, name, array=True, doc=doc, eol=eol, outfile=outfile)

class StructTypedef(CDefinition):
    ''' Generate a c struct typedef '''
    def __init__(self, name, doc="", outfile=None):
        CDefinition.__init__(self, 'struct', name, typedef=True, eol=";", doc=doc, outfile=outfile)

class UnionTypedef(CDefinition):
    ''' Generate a c union typedef '''
    def __init__(self, name, doc="", outfile=None):
        CDefinition.__init__(self, 'union', name, typedef=True, eol=";", doc=doc, outfile=outfile)
