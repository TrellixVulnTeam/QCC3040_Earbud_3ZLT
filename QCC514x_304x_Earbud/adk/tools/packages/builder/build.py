...
try:
    ...
    from vm_build.workspace_parse import ubuild_compat
    from vm_build.builder.adk_toolkit import AdkToolkit
    import vm_build.quickChargeHexGen
...
def launch_cmd(cmd_line, env, tgt_config, working_dir, elf_path, is_audio=False, attribs=None):
...
def launch_simple_cmd(cmd_lines, env=None, cwd=None, attribs=None):
    if env is None:
        env = os.environ.copy()
    if cwd is None:
        cwd = os.getcwd()

    return launch_cmd(cmd_lines, env, None, cwd, None, attribs=attribs)
...
def print_info(p_inst, dk_inst, script_args):
    ...
    if not isinstance(p_inst, Project):
        if hasattr(p_inst, 'proj_fname'):
            # Looks like a ubuild project object
            proj = ubuild_compat.create_project(p_inst)
        else:
            print("Wrong project type {type!s} for project {project!s}".format(type(p_inst), p_inst))
            return False

    print("\nSOURCE FILES")
    for src_file in proj.files:
        print(src_file)

    for config in proj.configurations:
        print("\nCONFIG NAME =", config)
        props = proj.get_properties(config)
...
"""
        ...
        output += '    %s \\\n' % os.path.relpath(os.path.join(proj.dirname, 'rsa_pss_constants.c'), start=proj.dirname)
        output += '$(DBS)\n'
        output += '\n'
...
def do_make(proj, devkit, tgt_config, build_style, is_audio, build_output_folder=None):
    ...
    if is_audio:
        ...
        if 'ALL_DM_AS_PM' in make_vars:
            cmd_line += [make_vars['ALL_DM_AS_PM']]
        if 'DATA_IN_EXTRA_HEAP' in make_vars:
            cmd_line += [make_vars['DATA_IN_EXTRA_HEAP']]
    else:
        cmd_line += [
            '-f', makefile_path,
            makefile_rules_path
...
def _run_prepare_fs(tool_root, root_dir, xuv_path, offset=0, appsFs=False):
    ...
    try:
        from vm_build.builder.prepare_fs import pack_dir
...
def build_filesystem(proj, devkit, script_args, crypto_key, build_output_folder):
    ...
    def __get_hydracode_sdbfile_path(proj):
        ...
        If the field is NOT defined or empty then walk up from the project file and search for the 'adk' folder, then:
            adk/bin/<chip_type>/hydracore_config.sdb
        """
        sdb_file = None
        config = proj.get_properties('filesystem')
        attribs = {
    ...
    def get_section_size(section_name, id_qualifier=None):
        ''' Get section size, without modification, from flash config file '''
        
        try:
            from vm_build.builder.prepare_single_image import PrepareSingleImage
        ...
        for section, attrs in flash_config.get("layout", None):
            id_qualifier_found = 'id' in attrs and attrs['id'] == id_qualifier
            if section == section_name and (id_qualifier is None or id_qualifier_found):
                section_size = attrs.get("capacity", 0)
                break
                
        if section_size == 0:
            raise Exception('Flash config layout must contain a valid %s section' % section_name)
        ...
        if ((section_size / block_size) % 2) != 0:
            raise Exception('%s size must be an even number of blocks' % section_name)
    ...
    if filesystem_type in ("firmware_config", "curator_config", "device_config"):
        ...
        if len(htf_files) > 0:
            ...
            ps_store_size =  get_section_size('rw_config') // 2
    ...
    elif filesystem_type == "customer_va":
        import fscat
        va_fs_config_size = get_section_size('ra_partition', 'VMdl')
        fscat_args = project_files + ['-s', str(va_fs_config_size), '-o','customer_va_filesystem_dfu','-b','-x']
        fscat.run_script(fscat_args)
...
def build_with_makefile(proj, devkit, build_style, devkit_root, build_output_folder=None):
    ...
    if 'CHIP_TYPE' in config:
        arguments_to_pass.append("CHIP_TYPE=%s" % config['CHIP_TYPE'])

    if 'CHIP_NAME' in config:
        arguments_to_pass.append("CHIP_NAME=%s" % config['CHIP_NAME'])

    if 'OS_VARIANT' in config:
        arguments_to_pass.append("OS_VARIANT=%s" % config['OS_VARIANT'])

    if 'NUM_PIO_BANKS' in config:
        arguments_to_pass.append("NUM_PIO_BANKS=%s" % config['NUM_PIO_BANKS'])

    if 'BUILD_TYPE' in config:
        arguments_to_pass.append("BUILD_TYPE=%s" % config['BUILD_TYPE'])

    if 'BUILD_LIBS_AS_PRIVATE' in config:
        arguments_to_pass.extend([
            "PRIVATE_LIBS=%s" % config['BUILD_LIBS_AS_PRIVATE'],
            "PRIVATE_LIBS_SEED=%d" % random.getrandbits(16)
        ])

    if 'INCPATHS' in config:
        arguments_to_pass.append("INCPATHS=%s" % config['INCPATHS'])

    if 'PRE_INCLUDES' in config:
        arguments_to_pass.append("PRE_INCLUDES=%s" % config['PRE_INCLUDES'])
    ...
    if build_output_folder is not None:
        ...
        arguments_to_pass.append("BUILDOUTPUT_PATH=%s" % output_suffix)
    ...
    cmd_line.extend(arguments_to_pass)
...
def build_configs(proj, devkit, script_args):
    ...
    if build_style == 'syntaxDefines':
        return get_compiler_defines(devkit, props)
...
def get_compiler_defines(devkit, props):
    cmd = [
        devkit['kcc.exe'],
        "-k", props['CHIP_NAME'],   # Compiler target
        "-c",                       # Compile only, don't link
        "-dM",                      # Print compiler defines
        "nul.c"                     # Null C file to satisfy kcc
    ]
    return launch_simple_cmd(cmd, attribs={'type': 'syntaxDefines'})
