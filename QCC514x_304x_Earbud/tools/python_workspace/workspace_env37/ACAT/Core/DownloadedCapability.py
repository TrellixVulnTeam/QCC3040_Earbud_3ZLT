...
# String used to format mapped table entries and header.
MAPTABLE_FORM = '{0:<20}{1:<10}{2:<15}{3:<15}{4:<5}'
...
class MappedTable(object):
    ...
    def __str__(self):
        ...
        self._update_table()
        out_str = 'Identify:  {0}  Entries: {1}\n'.format(
            hex(self.identify_field), self.num_of_entries
        )
        if self.num_of_entries:
            # Show all the entries in form of a table.
            out_str += MAPTABLE_FORM.format(
    ...
    def _update_table(self):
        ...
        if not self.chipdata.is_booted():
            # Nothing to update if the processor is not booted.
            return
    ...
    def load_bundle_from_internal_builds(self, cap_elf_id):
        ...
        from ACAT.Core.CoreUtils import load_ker_debuginfo
                ...
                if str(cap_elf_id) in ids_to_paths.keys():
                    ...
                    if sys.platform == "win32":
                        self.chipdata.debuginfo.update_bundles(
                            load_ker_debuginfo(ids_to_paths[str(cap_elf_id)][0])
                        )
                    else:
                        self.chipdata.debuginfo.update_bundles(
                            load_ker_debuginfo(ids_to_paths[str(cap_elf_id)][1])
                        )
        except IOError:
            pass
...
class MappedTableEntry(object):
    ...
    def __init__(self, device_address, length, file_address, file_id, debuginfo):
    ...
    def __str__(self):
        file_name = self.debuginfo.get_elf_file_name(elf_id=self.file_id)
        out_str = MAPTABLE_FORM.format(
            hex(self.device_address), self.length,
            hex(self.file_address), hex(self.file_id), file_name
        )
        out_str += ',\n'
        return out_str
