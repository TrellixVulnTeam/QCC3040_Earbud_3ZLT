...
def get_library_components(a_dir):
    exclusion_list = ['depend', 'debugobj', 'debugbin', 'debugextconst', 'struct_share']
    dirs = get_immediate_subdirectories(a_dir)
    sub_dirs = []
    for dir in dirs:
        all_sub_dirs = get_immediate_subdirectories(os.path.join(a_dir,dir))
        filtered_sub_dirs = filter(lambda x: x not in exclusion_list, all_sub_dirs)
        adjusted_sub_dirs = map(lambda x: dir + "/" + x, filtered_sub_dirs)
        sub_dirs.extend(adjusted_sub_dirs)
...
def get_library_filename_if_exists(library_path):
    library_filename = None

    makefile = os.path.abspath(os.path.join(library_path, 'makefile'))
    if os.path.isfile(makefile):
        f = open(makefile, 'r')
        for line in f.readlines():
            match = re.search(r'TARGET_LIB_ROOT\s*=\s*(\w+)', line)
            if match and match.group(1):
                library_filename = match.group(1)
...
class KCSMaker(object):
    ...
    def get_obj_files(self, chip, get_kcc_version):
        ...
        cap_names = get_library_components(cap_output_path)
        ...
        for line in f:
            if matched:
                if line.strip().rstrip("\\").strip().endswith(".a"):
                    prebuilt_lib_names.extend([line.strip().rstrip("\\").strip()])
        ...
        try:
            comp_names = get_library_components(comp_output_path)
        ...
        try:
            lib_names = get_library_components(lib_output_path)
        except OSError as err:
            # errno 2 means "No such file or directory"
            if err.errno == 2:
                lib_names = []
        ...
        try:
            lib_private_names = get_library_components(lib_private_output_path)
        except OSError as err:
            # errno 2 means "No such file or directory"
            if err.errno == 2:
                lib_private_names = []
        ...
        try:
            # There are 2 options regarding "support_lib":
            # - build it as a single private library
            # - build every component of support lib as an individual library 
            # Search for libsupport_lib.a filename inside the support_lib folder or
            # search for all the .a filenames inside each of the support_lib folders.

            priv_support_lib = False
            support_lib_root = os.path.abspath(os.path.join(self.kymera_src_path, 'support_lib'))
            lib_name = get_library_filename_if_exists(support_lib_root)
            if lib_name:
                support_lib_path = os.path.join(
                    support_lib_output_path,
                    "debugbin",
                    "lib{}.a".format(lib_name)
                )

                if os.path.isfile(support_lib_path):
                    # Found libsupport_lib.a file inside the support_lib folder
                    o_files.extend([support_lib_path])
                    support_lib_names['support_lib'] = lib_name
                    priv_support_lib = True

            if priv_support_lib == False:
                support_lib_folders = get_library_components(support_lib_output_path)
                for support_lib_folder in support_lib_folders:
                    ...
                    lib_name = get_library_filename_if_exists(lib_path)

                    # Found lib<name>.a files inside each of the support_lib folders
                    support_lib_path = os.path.join(
                        support_lib_output_path,
                        support_lib_folder,
                        "debugbin",
                    ...
                    if os.path.isfile(support_lib_path):
                        o_files.extend([support_lib_path])
                        support_lib_names[support_lib_folder] = lib_name
        ...
        for elem in obj_names_and_paths_list:
            ...
            for obj_name in obj_names:
                lib_name = obj_name.split("/")[-1]
                if obj_names == prebuilt_lib_names:
                    o_files.extend([os.path.join(obj_path, obj_name)])
                else:
                    o_files.extend(
                        [
                            os.path.join(
