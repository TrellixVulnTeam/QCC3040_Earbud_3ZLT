...
/* Event arguments for the additional data write event */
typedef struct
{
    const uint8* enc_data;
    uint16 size;
} fast_pair_state_event_additional_data_write_args_t;

/* Event arguments for the personalized write event */
typedef struct
{
    const uint8* pname_data;
    uint16 size;
} fast_pair_state_event_pname_write_args_t;
...
/*! \brief Handle Additional Data Characterstic Write by Fast Pair Seeker

    Use AES Key in case of naming flow 1 or account key in case of naming flow 2
    to decode additional data written in AES_CTR encrypted format. Verify the data with HMAC-SHA256.

    \param[in] enc_data Pointer to encrypted additional data written by FP seeker.
    \param[in] enc_data_len Length of encrypted additional data written by FP seeker.
 */
void fastPair_AdditionalDataWrite(const uint8 *enc_data, uint32 enc_data_len);

/*! \brief Store Name to persistent storage

    \param[in] enc_data Pointer to personalized name data
    \param[in] enc_data_len Length of personalized name data
 */
