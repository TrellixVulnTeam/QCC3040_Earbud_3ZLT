...
/*! \brief Returns the PIO bit position mask within a register bank.

    This function needs to be used in conjunction with PioCommonPioBank
    to ensure that the correct bank is also accessed.
...
/*!
    Set active high or low for a pio

    @param[in] pio number
    @param[in] level true for active hight, false for active low
