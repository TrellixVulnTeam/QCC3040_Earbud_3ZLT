...
/**
 *  \brief Read whether the pull is enabled on a PIO.
 *  \param bank PIO bank number.
 * 
 * \ingroup trapset_core
 */
...
/**
 *  \brief Set whether the pull is enabled on PIOs. Note that other PIO related functions
 *  may change this setting.
 *   \code
 *    PioSetPullEnable32Bank(0, (1<<4)|(1<<5), (1<<5));
 *   \endcode
 *  \param bank PIO bank number.
 *  \param mask Each bit in the mask corresponds to a PIO line. Bits set to 1 in this mask will
 *  be modified. Bits set to 0 in this mask will not be modified. 
 *  \param bits Each bit in the "bits" value corresponds to a PIO line. Bits set to 1 in this
 *  value will result in that PIO pull being enabled. Bits set to 0 in this value
 *  will result in that PIO being disabled.
 *  \return A 32 bit mask. If any bit in this mask is high then the PIO pull enable setting
 *  could not be changed; note that no action will have been taken on any PIOs.
 * 
 * \ingroup trapset_core
 */
...
/**
 *  \brief Read whether the fixed pull direction on a PIO is up or down.
 *  \param bank PIO bank number.
 * 
 * \ingroup trapset_core
 */
...
/**
 *  \brief Set whether the fixed pull direction on a PIO is pull-up or pull-down. Note
 *  that other PIO related functions may change this setting.
 *   \code
 *    PioSetPullDirection32Bank(0, (1<<4)|(1<<5), (1<<5));
 *   \endcode
 *  \param bank PIO bank number.
 *  \param mask Each bit in the mask corresponds to a PIO line. Bits set to 1 in this mask will
 *  be modified. Bits set to 0 in this mask will not be modified. 
 *  \param bits Each bit in the "bits" value corresponds to a PIO line. Bits set to 1 in this
 *  value will result in that PIO fixed pull direction as up. Bits set to 0 in
 *  this value will result in that PIO fixed pull direction being down.
 *  \return A 32 bit mask. If any bit in this mask is high then the PIO pull enable setting
 *  could not be changed; note that no action will have been taken on any PIOs.
 * 
 * \ingroup trapset_core
...
/**
 *  \brief Read whether the sticky direction pull on a PIO is enabled.
 *  \param bank PIO bank number.
 * 
 * \ingroup trapset_core
 */
...
/**
 *  \brief Set whether the sticky direction pull on a PIO is enabled. The direction of the
 * pull is controlled by the input level on the PIO.
 *         
 *   \code
 *    PioSetPullSticky32Bank(0, (1<<4)|(1<<5), (1<<5));
 *   \endcode
 *         
 *  \param bank PIO bank number.
 *  \param mask Each bit in the mask corresponds to a PIO line. Bits set to 1 in this mask will
 *  be modified. Bits set to 0 in this mask will not be modified. 
 *  \param bits Each bit in the "bits" value corresponds to a PIO line. Bits set to 1 in this
 *  value will result in that PIO using the input level for the pull direction.
 *  Bits set to 0 in this value will result in that PIO fixed using the fixed pull
 *  direction.
 *  \return A 32 bit mask. If any bit in this mask is high then the PIO pull enable setting
 *  could not be changed; note that no action will have been taken on any PIOs.
 * 
 ...
 *   This function configures a simple debounce engine for PIO input
 *   pins. It can detect transitions on PIOs and return stabilised
 *   readings for them, filtering out glitches. It supports three distinct groups
 *  of
 *   PIOs. The settings apply to all the selected PIOs in a group. A PIO can belong
 *   in all three groups at the same time and will fulfill al three functions.
 *   When the engine detects a change in state of one or more of the
 *   monitored pins, it repeatedly reads the pin state. If the state of
 *   all the monitored pins remains unchanged for \e count successive
 *   samples, it is considered stable and a \#MESSAGE_PIO_CHANGED message
 *   is sent to the task registered with MessagePioGroupTask(). 
 *   The pio state change handling depends on the pririty defined for that group.
 *   If it is high priority then new state is handled in pio interrupt context
 *   and \#MESSAGE_PIO_CHANGED message is sent to registered task for that group 
 *   immediately.
 *   If it is low priority then new state is handled in background.
 *   \note
 *   It is not possible to configure different debounce settings for different pins
 *   within the same group. If a pin is used in more than one group it will
 *   generate one message for each group after each group's conditions have been
 *   met.
 *   Examples:
 *   On group 0, enable events on PIO2 and PIO3, low priority handling, 
 *   with 4 reads 2ms apart (6ms total):
 *   \code
 *   PioDebounceGroup32Bank(0, 0, 1u<<2 | 1u<<3, 4, 2, 0);
 *   \endcode
 *   On group 1, enable events on any transition on PIO5 with no debouncing 
 *   and low priority:
 *   \code
 *   PioDebounceGroup32Bank(1, 0, 1u<<5, 1, 0, 0);
 *   \endcode
 *   On group 2, enable events on any transition on PIO4 with no debouncing
 *   and high priority handling:
 *   \code
 *   PioDebounceGroup32Bank(1, 0, 1u<<4, 1, 0, 1);
 *   \endcode
 *   Note: This trap is supported only on QCC517x and later devices. Devices
 *  before 
 ...
 *  of \#MessagePioChanged may show no change in state. 
 *  \param period The delay in milliseconds between successive reads of the pins. 
 *  \param priority The priority of the task to which response is sent. 
 *  \return A 32 bit mask. If any bit in this mask is high then monitoring could not be set
 *  up for that PIO; no action will have been taken on any PIOs (any previously
 *  set up monitoring will remain in force). The most likely reasons for a
 *  non-zero return are attempting to debounce nonexistent PIOs, or attempting to
 ...
 * \ingroup trapset_core
 */
uint32 PioDebounceGroupBankPriority(uint16 group, uint16 bank, uint32 mask, uint16 count, uint16 period, pio_group_priority priority);
#endif /* TRAPSET_CORE */
#if TRAPSET_KALIMBA
