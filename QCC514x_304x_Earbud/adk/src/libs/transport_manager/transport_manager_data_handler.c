...
uint8 *TransportMgrClaimData(transport_mgr_type_t type, uint16 trans_link_info, uint16 data_length)
{
    uint8 *ptr = NULL;
    transport_mgr_link_data_t *temp = NULL;
    ...
    switch (type)
    {
        ...
        case transport_mgr_type_rfcomm:
            ...
            if (temp)
            {
                uint8 *dest = SinkMap(temp->sink);
                uint16 claimed = SinkClaim(temp->sink, 0);

                if (dest == NULL || claimed == 0xFFFF)
                    break;
                if (data_length <= claimed)
                    ptr = dest;
                else if (0xFFFF != SinkClaim(temp->sink, data_length - claimed))
                    ptr = dest;
            }
            break;
