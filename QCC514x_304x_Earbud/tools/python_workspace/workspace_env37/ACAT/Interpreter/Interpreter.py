...
from ACAT.Core.exceptions import OutdatedFwAnalysisError, DebugInfoError
...
class Interpreter(object):
    ...
    def _load_analysis(self, analysis, processor):
        ...
        # All analyses depend on module Analysis. Python 3.6.0-3.6-2 demands
        # that it is imported first.
        importlib.import_module('ACAT.Analysis')

        extensions = ["", "Old1", "Old2", "Old3", "Old4"]
        for ext in extensions:
            try:
                ...
                try:
                    ...
                    # Analysis class cannot instantiate.
                    logger.warning(
                        "Unable to load the analysis %s. Error: %s",
                        class_name,
                        error
                    )
                    continue
    ...
    def _load_analyses(self):
        ...
        for processor in sorted(self.processors.keys()):
            ...
            current_proc = self.processors[processor]
            formatter = current_proc.formatter
            if current_proc.is_booted() is False:
                formatter.alert(
                    "Processor {} is not booted.".format(current_proc.processor)
                )
                continue
