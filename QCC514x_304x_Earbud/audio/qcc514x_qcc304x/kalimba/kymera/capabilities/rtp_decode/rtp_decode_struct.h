...
#define FRAME_LENGTH_EXTRA_BUFFER_SIZE 20
...
typedef struct RTP_DECODE_OP_DATA
{
    ...
    /**
     * Total number of RTP packets processed since the
     * operator started including bad packets
     */
    unsigned rtp_decode_packet_count;

    /**
     * Number of good ttp updated packets processed since reset.
     * This is same as rtp_decoder_packet_count - invalid packets.
     */
    unsigned rtp_decode_valid_packet_count;

    /**
     * Number of RTP packets to be processed for repeating the
     * ttp notification. value 0 is to disable the notification,
     */
    unsigned  ttp_notification_interval;
...
typedef struct RTP_FRAME_DECODE_DATA
{
    ...
    /** SBC specific - number of frames extracted from the RTP SBC header */
    uint16 nr_of_frames_rtp_header;

    /** SBC specific - total encoded data length  */
    uint16 total_encoded_data;

    /** Record all the frame lengths in a packet for SBC */
    uint16 *frame_length_extra;
