...
static bool sinkInsertHeaderAndFlushData(Sink sink, uint16 amount_to_flush, const uint8 *header, uint16 hdrlen)
{
    ...
    DEBUG_LOG_FN_ENTRY("sinkInsertHeaderAndFlushData prev_sink_claimed %u", prev_sink_claimed);
    ...
    if (prev_sink_claimed != 0xFFFF)
    {
        ...
        flush_result = SinkFlush(sink, amount_to_flush + hdrlen);
        DEBUG_LOG("sinkInsertHeaderAndFlushData flushing amount_to_flush %u hdrlen %u result %u", amount_to_flush, hdrlen, flush_result);
    ...
    DEBUG_LOG_INFO("sinkInsertHeaderAndFlushData not flushing");
...
static bool sendPacket(remote_device *device, uint8 signalling_header, uint8 signal_id, uint16 payload_size)
{
    ...
    DEBUG_LOG_V_VERBOSE("sendPacket device=%p signalling_header=%X signal_id=%u mtu=%u payload_size=%u  <",
                        (void*)device, signalling_header, signal_id, signalling->connection.active.bitfields.mtu, payload_size);
    for(i=0; i<payload_size; i++)  DEBUG_LOG_V_VERBOSE("%X ",sink_data[i]);
    DEBUG_LOG_V_VERBOSE(">\n");

    if (payload_size < ((uint16)signalling->connection.active.bitfields.mtu - 1))
    {   /* Enough space in MTU to fit packet and two byte header */
        DEBUG_LOG("sendPacket Enough space in MTU to fit packet and two byte header");
    ...
    {   /* Packet won't fit in MTU and needs to be fragmented */
        /* Start Packet */
        DEBUG_LOG("sendPacket Packet won't fit in MTU and needs to be fragmented");
...
void a2dpSendAccept (remote_device *device, avdtp_signal_id signal_id, uint16 payload_size)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpSendAccept device=%p enum:avdtp_signal_id:%d payload_size=%u", (void*)device, signal_id, payload_size);
...
void a2dpSendReject (remote_device *device, avdtp_signal_id signal_id, uint16 error_code)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpSendReject device=%p enum:avdtp_signal_id:%d error_code=%u", (void*)device, signal_id, error_code);
...
void a2dpSendGeneralReject (remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpSendGeneralReject device=%p", (void*)device);
...
bool a2dpSendCommand(remote_device *device, uint8 signal_id, const uint8* payload_data, uint16 payload_size)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpSendCommand device=%p enum:avdtp_signal_id:%d payload_size=%u", (void*)device, signal_id, payload_size);
...
static void processGeneralRejectResponse(remote_device *device)
{
    ...
    {
        DEBUG_LOG("processGeneralRejectResponse no pending issued transaction or invalid transaction id");
        return;
    }
...
static bool handleReceiveCommand(remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("handleReceiveCommand %d", device->bitfields.device_id);

    if (!sink || !SinkIsValid(sink))
    {
        DEBUG_LOG_INFO("handleReceiveCommand invalid sink");
        return FALSE;
    }

    if (device->signal_conn.status.pending_received_transaction)
    {
        DEBUG_LOG_INFO("handleReceiveCommand pending received transaction");
        return FALSE;
    }
    ...
    */
    ...
    {   /* Unknown signal id, send a general reject */
        DEBUG_LOG_INFO("handleReceiveCommand unknown signal id");
    ...
    {   /* PDU is too short for specified signal id */
        DEBUG_LOG_INFO("handleReceiveCommand PDU too short");
        a2dpSendReject(device, ptr[1], avdtp_bad_length);
        return TRUE;
    }

    avdtp_signal_id signal_id = ptr[1];
    DEBUG_LOG_INFO("handleReceiveCommand enum:avdtp_signal_id:%d", signal_id);
...
static void handleReceiveResponse(remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("handleReceiveResponse %u", device->bitfields.device_id);
    ...
    {   /* Transaction id does not match */
        DEBUG_LOG_INFO("handleReceiveResponse no pending issued transaction or invalid transaction id");
        return;
    }
    ...
    {   /* Must contain at least a header and signal id */
        DEBUG_LOG_INFO("handleReceiveResponse invalid PDU size");
        return;
    }

    avdtp_signal_id signal_id = ptr[1];
    DEBUG_LOG_INFO("handleReceiveResponse enum:avdtp_signal_id:%d", signal_id);
...
static void handleBadHeader(remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("handleBadHeader");
...
static bool processSignalPacket(remote_device *device)
{
    ...
    uint16 i;
    DEBUG_LOG_V_VERBOSE("processSignalPacket length=%u  <",device->signal_conn.connection.active.bitfields.received_packet_length);
    for(i=0;i<device->signal_conn.connection.active.bitfields.received_packet_length;i++) DEBUG_LOG_V_VERBOSE("%X",device->signal_conn.connection.active.received_packet[i]);
    DEBUG_LOG_V_VERBOSE(">");
    ...
    switch (message_type)
    {
        case avdtp_message_type_command:
            ...
            {   /* it was inappropriate to process the command, so go away and come back later */
                DEBUG_LOG_INFO("processSignalPacket deferred");
                return FALSE;
            }
            break;
