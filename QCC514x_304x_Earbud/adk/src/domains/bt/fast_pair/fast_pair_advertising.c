...
static le_adv_data_item_t fp_model_id_data_item;
...
static le_adv_data_item_t fastPairGetAccountKeyFilterAdvData(void)
{
    ...
    if (bloom_filter_size)
    {
         ...
         */
        adv_size = SIZE_GFPS_ID_ADV + bloom_filter_size + FP_BATTERY_NOTFICATION_SIZE;
...
static le_adv_data_item_t fastPair_GetDataUnIdentifiable(uint16 data_set_identifier)
{
    ...
    if (data_set_identifier==FAST_PAIR_ADV_ITEM_BLOOM_FILTER_ID)
    {
        ...
        {
            /* Fast pair can now get started whilst in the case, until we support pairing in the case
             * do not supply any fast pair advertising data when there are no account keys and still
             * in the case. */
        ...
        {
            /* Where we do have existing account keys, we still need to send fast pair adverts whilst
             * in the case. This enables battery notification data to be advertised for existing
             * paired handsets. */
