...
class KerDebugInfo(di.DebugInfoInterface):
    ...
    def read_debuginfo(self):
        ...
        except elfreaderlib.NoFileLoadedError:
    ...
    def type_has_union(self, type_id):
        ...
        for mem in itype.members:
            if mem.type_id == 0xFFFFFFFF:
                logger.warning(
                    'Type %s has an unknown type of %s as a member.',
                    itype.name,
                    mem.name
                )
                continue
    ...
    def get_type_info(self, type_name_or_id, elf_id=None):
        ...
        if size == 0:
            raise InvalidDebuginfoTypeError(
                type_name + " with type id " + hex(first_tid) +
                " has no size."
            )
    ...
    def get_type_name(self, type_id, elf_id=None):
        ...
        """
        if elf_id is not None and elf_id != self.elf_id:
            raise InvalidDebuginfoCallError()

        try:
            return self._get_type_name(self.types[type_id])

        except IndexError:
            raise InvalidDebuginfoTypeError(
                "Type ID {} not found.".format(type_id)
            )

    def _get_type_name(self, ker_type_info):
        if ker_type_info.form == 3:
            # It is an array.
            return ('{}[]'.format(
                self._get_type_name(self.types[ker_type_info.array_type_id])
            ))

        elif ker_type_info.form == 5:
            # It is a pointer.
            return ('{} *'.format(
                self._get_type_name(self.types[ker_type_info.ref_type_id])
            ))

        elif ker_type_info.form in (1, 2, 4):
            # It is either `typedef`, `struct` or a `union`.
            return ker_type_info.name

        else:
            raise InvalidDebuginfoTypeError(
                "Form {0} not found for {1}".format(
                    ker_type_info.form,
                    ker_type_info,
                )
            )
    ...
    def get_type_members(self, type_name_or_id, elf_id=None):
        ...
        """
        type_id = self.get_type_info(type_name_or_id, elf_id)[1]

        var_type = self.types[type_id]
        ref_type = var_type.ref_type_id
        if ref_type != 0xffffffff:
            var_type = self.types[ref_type]
        return [i.name for i in var_type.members]
    ...
    def _inspect_non_bitfield(self, var, mem):
        ...
        if member_size < Arch.addr_per_word and Arch.addr_per_word == 4:
            ...
            # Starting index is inclusive and ending index is exclusive in
            # Python slices. Using ``//`` removes the fractions, so it's
            # "floor" by definition. And ``math.ceil`` is going to include the
            # fractions.
            start_index = mem.offset // Arch.addr_per_word
            end_index = math.ceil(
                (mem.offset + member_size) / float(Arch.addr_per_word)
            )
    ...
    def inspect_var(self, var, elf_id=None):
        ...
        # set the debuginfo for the variable since is known.
        var.set_debuginfo(self)
        ...
        if ptr_typeid is not None:
            ...
            pass
        elif members_typeid == 0xffffffff:
            # Avoid inspecting an array, union or struct if the members type
            # id is invalid.
            pass
