...
#ifndef LOCAL_ADDR_H_
#define LOCAL_ADDR_H_
...
/*!
    \brief Save the programmed BT address of this device.
    \param addr Pointer to the programmed BT address.

    The programmed BT address is only visible during boot before any components
    override it. Save the address and so it is available to retrieve with a
    synchronous API.
*/
void LocalAddr_SetProgrammedBtAddress(const bdaddr* addr);

/*!
    \brief Retrieve the programmed BT address of this device.
    \param addr[out] Pointer to bdaddr structure into which address will be written if available.
    \return bool TRUE Address provided in addr parameter, FALSE otherwise.
*/
