...
from kats.library.types.int_types import IntHex, IntHexU32
...
class IO:
    ...
    @log_output(logging.INFO, formatters={'address': '0x%08x'})
    def mem_peek(self, memory_space, address, size):
        ...
        """
        return IntHex(self._kalcmd.peek(memory_space, address, size - 1), size=size)
    ...
    def mem_block_read(self, memory_space, address, num_dwords):
        ...
        """
        ret = self._kalcmd.block_mem_read(memory_space, address, num_dwords)
        return [IntHexU32(entry) for entry in ret]
