...
PRESERVE_TYPE_FOR_DEBUGGING(DM_ULP_EXT_SCAN_FILTERED_ADV_REPORT_IND_T)
PRESERVE_TYPE_FOR_DEBUGGING(DM_ULP_EXT_SCAN_CTRL_SCAN_INFO_IND_T)
PRESERVE_TYPE_FOR_DEBUGGING(DM_ULP_PERIODIC_SCAN_SYNC_ADV_REPORT_IND_T)
...
bool VmGetAclLinkQuality(const tp_bdaddr *tpaddr, uint16 *link_quality)
{
    IPC_VM_GET_ACL_LINK_QUALITY send_prim;
    IPC_VM_GET_ACL_LINK_QUALITY_RSP rsp_prim;
    TP_BD_ADDR_T tpaddr_fw;
    ...
    tpaddr_fw.addrt.addr.uap = tpaddr->taddr.addr.uap;
    tpaddr_fw.addrt.addr.nap = tpaddr->taddr.addr.nap;
    send_prim.acl_address = &tpaddr_fw;

    ipc_transaction(IPC_SIGNAL_ID_VM_GET_ACL_LINK_QUALITY, &send_prim, sizeof(send_prim),
                    IPC_SIGNAL_ID_VM_GET_ACL_LINK_QUALITY_RSP, &rsp_prim);

    if (link_quality != NULL)
    {
        *link_quality = rsp_prim.link_quality;
    }
...
void trap_api_send_bluestack_message(uint16 protocol, void *prim)
{
    ...
    {
        case ATT_PRIM:
            hdlr = trap_api_lookup_message_task(IPC_MSG_TYPE_ATT);
        break;
        case MDM_PRIM:
            hdlr = trap_api_lookup_message_task(IPC_MSG_TYPE_MDM);
        break;
        case VSDM_PRIM:
            hdlr = trap_api_lookup_message_task(IPC_MSG_TYPE_VSDM);
        break;
        default:
            hdlr = trap_api_lookup_message_task(IPC_MSG_TYPE_BLUESTACK);
        break;
    }
