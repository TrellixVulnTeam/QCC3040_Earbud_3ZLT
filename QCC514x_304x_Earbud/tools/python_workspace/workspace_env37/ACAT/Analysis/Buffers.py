...
Module responsible for analysing buffers in Kymera.
"""
import math
import numbers
import os
...
from ACAT.Core.exceptions import VariableMemberMissing
...
class Cbuffer(object):
    ...
    def __init__(self, var, is_old_version):
        ...
        # The indent is used to display the buffer with the right alignment.
        # The easiest thing to do is to use the debuginfo variable indent.
        self.indent = var._debuginfo_var.indent
...
class Buffers(Analysis.Analysis):
    ...
    def get_mmu_buffer_width(self, handle):
        ...
        try:
            ...
            handle_size = handle['buffer_size'].value
    ...
    def inspect_cbuffer(self, buffer_p, analyse_metadata=False):
        ...
        descriptor = buffer_var.var['descriptor'].value
        ...
        if buffer_var.buffer_type == "SW" or buffer_var.buffer_type == "MMU":
            ...
            # for an MMU buffer.
            base_addr = buffer_var.var['base_addr'].value
            buffer_var.base_addr = base_addr
            buffer_var.rd_offset = (
                buffer_var.var['read_ptr'].value - base_addr
            ) // Arch.addr_per_word
            buffer_var.wr_offset = (
                buffer_var.var['write_ptr'].value - base_addr
            ) // Arch.addr_per_word
        ...
        if buffer_var.buffer_type == "MMU":
            if descriptor & (1 << DESCRIPTOR_FLAGS.index('RD_PTR_TYPE_MMU')):
                handle_p = buffer_var.var['read_ptr'].value
                handle = self.chipdata.cast(handle_p, "audio_buf_handle_struc")
                rd_offset = handle['buffer_offset'].value
            ...
            if descriptor & (1 << DESCRIPTOR_FLAGS.index('WR_PTR_TYPE_MMU')):
                handle_p = buffer_var.var['write_ptr'].value
                handle = self.chipdata.cast(handle_p, "audio_buf_handle_struc")
                wr_offset = handle['buffer_offset'].value
            ...
            if descriptor & (1 << DESCRIPTOR_FLAGS.index('AUX_PTR_PRESENT')):
                handle_p = buffer_var.var['aux_ptr'].value
                handle = self.chipdata.cast(handle_p, "audio_buf_handle_struc")
                aux_offset = handle['buffer_offset'].value
        ...
        if buffer_var.buffer_type == "Remote MMU":
            ...
            packed_rdh = buffer_var.var['read_ptr'].value
            packed_wrh = buffer_var.var['write_ptr'].value
            # SSID, MMU index
        ...
        buffer_var.buffer_size = \
            buffer_var.var['size'].value // Arch.addr_per_word
        ...
        if descriptor & 1 << DESCRIPTOR_FLAGS.index('BUF_DESC_IN_PLACE_SHIFT'):
            ...
            # Cannot use dereference because aux_ptr is a void pointer.
            tail_buff = buffer_var.var['aux_ptr'].value
            if tail_buff != 0:
                ...
                new_rd_offset = (
                    tail_buffer_var['read_ptr'].value - base_addr
                ) // Arch.addr_per_word
    ...
    def _analyse_buffer_metadata(self, buffer_var):
        ...
        if buffer_var.var['metadata'].value:
            metadata_list = buffer_var.var['metadata'].deref
    ...
    def _get_linked_list_contents(self, metadata_list):
        ...
        try:
            head_value = metadata_list['tags']['head'].value  # ## AAA use deref
        except AttributeError:  # For backwards compatibility
            head_value = metadata_list['tags']['local']['head'].value

        if head_value != 0:
            head = self.chipdata.cast(head_value, 'metadata_tag')
            count = 0
            metadata_list_str += 'Metadata tags:\n'
            for tag in ch.parse_linked_list(head, 'next'):
                count += 1
...
class MetadataTag(object):
    ...
    def __init__(self, tag):
        ...
        for member_name in self.tag_member_names:
            self.tag_members[member_name] = tag[member_name].value
    ...
    def __str__(self):
        # The indent is used to display the tag with the right alignment.
        # The easiest thing to do is to use the debuginfo variable indent.
        tag_str = self.tag._debuginfo_var.indent
