...
typedef struct 
{
    ...
    /*! Prepare clients that have yet to respond */
    task_list_t                 *pending_prepare_tasks;
    ...
    /*! Record of the number of calls made to PeerFindRole_FindRole */
    uint16                      num_find_role_calls;

    /*! Record of the outcomes of Find Role.
        This is encoded as hex digits, with the last result in the lowest
        4 bits. Each digit is the lower bits from the enum 
        \ref peer_find_role_message_t */
    uint32                      outcomes;
...
/*! \brief Record a call to PeerFindRole_FindRole

    This function is only active if PEER_FIND_ROLE_RECORD_OUTCOMES is 1,
    having no effect if not.
 */
#define PeerFindRole_RecordCall() ((void)peer_find_role.num_find_role_calls++)

/*! \brief Return the number of calls to PeerFindRole_FindRole so far

    This function is only active if PEER_FIND_ROLE_RECORD_OUTCOMES is 1,
    otherwise returning 0
 */
#define PeerFindRole_NumCalls()  (peer_find_role.num_find_role_calls)

/*! \brief Record a peer find role outcome

    Maintain a history of peer find role completions.

    \param _msg The message being sent to the client
 */
#define PeerFindRole_RecordOutcome(_msg) \
                do {\
                    peer_find_role.outcomes = (peer_find_role.outcomes << 4) \
                                                + ((_msg)&0xF); \
                } while(0)

/*! \brief Get the outcome of previous peer find role execution

    \return A uint 32 representing the last 8 messages sent by Peer Find Role.
 */
#define PeerFindRole_GetOutcomes() (peer_find_role.outcomes)

/*! Message identifiers used for internal messages */
typedef enum
{
        /*! Update the score and notify GATT server (if any) */
    PEER_FIND_ROLE_INTERNAL_UPDATE_SCORE = INTERNAL_MESSAGE_BASE,
    ...
    PEER_FIND_ROLE_INTERNAL_PREPARED,
        ...
        /*! Advertising was not yet released when attempting to start */
    PEER_FIND_ROLE_INTERNAL_ATTEMPT_ADVERTISING,

        /* Timer messages grouped together */

        /*! Failed to discover peer within requested time */
    PEER_FIND_ROLE_INTERNAL_TIMEOUT_DISCOVERY,

        /*! Failed to connect to peer within requested time */
    PEER_FIND_ROLE_INTERNAL_TIMEOUT_PEER_CONNECTION,
    ...
    PEER_FIND_ROLE_INTERNAL_MESSAGE_END
} peer_find_role_internal_message_t;
ASSERT_INTERNAL_MESSAGES_NOT_OVERFLOWED(PEER_FIND_ROLE_INTERNAL_MESSAGE_END)
