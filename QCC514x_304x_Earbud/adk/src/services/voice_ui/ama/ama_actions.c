...
#ifdef INCLUDE_WUW
static void amaActions_ToggleWakeWordEnable(void)
{
    if (VoiceUi_IsWakeUpWordFeatureIncluded())
    {
        if (VoiceUi_WakeWordDetectionEnabled())
        {
            VoiceUi_DisableWakeWordDetection();
            AmaAudio_StopWakeWordDetection();
            VoiceUi_Notify(VOICE_UI_AMA_WUW_DISABLED);
        }
        else
        {
            VoiceUi_EnableWakeWordDetection();
            AmaAudio_StartWakeWordDetection();
            VoiceUi_Notify(VOICE_UI_AMA_WUW_ENABLED);
        }
    }
}
#endif  /* INCLUDE_WUW */
...
static void amaActions_ActionOnEvent(ama_button_events_t event_id)
{
    ...
    switch(event_id)
    {
        ...
        case AMA_BUTTON_DOUBLE_TAPPED:
            ...
            {
                DEBUG_LOG_VERBOSE("amaActions_ActionOnEvent: AMA_BUTTON_DOUBLE_TAPPED: Stop session");
                /*
                 * Stopping a session is achieved by a COMMAND__START_SPEECH with a new dialog ID,
                 * followed by a COMMAND__STOP_SPEECH with that new dialog ID once the
                 * COMMAND__START_SPEECH response is received.
                 */
                AmaData_SetStoppingSession(TRUE);
                AmaSpeech_StartTapToTalk();
            }
#ifdef INCLUDE_WUW
            else
            {
                DEBUG_LOG_VERBOSE("amaActions_ActionOnEvent: AMA_BUTTON_DOUBLE_TAPPED: Toggle WuW privacy");
                amaActions_ToggleWakeWordEnable();
            }
#endif
            break;
