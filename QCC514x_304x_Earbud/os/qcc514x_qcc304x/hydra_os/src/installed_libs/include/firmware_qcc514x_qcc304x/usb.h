 ...
 *         
 *         Only descriptors with correspondent bits set in the
 *  "descriptors_bitmask"
 ...
 *  USB_DT_CONFIG));
 *         
 *         Altenatively, "0" can be supplied to include all descriptors supported
 *         by the trap:
 *         
 *         uint32 hash = UsbDescriptorsHash(0);
 *         
 *         At the moment, the trap supports these descriptors:
