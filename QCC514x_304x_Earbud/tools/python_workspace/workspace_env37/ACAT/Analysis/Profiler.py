...
from ACAT.Core.exceptions import (DebugInfoNoVariableError, VariableMemberMissing)
...
class Profiler(Analysis.Analysis):
            ...
            for profiler in ch.parse_linked_list(profiler_list, 'next'):
                ...
                try:
                    peak_cpu_usage = float(
                        profiler['peak_cpu_fraction'].value
                    ) / 10
                except BaseException:
                    peak_cpu_usage = None
    ...
    def clear_peak(self):
        ...
        if profiler_head_ptr != 0:
            ...
            for profiler_entry in ch.parse_linked_list(
                    profiler_list, 'next'
            ):
                # clear the peak cpu usage.
                self.chipdata.set_data(
                    profiler_entry['peak_cpu_fraction'].address,
                    [0]
                )
