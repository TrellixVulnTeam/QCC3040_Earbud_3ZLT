...
/*! \brief Callback to authorise incoming connections. */
typedef struct
{
    ...
    /*! \returns Return TRUE to accept the connection, FALSE to reject */
    bool (*AuthoriseConnection)(const bdaddr* bd_addr, dm_protocol_id protocol_id, uint32 channel, bool incoming);
} con_manager_authorise_callback_t;
...
/*! \brief Return the current QoS setting

    \param tpaddr The address of the remote device

    \return QoS setting value
 */
...
/*! \brief Set a callback to accept or reject incoming connections for a 
           given type of device. This replaces the connection_manager limits 
           on number of devices of a particular type, but does not override
           restrictions set through ConManagerAllowConnection or 
           ConManagerAllowHandsetConnect

    \param[in] dev_type        Type of device to set callback for
