...
from ACAT.Core.exceptions import (
    DebugInfoNoVariableError, InvalidDebuginfoTypeError, VariableMemberMissing
)
...
class Endpoint(object):
    ...
    def __init__(self, ep, name, stream, is_state_type_equal_to_name=True):
        ...
        self.id = Stream.convert_ep_id(ep['id'].value, 1)
        self.can_be_closed = ep['can_be_closed'].value
        if self.can_be_closed == 0:
            self.can_be_closed = False
        else:
            self.can_be_closed = True

        self.can_be_destroyed = ep['can_be_destroyed'].value
        if self.can_be_destroyed == 0:
            self.can_be_destroyed = False
        else:
            self.can_be_destroyed = True

        self.destroy_on_disconnect = ep['destroy_on_disconnect'].value
        ...
        self.direction = stream.debuginfo.get_enum(
            'ENDPOINT_DIRECTION', ep['direction'].value
        )[0]
        self.is_real = ep['is_real'].value
        if self.is_real == 0:
            self.is_real = False
        else:
            self.is_real = True

        self.con_id = ep['con_id']
        ...
        self._connected_to = ep['connected_to'].value
        self._ep_to_kick = ep['ep_to_kick'].value
        ...
        if is_state_type_equal_to_name:
            setattr(self, name, self.state[name])
...
class AudioEndpoint(Endpoint):
    ...
    def desc_str(self):
        """Returns the description string of the endpoint."""
        ep_str = super().desc_str

        tp_ptr_var = self.audio['timed_playback']
        if tp_ptr_var is not None:
            # Timed playback is enabled in the build.
            timed_playback_ptr = tp_ptr_var.value

            if timed_playback_ptr != 0:
                # The endpoint has a timed playback instance. Cast the value and
                # replace the `member` text with it.
                ...
                ep_str = ep_str.replace(
                    "timed_playback: (TIMED_PLAYBACK *) {}".format(
                        hex(timed_playback_ptr)
                    ),
...
class OperatorEndpoint(Endpoint):
    ...
    def __init__(self, ep, name, stream):
        ...
        self.operator = self.state['operator']
...
class ScoEndpoint(Endpoint):
    """SCO/ISO Endpoint.
    
    SCO/ISO endpoint is treated differently to display the SCO driver along
    with the MMU buffer analysis.
    """

    def __init__(self, ep, name, stream):
        """Initialises the ScoEndpoint object.
        
        Args:
            ep: Raw endpoint Variable.
            name: Name specified to identify the endpoint type.
                  In most cases it should match with the endpoint type defined
                  in the firmware. This is further specified within the last
                  parameter when instatiating the Endpoint object.
            stream: Analysis.Analysis object
        """
        Endpoint.__init__(self, ep, name, stream, True)

        self._mmu_buffer = self.state.sco.cbuffer.value

        try:
            sco_drv = self.state.sco.sco_drv
            if self.direction == 'SOURCE':
                self.sco_drv_type = "SCO_SRC_DRV_DATA"
                self._sco_drv = sco_drv.sco_src_drv.value
                # Check if the sco_drv_data works
                stream.chipdata.cast(self._sco_drv, self.sco_drv_type)
            else:
                self.sco_drv_type = "SCO_SINK_DRV_DATA"
                self._sco_drv = sco_drv.sco_sink_drv.value
                # Check if the sco_drv_data works
                stream.chipdata.cast(self._sco_drv, self.sco_drv_type)
        except VariableMemberMissing:
            # Older firmware will not have a SCO driver associated with the
            # SCO endpoint.
            self._sco_drv = None

    @property
    def desc_str(self):
        """Returns the description string of the endpoint along with the
        SCO driver data.
        """
        ep_str = super().desc_str

        if self._sco_drv:
            sco_drv_data = self.stream.chipdata.cast(self._sco_drv,
                                                     self.sco_drv_type)
            ep_str += "\n" + str(sco_drv_data)
...
class Transform(object):
    ...
    def __init__(self, t, stream):
        ...
        self.id = Stream.convert_transform_id(t['id'].value, 1)
...
class Stream(Analysis.Analysis):
    ...
    def get_ep_audio_sink_timed(self):
        ...
        for ep in self.get_endpoints():
            if ep.name == 'audio' and ep.direction == 'SINK':
                if ep.audio['timed_playback'].value != 0:
    ...
    def get_ep_audio_sink_timed_dict(self):
        ...
        for ep in ep_audio_sink_timed:
            ...
            ep_chipdata = self.chipdata.cast(
                ep.audio['timed_playback'].value, "TIMED_PLAYBACK"
            )
            current_tag = ep_chipdata['current_tag']
            playback_time = current_tag['playback_time'].address
            timestamp = current_tag['timestamp'].address

            temp_dict['error'] = current_tag['error'].address
            temp_dict['timestamp_minus_playback_time'] = timestamp - playback_time
            temp_dict['warp'] = ep_chipdata['pid_state']['warp'].address
            temp_dict['sp_adjust'] = ep_chipdata['tag_reframe']['tag']['sp_adjust'].address
            ep_draw_dict[ep.id] = temp_dict
    ...
    def plot_transforms_buffers(self, screen_width=0, live_plotting=True):
        ...
        if screen_width == 0:
            try:
                ...
                # width.
                screen_width = int(shutil.get_terminal_size().columns * 0.9)

            except (AttributeError, OSError):
                # Set a default screen width.
                screen_width = 120
        ...
        if isinstance(self.chipdata, Coredump) and live_plotting:
            logger.warning("Live plotting is disabled in Coredump.")
            live_plotting = False
    ...
    def analyse_service_priv(self, ep):
        ...
        """
        try:
            srv_priv_p = getattr(ep, ep.name)['service_priv'].value
            ...
            self.formatter.output(
                self.chipdata.
                    cast(srv_priv['meta_desc'].value, "META_DESC")
            )

    def analyse_sco_ep_buffer(self, ep):
        """Display the MMU buffer associated to the SCO/ISO endpoint.
        ...
        buffers_analysis.analyse_cbuffer(ep._mmu_buffer)
    ...
    def analyse_endpoints(self, cbopsanalysis_on=False):
        ...
        for endpoint in self.endpoints:
            ...
            if endpoint.name == "audio" and endpoint.direction == "SINK":
                running = endpoint.audio['running'].value
                sync_started = endpoint.audio['sync_started'].value
                latency_ctrl_info_var = endpoint.audio['latency_ctrl_info']
                silence_samples = latency_ctrl_info_var['silence_samples'].value
            ...
            if endpoint.name in ("sco", "iso"):
                self.analyse_sco_ep_buffer(endpoint)
    ...
    def analyse_endpoint(self, ep_id, cbopsanalysis_on=True):
        ...
        if ep.name == "audio" and ep.direction == "SINK":
            running = ep.audio['running'].value
            sync_started = ep.audio['sync_started'].value
            latency_ctrl_info_var = ep.audio['latency_ctrl_info']
            silence_samples = latency_ctrl_info_var['silence_samples'].value
            if running and sync_started != 0 and \
                    silence_samples != 0:
                self.formatter.alert(
        ...
        if ep.name in ["sco", "iso"]:
            self.analyse_sco_ep_buffer(ep)
    ...
    def _display_rm_pair(self, rm_pair):
        """Helper function which displays a ratematch pair.

        Args:
            rm_pair: Ratematch pair.

        Returns:
            True if the endpoints are in this core, False otherwise.
        """
        # Get the endpoints
        ...
        if not ep1:
            # The endpoints are in the other core.
            return False
        # Get the endpoint name
        ep1 = ep1.compact_name()
        # Rest of the endnpoints must be on the same core as the first one.
        ep2 = self.get_endpoint_by_address(
            rm_pair['ep2'].value
        ).compact_name()
        enacting_ep = self.get_endpoint_by_address(
            rm_pair['enacting_ep'].value
        ).compact_name()
    ...
    def analyse_rm_pairs(self):
        ...
        pair_found = False
        try:
            rm_list = self.chipdata.get_var_strict('L_rm_list')
            for rm_pair in ch.parse_linked_list(rm_list, 'next'):
                if self._display_rm_pair(rm_pair):
                    pair_found = True
        except DebugInfoNoVariableError:
            pass
    ...
    def analyse_kick_objects(self):
        ...
        kick_object_list = self.chipdata.get_var_strict('L_kick_object_list')
        kick_object_found = False
        for kick_object in ch.parse_linked_list(kick_object_list, 'next'):
            sched_ep = self.get_endpoint_by_address(
                kick_object['sched_ep'].value
            )
            kick_ep = self.get_endpoint_by_address(
                kick_object['kick_ep'].value
            )
            self.formatter.section_start("")
            self.formatter.output(
    ...
    def _read_raw_transforms(self):
        transforms = []
        transform_list = self.chipdata.get_var_strict('$_transform_list')
        for transform in ch.parse_linked_list(transform_list, 'next'):
    ...
    def _read_raw_endpoints(self):
        endpoints = []
        source_endpoint_list = self.chipdata.get_var_strict(
            '$_source_endpoint_list'
        )
        for source in ch.parse_linked_list(source_endpoint_list, 'next'):
            endpoints.append(self._read_endpoint(source))

        sink_endpoint_list = self.chipdata.get_var_strict(
            '$_sink_endpoint_list'
        )
        for sink in ch.parse_linked_list(sink_endpoint_list, 'next'):
    ...
    def _read_endpoint(self, ep):
        ...
        ep_type = self.debuginfo.get_enum(
            'ENDPOINT_TYPE', ep['stream_endpoint_type'].value
        )[0]
        ...
        if ep_type == "sco" or ep_type == "iso":
            return ScoEndpoint(ep, ep_type, self)
