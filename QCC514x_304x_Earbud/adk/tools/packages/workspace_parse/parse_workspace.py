...
try:
    from vm_build.aliases.aliases import Aliases
...
class ParseWorkspace(object):
    ...
    def __init__(self, workspace_file, aliases=None):
        self.workspace_xml = parsers.get_xml(workspace_file)
        if aliases is None:
            aliases = dict()
        self.aliases = Aliases(workspace_file, **aliases)
    ...
    def parse(self):
        if len(self.workspace_xml.base_dir) > 111:
            print("================================")
            print("WARNING")
            print("Path '%s'" % self.workspace_xml.base_dir)
            print("is %d characters" % len(self.workspace_xml.base_dir))
            print("If include errors are seen, try to shorten the path.")
            print("================================")
