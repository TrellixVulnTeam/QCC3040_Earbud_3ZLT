...
/* Check the availability of a cap_id in a chain */
bool ChainCheckCapabilityId(kymera_chain_handle_t chain, capability_id_t cap_id)
{
    ...
    for (i = 0; i < config->number_of_operators; i++)
    {
        ...
        if(op_config)
        {
            if (op_config->capability_id == cap_id)
            {
                return TRUE;
            }
        }
...
void ChainLoadDownloadableCapsFromChainConfig(const chain_config_t * chain_config)
{
    ...
    for(i=0; i<chain_config->number_of_operators; i++)
    {
        if(CustomOperatorIsDownloadableCapability(chain_config->operator_config[i].capability_id))
        {
             CustomOperatorLoadBundle(chain_config->operator_config[i].capability_id);
        }
    }
}
...
void ChainUnloadDownloadableCapsFromChainConfig(const chain_config_t * chain_config)
{
    ...
    for(i=0; i<chain_config->number_of_operators; i++)
    {
        if(CustomOperatorIsDownloadableCapability(chain_config->operator_config[i].capability_id))
        {
             CustomOperatorUnloadBundle(chain_config->operator_config[i].capability_id);
        }
    }
}
