...
/*! \brief Logical Input screening decider callback, implemented by the Application,
           used when e.g. button presses should be ignored */
typedef bool (*li_screening_decider_t)(unsigned logical_input);
...
/*! \brief Inject a Logical Input, with originating device, into the CAA framework.

    This API is used to inject a UI Input into the system from the Peer Service
    Logical Input Switch. The Logical Input will be checked against the RAM and ROM
    UI configuration tables and may lead to a UI Input being raised if a matching UI
    Provider context is found.

    \param logical_input - The Logical Input to be injected
    \param is_right_device - TRUE if the logical input originated from the right
                             device, FALSE if it was from the left device.
*/
...
/*! \brief Register a function which shall indicate when logical inputs should be
           screened out by the UI domain.

    This mechanism allows the appliucation to register a callback function which
    shall indicate when the UI domain should silently dispose (i.e. screen out)
    any Logical Inputs that it receives.

    If no function is registered the Logical Inputs shall never be screened.

    \param li_screening_decider_func - The Application callback which shall indicate
                                   when logical inputs should be screened.
*/
void Ui_RegisterLogicalInputScreeningDecider(li_screening_decider_t li_screening_decider_func);
