...
/*! \brief Start advertising
    ...
    \note Should only be called if in an advertising state 

    \return TRUE if advertising request successful, FALSE otherwise.
                 adv_handle in the runtime date will have been set
 */
bool peer_pair_le_enable_advertising(void);

/*! \brief Check if the module is in a pairing state 

    \return TRUE if in a pairing state
*/
bool peer_pair_le_in_pairing_state(void);

/*! Check if a state is an advertising state

    \param state    The state to check

    \return TRUE if state is a state that can have advertising, otherwise FALSE
 */
