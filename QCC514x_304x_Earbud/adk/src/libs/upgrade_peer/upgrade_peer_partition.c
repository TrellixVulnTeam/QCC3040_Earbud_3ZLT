...
static bool getNextUpgradePskeyData(uint8 *peer_data, uint32 req_len)
{
    ...
    if(peerPartitionCtx == NULL || peerPartitionCtx->pskey > UpgradePartitionDataDfuHeaderPskeyEnd() ||
       req_len == 0)
    {
        return FALSE;
    }
...
static bool checkPskeyDataAvailable(uint32 size)
{
    ...
    DEBUG_LOG("checkPskeyDataAvailable : UpgradeCtxGetPartitionData()->dfuHeaderPskey %d", UpgardeCtxDfuHeaderPskey());
    DEBUG_LOG("checkPskeyDataAvailable : peer_ctx->pskey %d",peer_ctx->pskey);
    DEBUG_LOG("checkPskeyDataAvailable : UpgradeCtxGetPartitionData()->dfuHeaderPskeyOffset %d", UpgardeCtxDfuHeaderPskeyOffset());
    DEBUG_LOG("checkPskeyDataAvailable : peer_ctx->read_offset %d",peer_ctx->read_offset); 
    
    if(UpgardeCtxDfuHeaderPskey() < peer_ctx->pskey)
    {
    ...
    {
        diff_len = UpgardeCtxDfuHeaderPskey() - peer_ctx->pskey;
    }
    ...
    if (diff_len == 0)
    {
        ...
        if ((UpgardeCtxDfuHeaderPskeyOffset() / 2) < peer_ctx->read_offset)
        {
    ...
    readable_size = ((diff_len * PSKEY_MAX_STORAGE_LENGTH) + (UpgardeCtxDfuHeaderPskeyOffset() / 2)) - peer_ctx->read_offset;
                                                                ...
                                                                uint32 req_len)
{
    ...
    if(0 == strncmp((char *)data_ptr, UpgradeGetHeaderID(), UpgradePartitionDataIdFieldSize()))
    {
                                           ...
                                           UpgradePartitionDataIdFieldSize()))
    {
        ...
        if(peerPartitionCtx->part_data_length < UpgradePartitionDataPartitionSecondHeaderSize())
        {
            return UPGRADE_PEER_ERROR_BAD_LENGTH_UPGRADE_HEADER;
        }
        peerPartitionCtx->peerPartitionState =
    ...
    else if(0 == strncmp((char *)data_ptr, UpgradeGetFooterID(), 
                                           UpgradePartitionDataIdFieldSize()))
    {
                                                            ...
                                                            uint32 req_len)
{
    ...
    peerPartitionCtx->part_data_length -= UpgradePartitionDataPartitionSecondHeaderSize();
...
void UpgradePeerParititonInit(void)
{
    ...
    peerPartitionCtx->pskey = UpgradePartitionDataDfuHeaderPskeyStart();
}
