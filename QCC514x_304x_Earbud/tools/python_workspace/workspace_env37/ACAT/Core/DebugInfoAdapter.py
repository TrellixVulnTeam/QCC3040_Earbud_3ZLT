...
from ACAT.Core.exceptions import (
    ...
    DebugInfoNoVariableError, InvalidDebuginfoEnumError
)
...
class DebugInfoAdapter(di.DebugInfoInterface):
    ...
    def get_type_info(self, type_name_or_id, elf_id=None):
        ...
        Returns a tuple containing:
            ...
            * A boolean value showing if the type has uninion.
        ...
        Args:
            type_name_or_id (int or str): Type name or id.
    ...
    @method_logger(logger)
    def get_type_name(self, type_id, elf_id=None):
        """Returns a type name.

        Args:
            type_id (int): Type identification number.
            elf_id (int, optional)
        ...
        Raises:
            InvalidDebuginfoTypeError: When the provided id is not found.
        """
        return self._call_debuginfo_elfid(
            elf_id,
            'get_type_name',
    ...
    @method_logger(logger)
    def get_type_members(self, type_name_or_id, elf_id=None):
        """Returns a type's members. This will be a list of strings or an
        empty list if type has no members.

        Args:
            type_name_or_id (str or int): Type name or identification number.
            elf_id (int, optional)

        Returns:
            lst: list of member names (str). Empty list if type has no members.

        Raises:
            InvalidDebuginfoTypeError: When the provided type is not found.
        """
        return self._call_debuginfo_elfid(
            elf_id,
            'get_type_members',
    ...
    def get_mmap_lst(self, elf_id):
        """Returns the mmap_lst for the bundle.
    ...
    @method_logger(logger)
    def set_kymera_debuginfo(self, kymera_debuginfo):
        ...
        self.debug_infos[None][1] = kymera_debuginfo
        # Add to the general dictionary
        self.debug_infos[kymera_debuginfo.get_elf_id()] = kymera_debuginfo
    ...
    def _address_in_table(self, address):
        """Checks if and address is in the downloaded bundle table.
    ...
    def get_downloadable_debuginfos(self):
        """Returns all the downloadable debug information objects in a list.

        Returns:
            (list): List containing all the downloadable debug information.
        """
        blocked_elf_ids = []
        kymera = self.get_kymera_debuginfo()
        blocked_elf_ids.append(kymera.elf_id)
        patch = self.get_patch_debuginfo()
        if patch is not None:
            blocked_elf_ids.append(patch.elf_id)

        return [
            ker_debug_info
            for elf_id, ker_debug_info in self.debug_infos.items()
            if elf_id is not None and elf_id not in blocked_elf_ids
        ]

    def get_elf_file_name(self, elf_id):
        """Returns the file name for the specific elf_id

        Returns:
            str: Name of the elf file if loaded to ACAT.
        """
        if not elf_id in self.debug_infos:
            file_name = "Unknown"
        else:
            file_name = ntpath.basename(self.debug_infos[elf_id].elf_path)
    ...
    def _get_descriptive_exception(self, elf_id, function_name, *argv, **kwargs):
        """When _call_debuginfo_function failed to return any value.
        This method is searching for possible valid elf_ids and raises
        the appropriate error displaying more help.
        ...
        Args:
            ...
            **kwargs: Function named input parameter.
        """
        # Default error to raise
        error_type = DebugInfoError
        # Array holding the possible elf_ids
        valid_elf_ids = []
        for (cur_elf_id, cur_debug_info) in self.debug_infos.items():
            # None is reserved for Kymera and patch, but they are also part of
            # the rest of the dict mapping elf_ids to debug_info.
            if cur_elf_id is not None:
                # Try if the call fails
                cur_funct = cur_debug_info.__getattribute__(function_name)
                try:
                    cur_funct(*argv, **kwargs)
                    valid_elf_ids.append(cur_elf_id)
                except DebugInfoError as debug_info_error:
                    # Call failed. Save the error type.
                    error_type = type(debug_info_error)

        if valid_elf_ids:
            display_str = "Operation " + function_name
            display_str += "(" + str(argv) + str(kwargs) + ")"
            display_str += " to debug info " + self.get_elf_file_name(elf_id)
            display_str += "(elf_id=" + hex(elf_id) + ") fails. "
            display_str += "Debug_infos supporting this operation: "
            display_str += ", ".join(
                [
                    self.get_elf_file_name(cur_elf_id) +
                    "(elf_id=" + hex(cur_elf_id) + ")"
                    for cur_elf_id in valid_elf_ids
                ]
                )
            return error_type(display_str)
        else:
            None
    ...
    def _call_debuginfo_elfid(self, elf_id, function_name, *argv, **kwargs):
        ...
        if elf_id is None:
            ...
            patch, kymera = self.debug_infos[elf_id]
            if patch:
                try:
        ...
        except (DebugInfoNoVariableError, InvalidDebuginfoTypeError,
                InvalidDebuginfoEnumError)  as debug_info_error:
            # There is chance that the elf_id is invalid. Check other
            # debug_infos to give better error message.
            # The function is not called here to avoid "exception while
            # handling exception" message which can be confusing.
            last_error = debug_info_error

        better_error = self._get_descriptive_exception(
            elf_id,
            function_name,
            *argv, **kwargs
        )
        if better_error is not None:
            raise better_error
        else:
            # last_error must be different than none if we are here
            raise last_error
