...
static void handleScanParamsGattMsg(MessageId id)
{
    ...
    GATT_SCAN_PARAMS_CLIENT_DEBUG_INFO(("Unknown Message MESSAGE:0x%x received from GATT lib\n",id));
...
static void handleScanParamsGattManagerMsg(Task task, MessageId id, Message payload)
{
    ...
    switch (id)
    {
        ...
        default:
        {
            ...
            GATT_SCAN_PARAMS_CLIENT_DEBUG_PANIC(("GSPC_T: Scan Params GattMgr Msg not handled MESSAGE:0x%x\n", id));
        }
        break;
    }
}
...
static void  handleScanParamsInternalMsg(Task task, MessageId id)
{
    ...
    GATT_SCAN_PARAMS_CLIENT_DEBUG_INFO(("handleScanParamsInternalMsg MESSAGE:scan_params_client_internal_msg_t:0x%x\n",
                                        id));
...
void scanParamsClientMsgHandler(Task task, MessageId id, Message payload)
{
    ...
    {
        GATT_SCAN_PARAMS_CLIENT_DEBUG_PANIC(("GSPC_T: Client Msg not handled MESSAGE:0x%x\n", id));
    }
}
