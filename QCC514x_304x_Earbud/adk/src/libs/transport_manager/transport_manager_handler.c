...
static void transportMgrHandleConnectCfm(TRANSPORT_CONNECT_CFM_T * msg)
{
    ...
    PRINT(("transportMgrHandleConnectCfm\n"));
    {
        ...
        {
            trans_data = PanicNull(transportMgrFindDataWithTransInfo(msg->transport_id,transport_type));

            /* Update the sink in the data block*/
            transportMgrUpdateSink(msg->transport_id, msg->sink, transport_type);
        ...
        else
        {
            ...
            transportMgrListBroadcastGattMessage(TRANSPORT_MGR_LINK_CREATED_CFM,message,sizeof(TRANSPORT_MGR_LINK_CREATED_CFM_T));
        }
    }
...
static void transportMgrHandleDisconnectCfm(TRANSPORT_DISCONNECT_CFM_T * msg)
{
    ...
    if(transport_type != transport_mgr_type_gatt)
    {
        ...
        trans_data = PanicNull(transportMgrFindDataWithTransInfo(transport_link_id,transportMgrConvertAdaptationType(msg->transport)));
        message->trans_sink = msg->sink;
    ...
    else
    {
        ...
        /* When the transport type is GATT we would not know 
            about the start and end handle */
        transportMgrListBroadcastGattMessage(TRANSPORT_MGR_LINK_DISCONNECTED_CFM,message,sizeof(TRANSPORT_MGR_LINK_DISCONNECTED_CFM_T));
    }
    }
}
...
/***************************************************************************/
void TransportMgrDeleteTransLinkData(transport_type_t trans_type, int16 transport_id)
{
    transport_mgr_link_data_t *trans_data = NULL;
    uint16 link_id_key = 0;
    /* Get the trans link id  which is used as a key for searching the link instance from the data base. */
    link_id_key = (transportMgrConvertAdaptationType(trans_type) != transport_mgr_type_gatt) ? transport_id : INVALID_TRANS_LINK_ID;
    trans_data = transportMgrFindDataWithTransInfo(link_id_key, transportMgrConvertAdaptationType(trans_type));
    if(trans_data != NULL)
    {
        transportMgrRemoveLinkRef(trans_data);
        TransportDeregisterReq(transportMgrConvertManagerType(trans_type), transport_id);
    }
}

/***************************************************************************/
bool TransportMgrCreateTransLinkData(Task application_task, transport_mgr_link_cfg_t link_cfg, Sink snk)
{
    transport_mgr_link_data_t *trans_data = NULL;
    /* Find node with provided link_id */
    trans_data = transportMgrFindDataWithTransInfo(link_cfg.trans_info.non_gatt_trans.trans_link_id, link_cfg.type);
    if (trans_data)
    {
        /* Node found, Update sink */
        PRINT(("TransportMgrCreateTransLinkData: Node found, sink %u, trans_link_id %u\n", snk, link_cfg.trans_info.non_gatt_trans.trans_link_id));
        trans_data->sink = snk;
        memcpy(trans_data->link_cfg, &(link_cfg), sizeof(transport_mgr_link_cfg_t));
        return TRUE;
    }
    else
    {
        PRINT(("TransportMgrCreateTransLinkData - new Node created \n"));
        transport_mgr_link_data_t * trans_data_new;
        trans_data_new = (transport_mgr_link_data_t *)PanicNull(calloc(1, sizeof(transport_mgr_link_data_t)));
        trans_data_new->link_cfg = (transport_mgr_link_cfg_t *)PanicNull( calloc(1,  sizeof(transport_mgr_link_cfg_t) ) );
        trans_data_new->application_task = application_task;
        trans_data_new->next = NULL;
        trans_data_new->sink = snk;
        memcpy(trans_data_new->link_cfg, &(link_cfg), sizeof(transport_mgr_link_cfg_t));
        transportMgrAddRef((void *)trans_data_new, transport_mgr_link_data);
        
        /*Register the channel with bluestack's server list */
        TransportRegisterReq(transportMgrConvertManagerType(link_cfg.type), 
                             link_cfg.trans_info.non_gatt_trans.trans_link_id);
        return FALSE;
...
static void transportMgrHandleInternalConnectReq(TRANSPORT_MGR_INTERNAL_CONNECT_REQ_T *msg)
{
    ...
    {
        MAKE_TRANS_MGR_MESSAGE(TRANSPORT_MGR_LINK_CREATED_CFM,message);
        trans_data = PanicNull(transportMgrFindDataWithTransInfo(msg->link_id, msg->trans_type));
...
static void transportMgrHandleInternalDisConnectReq(TRANSPORT_MGR_INTERNAL_DISCONNECT_REQ_T *msg)
{
    ...
    {
        MAKE_TRANS_MGR_MESSAGE(TRANSPORT_MGR_LINK_DISCONNECTED_CFM,message);
        trans_data = PanicNull(transportMgrFindDataWithTransInfo(transportMgrFindTranslinkIdFromSink(msg->sink), msg->trans_type));
        message->status = transport_mgr_status_fail;
        message->trans_sink= msg->sink;
        memcpy(&(message->link_cfg),trans_data->link_cfg,sizeof(transport_mgr_link_cfg_t));
