...
try:
    from vm_build.codegen import CommentBlock, CommentBlockDoxygen, HeaderGuards, Enumeration, Array, StructTypedef, UnionTypedef, Include, Indented, EnumerationTypedef
    
except ImportError:
    file_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(file_dir)
    sys.path.insert(0, os.path.join(parent_dir, "codegen"))
    from codegen import CommentBlock, CommentBlockDoxygen, HeaderGuards, Enumeration, Array, StructTypedef, UnionTypedef, Include, Indented, EnumerationTypedef
...
class TypesGenerator(object):
    ...
    def _generate_marshal_type_table(self):
        ...
        print("\n#define " + self._types_table_name() + "(ENTRY)", end="")
        for t in self._root:
            if t.tag in ["typedef_struct", "typedef_union", "typedef_enum"]:
                name = t.get('name')
                print ("\\")
                with Indented():
                    print(Indented.indent() + "ENTRY(" + name + ")", end="")
        print("\n")
