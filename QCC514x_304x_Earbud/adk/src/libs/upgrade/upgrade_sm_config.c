    ...
    case UPGRADE_HOST_TRANSFER_COMPLETE_RES:
        {
            ...
            if(msg->action == 0)
            {
                ...
                DEBUG_LOG("UpgradeSMHandleValidated, move to completed state. MESSAGE:0x%x action:%d", id, msg->action);
            ...
            {
                DEBUG_LOG("UpgradeSMHandleValidated, move to abort state. MESSAGE:0x%x action:%d", id, msg->action);
