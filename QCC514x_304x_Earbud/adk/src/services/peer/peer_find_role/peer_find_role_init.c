...
static void peer_find_role_GattConnect(gatt_cid_t cid);
static void peer_find_role_GattDisconnect(gatt_cid_t cid);
...
bool peerFindRole_EnableAdvertising(void)
{
...
{
    MessageCancelAll(PeerFindRoleGetTask(), PEER_FIND_ROLE_INTERNAL_TIMEOUT_DISCOVERY);
}
...
static void peer_find_role_notify_clients_immediately(peer_find_role_message_t role)
{
    ...
    DEBUG_LOG_FN_ENTRY("peer_find_role_notify_clients_immediately role= 0x%x History:%8X",
                        role, PeerFindRole_GetOutcomes());
    ...
    PeerFindRole_RecordOutcome(role);
...
void peer_find_role_prepare_for_role_selection(PEER_FIND_ROLE_STATE state_if_no_clients)
{
    ...
    /* The use of PEER_FIND_ROLE_PREPARE_FOR_ROLE_SELECTION can cause pairing to
       be stopped, which will update the user_pairing flag. Keep a copy for use
       in scoring */
    pfr->scoring_info.saved_user_pairing = pfr->scoring_info.user_pairing;

    if (peer_find_role_prepare_client_registered())
    {
        DEBUG_LOG("peer_find_role_prepare_for_role_selection. %d clients", TaskList_Size(&pfr->prepare_tasks));
        if (TaskList_Size(pfr->pending_prepare_tasks))
        {
            DEBUG_LOG_WARN("peer_find_role_prepare_for_role_selection. pending list non-zero, already in progress ?");
            TaskList_Destroy(pfr->pending_prepare_tasks);
        }
        pfr->pending_prepare_tasks = TaskList_Duplicate(&pfr->prepare_tasks);

        TaskList_MessageSendId(&pfr->prepare_tasks, PEER_FIND_ROLE_PREPARE_FOR_ROLE_SELECTION);
    }
    else
    {
        peer_find_role_set_state(state_if_no_clients);
    }
}
...
/*! Handler for a change in user pairing status

    This should be called when pairing activity occurs.

    \param activity Activity message from pairing module
*/
static void peer_find_role_handle_pairing_activity(const PAIRING_ACTIVITY_T *activity)
{
    peerFindRoleTaskData *pfr = PeerFindRoleGetTaskData();
    bool pairing_is_user_requested = FALSE;

    DEBUG_LOG_FN_ENTRY("peer_find_role_handle_pairing_activity. status enum:pairingActivityStatus:%d User:%d",
                        activity->status, activity->user_initiated);

    switch (activity->status)
    {
        case pairingActivityInProgress:
            pairing_is_user_requested = activity->user_initiated;
            break;

        case pairingActivityNotInProgress:
        case pairingActivitySuccess:
            /* These statuses indicate we are no longer pairing
               So definitely not user pairing */
            break;

        case pairingActivityCompleteVersionChanged:
        case pairingActivityLinkKeyReceived:
            /* These are for an interim status, ignore */
            return;

        default:
            DEBUG_LOG_WARN("peer_find_role_handle_pairing_activity. Unknown PAIRING_ACTIVITY status, ignoring. enum:pairingActivityStatus:%d",
                            activity->status);
            return;
    }

    pfr->scoring_info.user_pairing = pairing_is_user_requested;
}

static inline void peer_find_role_connect_disconnect_handler(void)
{
    peerFindRoleTaskData *pfr = PeerFindRoleGetTaskData();
    switch (peer_find_role_get_state())
    {
        ...
        case PEER_FIND_ROLE_STATE_CONNECTING_TO_DISCOVERED:
            ...
            /* The peer has disconnected before completing role selection.
               When re-entering discover state, the
               PEER_FIND_ROLE_INTERNAL_TIMEOUT_DISCOVERY message will be
               re-sent with the original timeout that was passed to
               PeerFindRole_FindRole(). This ensures the procedure doesn't
               remain in discover state indefinitely. A consequence of
               re-entering discover (as opposed for completing role selection
               and becoming acting primary) is that in very marginal link
               conditions, the role selection may cycle through:
               1. discovery
               2. connection
               3. loss of connection
               4. rediscovery
               before one earbud times out and becomes acting primary. This
               behavior is considered preferable to both earbuds becoming
               primary. */
            peer_find_role_set_state(PEER_FIND_ROLE_STATE_DISCOVER);
            break;
...
static void peer_find_role_handle_connection_ind(const CON_MANAGER_CONNECTION_IND_T *ind)
{
    ...
    {
        peer_find_role_connect_disconnect_handler();
    }
}
...
{
    DEBUG_LOG("peer_find_role_handle_ble_security enum:ble_security_status:%d", cfm->status);
    ...
    switch (cfm->status)
    {
        ...
        case ble_security_pairing_in_progress:
            /* ignore - the request is still active message will be received again with the result */
            break;
        default:
            break;
    }
...
/*! Handler for the timeout message #PEER_FIND_ROLE_INTERNAL_TIMEOUT_DISCOVERY
...
/*! Handler for the timeout message #PEER_FIND_ROLE_INTERNAL_TIMEOUT_PEER_CONNECTION

    This timeout message is received when connection attempt to peer takes too
    long.
 */
static void peer_find_role_handle_connection_timeout(void)
{
    if (PEER_FIND_ROLE_STATE_CONNECTING_TO_DISCOVERED == peer_find_role_get_state())
    {
        DEBUG_LOG("peer_find_role_handle_connection_timeout");

        /* ConManagerReleaseTpAcl is called when exiting current state */
        peer_find_role_set_state(PEER_FIND_ROLE_STATE_DISCOVER);
    }
    else
    {
        DEBUG_LOG("peer_find_role_handle_connection_timeout. Expired after already left state");
    }
}
...
static void peerFindRole_HandleInternalAttemptAdvertising(void)
{
    DEBUG_LOG("peerFindRole_HandleInternalAttemptAdvertising");
    ...
    if (peer_find_role_is_in_advertising_state())
    {
        ...
        PanicFalse(peerFindRole_EnableAdvertising());
    }
}
...
static void peer_find_role_handler(Task task, MessageId id, Message message)
{
    ...
    if (handle_as_active)
    {
        switch (id)
        {
            ...
            case PEER_FIND_ROLE_INTERNAL_TIMEOUT_DISCOVERY:
            ...
            case PEER_FIND_ROLE_INTERNAL_TIMEOUT_PEER_CONNECTION:
                peer_find_role_handle_connection_timeout();
                break;
            ...
            case PEER_FIND_ROLE_INTERNAL_ATTEMPT_ADVERTISING:
                peerFindRole_HandleInternalAttemptAdvertising();
                break;

            default:
                /* We didn't handle the message. Clear the flag so drop
    ...
    /* We only get here with handle_as_active set if the message *was*
       handled above. */
    if (handle_as_active)
    {
        return;
    ...
    switch (id)
    {
        ...
        /* ---- Advertising Manager messages ---- */
        /* Release message could potentially be delayed until Initialised state. */
        ...
        case PAIRING_ACTIVITY:
            peer_find_role_handle_pairing_activity((const PAIRING_ACTIVITY_T *)message);
            break;
...
static inline void peer_find_role_init(void)
{
    peer_find_role.outcomes = 0xFFFFFFFFul;
    ...
    HfpProfile_RegisterStatusClient(PeerFindRoleGetTask());
