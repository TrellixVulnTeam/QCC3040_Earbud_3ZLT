...
from kats.core.stream_base import StreamBase, STREAM_TYPE_SOURCE, STREAM_TYPE_SINK, STREAM_NAME, \
...
class KalsimStream(StreamBase, metaclass=ABCMeta):
    ...
    def __init__(self, stream_type, *_, **kwargs):
        ...
        self._log_param = {} if not hasattr(self, '_log_param') else self._log_param
    ...
    def config(self, **kwargs):
        ...
        # This is only supported in kalsim 21f/2018a and after
    ...
    def stop(self):
        ...
        if self.__parameters[STREAM_FLOW_CONTROL_DRIVE] == STREAM_FLOW_CONTROL_DRIVE_KALSIM:
            ...
            if self._stream_type == STREAM_TYPE_SINK:
                # This is only supported in kalsim 21d/2018a and after
    ...
    def extract(self, samples, sign_extend=None):
        """Extracts data from the stream

        Args:
            samples (int): Number of samples to extract
            sign_extend (bool): Do we want to sign extend the data elements, None for autodetect

        Returns:
            data (list[int]): Data extracted
        """
        sign_extend = self.get_data_width() != 8 if sign_extend is None else sign_extend
