/*!
\copyright  Copyright (c) 2019-20 Qualcomm Technologies International, Ltd.\n
            All Rights Reserved.\n
...
#define IS_DATA_NOTIFICATION_INDICATION_ENABLED(conex)   ((conex.client_config.data) & ENABLE_NOTIFICATION_INDICATION)

#define FAST_PAIR_CHARACTERISTIC_BASE  (0x1234)
/*! This enum defines the common enums for FP Characteristics to be used between GATT FP Server interface and State Machine */
typedef enum 
{
    FAST_PAIR_KEY_BASED_PAIRING = FAST_PAIR_CHARACTERISTIC_BASE,
    FAST_PAIR_PASSKEY,
...
/*! Structure holding the information of GATT FP Read Only characteristics */
typedef struct __gatt_fp_read_only_char_t
{
    uint32 model_id;
    const char *fw_revision_string;
}gatt_fp_read_only_char_value_t;
...
typedef struct __gatt_fp_data_t
{
    ...
    /*! Read only characteristic being used by application */
    gatt_fp_read_only_char_value_t char_value;
}gatt_fp_data_t;
...
/*! @brief Send Additional data Notification to GATT Fast Pair Service library.

    \param      encr_additional_data_pname         Payload. Encrypted Additional data which has to be sent as a Notification
    \param      encr_additional_data_pname_len     Payload length. Encrypted Additional data length
*/
