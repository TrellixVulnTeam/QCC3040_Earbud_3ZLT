...
void MirrorProfile_HandleClSdpServiceSearchAttributeCfm(const CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM_T *cfm)
{
    ...
    switch (mirror_inst->audio_sync.l2cap_state)
    {
        ...
        default:
        {
            DEBUG_LOG("MirrorProfile_HandleClSdpServiceSearchAttributeCfm, unexpected sync state 0x%x mirror state 0x%x", mirror_inst->audio_sync.l2cap_state, MirrorProfile_GetState());
            Panic();
        }
        break;
    }
