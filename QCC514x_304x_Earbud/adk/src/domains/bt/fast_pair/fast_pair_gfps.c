...
void fastPair_SendFPAdditionalDataNotification(uint8 * encr_additional_data_pname,uint8 encr_additional_data_pname_len)
{
     DEBUG_LOG("fastPair_SendFPAdditionalDataNotification. ");
     /*! Check to ensure if notifications are enabled */
     if(IS_DATA_NOTIFICATION_INDICATION_ENABLED(FAST_PAIR_GATT_SERVER))
     {
         /*! Send Additional data Notification */
         GattFastPairServerAdditionalDataNotification(&FAST_PAIR_GATT_SERVER.gfps, FAST_PAIR_GATT_SERVER.cid,
                                                      encr_additional_data_pname,encr_additional_data_pname_len);
         DEBUG_LOG("fastPair_SendFPAdditionalDataNotification. Additional Data notification sent successfully");
     }
}
...
void fastPair_GattFPServerMsgHandler(Task task, MessageId id, Message message)
{
    ...
    switch(id)
    {
        ...
        case GATT_FAST_PAIR_SERVER_READ_MODEL_ID_IND:
        {
            GATT_FAST_PAIR_SERVER_READ_MODEL_ID_IND_T *model_id_value =
                (GATT_FAST_PAIR_SERVER_READ_MODEL_ID_IND_T *)message;

            DEBUG_LOG("fastPair_GattFPServerMsgHandler. Read Model ID being used for FP procedure");

            FAST_PAIR_GATT_SERVER.char_value.model_id = fastPair_GetModelId();

            GattFastPairServerReadModelIdResponse(
                    model_id_value->fast_pair_server,
                    model_id_value->cid,
                    FAST_PAIR_GATT_SERVER.char_value.model_id
                    );
        }
        break;

        case GATT_FAST_PAIR_SERVER_READ_FIRMWARE_REVISION_IND:
        {
            GATT_FAST_PAIR_SERVER_READ_FIRMWARE_REVISION_IND_T *firmware_revision = 
                (GATT_FAST_PAIR_SERVER_READ_FIRMWARE_REVISION_IND_T *)message;

            DEBUG_LOG("fastPair_GattFPServerMsgHandler. Read firmware revision");

            FAST_PAIR_GATT_SERVER.char_value.fw_revision_string = DeviceInfo_GetSoftwareVersion();

            GattFastPairServerReadFirmwareRevisionResponse(
                firmware_revision->fast_pair_server,
                firmware_revision->cid,
                FAST_PAIR_GATT_SERVER.char_value.fw_revision_string
                );
        }
        break;
        ...
        case GATT_FAST_PAIR_SERVER_WRITE_DATA_CONFIG_IND:
        {
            GATT_FAST_PAIR_SERVER_WRITE_DATA_CONFIG_IND_T *data_config_write =
                (GATT_FAST_PAIR_SERVER_WRITE_DATA_CONFIG_IND_T *)message;

            DEBUG_LOG("fastPair_GattFPServerMsgHandler. Write data Client Config Descriptor 0x%04x cid %d",
                (int)data_config_write->config_value, data_config_write->cid
                 );

            /*! Only last notification or indication written by client will be used */
            FAST_PAIR_GATT_SERVER.client_config.data = (int)data_config_write->config_value;
            GattFastPairServerWriteDataConfigResponse(
                    data_config_write->fast_pair_server,
                    data_config_write->cid,
                    gatt_status_success
                    );
            }
            break;

        case GATT_FAST_PAIR_SERVER_WRITE_DATA_IND:
        {
            GATT_FAST_PAIR_SERVER_WRITE_DATA_IND_T *data_write =
                (GATT_FAST_PAIR_SERVER_WRITE_DATA_IND_T *)message;

            /*! check the cid, if it matches then write name
                if not, then send the failure response to library */
            if(data_write->cid == FAST_PAIR_GATT_SERVER.cid)
            {
                DEBUG_LOG("fastPair_GattFPServerMsgHandler. Write Data Pairing with len %d",data_write->size_value);

                /* Store Data (Name) to persistent storage */
                fastPair_AdditionalDataWrite(data_write->value, data_write->size_value);

                GattFastPairServerWriteDataResponse(
                        data_write->fast_pair_server,
                        data_write->cid,
                        gatt_status_success
                        );
            }
            ...
            {
                DEBUG_LOG("fastPair_GattFPServerMsgHandler. Invalid cid for Data write ");
                GattFastPairServerWriteDataResponse(
                        data_write->fast_pair_server,
                        data_write->cid,
                        gatt_status_invalid_cid
        ...
        case GATT_FAST_PAIR_SERVER_DATA_NOTIFICATION_CFM:
        {
            DEBUG_LOG(
                    "fastPair_GattFPServerMsgHandler. Data Notify Cfm"
                    );
        }
        break;
        ...
        {
            /* Un-handled messages */
            DEBUG_LOG("fastPair_GattFPServerMsgHandler. GATT Fast Pair Unhandled msg MESSAGE:0x%x",
                      id);
        }
        break;
