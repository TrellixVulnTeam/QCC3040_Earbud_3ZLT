...
/* Check that replacing unsigned reserved with a SRC_SYNC_IN_OUT_BUFFER_SIZES
 * did not change the size of the operator data
 */
COMPILE_TIME_ASSERT(sizeof(unsigned)==sizeof(SRC_SYNC_IN_OUT_BUFFER_SIZES),
                    SRC_SYNC_IN_OUT_BUFFER_SIZES_is_one_word);
...
const opmsg_handler_lookup_table_entry src_sync_opmsg_handler_table[] =
{
    ...
    {OPMSG_COMMON_ID_SET_TERMINAL_BUFFER_SIZE,  src_sync_opmsg_set_terminal_buffer_size},
    {OPMSG_COMMON_SET_BACK_KICK_THRESHOLD,      src_sync_opmsg_set_back_kick_threshold},
    ...
    {OPMSG_SRC_SYNC_ID_SET_STALL_NOTIFICATION_ENABLE, src_sync_opmsg_set_stall_notification_enable},
...
bool src_sync_opmsg_set_buffer_size(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    /* If a negative value is given, store 0 in the input_buffer_size
     * and output_buffer_size to disable the separate input and output
     * buffer sizes.
     */
    uint16 in_out_buffer_size = (uint16)pl_max(0, op_extra_data->buffer_size);

    /* Write the value to output and input buffer size
     * half-word fields.
     */
    uint16* terminal_buffer_size = op_extra_data->terminal_buffer_sizes.terminal_buffer_sizes;
    *terminal_buffer_size++ = in_out_buffer_size;
    *terminal_buffer_size = in_out_buffer_size;
 ...
 * The operator accepts it at any time.
 */
bool src_sync_opmsg_set_terminal_buffer_size(
        OPERATOR_DATA *op_data, void *message_data,
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    SRC_SYNC_OP_DATA *op_extra_data =
            (SRC_SYNC_OP_DATA*)base_op_get_instance_data(op_data);

    uint16 buffer_size =
        OPMSG_FIELD_GET(message_data, OPMSG_COMMON_SET_TERMINAL_BUFFER_SIZE,
                        BUFFER_SIZE);

    uint16* terminal_buffer_sizes =
            op_extra_data->terminal_buffer_sizes.terminal_buffer_sizes;

    if (OPMSG_FIELD_GET(message_data, OPMSG_COMMON_SET_TERMINAL_BUFFER_SIZE,
                        SOURCES) != 0)
    {
        terminal_buffer_sizes[SRC_SYNC_OUTPUT_BUFFER_SIZE_INDEX] = buffer_size;
    }
    if (OPMSG_FIELD_GET(message_data, OPMSG_COMMON_SET_TERMINAL_BUFFER_SIZE,
                        SINKS) != 0)
    {
        terminal_buffer_sizes[SRC_SYNC_INPUT_BUFFER_SIZE_INDEX] = buffer_size;
    }
...
/**
 * Process the standard OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD.
 * The operator accepts it at any time.
 */
bool src_sync_opmsg_set_back_kick_threshold(
        OPERATOR_DATA *op_data, void *message_data,
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    SRC_SYNC_OP_DATA *op_extra_data =
    ...
    op_extra_data->back_kick_threshold =
            (int16)
            OPMSG_FIELD_GET(message_data,
                            OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD,
                            THRESHOLD);

    op_extra_data->back_kick_mode = (OPMSG_COMMON_BACK_KICK_MODE)
            OPMSG_FIELD_GET(message_data,
                            OPMSG_COMMON_MSG_SET_BACK_KICK_THRESHOLD,
                            MODE);
...
/**
 * The message is OPMSG_SOURCE_SYNC_SET_STALL_NOTIFICATION_ENABLE.
 * The operator accepts it at any time.
 */
bool src_sync_opmsg_set_stall_notification_enable(
        OPERATOR_DATA *op_data, void *message_data,
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    SRC_SYNC_OP_DATA *op_extra_data =
    ...
    op_extra_data->enable_sink_state_notification =
            (uint8)
            OPMSG_FIELD_GET(message_data,
        ...
        OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    switch(msg->key)
    {
        ...
        case OPMSG_OP_TERMINAL_SHIFT:
            /* value is cbops param 'shift', possibly used in
