...
void hdma_ShiftQueueBaseTimestamp(queue_t* queue)
{
	...
	HDMA_DEBUG_LOG("hdma_ShiftQueueBaseTimestamp: shifting timestamp");
...
void Hdma_QueueInsert(queue_t* queue,uint16 val, uint32 timestamp)
{
	uint32 tempstamp;
	
    if(Hdma_IsQueueEmpty(queue))
    {
    	DEBUG_LOG_ERROR("Queue is Empty");
        queue->rear = 0;
	...
	tempstamp = timestamp - queue->base_time;
	if(tempstamp >= 65535)
	{
		queue->size = 0;
		queue->rear = 0;
	...
	{
		queue->quality[queue->rear].timestamp = tempstamp;
	}
	queue->quality[queue->rear].data = val;
	HDMA_DEBUG_LOG("Hdma_QueueInsert: relative_time [%u] base_time [%u] data [%d]", queue->quality[queue->rear].timestamp, queue->base_time, (int16)val);
...
uint8 Hdma_QueueDelete(queue_t *queue, uint32 *timestamp)
{
    ...
    if (Hdma_IsQueueEmpty(queue))
    {
        DEBUG_LOG_ERROR("Queue Underflow \n");
        *timestamp = 0;
        return val;
    }
    else
