...
class HydraScoProcessingService:
    """Hydra SCO Processing Service
    ...
    def __init__(self, **kwargs):
        ...
        self.__config.role = kwargs.pop(ROLE)
        self.__config.air_compression_factor = kwargs.pop(AIR_COMPRESSION_FACTOR)
        self.__config.air_buffer_size = kwargs.pop(AIR_BUFFER_SIZE)
        self.__config.air_packet_length = kwargs.pop(AIR_PACKET_LENGTH)
        self.__config.tesco = kwargs.pop(TESCO)
        self.__config.wesco = kwargs.pop(WESCO)
        self.__config.slot_occupancy = kwargs.pop(SLOT_OCCUPANCY)
        ...
        if self.__config.role == ROLE_MASTER:
            self.__config.to_air_latency = 500
            self.__config.from_air_latency = (self.__config.wesco + 2) * \
        ...
        self.__data.channel_frame_length = [0, 0]  # default frame length
    ...
    def _message_received(self, msg):
        ...
        if len(msg) == 4 and \
            ...
            if direction == SCO_DIR_TO_AIR:
                ...
                self.__data.channel_frame_length[0] = msg[3]
                self._log.info('sco service to air ready frame_length:%s',
                               self.__data.channel_frame_length[0])
            elif direction == SCO_DIR_FROM_AIR:
                self.__data.channel_ready[1] = True
                self.__data.channel_frame_length[1] = msg[3]
                self._log.info('sco service from air ready frame_length:%s',
                               self.__data.channel_frame_length[1])
    ...
    def config(self):
        """Configure SCO processing service"""
    ...
    def get_from_air_read_handle(self):
        """Get SCO processing service from air read bac handle
    ...
    def get_from_air_write_handle(self):
        """Get SCO processing service from air write bac handle
    ...
    def get_to_air_read_handle(self):
        """Get SCO processing service to air read bac handle
    ...
    def get_to_air_write_handle(self):
        """Get SCO processing service to air read bac handle
    ...
    def get_frame_length(self, channel):
        """Request channel frame length
        ...
        return self.__data.channel_frame_length[channel]
    ...
    def start_channel(self, channel, callback):
        ...
        _ = self.__helper.uut.timer_add_relative(
