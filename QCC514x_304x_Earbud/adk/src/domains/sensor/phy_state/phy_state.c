...
LOGGING_PRESERVE_MESSAGE_TYPE(PHY_STATE_MSG)
LOGGING_PRESERVE_MESSAGE_TYPE(PHY_STATE_INTERNAL_MSG)

/*! There is checking that the messages assigned by this module do
not overrun into the next module's message ID allocation */
ASSERT_MESSAGE_GROUP_NOT_OVERFLOWED(PHY_STATE, PHY_STATE_MESSAGE_END)
ASSERT_INTERNAL_MESSAGES_NOT_OVERFLOWED(PHY_STATE_INTERNAL_MESSAGE_END)
 ...
 */
static void appPhyStateMsgSendStateChangedInd(phyState state, phy_state_event event, uint32 transition_delay)
{
...
static void appPhyStateEnterInCase(void)
{
    ...
    appPhyStateMsgSendStateChangedInd(PHY_STATE_IN_CASE, phy_state_event_in_case, appPhyStateInCaseNotificationsLimitMs());
}
...
static void appPhyStateEnterInEar(void)
{
    ...
    appPhyStateMsgSendStateChangedInd(PHY_STATE_IN_EAR, phy_state_event_in_ear, appPhyStateInEarNotificationsLimitMs());
}
...
static void appPhyStateEnterOutOfEarAtRest(void)
{
    ...
    appPhyStateMsgSendStateChangedInd(PHY_STATE_OUT_OF_EAR_AT_REST, phy_state_event_not_in_motion, appPhyStateOutOfEarNotificationsLimitMs());
}
...
static void appPhyStateExitInCase(void)
{
    ...
    appPhyStateMsgSendStateChangedInd(PHY_STATE_OUT_OF_EAR, phy_state_event_out_of_case, appPhyStateOutOfCaseNotificationsLimitMs());
}
...
static void appPhyStateExitInEar(void)
{
    ...
    appPhyStateMsgSendStateChangedInd(PHY_STATE_OUT_OF_EAR, phy_state_event_out_of_ear, appPhyStateOutOfEarNotificationsLimitMs());
}

/*! \brief Perform actions on exiting PHY_STATE_OUT_OF_EAR_AT_REST state. */
static void appPhyStateExitOutOfEarAtRest(void)
{
    appPhyStateMsgSendStateChangedInd(PHY_STATE_OUT_OF_EAR, phy_state_event_in_motion, appPhyStateInMotionNotificationsLimitMs());
}
...
static void appPhyStateHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case PROXIMITY_MESSAGE_NOT_IN_PROXIMITY:
            ...
            break;
        default:
            DEBUG_LOG("Unknown message received, MESSAGE:PHY_STATE_INTERNAL_MSG:0x%x", id);
            break;
    }
}
...
#ifdef INCLUDE_HALL_EFFECT_SENSOR
    HallEffectSensor_ClientRegister(&phy_state->task);
#else
...
will be received. This means the in-case state can never be entered */
    ...
    (void)Charger_ClientRegister(&phy_state->task);
#else
...
void appPhyStatePrepareToEnterDormant(void)
{
    ...
    TouchSensorClientUnRegister(&phy_state->task);
#ifdef INCLUDE_HALL_EFFECT_SENSOR
    HallEffectSensor_ClientUnRegister(&phy_state->task);
#endif
}
...
{
    phyStateTaskData* phy_state = PhyStateGetTaskData();
    return phy_state->reported_state;
}
