...
try:
    from vm_build.builder import nvscmd as nvs
    from vm_build.builder import deploy_single_image
...
try:
    import kalsim as sim
except:
    try:
        from vm_build.builder import sim_launch as sim
    except:
...
WORKSPACE_ROOT = os.path.abspath(os.path.join(HERE, ROOT_REL_PATH))
...
def deploy_built_project(proj_parser, dk_parser, parsed_args, build_output_folder=None):
    ...
    if build_output_folder is None:
        props = proj_parser.get_properties_from_config(parsed_args.configuration)
        per_config_depend = 'depend_{}_{}'.format(parsed_args.configuration, props.get('CHIP_TYPE', 'none'))
    ...
    image_path = os.path.join(proj_parser.proj_dirname, build_output_folder, image_file)
    ...
    if config_keys["SUBSYSTEM_NAME"] == "audio" and "KALSIM_MODE" in config_keys:
        if config_keys["KALSIM_MODE"] == "true":
            ...
            if audio_elf_path != fw_path and os.path.isfile(fw_path):
                ...
                audio_elf_path = ""
            keys = {key[len(KALSIM_OPT):].lower():value for (key,value) in config_keys.items() if key.startswith(KALSIM_OPT)}
            keys['fw_path'] = fw_path
            keys['patch_path'] = patch_path
            keys['bundle_path'] = audio_elf_path
            launcher = os.path.join(parsed_args.devkit_root, 'tools', 'bin', 'run_py3.bat')
            return sim.launch_kse(launcher, WORKSPACE_ROOT, **keys)
