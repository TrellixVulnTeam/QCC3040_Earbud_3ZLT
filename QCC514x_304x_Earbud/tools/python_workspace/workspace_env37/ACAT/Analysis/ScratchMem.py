...
class ScratchMem(Analysis.Analysis):
    ...
    def analyse_per_priority(self):
        ...
        for current in range(0, num_entries):
            dm1_size.append(
                pdd[current]['alloc_info_dm1']
                ['alloc_size'].value
            )
            dm2_size.append(
                pdd[current]['alloc_info_dm2']
                ['alloc_size'].value
            )
            none_size.append(
                pdd[current]['alloc_info_none']
                ['alloc_size'].value
            )
            users.append(pdd[current]['refcount'].value)
    ...
    def analyse_per_task(self, task_id=None):
        ...
        """
        per_task = self.chipdata.get_var_strict('$_first_scratch_mem')
        matching_id = False
        for sc_table in ch.parse_linked_list(per_task, 'next'):
