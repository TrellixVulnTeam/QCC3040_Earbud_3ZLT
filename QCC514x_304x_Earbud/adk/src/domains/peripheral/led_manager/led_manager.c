...
static void ledManager_SetPio(uint16 led_num, uint16 led_state, bool pull_pio_high_on)
{
    uint16 led_pio = ledManager_GetPioNumForLed(led_num);
    uint32 pio_mask = PioCommonPioMask(led_pio);
 
    if(pull_pio_high_on)
    {
        PioSet32Bank(PioCommonPioBank(led_pio), pio_mask, (led_state & 0x1<<led_num) ? pio_mask : 0);
    }
    else
    {
        PioSet32Bank(PioCommonPioBank(led_pio), pio_mask, (led_state & 0x1<<led_num) ? 0 : pio_mask);
    }
}
...
static void ledManager_Update(void)
{
    ...
    if (!led_mgr.force_enabled)
    {
        led_state = led_mgr.led_state;
    ...
    {
        led_state = led_mgr.led_forced_pattern;
    }
...
#ifdef HAVE_RDP_HW_18689
            case 3: ledManager_SetPio(2, led_state, TRUE);
#else
            case 3: ledManager_SetPio(2, led_state, FALSE);
#endif
            case 2: ledManager_SetPio(1, led_state, FALSE);
            case 1: ledManager_SetPio(0, led_state, FALSE);
            default: break;
        }
    }
    else
...
/*! \brief Handle internal message to force LEDs on or off

    If stopping force mode, drive the LEDs to Off first and only then
    re-establish any existing pattern

    \param force Message with parameters for the force mode
 */
static void ledManager_HandleForce(const LED_INTERNAL_FORCE_T *force)
{
    DEBUG_LOG_ALWAYS("ledManager_HandleForce Mask:0x%x Force:%d",
                        force->mask, force->enable);

    if (force->enable)
    {
        led_mgr.led_forced_pattern = force->mask;
        led_mgr.force_enabled = TRUE;

        ledManager_Update();
    }
    else
    {
        led_mgr.led_forced_pattern = 0;
        ledManager_Update();
    
        led_mgr.force_enabled = FALSE;
        if (led_mgr.enable)
        {
            ledManager_Update();
        }
...
static void ledManager_Handler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case LED_INTERNAL_FORCE:
            ledManager_HandleForce((const LED_INTERNAL_FORCE_T *)message);
            break;
...
{
    unsigned bank = PioCommonPioBank(led_pio);
    unsigned mask = PioCommonPioMask(led_pio);

    PioSetMapPins32Bank(bank, mask, mask);
    PioSet32Bank(bank, mask, mask); /* Set LEDs off before enabling them;
        Since the LEDs are open drain, setting the IO high ensures there
        is no voltage drop. */
...
{
    MAKE_LED_MESSAGE(LED_INTERNAL_FORCE);

    message->enable = TRUE;
    message->mask = led_mask;

    MessageSend(&led_mgr.task, LED_INTERNAL_FORCE, message);
}
