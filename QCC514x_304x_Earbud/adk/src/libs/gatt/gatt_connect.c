...
{
    /* Different handling for BLE or BR/EDR. NOTE: the CID is 0 on a BR/EDR connection
     * until the CFM message.
     */
    if ( L2CA_CONFLAG_IS_LE(flags) )
    {
        /* It is possible the connection has been disconnected before the
            call to GattConnectResponse(), so ignore if CID invalid */
        if (gattFindConn(cid) == NULL)
        {
            GATT_DEBUG_INFO(("GattConnectResponse: cid null, already disconnected?\n"));
            return;
        }
