...
/**
 * Schedule panic() after delay (usec)
 * 
 * Protest volubly and die after scheduled delay
 *
 * NOTE: 
 * 1) Unlike panic() function, this function returns the control
 * back to the caller. 
 * 2) The panic() will be called at or after "delay" has passed; 
 * the actual delay will depend on the timing behaviour of the
 * scheduler.
 */
extern void panic_later(panicid deathbead_confession, int32 delay);

#define panic_later(deathbead_confession, delay) \
        panic_diatribe_later(deathbead_confession, (DIATRIBE_TYPE)(0), delay)

/**
 * Schedule panic_diatribe() after delay (usec)
 * 
 * Protest volubly and die after scheduled delay
 *
 * Like panic_later(), but stores an extra argument in preserved memory --
 * this can be used to pinpoint the reason for a panic when debugging.
 * 
 * NOTE: 
 * 1) Unlike panic_diatribe(), this function returns the control
 * back to the caller. 
 * 2) The panic_diatribe() will be called at or after "delay" has 
 * passed; the actual delay will depend on the timing behaviour of the
 * scheduler.
 */
extern void panic_diatribe_later(panicid deathbead_confession, 
