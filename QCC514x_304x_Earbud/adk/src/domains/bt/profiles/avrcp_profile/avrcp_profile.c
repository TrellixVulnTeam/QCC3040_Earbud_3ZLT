                                                     ...
                                                     bool from_repeat_message)
{
    switch (appAvrcpGetState(theInst))
    {
        case AVRCP_STATE_CONNECTED:
        {
            ...
            DEBUG_LOG("appAvrcpHandleInternalAvrcpRemoteRequest(%p) enum:avc_operation_id:%d state=%d", (void *)theInst, req->op_id, req->state);
...
{
    DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectRequest(%p) %x,%x,%lx, clients %u, locks %u state enum:avAvrcpState:0%d",
                 (void *)theInst,
...
{
    DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectIndication(%p) clients %d state enum:avAvrcpState:%d",
                 (void *)theInst,
                 TaskList_Size(theInst->avrcp.client_list),
                 appAvrcpGetState(theInst));
                                              ...
                                              const AVRCP_CONNECT_CFM_T *cfm)
{
    ...
    DEBUG_LOG("appAvrcpHandleAvrcpConnectConfirm(%p) status %d reject_expected:%d",
...
*/
static void avrcpProfile_DoAvrcpOperation(AVRCP *avrcp, bool pressed, bool (*avrcpOp)(bool))
{
    if (avrcpOp == NULL || avrcpOp(pressed))
    {
       AvrcpPassthroughResponse(avrcp, avctp_response_accepted);
    }
    else
    {
       AvrcpPassthroughResponse(avrcp, avctp_response_rejected);
    }
}
        ...
        case AVRCP_STATE_CONNECTED:
        {
            ...
            switch (ind->opid)
            {
                ...
                case opid_volume_down:
                    ...
                    break;
                case opid_play:
                    avrcpProfile_DoAvrcpOperation(theInst->avrcp.avrcp, !ind->state, theInst->av_callbacks->OnAvrcpPlay);
                    break;
                case opid_pause:
                    avrcpProfile_DoAvrcpOperation(theInst->avrcp.avrcp, !ind->state, theInst->av_callbacks->OnAvrcpPause);
                    break;
                case opid_forward:
                    avrcpProfile_DoAvrcpOperation(theInst->avrcp.avrcp, !ind->state, theInst->av_callbacks->OnAvrcpForward);
                    break;
                case opid_backward:
                    avrcpProfile_DoAvrcpOperation(theInst->avrcp.avrcp, !ind->state, theInst->av_callbacks->OnAvrcpBackward);
                    break;
...
static void appAvrcpHandleRegisterNotificationInd(avInstanceTaskData *theInst, AVRCP_REGISTER_NOTIFICATION_IND_T *ind)
{
    ...
    switch (ind->event_id)
    {
        case avrcp_event_volume_changed:
            DEBUG_LOG("appAvrcpHandleRegisterNotificationInd(%p), volume %u", (void *)theInst, theInst->volume);
            AvrcpEventVolumeChangedResponse(ind->avrcp, response, theInst->volume);
            break;
        ...
        case avrcp_event_track_changed:
            ...
            AvrcpMetadata_HandleEventTrackChanged(ind, response);
            break;

        case avrcp_event_addressed_player_changed:
            DEBUG_LOG("appAvrcpHandleRegisterNotificationInd(%p), addressed_player_changed", (void *)theInst);
            AvrcpBrowsing_HandleEventAddressedPlayerChanged(ind, response);
            break;
