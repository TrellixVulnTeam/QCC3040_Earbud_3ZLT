...
from .constants import SERVICE_TYPE_AUDIO_DATA_FILE_MANAGER_SERVICE, \
    DEVICE_TYPE_FILE_MANAGER, AUDIO_DATA_SERVICE_AUX_MSG

MAX_BLOCK_SIZE = 128

AUDIO_DATA_SERVICE_FILE_MANAGER_ACK = 0x03

SAMPLE_WIDTH = 32


class HydraFileManagerService(HydraAudioDataService):
    """Hydra File Manager Service

    A service used to download and remove hydra files to and from Audio subsystem.
    """
    ...
    def __init__(self, stream, accmd, hydra_protocol, service_tag=1, **kwargs):
        ...
        super().__init__(
            hydra_protocol,
            SERVICE_TYPE_AUDIO_DATA_FILE_MANAGER_SERVICE,
            DEVICE_TYPE_FILE_MANAGER,
            service_tag,
            **kwargs)
    ...
    def download(self, filename, timeout=15, memory_type=1, auto_remove=0):
        """Download file using file manager

        Args:
            filename (str): Full path to file to download
            timeout (float): Timeout in seconds
            memory_type (int): Type of allocated memory
            auto_remove (int): Allow the operator to remove the file when it is no longer required

        Returns:
            int: file_id
        """
        file_id = self._accmd.file_allocation_request(
                      os.path.getsize(filename), memory_type, auto_remove)
        self._log.info('file_manager_id:%s', file_id)
        super().start(connect_endpoint=file_id)
        ...
        for _ in range(int(4 * timeout)):
            ...
            stream.destroy()
            super().stop()
            raise RuntimeError('timeout downloading file with file manager service')
        ...
        while True:
            ...
            try:
                ...
                if payload[0] != AUDIO_DATA_SERVICE_AUX_MSG:
                    raise RuntimeError(
                        'audio data service aux message invalid code:0x%04x' % (payload[0]))
                if payload[1] != self._service_tag:
                    raise RuntimeError(
                        'audio data service aux message invalid service tag:0x%04x' % (payload[1]))
                if payload[2] != AUDIO_DATA_SERVICE_FILE_MANAGER_ACK:
                    raise RuntimeError(
                        'audio data service aux message invalid parameter:0x%04x' % (payload[2]))
                break
            except RuntimeError:
                pass
    ...
    def remove(self, file_id):
        """Remove a previously allocated file

        Args:
            file_id (int): file identifier obtained in file allocation request
        """
        self._accmd.file_deallocation_request(file_id)
