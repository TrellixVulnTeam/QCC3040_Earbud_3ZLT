...
import shutil
from dfu.dfu_file_generator import dfu_file_generator
...
class UnrecognisedPartitionError(Exception):
    ''' No need to implement __str__ '''
    def __init__(self, partition):
        self.partition = partition
        msg = 'Unrecognised partition: {}.'.format(partition)
        super(UnrecognisedPartitionError, self).__init__(msg)
...
def process_apps1_project(build_runner, parsed_args, proj_parser, output, outpath, config, projName):
    """
    ...
    if parsed_args.build_output_folder is None:
        props = proj_parser.get_properties_from_config(config)
        per_config_depend = 'depend_{}_{}'.format(config, props.get('CHIP_TYPE', 'none'))
        build_output_folder = os.path.join(proj_parser.proj_dirname, per_config_depend)
    else:
        build_output_folder = os.path.join(parsed_args.build_output_folder, projName)

    elf_filename = output + ".elf"
    if build_runner.run_build(proj_parser.proj_fname, config) == True:
    ...
    source = os.path.abspath(os.path.join(build_output_folder, xuv_filename))
...
def process_project(build_runner, parsed_args, project, projects, x2p, outpath, flash_config):
    """
        ...
        if project_type is not None:
            if project_type not in projects.keys():
                ...
                if project_type == "apps0":
                    xuv_filename = process_apps0_project(parsed_args, project, proj_parser, outpath)
                elif project_type == "audio":
                    for i in range(len(flash_config["layout"])):
                        if flash_config["layout"][i][0] == "audio":
                            xuv_filename = process_audio_project(parsed_args, project, proj_parser, outpath)
                            break
                    else:
                        continue
                elif project_type == "curator_config":
                    xuv_filename = process_curator_config_project(build_runner, parsed_args, project, outpath, projName)
                elif project_type in ("firmware_config", "customer_ro", "user_ps"):
                    xuv_filename = process_apps_filesystem_project(build_runner, parsed_args, project, project_type, outpath, projName)
                    ...
                    if apps1_project_is_default(project, config):
                        xuv_filename = process_apps1_project(build_runner, parsed_args, proj_parser, output, outpath, config, projName)
                    else:
...
def add_layout(lines, file_type, xuv_filename, params, encrypted):
    ...
    for item, value in params.iteritems():
        if item == "id":
            # For "id" field, value is string so put it in quotes
            line = '            "{}": "{}",\n'.format(item, value)
            lines.append(line)
        else:
...
def prepare_dfu_file_generator_config(devkit_root, projects, dfg_filepath,
    ...
    for i in range(len(map)):
        ...
        if map[i] =="curator_fs":
            ...
            xuv_filename = None
            if "user_ps" in projects:
                xuv_filename = projects["user_ps"]
            lines = add_layout(lines, "rw_config", xuv_filename, section["rw_config"], None)
        elif map[i] == "ra_partition":
            lines = add_layout(lines, "ra_partition", None, section["ra_partition"], None)
        elif map[i] == "debug_partition":
            lines = add_layout(lines, "debug_partition", None, section["debug_partition"], None)
        elif map[i] == "rw_fs":
            ...
            raise UnrecognisedPartitionError(map[i])
...
def prepare_UpgradeFileGen_config(projects_selected, upd_filepath, output_path,
    ...
    if "user_ps" in projects_selected:
        line = '# Apps rw_config filesystem\n'
        lines.append(line)
        output_filepath = os.path.join(output_path,
            'user_ps_filesystem.xuv')
        line = 'partition 0 11 1 %s\n' % output_filepath
        lines.append(line)
...
def prepare_and_run_dfu_file_generator(devkit_root, projects, projects_selected,
    ...
    if dfu_file_generator(dfg_filepath).initiate():
        print('DFU file generation completed successfully')
        return True

    print('DFU file generation failed')
    return False
...
class show_partition_selection_ui(object):
    def __init__(self, root, x2p_files):
        ...
        """
        self.root = root
        self.user_ps = 'user_ps'
        self.projects = {k: v for k, v in x2p_files.iteritems() if k != self.user_ps}
        self.user_ps_x2p = x2p_files[self.user_ps]
        self.button_vars = {}
        self.all_button_var = None
        self.value = 0

        self._create_button_vars()
        self._create_widget()
        self.add_all_checkbox()
        self.add_project_checkboxes()
        self.add_user_ps_label()
        self.add_user_ps_checkbox()
    ...
    def _create_frame(self, level):
        """Create Tkinter Frame."""
        frame = Tkinter.Frame(level)
    ...
    def _create_widget(self):
        ...
        # Create a frame to contain checkboxes for all projects.
        self.projects_frame = self._create_frame(self.top_level)
        ...
        # Create a separate frame for selecting user_ps x2p files.
        self.user_ps_frame = self._create_frame(self.top_level)

        # Privide an area at the bottom of the widget with "Proceed"
        # and "Cancel" buttons
        self.button_frame = self._create_frame(self.top_level)

    def _create_popup_widget(self):
        """Create popup widget."""
         # Create the popup widget
        popup = Tkinter.Toplevel(self.top_level)        
        popup.grab_set()
        popup.focus_set()        
        popup.title('Confirm')
        ...
        Tkinter.Label(
            frame,
            text='Confirm your current image on device supports DFU of the PS store.',
        ).pack()

        Tkinter.Button(frame, text="OK", command=popup.destroy).pack()

    def _create_button_vars(self):
        """Create the necessary variables to control checkboxes.
        
        These will be bound to the CheckBoxes.
        """
        for project in self.projects.keys():
            self.button_vars[project] = Tkinter.IntVar()
        self.button_vars[self.user_ps] = Tkinter.IntVar()
        self.all_button_var = Tkinter.IntVar()

    def _create_checkbox(self, project, label, button_var, frame, user_ps=False):
        """Creates and returns a checkbox."""
        b = Tkinter.Checkbutton(
           frame, text=label, variable=button_var,
        )
        b.pack(anchor=Tkinter.W, expand=Tkinter.YES)
        if not user_ps:
            b.select()
            def cb_handler (event, self=self, project=project):
                return self.cb_handler(project)
            b.bind("<Button-1>", cb_handler)
        else:
            b.bind("<Button-1>", self.user_ps_cb_handler)
        return b
        
    def add_all_checkbox(self):
        """Add a checkbox for controlling all other non user_ps checkboxes."""
        self._create_checkbox('All', 'All', self.all_button_var, self.projects_frame)            
        
    def add_project_checkboxes(self):
        """ Add checkboxes for all non user_ps projects."""
        for proj, x2p in self.projects.iteritems():
            self._create_checkbox(
                proj, x2p, self.button_vars[proj], self.projects_frame,
            )
    
    def add_user_ps_label(self):
        """Ask if user wants user_ps config as well."""
        Tkinter.Label(
            self.user_ps_frame,
            text="Do you want to include user_ps x2p file as well?",
        ).pack()

    def add_user_ps_checkbox(self):
        """Add a checkbox to select user_ps config.
        
        This button is simpler - its not selected and has no callback
        handler.
        """
        self._create_checkbox(
            self.user_ps,
            self.user_ps_x2p,
            self.button_vars[self.user_ps],
            self.user_ps_frame,
            user_ps=True,
        )

    def cb_handler(self, project):
        """ Manage all other projects when all is clicked or unclicked."""
        if project == 'All':
            for proj, button_var in self.button_vars.iteritems():
                # Keep this project out of bulk selection.
                if proj == self.user_ps:
                    continue
                button_var.set(not self.all_button_var.get())
        else:
            # Some other button has been clicked.
            selected = []
            for proj in self.projects.keys():
                if proj != project and self.button_vars[proj].get():
                    selected.append(proj)
            # The button_var after the click hasn't been updated yet,
            # so account for that.
            if not self.button_vars[project].get():
                selected.append(project)
                
            self.all_button_var.set(len(selected) == len(self.projects))

    def user_ps_cb_handler(self, event):
        """Handle selection of ps_cfg.x2p."""
        if not self.button_vars[self.user_ps].get():
            # ps_cfg option has been clicked.
            
            # Manually set the button because the popup widget doesn't return.
            self.button_vars[self.user_ps].set(1)
...
class show_encryption_selection_ui(object):
    def __init__(self, root):
        ...
        self.button_frame.pack(fill=Tkinter.X, padx=10, pady=10)
...
def parse_args(args):
    ...
    # For example: 
    # -p "['firmware_config', 'customer_ro', 'app/p1', 'curator_config', 'apps0', 'user_ps']"
    # -p "['app/p1', 'apps0']"
