...
/**
 *  \brief Query parameters associated with a transform.
 *             This function allows customer applications to query transforms to
 *             obtain internal information.
 *         
 *  \param transform The transform to query.
 *  \param key Valid values depend on the transform.
 *  \param value Memory location to store result.
 *  \return Returns FALSE if the key was unrecognised.
 * 
 * \ingroup trapset_stream
 */
bool TransformQuery(Transform transform, vm_transform_query_key key, uint32 * value);
