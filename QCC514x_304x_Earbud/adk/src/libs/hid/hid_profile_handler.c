...
#define hidHandleUnexpected(hid, id) \
    HID_DEBUG(("hid profile handler %p - message type not yet handled MESSAGE:hid_internal_messages:0x%x, state %d\n", (void *)hid, id, hidGetState(hid)));
...
void hidLibProfileHandler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        default:
            /* Received an unknown message */
            HID_PRINT(("hid lib profile handler - message type not yet handled MESSAGE:hid_internal_messages:0x%x\n", id));
            break;
    }
}
