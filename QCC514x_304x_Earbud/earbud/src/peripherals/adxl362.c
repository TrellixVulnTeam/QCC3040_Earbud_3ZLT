...
/*! There is checking that the messages assigned by this module do
     not overrun into the next modules message ID allocation */
ASSERT_MESSAGE_GROUP_NOT_OVERFLOWED(ACCELEROMETER, ACCELEROMETER_MESSAGE_END)
...
static MessageId getMessage(unsigned masked_bits)
{
    return masked_bits ? ACCELEROMETER_MESSAGE_IN_MOTION : ACCELEROMETER_MESSAGE_NOT_IN_MOTION;
}
        ...
        case MESSAGE_PIO_CHANGED:
            {
                ...
                if (PioMonitorIsPioInMessage(mpc, accel->config->pios.interrupt, &pio_set))
                {
                    TaskList_MessageSendId(accel->clients, getMessage(pio_set));
                }
            }
            break;
...
bool appAccelerometerClientRegister(Task task)
{
    ...
    uint16 bank = PioCommonPioBank(accel->config->pios.interrupt);
    uint32 mask = PioCommonPioMask(accel->config->pios.interrupt);
