...
#define IO_DEFS_MODULE_BITSERIAL2
#endif
...
#endif
#if HAVE_NUMBER_OF_BITSERIALS > 2
    BITSERIAL_INSTANCE_2 = 0x02,
#endif
    BITSERIAL_INSTANCE_NULL = 0xff
}bitserial_instance;

/** Helper macro for converting bitserial indices to instances */
#define hal_bitserial_get_instance_from_index(index) ( \
            (index) >= HAVE_NUMBER_OF_BITSERIALS ? BITSERIAL_INSTANCE_NULL \
            : (bitserial_instance)(index) \
        )

/** Helper macro for checking an instance is within the supported range */
#define hal_bitserial_instance_is_valid(x) ((x) < HAVE_NUMBER_OF_BITSERIALS)

/** Helper macro for getting a CCP resource ID from an instance */
/* CCP_RESOURCE_ID_BITSERIALs are not sequential. */
#define hal_bitserial_instance_to_ccp(x) ( \
            (x) >= HAVE_NUMBER_OF_BITSERIALS ? CCP_RESOURCE_ID_INVALID \
            : (x) == 0 ? CCP_RESOURCE_ID_BITSERIAL0 \
            : (x) == 1 ? CCP_RESOURCE_ID_BITSERIAL1 \
            : CCP_RESOURCE_ID_BITSERIAL2 \
        )

/**
 * Type definition for bitserial events(interrupts).
 * The bitserial event types have the same values for all bitserials.
 */
...
/**
 * Type definition for act on nak bits
 */
typedef enum bitserial_act_on_nak_bits
{
    BITSERIAL_ACT_ON_NAK_LEGACY = BITSERIAL0_CONFIG3_ACT_ON_NAK_LEGACY,
    BITSERIAL_ACT_ON_NAK_STOP = BITSERIAL0_CONFIG3_ACT_ON_NAK_STOP,
    BITSERIAL_ACT_ON_NAK_RESTART = BITSERIAL0_CONFIG3_ACT_ON_NAK_RESTART
} bitserial_act_on_nak_bits;
...
/**
 * Handy macro for referring to all error case bitserial interrupts.
 */
#define BITSERIAL_EVENT_IS_FAIL(e) ( (e) & (BITSERIAL_EVENT_ERROR_IN_BUFFER_MASK | \
                                      BITSERIAL_EVENT_ERROR_IN_READ_MASK | \
                                      BITSERIAL_EVENT_SLAVE_START_STOP_BITS_ERROR_MASK | \
                                      BITSERIAL_EVENT_FIFO_UNDERFLOW_MASK | \
                                      BITSERIAL_EVENT_FIFO_OVERFLOW_MASK | \
...
/**
 * Return the handler of the software interrupt.
 */
...
/**
 * Abstraction for getting the bitserial act on nak value.
 * \param i Bitserial instance.
 * \return act on nak value.
 */
...
/**
 * Abstraction for getting the bitserial clock rate registers.
 * \param i Bitserial instance.
 * \param hi Location to return the HI register.
 * \param lo Location to return the LO register.
 */
...
/**
 * Abstraction for getting the bitserial clock sample offset registers.
 * \param i Bitserial instance.
 */
...
/**
 * Abstraction for getting the number of words sent
 * \param i Bitserial instance
 * \return number of words sent
 */
...
/**
 * Abstraction for getting the bitserial TX/RX length.
 * \param i Bitserial instance.
 * \return Size of data.
 */
...
/**
 * Abstraction for getting the bitserial TX/RX length for the second part of 
 * the COMBO sequence.
 * \param i Bitserial instance.
 * \return Size of data.
 */
...
#ifndef hal_set_reg_bitserial2_debug_sel
#define hal_set_reg_bitserial2_debug_sel(v) UNUSED(v)
#endif /* hal_set_reg_bitserial2_debug_sel */
...
#define hal_get_bitserial2_event() hal_get_bitserial_event_bitserial2_status()
