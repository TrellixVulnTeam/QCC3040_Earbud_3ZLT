...
LOGGING_PRESERVE_MESSAGE_TYPE(SppMessageId)
LOGGING_PRESERVE_MESSAGE_TYPE(SppInternalMessageId)
...
void sppSendConnectCfm(SPP *spp, MessageId id, spp_connect_status status)
{
    ...
    {
        SPP_DEBUG(("Invalid Message id MESSAGE:0x%x\n", id));
    }
...
void sppHandleUnexpectedMsg(sppState state, MessageId id)
{
    state = state;
    id = id;
    SPP_DEBUG(("Unhandled message MESSAGE:0x%x, in SPPC State %d\n", id, state));
}
