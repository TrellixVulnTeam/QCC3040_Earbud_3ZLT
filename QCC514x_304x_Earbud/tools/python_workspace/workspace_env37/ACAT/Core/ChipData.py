...
class ChipData(object):
    ...
    def get_var_strict(self, identifier, elf_id=None):
        ...
        var.members = []
        return ct.ChipdataVariable(self.debuginfo.inspect_var(var, elf_id), self)
    ...
    def cast(self, addr, type_def, deref_type=False, section="DM",
        ...
        if section == 'PM':
            ...
            if pm_region not in ('PMRAM', 'DM_AS_PM'):
                raise ValueError(
                    "Address " +
                    hex(addr) +
        ...
        # with the size we extracted from typeinfo.
            ...
            elif (dm_region not in ('DM1RAM', 'DM2RAM', 'DMSRAM', 'DM_AS_PM') and
        ...
        return ct.ChipdataVariable(
            self.debuginfo.inspect_var(var, type_elf_id), self
        )

    @method_logger(logger)
    def run_poll(self, seconds, identifier):
        """Measures the value of variable(s) over a given period of time.

        Args:
            seconds (int): Seconds to run the poll.
    ...
    def is_booted(self):
        ...
        The booted state is decided by the memory mapped STACK_POINTER value.

        Returns:
            bool: True if the processor was booted, False otherwise.
        """
        proc_stack_pointer = self.get_reg_strict("$_STACK_POINTER").value

        return proc_stack_pointer != 0
