...
"""
from ACAT.Analysis.HeapMemOld2 import HeapMem as NewHeapMem
...
class HeapMem(NewHeapMem):
    ...
    def _get_heap_config(self, processor_number, heap_number):
        ...
        heap_size = self.heap_info_list[processor_number][
            'heap%d' % (heap_number + 1)
        ]['heap_size'].value
        heap_start = self.heap_info_list[processor_number][
            'heap%d' % (heap_number + 1)
        ]['heap_start'].value
        heap_end = heap_start + heap_size - 1
