    ...
    def get_data_pm(self, address, length=0):
        ...
        try:
            if ('PMRAM' in mem_region or 'PMCACHE' in mem_region or
                    'SLT' in mem_region or 'DM_AS_PM' in mem_region):
                if length == 0:
        ...
        while last != "\0":
            ...
            if unpacked_string:
                if Arch.addr_per_word == 4:
                    # Two char per word
                    build_string += chr(word & 0x00FFFF)
                    if build_string[-1] == "\0":
                        break
                    last = chr((word & 0xFFFF0000) >> 16)
                    build_string += last
                else:
                ...
                if Arch.addr_per_word == 4:
                    ...
                    # Two chars per word
                    build_string += chr((word & 0xFF00) >> 8)
                    if build_string[-1] == "\0":
                        break
                    last = chr(word & 0x00FF)
                    build_string += last
