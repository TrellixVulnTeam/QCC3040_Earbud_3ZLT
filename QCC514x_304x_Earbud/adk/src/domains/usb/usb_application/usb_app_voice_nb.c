...
\brief      USB Voice application - enumerates HID consumer transport,
            HID datalink and USB Audio (for voice calls with only narrowband support) classes.
*/
...
#define NB_VOICE_SPKR_SUPPORTED_FREQUENCIES   1
#define NB_VOICE_MIC_SUPPORTED_FREQUENCIES    1

/** Default USB streaming interface descriptors for speaker */
static const uint8 streaming_intf_desc_voice_nb_spkr[] =
{
    ...
    UAC_FORMAT_DESC_SIZE(NB_VOICE_SPKR_SUPPORTED_FREQUENCIES),  /* bLength 8+((number of sampling frequencies)*3) */
    UAC_CS_DESC_INTERFACE,              /* bDescriptorType = CS_INTERFACE */
    ...
    NB_VOICE_SPKR_SUPPORTED_FREQUENCIES,/* bSampleFreqType = 1 discrete sampling frequencies */
...
static const uint8 streaming_intf_desc_voice_nb_mic[] =
{
    /* Class Specific AS interface descriptor */
    UAC_AS_IF_DESC_SIZE,                /* bLength */
    UAC_CS_DESC_INTERFACE,              /* bDescriptorType = CS_INTERFACE */
    ...
    /* Type 1 format type descriptor */
    UAC_FORMAT_DESC_SIZE(NB_VOICE_MIC_SUPPORTED_FREQUENCIES),/* bLength */
    UAC_CS_DESC_INTERFACE,              /* bDescriptorType = CS_INTERFACE */
    UAC_AS_DESC_FORMAT_TYPE,            /* bDescriptorSubType = FORMAT_TYPE */
    UAC_AS_DESC_FORMAT_TYPE_I,          /* bFormatType = FORMAT_TYPE_I */
    ...
    UAC1D_USB_AUDIO_SAMPLE_SIZE,        /* bSubframeSize = 2 bytes */
    UAC1D_USB_AUDIO_SAMPLE_SIZE * 8,    /* bBitsResolution */
    NB_VOICE_MIC_SUPPORTED_FREQUENCIES, /* bSampleFreqType = 1 discrete sampling freq */
    SAMPLE_RATE_8K & 0xff,              /* tSampleFreq = 8000 */
    (SAMPLE_RATE_8K >> 8) & 0xFF,
    (SAMPLE_RATE_8K >> 16) & 0xFF,
...
static const uac_streaming_config_t voice_nb_streaming_mic_desc = {
    streaming_intf_desc_voice_nb_mic,
    sizeof(streaming_intf_desc_voice_nb_mic)
};

static const uac_streaming_config_t voice_nb_streaming_spkr_desc = {
    streaming_intf_desc_voice_nb_spkr,
    sizeof(streaming_intf_desc_voice_nb_spkr)
};

static const usb_audio_interface_config_t voice_nb_interface_list[] =
{
    {
        .type = USB_AUDIO_DEVICE_TYPE_VOICE_MIC,
        .control_desc =   &uac1_voice_control_mic_desc,
        .streaming_desc = &voice_nb_streaming_mic_desc,
        .endpoint =       &uac1_voice_mic_endpoint
    },
    {
        .type = USB_AUDIO_DEVICE_TYPE_VOICE_SPEAKER,
        .control_desc =   &uac1_voice_control_spkr_desc,
        .streaming_desc = &voice_nb_streaming_spkr_desc,
        .endpoint =       &uac1_voice_spkr_endpoint
    }
};

static const usb_audio_interface_config_list_t voice_nb_interfaces =
{
    .intf = voice_nb_interface_list,
    .num_interfaces = ARRAY_DIM(voice_nb_interface_list)
};

static const usb_audio_config_params_t usb_voice_nb_config =
{
...
static const usb_class_interface_t usb_voice_nb_if =
{
    .cb = &UsbAudio_Callbacks,
    .config_data = (usb_class_interface_config_data_t)&usb_voice_nb_config
};
...
static void usbAppVoiceNb_ConfigDevice(usb_device_index_t dev_index)
{
    usb_result_t result;
...
static void usbAppVoiceNb_Create(usb_device_index_t dev_index)
{
    usb_result_t result;

    DEBUG_LOG_INFO("UsbAppVoiceNb: Create");
...
static void usbAppVoiceNb_Attach(usb_device_index_t dev_index)
{
    usb_result_t result;
...
static void usbAppVoiceNb_Detach(usb_device_index_t dev_index)
{
    usb_result_t result;
...
static void usbAppVoiceNb_Close(usb_device_index_t dev_index)
{
    UNUSED(dev_index);

    DEBUG_LOG_INFO("UsbAppVoiceNb: Close");

    /* nothing to clear */
}

const usb_app_interface_t usb_app_voice_nb =
{
    .Create = usbAppVoiceNb_Create,
    .Attach = usbAppVoiceNb_Attach,
    .Detach = usbAppVoiceNb_Detach,
    .Destroy = usbAppVoiceNb_Close
};
