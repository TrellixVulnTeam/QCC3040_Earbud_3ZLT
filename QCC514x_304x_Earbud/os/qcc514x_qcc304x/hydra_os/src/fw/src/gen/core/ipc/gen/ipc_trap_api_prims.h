...
typedef struct IPC_USB_CONNECT {
    IPC_HEADER header;
    bool connect;
} IPC_USB_CONNECT;

typedef struct IPC_USB_DESCRIPTORS_HASH {
    IPC_HEADER header;
    uint32 descriptors_bitmask;
} IPC_USB_DESCRIPTORS_HASH;
...
typedef struct IPC_TRANSFORM_QUERY {
    IPC_HEADER header;
    uint16 transform;
    vm_transform_query_key key;
    uint32 * value;
} IPC_TRANSFORM_QUERY;
...
typedef struct IPC_GENERATE_MASTER_CLOCK_OUTPUT {
    IPC_HEADER header;
    uint32 frequency;
} IPC_GENERATE_MASTER_CLOCK_OUTPUT;
...
#if TRAPSET_LE_AUDIO
typedef struct IPC_STREAM_ISO_SOURCE {
    IPC_HEADER header;
    uint16 handle;
} IPC_STREAM_ISO_SOURCE;

typedef struct IPC_STREAM_ISO_SINK {
    IPC_HEADER header;
    uint16 handle;
} IPC_STREAM_ISO_SINK;
...
typedef struct IPC_IMAGE_UPGRADE_STREAM_GET_SOURCE_FROM_OFFSET {
    IPC_HEADER header;
    uint16 qspi;
    image_section_id image_section;
    uint32 offset;
} IPC_IMAGE_UPGRADE_STREAM_GET_SOURCE_FROM_OFFSET;
...
typedef struct IPC_STREAM_CHARGER_COMMS_UART_SINK {
    IPC_HEADER header;
} IPC_STREAM_CHARGER_COMMS_UART_SINK;

typedef struct IPC_CHARGER_COMMS_UART_CONFIGURE {
    IPC_HEADER header;
    charger_comms_uart_config_key_t config_key;
    uint32 value;
} IPC_CHARGER_COMMS_UART_CONFIGURE;
...
typedef struct IPC_CHARGER_COMMS_UART_RESET_SEQ_NUM {
    IPC_HEADER header;
    charger_comms_uart_address address;
} IPC_CHARGER_COMMS_UART_RESET_SEQ_NUM;
...
typedef struct IPC_AUDIO_ANC_CONFIG {
    IPC_HEADER header;
    audio_anc_instance anc_instance;
    uint16 payload_length;
    const void * payload;
    audio_anc_response * debug_info;
} IPC_AUDIO_ANC_CONFIG;
...
#if TRAPSET_LE_AE
typedef struct IPC_STREAM_EXT_SCAN_SOURCE {
    IPC_HEADER header;
} IPC_STREAM_EXT_SCAN_SOURCE;

typedef struct IPC_STREAM_PERIODIC_SCAN_SOURCE {
    IPC_HEADER header;
    uint16 sync_handle;
} IPC_STREAM_PERIODIC_SCAN_SOURCE;
...
typedef struct IPC_VM_GET_SCO_STATISTICS {
    IPC_HEADER header;
    uint16 handle;
    sco_statistics_t * sco_stats;
} IPC_VM_GET_SCO_STATISTICS;

typedef struct IPC_VM_GET_ACL_LINK_QUALITY {
    IPC_HEADER header;
    const TP_BD_ADDR_T * acl_address;
} IPC_VM_GET_ACL_LINK_QUALITY;
...
typedef struct IPC_VM_GET_ACL_LINK_QUALITY_RSP {
    IPC_HEADER header;
    bool ret;
    uint16 link_quality;
} IPC_VM_GET_ACL_LINK_QUALITY_RSP;
...
typedef struct IPC_OPERATOR_DATA_LOAD_RA {
    IPC_HEADER header;
    ...
    OperatorDataStorageType storage;
    bool auto_remove;
} IPC_OPERATOR_DATA_LOAD_RA;

typedef struct IPC_OPERATOR_DATA_UNLOAD_RA {
    IPC_HEADER header;
    DataFileID file_id;
} IPC_OPERATOR_DATA_UNLOAD_RA;
...
typedef struct IPC_RA_PARTITION_NAMED_OPEN {
    IPC_HEADER header;
    const char * name;
    raPartition_handle * handle;
    raPartition_info * partition_info;
} IPC_RA_PARTITION_NAMED_OPEN;
...
typedef struct IPC_SINK_L2CAP_FILTER_PACKETS {
    IPC_HEADER header;
    uint16 sink;
    bool enable;
    sink_filter_path path;
    uint8 filter_index;
    uint8 filter_mask;
    uint8 filter_result;
} IPC_SINK_L2CAP_FILTER_PACKETS;

typedef struct IPC_STREAM_ACL_ESTABLISH_SINK {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpaddr;
} IPC_STREAM_ACL_ESTABLISH_SINK;

typedef struct IPC_ACL_ESTABLISH_COMMIT {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpaddr;
} IPC_ACL_ESTABLISH_COMMIT;

typedef struct IPC_ACL_HANDOVER_ROLE_COMMIT {
    IPC_HEADER header;
    uint16 handle;
    mirror_role_t role;
} IPC_ACL_HANDOVER_ROLE_COMMIT;
...
typedef struct IPC_CHARGER_MESSAGE_REQUEST {
    IPC_HEADER header;
} IPC_CHARGER_MESSAGE_REQUEST;
...
typedef union IPC_RSP {
    ...
    IPC_VM_GET_ACL_LINK_QUALITY_RSP ipc_vm_get_acl_link_quality_rsp;
