...
/**
 * block_interrupts_before_sleep() and unblock_interrupts_after_sleep() are
 * required if RECORD_LATENCY is enabled and the OS sleeps with interrupts
 * blocked. Currently, RECORD_LATENCY is only supported on FreeRTOS and Oxygen
 * does not sleep with interrupts blocked.
 */
#ifdef OS_FREERTOS
/**
 * Block interrupts before going to sleep
 *
 * Functionally equivalent to block_interrupts but doesn't record how long
 * interrupts were blocked for when RECORD_LATENCY is enabled.
 */
extern void block_interrupts_before_sleep(void);

/**
 * Unblock interrupts after waking from sleep
 *
 * Functionally equivalent to unblock_interrupts but doesn't record how long
 * interrupts were blocked for when RECORD_LATENCY is enabled.
 */
extern void unblock_interrupts_after_sleep(void);
#endif /* OS_FREERTOS */
