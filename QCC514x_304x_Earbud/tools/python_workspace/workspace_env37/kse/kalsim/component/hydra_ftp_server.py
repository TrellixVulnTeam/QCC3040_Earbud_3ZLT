...
from kats.kalsim.hydra_service.constants import FTP_FW_TO_SERV_CTRL, FTP_FW_TO_SERV_DATA, \
    CCP_SIGNAL_ID_OPERATIONAL_IND
from kats.library.registry import get_instance

MESSAGE_TX_DELAY = 0.01
CHIP_READY_TIMEOUT = 3
...
class HydraFtpServer:  # pylint: disable=too-few-public-methods
    ...
    def _cache_files(self):
        ...
        if self.__config.directory is not None:
            ...
            # create special files subsysX_config0.hcf
            # these are a special concatenation of subsysX_config1.hcf to subsysX_config15.hcf files
            subsystems = []
            pattern = re.compile(r'^%ssubsys([0-9]+)_%s([0-9]+).%s$' %
                                 (self.__config.prefix, FORMAT_STR[FORMAT_CONFIG], FILE_EXTENSION))
            for filename in files:
                search = pattern.search(os.path.basename(filename))
                if search:
                    subsystem = int(search.group(1))
                    layer = int(search.group(2))
                    if LAYER_MIN <= layer <= LAYER_MAX and subsystem not in subsystems:
                        subsystems.append(subsystem)

            for subsystem in subsystems:
                zero_file = '%ssubsys%s_%s0.%s' % \
                            (self.__config.prefix, subsystem, FORMAT_STR[FORMAT_CONFIG],
                             FILE_EXTENSION)
                header = bytearray([0, 0, 0, 0, 0, 0, subsystem << 4, FORMAT_CONFIG])
                self.__data.file_data[zero_file] = header
                for ind in range(LAYER_MIN, LAYER_MAX + 1):
                    filename = '%ssubsys%s_%s%s.%s' % \
                               (self.__config.prefix, subsystem, FORMAT_STR[FORMAT_CONFIG], ind,
                                FILE_EXTENSION)
                    filename = os.path.join(self.__config.directory, filename)
                    try:
                        with open(filename, 'rb') as handler:
                            # remove first 64 bits header
                            self.__data.file_data[zero_file] += handler.read()[HCF_HEADER_SIZE:]
                            self._log.info('ftp_server added %s to %s', filename, zero_file)
    ...
    def _send_control_now(self, payload, timer_id):
        _ = timer_id
        self.__config.hydra_protocol.send_ftp_server_to_fw_control(payload)

    def _send_data_now(self, payload, timer_id):
        _ = timer_id
        self.__config.hydra_protocol.send_ftp_server_to_fw_data(payload)

    def _send_control(self, payload):
        # self.__config.hydra_protocol.send_ftp_server_to_fw_control(payload)
        # FIXME this is a workaround for a bug in current firmware implementation
        self.__config.uut.timer_add_relative(MESSAGE_TX_DELAY,
                                             partial(self._send_control_now, payload))

    def _send_data(self, payload):
        # self.__config.hydra_protocol.send_ftp_server_to_fw_data(payload)
        # FIXME this is a workaround for a bug in current firmware implementation
        self.__config.uut.timer_add_relative(MESSAGE_TX_DELAY,
                                             partial(self._send_data_now, payload))
