...
typedef bool(*InitialiseFunc)(const struct _gatt_client_t* instance, gatt_cid_t cid, uint16 start, uint16 end);
typedef bool (*DeinitialiseFunc)(const struct _gatt_client_t* instance);

/* Client specific functions */
typedef bool(*ClientLibInitFunc)(Task task, gatt_cid_t cid, uint16 start_handle, uint16 end_handle, void *client_data_dynamic, void* client_data_const);
...
bool GattClient_Protected_ClientLibInit(gatt_client_t* instance, gatt_cid_t cid, uint16 start_handle, uint16 end_handle);
...
gatt_cid_t GattClient_Protected_GetConnectionID(gatt_client_t*instance);
