...
import shutil
from maker.adk import  prepare_single_image as psi
from workspace_parse.workspace import Workspace
from maker.build import build_configs
...
def main(args):
    ...
    workspace = os.path.abspath(build_output_folder if build_output_folder else os.path.dirname(parsed_args.workspace))

    # Filename of the flash usage report that will be automatically generated
    flash_usage_datafile = os.path.join(workspace, 'FlashLayout.json')
    
    # Filename of report file containing break down of usage of ro_fs this will be automatically generated
    ro_fs_usage_datafile = os.path.join(workspace, 'ro_fs.json')

    # If the user have provided their own folder path use it, otherwise
    # obtain a string with the current date and time to use for the folder and filenames.
    nowstring = parsed_args.nowstring if parsed_args.nowstring else datetime.strftime(datetime.now(), '%Y%m%d%H%M%S')

    # Create a workspace folder image\<nowstring> folder
    outpath = os.path.join(workspace, "image", nowstring)
    ...
    if success:
        single_image.create_flash_image(devkit=parsed_args.devkit_root,
                                        ...
                                        flash_usage_datafile=flash_usage_datafile,
                                        ro_fs_usage_datafile=ro_fs_usage_datafile)
    else:
        print("Cancelled")
