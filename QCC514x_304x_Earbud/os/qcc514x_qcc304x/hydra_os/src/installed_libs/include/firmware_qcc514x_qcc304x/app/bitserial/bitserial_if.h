...
typedef enum bitserial_block_index
{
    ...
    BITSERIAL_BLOCK_1 = 1,
    /** Bitserial instance 2. */
    BITSERIAL_BLOCK_2 = 2,

    /* Aliases for when the transfer code is to run on the customer processor.
     * These cannot be opened at the same time as the non-P1 equivalent.
     */
    P1_BITSERIAL_BLOCK_0 = 0x80,
    P1_BITSERIAL_BLOCK_1 = 0x81,
    P1_BITSERIAL_BLOCK_2 = 0x82
...
typedef enum bitserial_spi_flags
{
    ...
    /* DEPRECATED: clock offset is "0" by default and can be changed by
     * setting "clock_sample_offset", so this flag is not necessary. */
...
typedef struct
{
    /** Clock sample offset, calculated as:
     * T(offset) = T(half_serial_clock) * clock_sample_offset / 65536.
     * Resulting value is within the range [0, 1/2 serial clock period),
     * not including the upper boundary.
     * Depending on the serial clock frequency, offset value can be
     * rounded down to the nearest supported value.
     * Default and recommended value is "0" which disables the offset. */
    uint16 clock_sample_offset;
    /** Deprecated: "interword_spacing".
     * Changing interword spacing is no longer supported,
     * the field is reserved and must be initialised to "0", otherwise
     * the firmware will panic with
     * PANIC_BITSERIAL_CONFIGURATION_NOT_SUPPORTED. */
    uint16 reserved;
...
typedef struct bitserial_config
{
     ...
     *
     * In all cases the frequency will be limited to 8MHz. 
     */
...
typedef enum bitserial_changeable_params
{
    ...
    /** Changes the clock frequency for future Bitserial transfers. */
    BITSERIAL_PARAMS_CLOCK_FREQUENCY_KHZ
