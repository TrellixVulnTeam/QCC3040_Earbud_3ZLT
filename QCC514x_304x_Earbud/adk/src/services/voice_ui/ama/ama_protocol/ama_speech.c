...
static void amaSpeech_RequestAlexaLaunch(void);
...
static void amaSpeech_RequestAlexaLaunch(void)
{
    ...
    if(bd_addr)
    {
        AccessoryFeature_RequestAppLaunch(*bd_addr, "com.amazon.echo", launch_without_user_alert);
    }
#endif
}
...
/******************************************************************/
bool AmaSpeech_StartWakeWord(uint32 pre_roll, uint32 start_timestamp, uint32 end_timestamp)
{
    ama_speech_msg_start_t msg;

    msg.initiator = ama_speech_initiator_wakeword;
    msg.pre_roll = pre_roll;
