...
transport_mgr_link_data_t *transportMgrFindDataWithTransInfo(uint16 link_param,transport_mgr_type_t trans_type)
{
    ...
    PanicFalse(trans_type != transport_mgr_type_none);
    ...
    while(temp != NULL)
    {
        ...
        else
        {
             if ((temp->link_cfg->trans_info.non_gatt_trans.trans_link_id == link_param) &&
                 (temp->link_cfg->type == trans_type))
                return temp;
        }
        temp = temp->next;
    }
    DEBUG_LOG_WARN("transportMgrFindDataWithTransInfo, no match for link_param %u, trans_type %u", link_param, trans_type);
    return NULL;
}
...
void transportMgrListFindAndSendMessage(uint16 trans_link_id,MessageId id,void *message,transport_mgr_type_t trans_type)
{
    ...
    {
        DEBUG_LOG_ERROR("transportMgrListFindAndSendMessage, could not find task for trans_link_id %d, trans_type %d", trans_link_id, trans_type);
        free(message);
    }
}

/******************************************************************************/
void transportMgrListBroadcastGattMessage(MessageId id,void *message, uint16 message_size)
{
    ...
    while(temp != NULL)
    {
        ...
        {
            /* Send copy of original message */
            uint8 *copy = PanicUnlessMalloc(message_size);
            memcpy(copy, message, message_size);
            MessageSend(temp->application_task, id, copy);
        }
        temp = temp->next;
    }
