...
/****************************************************************************
NAME
    HfpUpdateHandsetSwbSupportStatus

DESCRIPTION
    Updates link swb support status based on information if handset is blocked
    for swb or not.
...
{
    hfp_link_data* link = hfpGetLinkFromBdaddr(addr);

    if (link)
    {
        /* Update link swb support flag */
        link->bitfields.link_supports_swb = swp_support_status;
    }
}
