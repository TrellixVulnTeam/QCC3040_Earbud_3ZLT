...
/*! The goal engine is a generic engine that can run goals from different applications.
    To improve the readibility of the logs produced by this module, the application
    may specify the name of the application-specific enumeration defining the goal
    ids/names.

    The format specifier GOAL_ID_FS is used within this module when printing goal
    ids to the log.
*/
#ifndef GOAL_ID_ENUM_NAME
/* If the application has not provided the goal enum name, precede with a simple label */
#define GOAL_ID_FS "goal:%d"
#else
/* Add the application defined goal id enum to log prints */
#define GOAL_ID_FS "enum:"CSR_EXPAND_AND_STRINGIFY(GOAL_ID_ENUM_NAME)":%d"
#endif
...
static void goalsEngine_DebugLogGoalQueue(const goal_set_t gs)
{
    ...
    FOR_ALL_QUEUED_GOALS(gs, i)
    {
        ...
        DEBUG_LOG("  "GOAL_ID_FS" rule_id %u lock 0x%x lock_mask 0x%08lx%08lx",
 ...
 * Goal Queue Handling
 *****************************************************************************/
/*! \brief Clear an entry in the goal queue by resetting the values. */
static void goalsEngine_ClearGoalEntry(pending_goal_entry_t *queue_entry)
{
    memset(queue_entry, 0, sizeof(pending_goal_entry_t));
}
                                  ...
                                  goal_mask wait_mask, goal_id new_goal)
{
    ...
    FOR_ALL_QUEUED_GOALS(goal_set, i)
    {
        ...
        {
            DEBUG_LOG("goalsEngine_QueueGoal re-using slot %d for "GOAL_ID_FS, i, new_goal);
            queue_index = i;
            break;
        }
    }
    ...
    DEBUG_LOG("goalsEngine_QueueGoal "GOAL_ID_FS" queue_idx %d rule_id 0x%x wait_mask 0x%08lx%08lx",
...
static void goalsEngine_DequeGoal(goal_set_t goal_set, goal_id goal)
{
    ...
    DEBUG_LOG("goalsEngine_DequeGoal "GOAL_ID_FS, goal);
    ...
    FOR_ALL_QUEUED_GOALS(goal_set, i)
    {
        ...
        if (entry->id == goal)
        {
            ...
            goalsEngine_ClearGoalEntry(entry);
            break;
        }
    }
}
...
bool GoalsEngine_IsGoalQueued(goal_set_t goal_set, goal_id goal)
{
    FOR_ALL_QUEUED_GOALS(goal_set, i)
    {
        ...
        {
            DEBUG_LOG("GoalsEngine_IsGoalQueued, "GOAL_ID_FS" found at slot %d", goal, i);
            return TRUE;
        }
    }

    DEBUG_LOG("GoalsEngine_IsGoalQueued, "GOAL_ID_FS" not found", goal);
    return FALSE;
}
...
static void goalsEngine_CancelGoal(goal_set_t goal_set, goal_id cancel_goal)
{
    ...
    {
        DEBUG_LOG("goalsEngine_CancelGoal cancelling scripted "GOAL_ID_FS, cancel_goal);
        ScriptEngine_CancelScript(goal_set->proc_result_task,
                                           goal_set->proc_cancel_cfm_fn);
    }
    else
    {
        DEBUG_LOG("goalsEngine_CancelGoal cancelling singular "GOAL_ID_FS, cancel_goal);
                                             ...
                                             goal_mask* wait_mask)
{
    ...
    {
        DEBUG_LOG("goalsEngine_HandleExclusiveGoals exclusive "GOAL_ID_FS, new_goal);
    ...
    goal_id new_goal, goal_mask *wait_mask)
{
    ...
    if (goalsEngine_GoalCancelsActive(goal_set, new_goal))
    {
        ...
        DEBUG_LOG("goalsEngine_CheckIfNewGoalCancelsActiveGoals perform cancellations for "GOAL_ID_FS, new_goal);
...
static bool goalsEngine_HandleDuplicateCancellationGoal(goal_set_t goal_set, goal_id new_goal)
{
    ...
    {
        DEBUG_LOG("goalsEngine_HandleDuplicateCancellationGoal "GOAL_ID_FS, new_goal);
        goalsEngine_ClearPendingGoalsQueue(goal_set);
                                              ...
                                              goal_mask *wait_mask)
{
    ...
    if (supports_concurrency)
    {
        ...
        *wait_mask &= ~mask;
        ...
        DEBUG_LOG("goalsEngine_HandleConcurrentGoals Ok "GOAL_ID_FS" (0x%08lx%08lx) mask 0x%08lx%08lx",
                    new_goal, PRINT_ULL(GOAL_MASK(new_goal)), PRINT_ULL(mask));
    }
    else
    {
        DEBUG_LOG("goalsEngine_HandleConcurrentGoals Issue failed "GOAL_ID_FS" (0x%08lx%08lx) contention:%d",
...
/*! Update wait masks for queued goals and clear goals no longer queued.
    ...
    Any entries in the queue for which there are no queued goal decision messages
    are removed from the queue. This occurs when a goal has just been started after
    having been queued.
...
static void goalsEngine_UpdateQueueMasks(goal_set_t goal_set)
{
    if (goal_set->pending_goals.queue_msg_count)
    {
        ...
        FOR_ALL_QUEUED_GOALS(goal_set, i)
        {
            ...
            /* If there are no queued rule decisions for this goal entry,
               then there's no point in updating locks/masks for it, as it
               is no longer queued, so clear it from the queue before proceeding. 

               This occurs when a queued goal (for which there was only one instance
               on the queue) has just been started.
            */
            if (!MessagePendingFirst(goal_set->pending_goals.queue_task, queue_entry->rule_id, NULL))
            {
                goalsEngine_ClearGoalEntry(queue_entry);
                /* this entry has just been cleared, no further processing required,
                   continue to the next entry in the queue. */
                continue;
            }
            ...
            if (queue_entry->rule_id)
            {
                ...
                DEBUG_LOG("goalsEngine_UpdateQueueMasks i %d, "GOAL_ID_FS, i, goal);
                ...
                if (wait_mask != old_mask)
                {
                    if (!wait_mask)
                    {
                        ...
                        DEBUG_LOG_INFO("goalsEngine_UpdateQueueMasks. "GOAL_ID_FS" (0x%08lx%08lx) released concurrent. Mask changed from 0x%08lx%08lx",
                                  goal, PRINT_ULL(GOAL_MASK(goal)), PRINT_ULL(old_mask));
                    }
                    else
                    {
                        DEBUG_LOG("goalsEngine_UpdateQueueMasks. "GOAL_ID_FS" (0x%08lx%08lx) mask changed from 0x%08lx%08lx to 0x%08lx%08lx",
...
static void goalsEngine_StartGoal(goal_set_t goal_set, goal_id goal, Message goal_data)
{
    ...
    DEBUG_LOG_INFO("goalsEngine_StartGoal "GOAL_ID_FS, goal);
    ...
    /* Update masks now that a new goal is running.
       If this goal came off the queue this will also clear up the queue
       entry for this goal. */
                         ...
                         Message goal_data, size_t goal_data_size)
{
    ...
    DEBUG_LOG("GoalsEngine_AddGoal "GOAL_ID_FS" is_new_goal %d existing goals 0x%08lx%08lx", new_goal, is_new_goal, PRINT_ULL(goal_set->active_goals_mask));
    ...
    {
        DEBUG_LOG("GoalsEngine_AddGoal cancelling "GOAL_ID_FS" already running, ignore", new_goal);
        return;
    }

    if (goalsEngine_GoalCanRunNow(goal_set, is_new_goal))
    {
        DEBUG_LOG("GoalsEngine_AddGoal start "GOAL_ID_FS" immediately", new_goal);
    ...
    else
    {
        ...
        {
            DEBUG_LOG("GoalsEngine_AddGoal "GOAL_ID_FS" is a cancel goal", new_goal);
...
void GoalsEngine_ClearGoal(goal_set_t goal_set, goal_id goal)
{
    ...
    DEBUG_LOG_INFO("GoalsEngine_ClearGoal "GOAL_ID_FS". Mask now 0x%08lx%08lx", goal, PRINT_ULL(goal_set->active_goals_mask));
