...
// *****************************************************************************
// MODULE:
//    $_stream_usb_audio_silence_sink_buffer
//
// DESCRIPTION:
//    silences already written usb audio packets
//
// INPUTS:
//    - r0 = usb audio sink buffer, this is normally a local mmu buffer
//    - r1 = tatal size of packets in octets
//
// OUTPUTS:
//    - none
//
...
// NOTES:
// *****************************************************************************
.MODULE $M.stream_usb_audio_silence_sink_buffer;
   .CODESEGMENT PM_FLASH;
   .DATASEGMENT DM;

$_stream_usb_audio_silence_sink_buffer:
   /* save the registers that are used */
   pushm <FP(=SP), r1, rLink>;
   pushm <I4, L4>;
   push B4;

   // patch point
   LIBS_PUSH_R0_SLOW_SW_ROM_PATCH_POINT($usb_audio_ep.STREAM_USB_AUDIO_EP_ASM.SILENCE_SINK_BUFFER.PATCH_ID_0, r2)

   // get the write address of the buffer where the last octet
   // of last packet is written
   // Note: Silencing will be done in reverse order
   call $cbuffer.get_write_address_and_size_and_start_address;
   I4 = r0;
   L4 = r1;
   push r2;
   ...
   // r10 = octets to silence
   r10 = M[FP + 1*ADDR_PER_WORD];
   // convert to words
   r10 = r10 LSHIFT -LOG2_ADDR_PER_WORD;

   // add 2 extra words for covering possible
   // non-aligned octets at the beginning and
   // at the end. This is for simplicity else
   // we could carefully silence those parts
   // separately but really not needed.
   // RD-|---+---+---+---+---|--WR
   r10 = r10 + 2;
   ...
   I4 = I4 - r0;
   r0 = 0;
   do align_silence_loop;
       M[I4, -ADDR_PER_WORD] = r0;
   align_silence_loop:
