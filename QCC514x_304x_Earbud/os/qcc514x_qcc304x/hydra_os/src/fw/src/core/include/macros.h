...
#define CONVERT_FROM_LE_UINT32(x,y) ((x)[3] = (uint8)((y) >> 24), \
                                     ...
                                     (x)[1] = (uint8)((y) >> 8),  \
                                     (x)[0] = (uint8)((y) & 0xFF))
#define CONVERT_TO_LE_UINT24(x) (((uint32)((x)[2]) << 16) | \
                                 ((uint32)((x)[1]) << 8)  | \
                                 ((x)[0]))
 ...
 */
#ifdef __KALIMBA__
#define PRESERVE_TYPE_FOR_DEBUGGING_(typename, section_name, tag) \
    /*lint --e{19} --e{129} Useless declarations */\
    _Pragma(CSR_STRINGIFY(datasection section_name)) tag typename preserve_##tag##_##typename##_for_debugging;
#else
#define PRESERVE_TYPE_FOR_DEBUGGING_(typename, section_name, tag)
#endif
#define PRESERVE_ENUM_FOR_DEBUGGING(typename) \
        PRESERVE_TYPE_FOR_DEBUGGING_(typename, UNREFERENCED, enum)

#define PRESERVE_STRUCT_FOR_DEBUGGING(typename) \
        PRESERVE_TYPE_FOR_DEBUGGING_(typename, UNREFERENCED, struct)

#define PRESERVE_TYPE_FOR_DEBUGGING(typename) \
        PRESERVE_TYPE_FOR_DEBUGGING_(typename, UNREFERENCED,)

#define PRESERVE_ENUM_IN_SECTION(typename, section_name) \
        PRESERVE_TYPE_FOR_DEBUGGING_(typename, section_name, enum)
#define PRESERVE_TYPE_IN_SECTION(typename, section_name) \
        PRESERVE_TYPE_FOR_DEBUGGING_(typename, section_name,)
...
/* Macro to tell linker to keep this function even if it's unreferenced so
 * that it isn't garbage-collected and can be called from pydbg.
 */
#if defined(__KALIMBA__) && !defined(DESKTOP_TEST_BUILD)
#define TEST_FUNCTION _Pragma("codesection testfn")
#else
