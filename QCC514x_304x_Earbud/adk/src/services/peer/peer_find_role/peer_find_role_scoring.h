...
typedef struct
{
    ...
    grss_figure_of_merit_t  peer_score;
        /*! Is user pairing in progress */
    bool                    user_pairing;
        /*! Was user pairing in progress when asked to prepare for role selection */
    bool                    saved_user_pairing;
...
#define SCORE_WEIGHTS(XFUNC) \
    ...
    XFUNC(BROADCAST_RECEIVE,    1,                 13, 1) \
    XFUNC(USER_PAIRING,         1,                 14, 1) \
    XFUNC(HANDSET_CONNECTED,    1,                 15, 1)
