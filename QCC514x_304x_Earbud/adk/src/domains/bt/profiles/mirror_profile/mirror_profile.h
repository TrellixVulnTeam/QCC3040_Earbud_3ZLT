...
The handset must be master of the ACL connection to the primary earbud.
The link policy module sets the handset link policy. Mirror profile manages the
peer earbud link policy.
...
does not typically initiate mirror link disconnection.

A2DP mirroring start sequence is shown below. The mirror L2CAP and the audio
chains on both devices are started before the media stream from the handset is
accepted.
@startuml
...
typedef enum
{
    ...
    */
    ...
    /*! The audio should start synchronised at primary and secondary. This mode
        is reported on the primary earbud. It is used when the primary and
        secondary earbuds are connected but audio is already streaming when
        the earbuds start to mirror the A2DP stream. In this case, both earbuds
        mute their outputs and the secondary defines a unmute instant which it
        sends to the primary. Both earbuds unmute at this instant.
    */
    MIRROR_PROFILE_A2DP_START_PRIMARY_SYNC_UNMUTE,

    /*! As MIRROR_PROFILE_A2DP_START_PRIMARY_SYNC_UNMUTE, but for the secondary.
        It is also used when the secondary connects to a primary with an already
        active audio stream. */
    MIRROR_PROFILE_A2DP_START_SECONDARY_SYNC_UNMUTE,

    /*! In Q2Q mode, the handset defines the rendering time, therefore no
        audio synchronisation is required as both earbuds receive the same
        rendering times from the handset.
    */
...
/*! \brief Query if the mirror profile's current role is primary.
...
/*! \brief Test if any CIS delegation is connected.

    \retrun TRUE if any CIS delegation is connected, FALSE otherwise.
*/
...
/*! \brief Query if eSCO audio should be started synchronously on primary and secondary.
    \param source The voice source being started. */
...
/*! \brief Get address of connected handset */
bdaddr * MirrorProfile_GetMirroredDeviceAddress(void);

/*! \brief Get Audio context for LE unicst audio */
...
#define MirrorProfile_IsCisMirroringConnected() (FALSE)
...
#define MirrorProfile_EnableMirrorA2dp() /* Nothing to do */

#define MirrorProfile_DisableMirrorA2dp()  /* Nothing to do */

#define MirrorProfile_GetMirroredDeviceAddress() ((bdaddr *)NULL)

#define MirrorProfile_IsRolePrimary() TRUE

#define MirrorProfile_GetLeAudioUnicastContext() ((uint16) 0x0)

#define MirrorProfile_ShouldEscoAudioStartSynchronously(source) FALSE
