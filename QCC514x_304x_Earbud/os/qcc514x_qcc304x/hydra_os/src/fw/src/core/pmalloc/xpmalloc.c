...
#ifdef PMALLOC_STATS
    /* Remember this pool as the one we'd have liked to use */
    ideal_pool = pool;
#endif
        ...
        if (pool->free != PMALLOC_SCRAMBLE_ADDRESS(NULL))
        {
...
#if defined(PMALLOC_TRACE_OWNER_ANY) || defined(PMALLOC_RECORD_LENGTHS)
            {
...
#endif
                n += (size_t)udiv3216(NULL, (uint16)(pool->size),
                ...
                if (n < PMALLOC_MAX_DEBUG_BLOCKS)
                {
                    ...
                    {
                        PMALLOC_SET_LENGTH_ATOMIC(n, (uint8_t)requested_size);
#ifdef PMALLOC_STATS
                        /* The requested_size is needed for the pfree unwinding,
                         * so we can now increment the 'ideal' counter safely.
                         */
                        ideal_pool->curr_ideal_size++;
                        if (ideal_pool->max_ideal_size < ideal_pool->curr_ideal_size)
                        {
                            ideal_pool->max_ideal_size = ideal_pool->curr_ideal_size;
                        }
#endif
#ifdef PMALLOC_BLOCK_OVERRUN_CHECK
...
#endif /* PMALLOC_TRACE_OWNER_ANY || PMALLOC_RECORD_LENGTHS */
