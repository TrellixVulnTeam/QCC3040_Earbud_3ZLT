...
typedef enum dorm_ids {
    /**
     * Bitserial: avoid deep sleep while there are active transfers.
     */
    DORMID_BITSERIAL,
    /**
    ...
    DORMID_TEST_SLEEP,
    /**
     * The P1 IPC module prevents deep sleep when blocked on P0 responses.
     */
    DORMID_IPC,
    /**
...
#ifdef OS_FREERTOS
/**
 * Indicate to P0 that P1 no longer needs to deep sleep.
 *
 * Only sends the IPC message if P0 thinks P1 can deep sleep.
 * Called from the ISR.
 */
extern void dorm_disable_deep_sleep_if_enabled(void);
