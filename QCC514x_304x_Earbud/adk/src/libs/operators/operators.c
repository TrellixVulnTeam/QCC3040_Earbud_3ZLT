...
#define ADAPTIVE_ANC_PARALLEL_CONFIG_MASK 0x0100
...
{
    uint16 id;
    uint16 gains[adaptive_anc_static_gain_max];
} aanc_set_factory_gain_with_rxmix_msg_t;
...
typedef struct
{
    uint16 msg_id;
    uint16 packet_len;
}lc3_dec_sco_iso_packet_len_t;

typedef struct
{
    uint16 msg_id;
    uint16 packet_len;
}lc3_enc_sco_iso_packet_len_t;

typedef struct
{
    uint16 msg_id;
    uint16 frame_duration;
} lc3_enc_sco_iso_frame_duration_t;

typedef struct
{
    uint16 msg_id;
    uint16 bit_error_resilience;
} lc3_enc_sco_iso_error_resilience_level_t;
...
static ssrc_to_ttp_mapping_t getSsrcToTtpMappingEntry(aptx_adaptive_ssrc_ids_t ssrc_id, unsigned ttp_in_ms)
{
    ...
    entry.ttp_in_us = split32BitWordTo16Bits(MS_TO_US(ttp_in_ms));
...
void OperatorsAecSetSampleRate(Operator op, unsigned in_rate, unsigned out_rate)
{
    ...
    aec_set_sample_rate_msg.id = AEC_REF_SET_INPUT_OUTPUT_SAMPLE_RATES;
    aec_set_sample_rate_msg.in_rate = getSampleRateUnitsForAudioSubsystem(in_rate);
    aec_set_sample_rate_msg.out_rate = getSampleRateUnitsForAudioSubsystem(out_rate);
...
void OperatorsAptxAdEncoderSetEncodingParams(Operator op, aptxad_encoder_params_t *params)
{
    ...
    msg.compatibility = (uint16)params->compatibility;
...
/* Scaling factor to convert a dB gain to a scaled dB gain in log2 Q6N format.
 * The scaling factor is (1<<26)*20*log10(2)
 */
#define GAIN_DB_TO_LOG2_Q6N_SF (0xAA152D)

void OperatorsSetPassthroughGain(Operator op, int32 gain_db)
{
    int32 value = (gain_db * GAIN_DB_TO_LOG2_Q6N_SF);
...
static void OperatorsStandardGetStatus(Operator op, get_status_data_t* get_status_data)
{
    ...
    /* P0 allows requests of max. 128 bytes. Otherwise P0 panics: PANIC_SUBMSG_INVALID_TX_LENGTH */
...
void OperatorsCvcSendEnableOmniMode(Operator cvc_snd_op)
{
    ...
    cvc_msg.control_id = CVC_SEND_OMNI_MODE;
    cvc_msg.value = TRUE;
...
void OperatorsCvcSendSetIntMicMode(Operator cvc_snd_op, uint16 value)
{
    ...
    parameter.id = CVC_SEND_INT_MIC_MODE;
    parameter.value = value;
    operatorsSetSingleParameter(cvc_snd_op, &parameter);
}

void OperatorsCvcSendSetDmssConfig(Operator cvc_snd_op, uint32 value)
{
    standard_param_t parameter;
    parameter.id = CVC_SEND_DMSS_CONFIG;
    parameter.value = value;
    operatorsSetSingleParameter(cvc_snd_op, &parameter);
}
...
{
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = CVC_SEND_DMSS_CONFIG;
    OperatorsStandardGetParameters(cvc_snd_op, get_params_data);
    return(get_params_data->standard_params->value);
}
...
void OperatorsAdaptiveAncSetParallelTopology(Operator op, adaptive_anc_filter_config_t filter_config)
{
    OperatorsStandardSetControl(op,ADAPTIVE_ANC_FILTER_CONFIG_CTRL,(unsigned)filter_config);
}

void OperatorsAdaptiveAncSetSampleRate(Operator op, adaptive_anc_sample_rate_config_t sample_rate)
{
    OperatorsStandardSetControl(op, ADAPTIVE_ANC_SET_SAMPLE_RATE_CTRL, (unsigned) sample_rate);
}
...
void OperatorsAdaptiveAncSetStaticGainWithRxMix(Operator op, uint16 *gains)
{
    aanc_set_factory_gain_with_rxmix_msg_t gain_msg;
    gain_msg.id = ADAPTIVE_ANC_SET_STATIC_GAIN;

    memcpy(&gain_msg.gains[0], gains, sizeof(uint16) * adaptive_anc_static_gain_max);
...
static void OperatorsAdaptiveAncSetCoefficients(Operator op,audio_anc_instance inst, uint16 msg_id, const adaptive_anc_coefficients_t *numerator_coefficients, const adaptive_anc_coefficients_t *denominator_coefficients)
{
    ...
    if(msg_id == ADAPTIVE_ANC_SET_CONTROL_MODEL && inst == AUDIO_ANC_INSTANCE_1)
    {
        set_model_msg->num_denominator |= ADAPTIVE_ANC_PARALLEL_CONFIG_MASK;
    }
...
{
    OperatorsAdaptiveAncSetCoefficients(op,AUDIO_ANC_INSTANCE_NONE, ADAPTIVE_ANC_SET_PLANT_MODEL, numerator_coefficients, denominator_coefficients);
}
...
{
    OperatorsAdaptiveAncSetCoefficients(op,AUDIO_ANC_INSTANCE_NONE, ADAPTIVE_ANC_SET_CONTROL_MODEL, numerator_coefficients, denominator_coefficients);
}

void OperatorsParallelAdaptiveAncSetControlModel(Operator op,audio_anc_instance inst, const adaptive_anc_coefficients_t *numerator_coefficients, const adaptive_anc_coefficients_t *denominator_coefficeints)
{
    OperatorsAdaptiveAncSetCoefficients(op,inst, ADAPTIVE_ANC_SET_CONTROL_MODEL, numerator_coefficients, denominator_coefficeints);
}
...
{
    lc3_dec_sco_iso_packet_len_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_DEC_SCO_ISO_SET_PACKET_LEN;
...
{
    lc3_enc_sco_iso_frame_duration_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_DEC_SCO_ISO_SET_FRAME_DURATION;
    msg.frame_duration = frame_duration;
...
{
    lc3_enc_sco_iso_packet_len_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_ENC_SCO_ISO_SET_PACKET_LEN;
    msg.packet_len = packet_len;
...
{
    lc3_enc_sco_iso_error_resilience_level_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_ENC_SCO_ISO_SET_ERROR_RESILIENCE;
    msg.bit_error_resilience = bit_error_resilience;
...
{
    OperatorsStandardSetControl(op, EARBUD_FIT_TEST_IN_OUT_EAR_CTRL, (unsigned)enable);
}
