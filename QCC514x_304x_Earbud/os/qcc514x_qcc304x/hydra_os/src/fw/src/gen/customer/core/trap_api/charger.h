...
/**
 *  \brief Request another \#MESSAGE_CHARGER_DETECTED to be sent to the application
 *         If the charger is attached at boot time then application can miss
 *         \#MESSAGE_CHARGER_DETECTED because the task to receive it was registered
 *  too late.
 *         To avoid this problem application can call this trap after registering
 *         the charger task to ensure the message is re-sent if neccessary.
 *         The message is sent immediately if at the time of the trap call charger
 *         type is resolved. Otherwise it arrives in the usual way on completion
 *         of charger detection.
 *         
 * 
 * \ingroup trapset_chargermessage
 */
void ChargerMessageRequest(void );
