...
The object supports dict and list containers and any values.
...
def json_merge(data1, data2, **kwargs):
    ...
    Args:
        data1 (dict/list): Base input data
        data2 (dict/list): New data
        ...
        dict_mode (str). How to handle dict
            ...
            - DICT_MODE_APPEND keys only in data1 will be left intact, both in data1 and data2 will
              return data2 values and only in data2 will return data2 values (default)
...
def json_unflatten(val, separator=SEPARATOR_DEFAULT, object_pairs_hook=None):
    """Un-flatten a dictionary or a list.
    ...
    Get the following un-flattened output::
