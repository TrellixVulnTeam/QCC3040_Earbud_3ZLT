        ...
        if objfile:
            ...
            if (m is not None):
                ...
                if found:
                    ...
                    # is it the object file we are interested in?
                    obj = l.split(":")[0]
                    if (objfile == obj):
                        found = True
...
def parseRelocObjFile(lines, prog, var, const, filename, mappings):
    ...
    while i < len(lines):
        ...
        if m is not None:
            ...
            logging.debug("Parsing object file(1): {}".format(filename_obj))
        else:
...
def parseDisObjFile(lines, func, filename, mappings, func_export, kdc_maker):
    ...
    while i < len(lines):
        ...
        if m is not None:
            ...
            # form the object path name found in the archive
            filename_obj = generateFileNameObj(filename, m.group(1))
            if filename_obj in filename_obj_seen:
                logging.info("object file(2) already parsed, ignoring: {}".format(filename_obj))
                ignore_file = True
            else:
                logging.debug("Parsing object file(2): {}".format(filename_obj))
                filename_obj_seen.append(filename_obj)
        if not ignore_file:
            # is this is a start of a PM section?
            m = sectionDissRe.match(l)
            if m is not None:
        ...
        if l1 != l2 or l1.find("file format") != -1:
            ...
            if m1 != None:
                ...
                if m1 is not None:
                    ...
                    name_left = m2.group(1).strip().split("?")[1]
                    name_right = m1.group(1).strip().split("?")[1]
                    ...
                    if m1 is not None:
                        ...
                        name_left = m2.group(1).strip()
                        name_right = m1.group(1).strip()
                        if name_left.find("?") != -1:
    ...
    for l2 in lines2:
        ...
        if m2 is not None:
            ...
            while m1 == None:
                ...
                i1 = i1 + 1
                l1 = lines1[i1]
            name_left = m2.group(2).split()[0].strip()
            name_right = m1.group(2).split()[0].strip()
            insertMapping(mappings, name_right, name_left)
