 ...
 *  timeout value is 2147 * 10^6 micro-seconds ~ 2147 secs. The trap will return 
 ...
 *  role.
 *         
 * @note This function has been deprecated. Please use AclHandoverRoleCommit
 *  instead.
...
/**
 *  \brief  This trap should be called by application on secondary device to
 * commit to new primary role for non-mirrored ACL link with given remote device.
 * Application must call AclHandoverCommit to commit to new primary role for
 * mirrored ACL link with a separate remote device prior to this trap call.
 *         
 * On the secondary this trap should be called to complete the handover for a
 * non-mirrored ACL link after SinkFlushBlocking is successful.
 * This trap should not be used on primary or for a mirrored link.
 *         
 ...
 * \ingroup trapset_mirroring
 */
bool AclEstablishCommit(const tp_bdaddr * tpaddr);
 ...
 *  "AclHandoverPrepare()". On the secondary device, this will be
 *  connection_handle parameter in mirror acl link create indication 
 *  \param role               The new role to commit, 0x00 for Primary, 0x01 for Secondary
 *             
 *  \return TRUE if operation is success, FALSE otherwise. 
 * 
 * \ingroup trapset_mirroring
 */
bool AclHandoverRoleCommit(uint16 handle, mirror_role_t role);
