...
typedef struct pmalloc_pool
{
    ...
    /** Number of blocks in total up to the start of this pool */
    /* It's a different name to the previous 'blocks' so that pylib
     * can tell the difference and use it appropriately
     */
    size_t cblocks;
#else
...
#ifdef PMALLOC_STATS
    /** How big the pool needed to be to prevent overflows */
    uint16 max_ideal_size;
    uint16 curr_ideal_size;
#endif /* PMALLOC_STATS */
...
#ifdef PMALLOC_CUMULATIVE_BLOCKS
extern size_t pmalloc_total_blocks;
#endif
