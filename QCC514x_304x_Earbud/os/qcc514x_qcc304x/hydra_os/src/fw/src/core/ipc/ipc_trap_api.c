...
void ipc_trap_api_handler(IPC_SIGNAL_ID id, const void *prim, uint16 length_bytes)
{
    switch(id)
    {
        ...
        case IPC_SIGNAL_ID_APP_SINK_SOURCE_MSG:
        {
            ...
            trap_api_send_sink_source_message_filtered(ipc_msg->stream_id,
                                                       ipc_msg->id,
                                                       ipc_msg->message,
 ...
 * and check for compatibility criteria:
