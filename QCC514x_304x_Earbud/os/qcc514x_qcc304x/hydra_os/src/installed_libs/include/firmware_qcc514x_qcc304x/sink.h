...
/**
 *  \brief Filter incoming packets on the given L2Cap sink.
 *  \param sink The sink to apply the filter to.
 *  \param enable TRUE to enable the filter as specified, FALSE to disable an existing filter.
 *  \param path The path to apply the filter to.
 *  \param filter_index The octet offset into the packet where the filter mask will be applied.
 *  \param filter_mask The filter mask to be applied to the 'index' octet.
 *  \param filter_result Value to check against after applying the mask.
 *  \return TRUE if filter applied successfully, FALSE otherwise.
 * 
 * \ingroup trapset_mirroring
 */
bool SinkL2capFilterPackets(Sink sink, bool enable, sink_filter_path path, uint8 filter_index, uint8 filter_mask, uint8 filter_result);
 ...
 *   @note
 *   If the sink is a StreamFileSink, StreamFilesystemSink, StreamAclEstablishSink
 *   or an ImageUpgradeStreamGetSink then further data must not be written to the
 *   sink before MESSAGE_MORE_SPACE has been received. Alternatively the
 *   SinkFlushBlocking trap can be called instead of SinkFlush on the above sink
 *   types. SinkFlushBlocking provides a guarantee that the data has been written
 *   by the time the trap returns.
 *   @note
