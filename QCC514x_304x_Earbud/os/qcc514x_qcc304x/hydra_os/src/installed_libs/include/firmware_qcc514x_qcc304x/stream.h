...
/*!
@brief Find the Source associated with the stream ChargerCommsUart.
** 
** 
Returns zero if it is unavailable (for example there is not enough buffer.)
*/
#define StreamChargerCommsUartSource() StreamSourceFromSink(StreamChargerCommsUartSink())
/*!
...
/**
 *  \brief Find the Sink associated with the ChargerCommsUart.
 *         Before StreamChargerCommsUartSink is called, PioSetMapPins32Bank and
 *  PioSetFunction
 *         should be called to assign the pios to UART signals and
 *  ChargerCommsUartConfigure is called
 *         to configure the UART.
 *         To transmit a ChargerComms message, SinkFlushHeader should be called
 *  with the message header defined by
 *         charger_comms_uart_tx_msg_header_t. 
 *         The received ChargerComms message in source buffer has a message header
 *  is defined by
 *         charger_comms_uart_rx_msg_header_t which includes the device ID which
 *  the message is sent by.
 *         Returns zero if it is unavailable. For example there is not enough
 *  buffer.
 *         
 * 
 * \ingroup trapset_chargercomms
 */
Sink StreamChargerCommsUartSink(void );
...
/**
 *  \brief Get a handle to the extended scanning stream source
 * There is one extended scanning stream source that provides the extended advert
 *  data for all scanners.
 * It is created either when this function is called or if extended advert data is
 *  received for an enabled scanner.
 * It is automatically closed when there are no longer any scanners enabled,
 *  calling SourceClose() on this source will fail.
 * This source is message based and each advert has a header of type
 *  DM_ULP_EXT_SCAN_FILTERED_ADV_REPORT_IND_T.
 * A pointer to the next advert's header can be acquired with SourceMapHeader()
 *  and a pointer to the advert data via SourceMap().
 * This source does not support being connected to sinks via StreamConnect().
 * When this source is full new adverts coming from the air destined for this
 *  stream are dropped.
 * SourceSizeHeader() will return non zero if there are any messages to read from
 *  the source. SourceSize() and SourceBoundary()
 * are not a reliable way to determine if the source is empty as they will return
 *  zero if there are messages in the source with
 * zero length data.
 *         
 *  \return The source handle for the global extended scanning stream.
 * 
 * \ingroup trapset_le_ae
 */
Source StreamExtScanSource(void );

/**
 *  \brief Get a handle to a periodic scanning stream source for a sync handle
 * Multiple periodic scanning sources can exist simultaneously, one for each
 *  active sync handle. Multiple calls to this function
 * for a given sync handle will return the same stream. This function should only
 *  be called after obtaining the sync_handle from a
 * DM_ULP_PERIODIC_SCAN_SYNC_TO_TRAIN_CFM prim. If called with an invalid sync
 *  handle, one that is not currently synced,
 * this function will return zero.
 * This source is message based and each advert has a header of type
 *  DM_ULP_PERIODIC_SCAN_SYNC_ADV_REPORT_IND_T.
 * A pointer to the next advert's header can be acquired with SourceMapHeader()
 *  and a pointer to the advert data via SourceMap().
 * This source is closed automatically when a periodic sync is terminated via the
 *  DM_ULP_PERIODIC_SYNC_TERMINATE prim,
 * or if the sync is lost. Calling SourceClose() on this source will fail.
 * When this source is full new adverts coming from the air destined for this
 *  stream are dropped.
 * SourceSizeHeader() will return non zero if there are any messages to read from
 *  the source. SourceSize() and SourceBoundary()
 ...
 * zero length data.
 *         
 *  \return The source handle for the periodic scanning stream associated with the sync
 *  handle.
 * 
 * \ingroup trapset_le_ae
 */
Source StreamPeriodicScanSource(uint16 sync_handle);
#endif /* TRAPSET_LE_AE */
#if TRAPSET_NFC
...
/**
 *  \brief Returns a Source from LE ISO CIS or BIS connection handle. 
 * This trap must be called after setting up data path(s) for LE ISO connection.
 *         
 *  \param handle LE ISO CIS or BIS connection handle to fetch the Source.
 *             
 *  \return  Source ID or 0 if handle is not valid or Source doesn't exist yet.
 * 
 * \ingroup trapset_le_audio
 */
Source StreamIsoSource(uint16 handle);

/**
 *  \brief Returns a Sink from LE ISO CIS or BIS connection handle. 
 * This trap must be called after setting up data path(s) for LE ISO connection.
 *         
 *  \param handle LE ISO CIS or BIS connection handle to fetch the Sink.
 *  \return  Sink ID or 0 if handle is not valid or Sink doesn't exist yet.
 *           
 * 
 * \ingroup trapset_le_audio
 */
Sink StreamIsoSink(uint16 handle);
...
/**
 *  \brief  Return the sink to unmarshal data for non-mirrored ACL link with given remote
 *  device 
 *         
 * SinkFlushBlocking should be used for StreamAclEstablishSink stream to ensure
 *  that the data written into sink stream has been unmarshalled before trap
 *  return. The return status can be used to detect failure to unmarshal data for
 *  non-mirrored ACL link
 *          
 *  \param tpaddr Bluetooth address of remote device. 
 * 
 * \ingroup trapset_mirroring
 */
Sink StreamAclEstablishSink(const tp_bdaddr * tpaddr);
