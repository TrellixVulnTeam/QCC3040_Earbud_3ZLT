...
LOGGING_PRESERVE_MESSAGE_TYPE(scanMessages)
...
ASSERT_MESSAGE_GROUP_NOT_OVERFLOWED(LE_SCAN_MANAGER, LE_SCAN_MANAGER_MESSAGE_END)
...
static le_scan_settings_t* leScanManager_AcquireScan(le_scan_interval_t scan_interval, le_advertising_report_filter_t* filter, Task task)
{
    ...
    if (settings_index < MAX_ACTIVE_SCANS)
    {
        ...
        /* Copy filter directly, then re-allocate pointers (if any) */
        sm_data->active_settings[settings_index]->filter = *filter;
...
static void leScanManager_SetScanAndAddAdvertisingReport(le_scan_interval_t scan_interval, le_advertising_report_filter_t* filter)
{
    ...
    ConnectionBleAddAdvertisingReportFilter(filter->ad_type,
                                            ...
                                            filter->size_pattern,
                                            filter->pattern);
    if (filter->ad_type_additional)
    {
        ConnectionBleAddAdvertisingReportFilter(filter->ad_type_additional,
                                                filter->interval,
                                                filter->size_pattern,
                                                filter->pattern);
    }
...
static void leScanManager_HandlePeriodicStopCfm(const LE_SCAN_MANAGER_PERIODIC_STOP_CFM_T* cfm)
{
    scanCommand scan_command = LeScanManagerGetCurrentCommand();
    le_scan_manager_data_t * sm_data = LeScanManagerGetTaskData();
    
    if(scan_command == LE_SCAN_MANAGER_CMD_STOP)
    {
        if (cfm->result.status == LE_SCAN_MANAGER_RESULT_BUSY)
        {
            PanicFalse(leScanManager_PeriodicScanStop(LeScanManagerGetTask(), cfm->scan_task));
        }
        else
        {
            sm_data->is_busy = FALSE;
            leScanManager_SendStopCfm(sm_data->requester,cfm->result);
        }
    }
}

static void leScanManager_HandlePeriodicDisableCfm(const LE_SCAN_MANAGER_PERIODIC_DISABLE_CFM_T* cfm)
{
    scanCommand scan_command = LeScanManagerGetCurrentCommand();
    le_scan_manager_data_t * sm_data = LeScanManagerGetTaskData();
    if(scan_command == LE_SCAN_MANAGER_CMD_DISABLE)
    {
        if (cfm->result.status == LE_SCAN_MANAGER_RESULT_BUSY)
        {
            PanicFalse(leScanManager_PeriodicScanDisable(LeScanManagerGetTask()));
        }
        else
        {
            sm_data->is_busy = FALSE;
            leScanManager_HandleDisable(sm_data->requester);
        }
    }
}

static void leScanManager_HandlePeriodicEnableCfm(const LE_SCAN_MANAGER_PERIODIC_ENABLE_CFM_T* cfm)
{
    scanCommand scan_command = LeScanManagerGetCurrentCommand();
    le_scan_manager_data_t * sm_data = LeScanManagerGetTaskData();
    if(scan_command == LE_SCAN_MANAGER_CMD_ENABLE)
    {
        if (cfm->result.status == LE_SCAN_MANAGER_RESULT_BUSY)
        {
            PanicFalse(leScanManager_PeriodicScanEnable(LeScanManagerGetTask()));
        }
        else
        {
            sm_data->is_busy = FALSE;
            leScanManager_HandleEnable(sm_data->requester);
        }
    }
}
...
void leScanManager_Handler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        case CL_DM_BLE_SET_SCAN_ENABLE_CFM:
        {
            ...
            break;
        case LE_SCAN_MANAGER_PERIODIC_STOP_CFM:
            leScanManager_HandlePeriodicStopCfm((LE_SCAN_MANAGER_PERIODIC_STOP_CFM_T*)message);
            break;
        case LE_SCAN_MANAGER_PERIODIC_DISABLE_CFM:
            leScanManager_HandlePeriodicDisableCfm((LE_SCAN_MANAGER_PERIODIC_DISABLE_CFM_T*)message);
            break;
        case LE_SCAN_MANAGER_PERIODIC_ENABLE_CFM:
            leScanManager_HandlePeriodicEnableCfm((LE_SCAN_MANAGER_PERIODIC_ENABLE_CFM_T*)message);
            break;
        default:
            break;
    }
...
static void leScanManager_HandleEnable(Task task)
{
    ...
    {
        if (leScanManager_PeriodicScanEnable(LeScanManagerGetTask()))
        {
            /* Wait for confirmation message from Periodic enable */
...
static void leScanManager_HandleDisable(Task task)
{
    ...
    {
        if (leScanManager_PeriodicScanDisable(LeScanManagerGetTask()))
        {
            /* Wait for confirmation message from Periodic disable */
            sm_data->requester = task;
            sm_data->is_busy = TRUE;
...
static void leScanManager_HandleStop(Task task)
{
    ...
    {
        if (leScanManager_PeriodicScanStop(LeScanManagerGetTask(), task))
        {
            /* Wait for confirmation message from Periodic stop */
            sm_data->requester = task;
            sm_data->is_busy = TRUE;
...
bool LeScanManager_Init(Task init_task)
{
    ...
    leScanManager_PeriodicScanInit();
                                                          ...
                                                          bool already_handled)
{
    ...
    switch (id)
    {
        ...
        default:
            return leScanManager_HandlePeriodicClMessages(id, message);
    }
}
bool LeScanManager_IsTaskScanning(Task task)
{
    if (LeScanManager_IsPeriodicTaskScanning(task))
    {
        return TRUE;
    }
...
void LeScanManager_StartPeriodicScanFindTrains(Task task, le_periodic_advertising_filter_t* filter)
{
    ...
    leScanManager_StartPeriodicScanFindTrains(task, filter);
}
