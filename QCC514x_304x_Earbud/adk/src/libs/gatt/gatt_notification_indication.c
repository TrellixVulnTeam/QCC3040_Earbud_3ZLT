        ...
        )
{
    ...
    if(conn && !conn->closing)
    {
        ...
        GATT_DEBUG_INFO(("GattNotificationRequest: gatt_task: %p, connection: %p\n", gattGetTask(), &conn));
        ...
        )
{
    if ( gattCidIsValid(req->cid) )
    {
        ...
        if (req->size_value)
        {
            /* Truncate the value sent to MTU-3 if it is longer than that,
             * otherwise Bluestack will reject it due to resources.
             */
            size_t val_len = MIN(req->size_value, conn->mtu-3) * sizeof(uint8);
            uint8 *value = (uint8 *)PanicUnlessMalloc(val_len);

            memmove(value, req->value, val_len);
            prim->size_value    = val_len;
