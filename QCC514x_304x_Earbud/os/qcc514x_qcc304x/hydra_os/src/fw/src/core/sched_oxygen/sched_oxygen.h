 ...
 * It is assumed that this function will only be called
 ...
 * possible.
 *
 * This function is called by dorm_wake() from the timer interrupt.
 *
 * For Oxygen this means temporarily disallowing shallow sleep so that on return
 * from the interrupt handler Oxygen exits sleep to check for expired casual
 * events.
 */
#if (CHIP_HAS_SHALLOW_SLEEP_REGISTERS) && !defined(DESKTOP_TEST_BUILD)
#define sched_wake() hal_set_reg_allow_goto_shallow_sleep(0)
#else
#define sched_wake() do { } while(0)
#endif
