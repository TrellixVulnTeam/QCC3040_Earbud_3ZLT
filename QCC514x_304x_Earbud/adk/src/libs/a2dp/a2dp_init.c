...
LOGGING_PRESERVE_MESSAGE_TYPE(A2dpMessageId)
...
void A2dpInit(Task clientTask, uint16 role, service_record_type *service_records, uint8 size_seps, sep_data_type *seps, uint16 linkloss_timeout)
{
    ...
    if ( !a2dp )
    {
        ...
        DEBUG_LOG_INFO("A2dpInit sizeof(A2DP)=0x%u", sizeof(A2DP));
        ...
        else
        {
            ...
            if (role & A2DP_INIT_ROLE_SINK)
            {
                ...
                DEBUG_LOG_INFO("A2dpInit Register Sink Service Record");
                a2dp->bitfields.sdp_register_outstanding++;
            }
            if (role & A2DP_INIT_ROLE_SOURCE)
            {
                ...
                DEBUG_LOG_INFO("A2dpInit Register Source Service Record");
                a2dp->bitfields.sdp_register_outstanding++;
            }
        }
        ...
        StreamConfigure(VM_STREAM_L2CAP_ADD_LARGE_BUFFER_ON_PSM, AVDTP_PSM);
    }    
