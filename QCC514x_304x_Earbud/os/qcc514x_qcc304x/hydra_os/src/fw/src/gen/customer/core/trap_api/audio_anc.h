...
/**
 *  \brief This command sent a sequence of ANC configurations for a given ANC channel
 *  instance.
 * Processing continues if a command fails however in the response only the first
 *  failed command in the sequence is identified. Not supported on all devices
 *  will return FALSE on unsupported devices. 
 * Note that the DSP software/operator framework must be loaded before configuring
 *  ANC.
 * Check OperatorFrameworkEnable documentation for more details. 
 *         
 *  \param anc_instance The anc_instance parameter is an ID that identifies the ANC channel instance to
 *  which the configuration is to be applied (ANC0/ANC1).
 *  \param payload_length The number of uint16 words making up the command sequence.
 *  \param payload An array words, where the first word gives the number of commands, followed by
 *  command triplets of key ID, size and value.
 *  \param debug_info Optional structure for debugging, if not required pass as NULL pointer.
 *  \return TRUE if the ANC command sequence was set, FALSE if the function failed.
 * 
 * \ingroup trapset_audio_anc
 */
bool AudioAncConfig(audio_anc_instance anc_instance, uint16 payload_length, const void * payload, audio_anc_response * debug_info);
