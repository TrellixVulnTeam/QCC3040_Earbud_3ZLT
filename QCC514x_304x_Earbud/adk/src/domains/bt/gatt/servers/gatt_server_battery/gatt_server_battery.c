...
static void gattServerBattery_AddClient(gatt_cid_t cid);
static void gattServerBattery_RemoveClient(gatt_cid_t cid);
...
/*! Default value of number of battery servers */
uint8 number_battery_servers = NUMBER_BATTERY_SERVERS;
...
static uint16 gattServerBattery_GetBatteryLevel(const GBASS *battery_server)
{
    ...
    {
        DEBUG_LOG("gattServerBattery_GetBatteryLevel. Devices are in pair eg. Earbud case");
    ...
    /*! It's single or standalone device */
    else
    {
        DEBUG_LOG("gattServerBattery_GetBatteryLevel. Standalone or single device case");
        battery_level_left = appBatteryGetVoltageAverage();
        battery_level_right = 0;
    }
    ...
    if (battery_server == GetGattServerBatteryGbass(gatt_server_battery_type_left))
    {
        ...
        {
            battery_percent = Soc_ConvertLevelToPercentage(battery_level_left);
        }
        else
        {
            battery_percent = 0;
    ...
    else if (battery_server == GetGattServerBatteryGbass(gatt_server_battery_type_right))
    {
        ...
        {
            battery_percent = Soc_ConvertLevelToPercentage(battery_level_right);
        }
        else
        {
            battery_percent = 0;
...
static bool gattServerBattery_SendBatteryLevelIfChanged(const gattServerBatteryInstanceInfo *instance)
{
    ...
    gatt_cid_t cids[NUMBER_BATTERY_CLIENTS];
...
static gattServerBatteryClientData *gattServerBattery_GetClientDataByCid(const gattServerBatteryInstanceInfo *instance, gatt_cid_t cid)
{
    uint16 client_count = 0;
...
static bool gattServerBattery_ReadClientConfigFromStore(gatt_cid_t cid, uint16 battery_type, uint16 *config)
{
    uint16 stored_client_config = 0;
    bdaddr public_addr;
    bool client_config_read = FALSE;
    
    if (GattHandler_GetPublicAddressFromConnectId(cid, &public_addr))
    {
...
static void gattServerBattery_WriteClientConfigToStore(gatt_cid_t cid, uint16 battery_type, uint16 client_config)
{
    bdaddr public_addr;
...
static void gattServerBattery_MessageHandler(Task task, MessageId id, Message message)
{
    ...
    DEBUG_LOG("gattServerBattery_MessageHandler MESSAGE:0x%x", id);
    ...
    switch (id)
    {
        ...
        default:
            DEBUG_LOG("appGattBatteryMessageHandler. Unhandled message MESSAGE:0x%x", id);
            break;
    }
}
...
static void gattServerBattery_init(void)
{
    ...
    for (index = 0; index < number_battery_servers; index++)
    {
        ...
        if (!GattBatteryServerInit(GetGattServerBatteryGbass(index),
                                   GetGattServerBatteryTask(),
                                   &battery_server_params,
...
void GattServerBattery_SetNumberOfBatteryServers(uint8 no_battery_servers)
{
    number_battery_servers = no_battery_servers;
}
