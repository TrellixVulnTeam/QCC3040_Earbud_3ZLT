...
{
    DEBUG_LOG_INFO("a2dpSetStreamState %d: O:enum:avdtp_stream_state:%d N:enum:avdtp_stream_state:%d", device->bitfields.device_id, device->signal_conn.status.stream_state, state);
...
static bool buildPreferredList (remote_device *device, const uint8 *seid_list, uint16 seid_list_size)
{
    ...
    DEBUG_LOG_INFO("buildPreferredList seid_list=%p seid_list_size=%u", seid_list, seid_list_size);
    ...
    {   /* Build preferred list from local seid list */
        ...
        if (available_seps)
        {
            ...
            DEBUG_LOG_VERBOSE("buildPreferredList preferred list=[");
            ...
            while (sep_list_size--)
            {
                if (!sep_list->in_use)
                {
                    ...
                    DEBUG_LOG_VERBOSE("[s:%u,r:%u,c:%u]", preferred_list->seid, preferred_list->role, preferred_list->codec);
                    preferred_list++;
                }
...
{
    DEBUG_LOG_V_VERBOSE("doesCodecMatch - local_sep:%p = seid:0x%X role:%u codec:0x%X", (void*)local_sep, local_sep->seid, local_sep->role, local_sep->codec);
    DEBUG_LOG_V_VERBOSE("doesCodecMatch - remote_sep:%p = seid:0x%X role:%u codec:0x%X", (void*)remote_sep, remote_sep->seid, remote_sep->role, remote_sep->codec);
...
static bool selectNextSeid (remote_device *device)
{
    ...
    if ((remote_sep = (sep_info *)blockSetCurrent( (uint8)device->bitfields.device_id, data_block_list_discovered_remote_seids, DATA_BLOCK_INDEX_NEXT )) == NULL)
    {
        ...
        {
            DEBUG_LOG_VERBOSE("selectNextSeid sep lists exhausted");
    ...
    DEBUG_LOG_VERBOSE("selectNextSeid local_sep = codec:0x%X seid:%u, remote_sep = codec:0x%X seid:%u", device->local_sep.codec, device->local_sep.seid, device->remote_sep.codec, device->remote_sep.seid);
    return TRUE;
}
...
static void issueGetCapabilities (remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("issueGetCapabilities");
...
static bool issueSetConfiguration (remote_device *device)
{
    ...
    case CONFIGURATION_BY_CLIENT:       
        ...
        DEBUG_LOG_INFO("issueSetConfiguration CONFIGURATION_BY_CLIENT");
    ...
    case CONFIGURATION_SELECTED:        
        ...
        DEBUG_LOG_INFO("issueSetConfiguration CONFIGURATION_SELECTED");
    ...
    case CONFIGURATION_NOT_SELECTED:    
        DEBUG_LOG_INFO("issueSetConfiguration CONFIGURATION_NOT_SELECTED");
        break;
    }
...
{
    DEBUG_LOG_FN_ENTRY("issueClientSetConfiguration");
...
{
    DEBUG_LOG_FN_ENTRY("requestDelayReport");
...
{
    DEBUG_LOG_FN_ENTRY("waitDelayReport");
...
{
    DEBUG_LOG_FN_ENTRY("continueStreamConnect(%p) enum:avdtp_stream_state:%d", (void*)device, device->signal_conn.status.stream_state);
    ...
    do
    {
        ...
        default:
            ...
            DEBUG_LOG_WARN("continueStreamConnect UNEXPECTED A2DP LIBRARY STATE enum:avdtp_stream_state:%d", device->signal_conn.status.stream_state);
            a2dpStreamAbort(device);
            break;
        }
...
void a2dpHandleDiscoverResponse (remote_device *device)
{
    ...
    {   /* Process discover reponse fn will have generated a new list_discovered_remote_seids data block */
        ...
        DEBUG_LOG_VERBOSE("a2dpHandleDiscoverResponse local_sep = codec:0x%X seid:%u, remote_sep = codec:0x%X seid:%u",
...
bool a2dpHandleStartCommand (remote_device *device)
{
    ...
    DEBUG_LOG_FN_ENTRY("a2dpHandleStartCommand(%p): enum:avdtp_stream_state:%d", device , device->signal_conn.status.stream_state);
